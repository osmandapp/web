...
import { TEST_LOGIN3, TEST_PASSWORD3 } from '../../options.mjs';
...
export default async function test() {
    await actionOpenMap();
    await actionLogIn({ login: TEST_LOGIN3, password: TEST_PASSWORD3 });
    await clickBy(By.id('se-show-menu-tracks'));
    await driver.navigate().refresh();
    await waitByRemoved(By.id('se-loading-page'), true);
    const loadedInDelay = await driver
        .wait(async () => {
            return await waitBy(By.id(`se-menu-cloud-${SMART_FOLDER_NAME}`));
