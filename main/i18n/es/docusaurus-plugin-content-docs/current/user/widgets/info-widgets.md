...
Los widgets informativos le proporcionan acceso rápido a datos importantes. Esto puede ser datos sobre la velocidad, el clima, la posición de estacionamiento, datos de sensores externos y mucho más. Los widgets se pueden configurar de acuerdo con sus requisitos específicos, lo que permite su visibilidad en la pantalla de la aplicación o los desactiva si es necesario.
...
En algunos dispositivos Android, la altitud puede mostrarse de forma imprecisa. Para solucionar este problema, descargue un mapa con corrección de altitud.
...
Puede utilizar el widget para averiguar la elevación del centro del mapa actual sobre el nivel del mar. Se necesita un [mapa de terreno (sombreado y pendiente)](../plugins/topography.md#download-maps) del área requerida para mostrar información en el **widget de elevación: centro del mapa**.
...
| Al tocar | Sin cambios |
| Pulsación larga | Abre el [menú contextual del widget](../widgets/configure-screen.md#widget-context-menu) |
| Formato | La configuración del dispositivo configura la hora y el formato. |
...
El widget **Velocidad media** calcula y muestra su velocidad media durante un intervalo de tiempo seleccionado sin necesidad de navegación o grabación de rutas. Es útil para controlar el cumplimiento de la velocidad en autopistas o para seguir la velocidad media durante los últimos minutos de ciclismo.
...
2. ***Intervalo de tiempo.***
   ...
   `Velocidad media = MEDIA(GPS_LOCATION.speed)`.
...
| Al tocar | Sin cambios |
| Pulsación larga | Abre el [menú contextual del widget](../widgets/configure-screen.md#widget-context-menu). <br/> Se puede utilizar para restablecer la velocidad media. |
...
El [simple](../widgets/configure-screen.md#widgets-for-all-panels) widget *Relación de planeo* proporciona información valiosa sobre la relación de planeo para ayudar a tomar decisiones informadas al planificar rutas aéreas. El widget *Relación de planeo* en OsmAnd le ayuda a evaluar la eficiencia de su descenso o ascenso al navegar a una ubicación específica (posición del [marcador](../personal/markers.md) en el mapa). Calcula la relación de planeo como la relación entre la distancia y la diferencia de altitud entre su posición actual y su destino previsto. Puede leer más sobre esto [aquí](https://en.wikipedia.org/wiki/Lift-to-drag_ratio#Glide_ratio).
...
- *Optimizar ascensos y descensos.* La relación de planeo promedio permite a los pilotos optimizar ascensos y descensos en función de las condiciones actuales. Los pilotos pueden elegir el momento óptimo para los ascensos para maximizar el tiempo de vuelo, o para los descensos para alcanzar un punto objetivo.
...
- **Los valores negativos** indican que su destino está a una altitud superior, lo que requiere un ascenso. Por ejemplo, "-90:1" significa que después de recorrer una distancia de 90 kilómetros, necesita ascender 1 kilómetro para llegar a su destino.
...
- La **elevación del objetivo** muestra la elevación del punto objetivo. El [marcador de mapa](../personal/markers.md) debe usarse como punto objetivo.
...
| Al tocar | Cambia la *relación de planeo al objetivo* o la *elevación del objetivo* |
| Pulsación larga | Abre el [menú contextual del widget](../widgets/configure-screen.md#widget-context-menu) |
...
- **iOS 17 y posteriores** — Debido a las **restricciones de privacidad**, los datos de nivel de batería solo están disponibles en incrementos del 5% (por ejemplo, 35%, 60% u 85%). Esta es una limitación del sistema impuesta por Apple.
...
El **widget de velocímetro** es una visualización gráfica de la velocidad actual del vehículo basada en *datos recibidos del GPS*. Le permite controlar en tiempo real el cumplimiento entre su velocidad y el límite de velocidad permitido. El límite de velocidad permitido lo proporcionan los [datos de OpenStreetMap](https://wiki.openstreetmap.org/wiki/Key:maxspeed) para una sección de carretera en particular y la configuración de [Tolerancia de límite de velocidad](../navigation/guidance/voice-navigation.md#speed-limit) de OsmAnd.
...
Estos widgets se habilitan y funcionan en combinación con sus plugins correspondientes.
...
Los widgets de grabación de viaje se refieren al [plugin <Translate android="true" ids="record_plugin_name"/>](../plugins/trip-recording.md#widgets). Puede usar los widgets para ver información breve sobre la grabación de la ruta en la pantalla. Como *Distancia*, *Duración*, *Cuesta arriba* y *Cuesta abajo*.
...
- Utilice el [plugin de grabación de viaje](../plugins/trip-recording.md#recording-settings) para registrar datos del sensor.
...
Los widgets del tiempo muestran información meteorológica actual para un punto central en el mapa. Se habilitan automáticamente en la pantalla cuando se ven con el plugin (*Menú → Tiempo*), y puede elegir cuáles poner en la pantalla de inicio para uso diario (*Menú → Personalizar pantalla*). Puede leer más sobre esto en el artículo [Plugin del tiempo](../plugins/weather.md).
...
:::note
Para añadir el widget de Mapillary a la pantalla del mapa, habilite el [plugin de Mapillary](../plugins/mapillary.md) de OsmAnd.
:::

![Widget de Mapillary](@site/static/img/widgets/mapillary_widget.png)

Este es un widget del plugin de Mapillary que proporciona acceso rápido a la aplicación de Mapillary, lo que le permite añadir [*imágenes a nivel de calle*](../plugins/mapillary.md#enable-layer). El widget se añade al mapa automáticamente cuando el plugin está habilitado.
...
### Widget de notas de audio/video {#audiovideo-notes-widget}

<InfoAndroidOnly />

:::note
Para añadir widgets de notas de audio/video a la pantalla del mapa, habilite el [plugin de notas de audio/video](../plugins/audio-video-notes.md) de OsmAnd.
:::

![Widget de notas de audio-video](@site/static/img/plugins/audio-video-notes/audio_video_notes_widget.png)

Estos son widgets del plugin de notas de audio/video que proporcionan acceso rápido para iniciar/detener la toma de notas de audio, video o fotos.
...
- *Grabar video* - le permite grabar un video.
...
:::note
Para añadir widgets de desarrollador a la pantalla del mapa, habilite el [plugin de desarrollo de OsmAnd](../plugins/development.md).
:::
...
Los **widgets de cámara** le permiten controlar la alineación entre el mapa de OsmAnd y la superficie terrestre.
...
- **Uso de RAM de la aplicación** — Muestra el consumo actual de RAM por OsmAnd.

- **RAM total de la aplicación** — Indica la memoria total asignada a OsmAnd, incluyendo la memoria usada y la disponible.

> **NOTA**. *La *RAM total disponible* puede variar según los procesos en segundo plano y otras aplicaciones que se ejecuten en su dispositivo. Estos valores son dinámicos y ofrecen una forma de rastrear y administrar el uso de la memoria para un rendimiento consistente y eficiente de OsmAnd.*
