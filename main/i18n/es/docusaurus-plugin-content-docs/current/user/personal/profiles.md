...
OsmAnd ofrece una variedad de perfiles predeterminados adaptados a diferentes actividades, como conducir, andar en bicicleta, caminar y actividades más especializadas como esquiar y navegar. Estos perfiles están diseñados para mejorar tu experiencia de navegación según la actividad en la que estés involucrado. Cada perfil incluye opciones configurables, lo que te permite ajustar la configuración e incorporar complementos adicionales para que coincidan con tus preferencias.

- Ciertos perfiles, como *Esquí* y *Barco*, tienen **características adicionales** que solo están disponibles cuando los complementos correspondientes están habilitados.

     - Los complementos se pueden activar navegando a *Menú → Complementos → Vista de mapa de esquí* o *Vista de mapa náutico*.
     - Habilitar los complementos proporciona a los perfiles las características esenciales para satisfacer los requisitos específicos de estas actividades.

- Es importante tener en cuenta que algunos perfiles, como **Camión**, **Motocicleta**, **Ciclomotor**, **Tren**, **Avión**, **Barco** y **Paseo a caballo**, no están activados por defecto.

     - Para acceder y usar estos perfiles, debes habilitarlos manualmente en *<Translate android="true" ids="shared_string_menu,shared_string_settings,application_profiles"/>*.
     - Este método ayuda a mantener una lista organizada y relevante de perfiles activando solo aquellos necesarios para actividades específicas.
...
:::caution Aplicar la configuración solo a un perfil específico
Todas las configuraciones se aplican exclusivamente al perfil de aplicación actualmente seleccionado.
:::
...
Esta sección contiene todas las configuraciones relacionadas con la apariencia, opciones de navegación, visualización del mapa, menú de configuraciones y pantalla de perfil.
...
Debes tener una cuenta de [OsmAnd Cloud](../personal/osmand-cloud.md#login) para usar la *Copia de seguridad de configuración gratuita*. Si tienes una suscripción a *OsmAnd Pro* o una cuenta activa de *OsmAnd Cloud*, el banner de promoción no se mostrará.


## Configuración general {#general-settings}

Esta sección contiene configuraciones para el tema de la aplicación y el botón de la brújula, unidades y formatos de datos de mapas y perfiles, dispositivos de entrada externos y muchas otras configuraciones para crear la configuración óptima para el perfil elegido.
...
- **[<Translate android="true" ids="screen_control"/>](../navigation/guidance/voice-navigation.md#screen-control)** (*Solo Android*). Abre el menú de control de pantalla, donde puedes seleccionar opciones para la pantalla del dispositivo mientras navegas para ahorrar el consumo de batería del dispositivo.
...
En la sección **Unidades y Formatos**, puedes ajustar las unidades de medida y los formatos para que se ajusten a tus preferencias personales y estándares regionales, asegurando que todos los datos se muestren de manera conveniente y familiar.
...
| | <Translate android="true" ids="navigate_point_format_mgrs"/> | Ejemplo: 34U DA 23678 52873 . [<Translate android="true" ids="mgrs_format_descr"/>](https://es.wikipedia.org/wiki/Sistema_de_coordenadas_de_la_cuadr%C3%ADcula_militar) |
...
La sección **Otros** proporciona configuraciones de perfil adicionales para personalizar la interfaz de OsmAnd y los controles de entrada. Estas configuraciones te permiten configurar dispositivos externos, ajustar las preferencias de animación y alternar el modo de pantalla completa.
...
Para una descripción detallada de las opciones de navegación, consulta el artículo [Configuración de navegación](../navigation/guidance/navigation-settings.md), que explica cómo configurar OsmAnd para una experiencia de navegación óptima.

- [Tipo de navegación](../navigation/guidance/navigation-settings.md#navigation-type) - Elige el modo de navegación adecuado según tu método de viaje y la disponibilidad de internet.
...
- [Alertas en pantalla](../navigation/guidance/navigation-settings.md#screen-alerts) - Habilita el *Widget de alertas* para mostrar notificaciones en tiempo real sobre límites de velocidad, giros y otras condiciones importantes de la carretera.
...
- [Guía detallada de la ruta](../navigation/guidance/navigation-settings.md) – Mejora la navegación paso a paso alineando tu ruta con las carreteras mapeadas. Configuraciones disponibles: *Preguntar siempre* o *Siempre*.
...
- **Mostrar rutas y archivos GPX específicos**, incluidas pistas de terceros para navegación o análisis.
...
- **Habilitar la visualización del transporte público**, mostrando [rutas y paradas](../map/public-transport.md) para una mejor planificación de viajes.
...
La configuración de **Apariencia del perfil** te permite personalizar el nombre y la representación visual de tu perfil. También puedes modificar la apariencia de [Mi ubicación](#my-location-appearance) aquí. Si bien el *nombre del perfil* debe ser único, los *iconos* y *colores* pueden ser los mismos en diferentes perfiles. Estos cambios se aplicarán en varias partes de la aplicación, como el menú de [Preparación de ruta](../navigation/setup/route-navigation.md).
...
1. **Crear un modelo 3D**. Diseña tu icono en [formatos MTL y OBJ](https://es.wikipedia.org/wiki/Wavefront_.obj_file).

2. **Integrar el modelo en OsmAnd:**
   ...
   - Alternativamente, copia los **archivos MTL y OBJ** directamente en la carpeta OsmAnd usando la misma estructura de directorio.
...
- Si el icono de ubicación **se mueve o gira** mientras está parado, cambia del **Modo de dirección de movimiento** ([Rotar mapa por rumbo](../map/interact-with-map.md#rotate-map-by-bearing)) a otro modo.
...
Con la función de personalización de la interfaz de usuario, puedes ajustar el número de elementos en acciones como [<Translate android="true" ids="shared_string_drawer"/>](../start-with/main-menu.md#customize-advanced-use-of-android), [<Translate android="true" ids="configure_map"/>](../map/configure-map-menu.md) y [<Translate android="true" ids="context_menu_actions"/>](../map/map-context-menu.md). La información sobre el número de elementos añadidos de todos los elementos posibles se puede encontrar bajo el título de cada función.
...
- **Elementos principales visibles** - En la sección *Personalizar interfaz de usuario*, puedes *reordenar, ocultar y restaurar* elementos del [Cajón](../start-with/main-menu.md#customize-advanced-use-of-android) para que coincidan con tus preferencias.
...
- **Menú de configuración del mapa** - En la sección *Personalizar interfaz de usuario*, puedes *reordenar u ocultar elementos* del [Configurar mapa](../map/configure-map-menu.md), lo que permite un acceso rápido a las configuraciones de uso frecuente.

- **<Translate android="true" ids="reset_to_default"/>** - Restaura la *lista de elementos original* en el menú Configurar mapa, eliminando cualquier cambio.
...
- **Menú contextual del mapa** - En la sección Personalizar interfaz de usuario, puedes reordenar u ocultar elementos en el [menú contextual del mapa](../map/map-context-menu.md) para optimizar el acceso a las funciones de uso frecuente.
...
- **<Translate android="true" ids="copy_from_other_profile"/>** – Copia la *disposición del menú contextual* de otro perfil de OsmAnd para mantener la coherencia entre diferentes perfiles.


## Configuración de complementos {#plugin-settings}
...
Los complementos desbloquean características adicionales de la aplicación y pueden ser [de pago o gratuitos](../plugins/index.md#purchase). Algunos complementos tienen su propia [configuración](../plugins/index.md#plugin-settings), otros no. OsmAnd te da la posibilidad de configurar los complementos para cada perfil por separado.


- **Grabación de viajes**. Este elemento abre la [configuración de grabación de viajes](../plugins/trip-recording.md#recording-settings) para el perfil seleccionado, donde puedes elegir cualquier parámetro para grabar tus viajes.

- **Clima**. Las capas interactivas del mapa de [Clima](../plugins/weather.md#weather-settings) te permiten monitorear la temperatura, la presión del aire, la cobertura de nubes, la velocidad del viento y las precipitaciones en tu ciudad o en cualquier otra ubicación en un mapa global.

- **Notas de audio/video** (*Solo Android*). Este elemento abre la [configuración del complemento de audio/video](../plugins/audio-video-notes.md#plugin-settings) para el perfil seleccionado. El complemento *Notas de audio/video* amplía la funcionalidad de OsmAnd al permitirte crear tus notas en varios formatos, como foto, video o audio, y asociarlas con una ubicación geográfica o la ubicación actual.

- **Edición de OpenStreetMap**. Este elemento abre la [configuración del complemento de edición de OpenStreetMap](../plugins/osm-editing.md#settings) para el perfil seleccionado. Con OsmAnd y el complemento de edición de OSM, puedes contribuir a OpenStreetMap.org creando o modificando POI, añadiendo o comentando notas y subiendo pistas GPX grabadas.

- **Sensores externos**. Cuando tocas un elemento en la sección *Configuración de complementos*, se abren las [configuraciones de los sensores externos](../plugins/external-sensors.md#sensors-settings) para el perfil seleccionado. El complemento Sensores externos te permite leer y registrar datos de sensores externos inalámbricos y mostrarlos usando widgets en la aplicación OsmAnd.

- **Accesibilidad** (*Solo Android*). Este elemento abre la [configuración del complemento de accesibilidad](../plugins/accessibility.md#plugin-settings) para el perfil seleccionado. La configuración del complemento de accesibilidad te permite personalizar la aplicación según tus necesidades. Todas las configuraciones se relacionan con el proceso de navegación y se establecen individualmente para cada perfil.

- **Desarrollo de OsmAnd**. Este menú abre la [configuración del complemento de desarrollo de OsmAnd](../plugins/development.md#plugin-settings), donde puedes configurar la aplicación OsmAnd para fines de prueba o explorar las próximas funciones. Estas configuraciones están destinadas a desarrolladores y no son necesarias para el uso normal de la aplicación.

  > ***Cambiar la configuración del complemento de desarrollo de OsmAnd afecta a todos los perfiles.***

- **Wikipedia**. Este menú abre la *[configuración del complemento de Wikipedia](../plugins/wikipedia.md#wikipedia-settings)*. Puedes seleccionar el idioma en el que se mostrarán los artículos y elegir si deseas descargar imágenes de Wikipedia. Tener Wikipedia en tu viaje mejora tu experiencia al proporcionar información sobre los lugares que visitas. Funciona sin conexión y muestra artículos relevantes de Wikipedia directamente en el mapa relacionados con puntos de interés.
...
- [Exportar perfil](https://osmand.net/docs/user/personal/import-export#export) - Exporta todas las configuraciones del perfil seleccionado en formato OSF.

- **Copiar de otro perfil** - Copia todas las configuraciones de otro perfil existente en tu aplicación OsmAnd.

- **Restablecer a valores predeterminados** - Restaura todas las configuraciones al estado inicial.
