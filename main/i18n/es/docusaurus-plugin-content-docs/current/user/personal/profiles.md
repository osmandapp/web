...
- Ciertos perfiles, como *Esquí* y *Barco*, tienen **características adicionales** que solo están disponibles cuando se activan los complementos correspondientes.

     - Los complementos se pueden activar navegando a *Menú → Complementos → Vista de mapa de esquí* o *Vista de mapa náutico*.
     - La activación de los complementos proporciona a los perfiles las características esenciales para satisfacer los requisitos específicos de estas actividades.
...
En la sección **Unidades y formatos**, puedes ajustar las unidades y formatos de medida para que se ajusten a tus preferencias personales y estándares regionales, asegurando que todos los datos se muestren de forma conveniente y familiar.
...
|          | <Translate android="true" ids="navigate_point_format_olc"/> | Ejemplo:  9F2X4WFJ+7W ([Open Location Code](https://en.wikipedia.org/wiki/Open_Location_Code) representa un área de 9m x 14m)  |
|          | <Translate android="true" ids="navigate_point_format_swiss_grid"/> (Solo versión Android) | Ejemplo: 2 215 227.87, 830 915.9 ([Sistema de coordenadas suizo](https://en.wikipedia.org/wiki/Swiss_coordinate_system#:~:text=The%20Swiss%20coordinate%20system%20(or,Office%20of%20Topography%20(Swisstopo).)))  |
|          | <Translate android="true" ids="navigate_point_format_swiss_grid_plus"/> (Solo versión Android) | Ejemplo: 4 215 227.87, 1 830 915.9 ([Sistema de coordenadas suizo](https://en.wikipedia.org/wiki/Swiss_coordinate_system#:~:text=The%20Swiss%20coordinate%20system%20(or,Office%20of%20Topography%20(Swisstopo).))) |
|  |  |  |
...
|          | <Translate android="true" ids="shared_string_milliradians"/> | Todos los valores angulares tienen [valor en milirradianes](https://en.wikipedia.org/wiki/Milliradian).  |
|  |  |  |
...
|  |  |  |
| **<Translate android="true" ids="unit_of_volume"/>** (*Solo Android*) | <Translate android="true" ids="litres"/> | Europa, Asia, América Latina, Canadá, Japón, India, Australia |
...
|          | <Translate android="true" ids="round_up"/> | *Para una mejor legibilidad*, los números que se muestran en los widgets de navegación (distancia a un punto, siguiente giro o carril) se redondearán a la baja para tener menos dígitos. <br/> Por ejemplo: 3672 m *→* 3.6 km,&nbsp; 462 m *→* 400 m,&nbsp; 184 m *→* 150 m,&nbsp; 47 m *→* 40 m,&nbsp; 18 m *→* 15 m. |
...
- [<Translate android="true" ids="use_magnetic_sensor"/>](../map/interact-with-map.md#extra-compass-settings) (*Solo Android*) – Utiliza el [sensor magnético](https://en.wikipedia.org/wiki/Kalman_filter) de tu dispositivo para estabilizar la rotación del mapa, reduciendo los cambios bruscos. Esto también introduce un ligero retraso.
...
Para obtener una descripción detallada de las opciones de navegación, consulta el artículo [Ajustes de navegación](../navigation/guidance/navigation-settings.md), que explica cómo configurar OsmAnd para una experiencia de navegación óptima.
...
- [Parámetros del vehículo](../navigation/guidance/navigation-settings.md#vehicle-parameters) – Configura ajustes específicos del vehículo, como la velocidad máxima y la altura del vehículo, para garantizar un enrutamiento preciso y evitar carreteras restringidas.
...
- [Guía de seguimiento detallada](../navigation/guidance/navigation-settings.md) – Mejora la navegación paso a paso alineando tu ruta con las carreteras mapeadas. Ajustes disponibles: *Preguntar siempre* o *Siempre*.
...
- **Superponer capas de mapa adicionales**, como [información del terreno](../plugins/topography.md), **imágenes satelitales** u otros [mapas rasterizados](../map/raster-maps.md) disponibles.
...
1. **Crea un modelo 3D**. Diseña tu icono en [formatos MTL y OBJ](https://en.wikipedia.org/wiki/Wavefront_.obj_file).
...
2. **Integra el modelo en OsmAnd:**
   ...
   - Coloca tus archivos de iconos 3D en las siguientes rutas:
   ...
   - Alternativamente, copia los **archivos MTL y OBJ** directamente en la carpeta OsmAnd utilizando la misma estructura de directorios.
...
- Ajusta la configuración adicional en *Menú → Ajustes → Perfiles de aplicación → Apariencia del perfil → Opciones*.
- Obtén más información sobre el seguimiento de rumbo y dirección en la sección [Widget de rumbo](../widgets/nav-widgets#bearing-widget).
...
- **Menú Configurar mapa** - En la sección *Personalizar UI*, puedes *reordenar u ocultar elementos* del [Configurar mapa](../map/configure-map-menu.md), permitiendo un acceso rápido a la configuración utilizada con frecuencia.
...
- **Menú contextual del mapa** - En la sección Personalizar UI, puedes reordenar u ocultar elementos en el [menú contextual del mapa](../map/map-context-menu.md) para optimizar el acceso a las funciones utilizadas con frecuencia.
...
- **<Translate android="true" ids="copy_from_other_profile"/>** – Copia la *disposición del menú contextual* de otro perfil de OsmAnd para mantener la coherencia en diferentes perfiles.


## Ajustes de complementos {#plugin-settings}

:::caution Acceso a los ajustes de los complementos
Para acceder a los ajustes del complemento, primero debes [**habilitar el complemento**](../plugins/index.md#enable--disable) en la *sección de complementos del menú principal*.
:::
...
- **Desarrollo de OsmAnd**. Este menú abre los [ajustes del complemento de desarrollo de OsmAnd](../plugins/development.md#plugin-settings), donde puedes configurar la aplicación OsmAnd para fines de prueba o explorar las próximas funciones. Estos ajustes están destinados a desarrolladores y no son necesarios para el uso normal de la aplicación.
  ...
  > ***Cambiar los ajustes del complemento de desarrollo de OsmAnd afecta a todos los perfiles.***
...
- **Eliminar perfil**:
    ...
    - Para añadir un perfil, ve a *Menú → Ajustes → Perfiles de aplicación → + Nuevo perfil*.
