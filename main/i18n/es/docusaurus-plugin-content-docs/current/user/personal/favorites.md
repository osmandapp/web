...
Al añadir un punto a partir de datos OSM, la información relevante del PDI se incluye automáticamente. También puedes crear un favorito [pulsando prolongadamente](../map/map-context-menu.md#select-any-point-long-tap) cualquier ubicación del mapa.
...
- **Eliminar** — Eliminar el favorito utilizando la opción de eliminar, accesible a través del menú Editar o mediante el botón de eliminar en Android.
...
Una amplia gama de iconos para PDI y waypoints está disponible en OsmAnd. Puedes:
...
Ve a: *Mantén pulsada la carpeta → Cambiar apariencia*
...
Utiliza el ***Menú de tres puntos*** (*Android*) junto a cada carpeta y ***mantén pulsada*** (*iOS*) la carpeta para gestionar grupos de favoritos:
...
- **Coherencia** — Los nuevos favoritos añadidos al grupo heredan la apariencia predeterminada automáticamente.
...
- **No se requiere pago**. Utiliza la función de copia de seguridad sin comprar suscripciones de pago.
...
Puedes exportar e importar tus favoritos utilizando los botones especiales en la parte inferior de la pantalla de Favoritos. Un [archivo .gpx](../../technical/osmand-file-formats/osmand-gpx.md) (*favorites.gpx*) se puede enviar a Dropbox, correo electrónico, mensajería y otras aplicaciones instaladas en tu dispositivo que admitan esta función.

- Botón **Importar** (*Android*) / **Importar favorito** (*iOS*). Te permite importar puntos favoritos (*favorites.gpx*) como waypoints desde un archivo *GPX* (un formato de datos GPS común) desde el almacenamiento de tu dispositivo.
...
Ve a: *Archivos → En mi iPhone → OsmAnd Maps → favourites_backup*
...
- **Android**: Las copias de seguridad se almacenan en *Android → data → net.osmand → files → backup*. Utiliza un gestor de archivos de terceros para acceder a ellas.
...
- Exporta el archivo `.osf` al almacenamiento local, a servicios en la nube o compártelo directamente.

:::caution
En las últimas versiones de ***Android***, el acceso a los directorios del sistema está limitado. Sin embargo, después de extraer el archivo de favoritos del archivo, sigue siendo posible reenviarlo a mensajería o servicios en la nube, etc.
:::


### Favoritos en archivo GPX {#favorites-in-gpx-file}

Toda la información sobre un Favorito se almacena y describe utilizando etiquetas. Al crear un punto Favorito, puedes escribir tu propia [descripción](#favorite-point) o utilizar información de los datos de [punto de interés (PDI)](../map/point-layers-on-map.md#points-of-interest-pois) de OSM a partir de los cuales se creó tu punto. Los puntos favoritos, los PDI y los Waypoints utilizan las mismas **etiquetas** para almacenar información y se guardan en **formato de archivo GPX**.
