...
* [**Redesigned Graph Axis Selection**](https://osmand.net/docs/user/map/tracks/#analyze-track-on-map)
    The "Analyze on map" graph now includes a redesigned axis selection menu. The new interface allows configuring both the X-axis and Y-axis in a single sheet and supports selecting up to two data types for the Y-axis. Additional data sources such as **external sensors** and **vehicle metrics (OBD-II)** are now available when present in the track data, making it easier to analyse different aspects of a recorded track.
...
* [**Widget Icon Visibility**](https://github.com/osmandapp/OsmAnd-iOS/issues/4902)
    Added a new option [to show or hide icon on widgets](https://osmand.net/docs/user/widgets/configure-screen#widget-settings). This allows for a cleaner, more minimalist dashboard by displaying only data, which is especially useful for medium and large widget layouts.
...
- Fixed [map behavior for trails/routes opening on the map](https://github.com/osmandapp/OsmAnd-iOS/issues/5129);
- Fixed [wrong language displayed in Wikipedia POI details](https://github.com/osmandapp/OsmAnd-iOS/issues/5235);
- Fixed [broken UI and corrupted X-axis scale in "Analyze on Map" for segments](https://github.com/osmandapp/OsmAnd-iOS/issues/5242);
- Fixed [crash during navigation by track with or without "Snap to road" enabled](https://github.com/osmandapp/OsmAnd-iOS/issues/5153);
- Fixed [Selection Pin offset when selecting a point from the "What's here" menu](https://github.com/osmandapp/OsmAnd-iOS/issues/5224);
- Fixed [CarPlay: Next turn widget background unexpectedly turning red during navigation](https://github.com/osmandapp/OsmAnd-iOS/issues/5231);
- Fixed [3D map view not being preserved after a route preview](https://github.com/osmandapp/OsmAnd-iOS/issues/5151);
- Resolved [issue where Waypoint and folder icons would disappear after renaming a folder and restarting the app](https://github.com/osmandapp/OsmAnd-iOS/issues/5131);
- Resolved [UI issue where character descenders were cropped in the Next turn widget](https://github.com/osmandapp/OsmAnd-iOS/issues/5072);
- Resolved [issue where temporary GPX files were visible in search results](https://github.com/osmandapp/OsmAnd-iOS/issues/5138);
- Resolved [issue where vehicle parameters did not correctly respect changes to measurement units or region settings](https://github.com/osmandapp/OsmAnd-iOS/issues/2433);
- Fixed [**issue where tapping Favorites on the map would fail if the map was rotated**](https://github.com/osmandapp/OsmAnd-iOS/issues/5168);
- Fixed [issue where POIs would fail to render if the OSM notes layer was enabled under poor internet connectivity](https://github.com/osmandapp/OsmAnd-iOS/issues/5187);
- Resolved [issue where the POI context menu displayed redundant categories or tags](https://github.com/osmandapp/OsmAnd-iOS/issues/5196);
- Fixed [issue where the customized route line color would not update until the page was refreshed](https://github.com/osmandapp/OsmAnd-iOS/issues/5051);
- Fixed [issue where Bremen was displayed inconsistently compared to other German regions on the map](https://github.com/osmandapp/OsmAnd-iOS/issues/2712).
