...
Топографічна інформація, як-от [Горизонталі](#contour-lines), [Рельєф](#terrain) (*Затінення рельєфу* та *Ухил*), а також [3D-рельєф](#3d-relief), допомагає візуально оцінити місцевість, побачивши висоту, рельєф, екстремуми, крутизну або точки однакової висоти.
...
- [Затінення рельєфу](#hillshade-slope-and-altitude-layers). Типи карт із затіненням пагорбів та схилів роблять рельєф більш видимим та допомагають візуально інтерпретувати місцевість.
...
| Горизонталі | Затінення рельєфу | Ухил |
|:---|:---|:---|
...
Дані про висоту на карті (між 70 градусами північної широти та 70 градусами південної широти) були отримані в результаті вимірювань, проведених у рамках *Shuttle Radar Topography Mission (SRTM)*. У ній використовувався *Advanced Spaceborne Thermal Emission and Reflection Radiometer (ASTER)*, основний інструмент зображення в *Earth Observation System NASA*.
Для повної інформації див. [Ліцензію](https://github.com/osmandapp/OsmAnd/blob/master/LICENSE#L146).
...
Щоб відобразити дані **Горизонталей** та **Рельєфу (Затінення рельєфу, Ухил)** на карті:
...
3. [Завантажте](#download-maps): карти Горизонталей, Затінення рельєфу, Ухилу або Рельєфу (3D).
4. **Увімкніть та налаштуйте**: Горизонталі, Затінення рельєфу або Ухил для перегляду карти.
...
Щоб почати працювати з функціоналом плагіна, вам потрібно завантажити карти, які вас цікавлять. Деякі карти, наприклад, Горизонталі для гірських регіонів, можуть бути досить великими, перевищуючи 2 ГБ, і можуть не підтримуватися на застарілих пристроях.
...
Вам потрібно завантажити карти **Рельєфу (3D)**, щоб відобразити Затінення рельєфу, Ухил та 3D-рельєф. Після завантаження карт ви можете відобразити **Горизонталі** та **Рельєф** за допомогою розділу [Налаштувати карту](../map/configure-map-menu.md) *Головного меню*.

Якщо карта, що відображається на екрані, не завантажена, то в *Меню → Налаштувати карту → Розділ Топографія → Рельєф* внизу списку функцій буде відображено розділ *Завантажити карти* з запропонованими додатковими картами.
...
Для [**Горизонталей**](#contour-lines) вам потрібно визначити, в яких [одиницях](../personal/profiles.md#units--formats) (метри або фути) вони будуть відображатися на карті, та завантажити відповідну версію карти на ваш пристрій.

**Параметри одиниць не взаємозамінні**, тому якщо вам потрібно переключитися з метрів на фути або навпаки, вам потрібно видалити попередню версію карти Горизонталей, щоб завантажити нову версію.
...
[Горизонталі](../map/vector-maps.md#-contour-lines) — це графічне представлення висот на карті, доступне у вигляді векторних карт. Вони утворюють лінії, що відповідають точкам з однаковою висотою, які формують контури, що дозволяють визначити, в якому напрямку та наскільки нахилена поверхня.

При використанні [Механізму візуалізації карти](../personal/global-settings.md#map-rendering-engine):
...
- *<Translate android="true" ids="rendering_attr_contourDensity_name"/>*. Вибрати щільність горизонталей (Низька, Середня, Висока). Вища щільність може впливати на швидкість завантаження.
...
Опція **Рельєф** дозволяє увімкнути та налаштувати три функції, такі як *Затінення рельєфу*, *Ухил* та *Висота*.
Особливості:

- Одночасно можна увімкнути лише одну опцію: Затінення рельєфу, Ухил або Висота.
- Якщо ви не бачите жодних змін після завантаження та увімкнення відповідної карти, рекомендується перезапустити застосунок.

Меню **Рельєф** включає вибір [колірної схеми](#default-color-scheme) з можливістю [її зміни](#modify-color-scheme) (для [підписників Pro](../../user/purchases/index.md)), можливість зміни прозорості шару на карті ([видимість](#visibility)), вибір [рівня масштабування](#zoom-levels) для його відображення, інформацію про розмір [кешованих даних](#cache-size) та список [карт](../../user/personal/maps-resources.md), необхідних для відображення шару.


## Шари Затінення рельєфу, Ухилу та Висоти {#hillshade-slope-and-altitude-layers}

| Затінення рельєфу | Ухил | Висота |
| ------ | ------- | ------- |
...
**Затінення рельєфу** базується на симуляції освітлення поверхні за допомогою даних про рельєф. Цей метод передбачає створення тіней та світлих ділянок на основі кута поверхні відносно джерела світла. В результаті ви бачите природні пагорби, долини та інші деталі рельєфу на карті.
...
Растрові карти **Затінення рельєфу**, **Ухилу** та **Висоти** створюються на основі растрових даних рельєфу, таких як цифрові моделі висот (DEM).

**Використання:**

- *Навігація.* Допомагає визначити круті схили, як вниз, так і вгору, що може бути критично важливим для безпечної навігації.
...
- *Затінення рельєфу* використовує темні відтінки для відображення схилів, вершин та низин. Віртуальне Сонце має фіксований азимут (напрямок) 315 градусів.
...
Функція *Видимість* використовується для налаштування прозорості тіней для Затінення рельєфу та кольорів, що використовуються для представлення кута в параметрі Ухил.

### Рівні масштабування {#zoom-levels}

![Zoom](../../../blog/2023-08-28-terrain/img/zoom.png)  

Функція *Рівні масштабування* дозволяє встановити мінімальні та максимальні значення рівнів масштабування карти, від 4 до 19, на яких будуть відображатися шари карти Затінення рельєфу або Ухилу.

### Розмір кешу {#cache-size}

**Розмір кешу** — це інформаційний розділ, який відображає обсяг пам'яті на вашому пристрої, що наразі використовується для даних *Рельєфу*. Щоразу, коли ви переглядаєте інформацію про *Затінення рельєфу* або *Ухил* на карті, всі ці дані тимчасово зберігаються в кеші для швидкого доступу та подальшого використання, уникаючи додаткового навантаження на процесор вашого пристрою.
...
### Механізм візуалізації (Android) {#rendering-engine-android}

**Затінення рельєфу** та **Ухил** відображаються та налаштовуються в будь-якому вибраному [Механізмі візуалізації карти](../personal/global-settings.md#map-rendering-engine).

1. Якщо ви використовуєте **Механізм візуалізації карти Версія 1**, вам потрібно використовувати звичайне [завантаження](../start-with/download-maps.md) растрових карт Затінення рельєфу та Ухилу.

2. Якщо ви використовуєте **Механізм візуалізації карти Версія 2 (OpenGL)**:
    - Ви можете продовжувати використовувати звичайний тип завантаження растрових карт Затінення рельєфу та Ухилу. Однак для цього вам потрібно буде активувати [плагін розробки OsmAnd](../plugins/development.md) та увімкнути налаштування [Використовувати растровий формат SQLite для затінення рельєфу та ухилу](../plugins/development.md#terrain).

    - Альтернативно, ви можете використовувати [Завантаження карти рельєфу (3D)](../personal/maps-resources.md#paid-features-for-the-selected-location). Це економить місце в пам'яті на вашому пристрої, а ефекти Затінення рельєфу, Ухилу та 3D-рельєфу будуть генеруватися з них за допомогою вашого пристрою.
...
**Затінення рельєфу** — це тип карти, що відображає рельєф за допомогою тіней, створюючи візуальне представлення нахилу та форми земної поверхні.
**3D-рельєф** — це функція, що додає тривимірні ефекти до карти.

Якщо ви **вимкнете** *Затінення рельєфу* та **увімкнете** *3D-рельєф*, тіні рельєфу все одно будуть видимі, оскільки *Затінення рельєфу* та *3D-рельєф* — це два різні способи візуалізації карти. *Затінення рельєфу* створює тіні на основі рельєфу та додає їх до карти, тоді як *3D-рельєф* моделює 3D-елементи для відображення глибини та форми рельєфу, а тіні є частиною візуалізації. Ці функції можуть працювати паралельно, і вимкнення *Затінення рельєфу* не впливає на те, як відображаються 3D-ефекти.

Коли **Затінення рельєфу** **увімкнено**, зображення з тінями рельєфу виглядає більш детальним, темнішим та ступінчастим, ніж зображення *3D-рельєфу*. Пояснення полягає в тому, що *Затінення рельєфу* підкреслює градієнти та контрасти рельєфу, створюючи більш чітке та детальне зображення. Функція *3D-рельєфу* надає карті більш плавний та згладжений вигляд, пом'якшуючи рельєф та потенційно зменшуючи видимість деяких дрібніших деталей.
...
- Комбінація **Горизонталей** та **Затінення рельєфу** є оптимальною для візуальної та числової оцінки крутизни гірських схилів.
- Комбінація шарів **Горизонталей** та **Ухилу** найкраще підходить для оцінки крутизни схилів та пошуку точок з однаковою висотою.
- Комбінація шарів **3D-рельєфу** та **Затінення рельєфу** дозволяє отримати більш реалістичне та візуальне представлення рельєфу, ландшафту та деталей місцевості. Ця комбінація особливо підходить для гірської та пагорбистої місцевості.
