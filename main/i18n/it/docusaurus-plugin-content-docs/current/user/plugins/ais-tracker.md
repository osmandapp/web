...
:::caution DISCLAIMER
**Questo plugin è un progetto amatoriale e non è progettato per affidabilità o accuratezza. NON fare affidamento su questo software per la navigazione o la sicurezza della vita.**
:::
...
1. Informazioni statiche (dati invariati).
    *Inviati ogni **6 minuti** o su richiesta.*

    - **Nome della nave** (*se impostato*)
    - **MMSI (Maritime Mobile Service Identity)** (*Identificatore univoco di 9 cifre della nave*)
...
2. Informazioni dinamiche (dati in tempo reale).
    *Inviati a intervalli diversi a seconda della velocità e delle manovre.*

    - **Coordinate della nave (Latitudine e Longitudine)**
    - **Rotta sul fondo (COG)**
    - **Velocità sul fondo (SOG)**
    - **Prua** (*Direzione in cui punta la prua*)
...
### Simboli e Leggende AIS {#ais-symbols-and-legends}
...
| *Triangolo arancione*   | Imbarcazione speciale (es. pilota, rompighiaccio) |
| *Triangolo rosso lampeggiante* | Imbarcazione in difficoltà |
| | |
| **Simboli degli ausili alla navigazione** |    |
| *Boa verde*              | Segnalatore di dritta (lato destro del canale) |
| *Boa rossa*                | Segnalatore di sinistra (lato sinistro del canale) |
| *Simbolo dell'ancora*           | Area di ancoraggio |
| *Simbolo della nave (SS)*        | Terminal traghetti o passeggeri |
| *Simbolo della gru portuale*       | Porto merci |
...
| *Icona orologio*      | Ora stimata di arrivo (ETA) |
| *Icona onda*       | Velocità sull'acqua |
...
Il plugin *AIS vessel tracker* offre varie impostazioni per personalizzare la navigazione e l'interazione per gli utenti con disabilità. Queste impostazioni vengono applicate per tutti i [profili](../personal/profiles.md) in OsmAnd.
...
| Timeout per oggetti AIS persi | Le navi scompaiono se nessun segnale viene ricevuto per un tempo impostato | `3 - 20 min` |
| Timeout per la visibilità della nave | Le icone delle navi cambieranno stato quando nessun segnale viene ricevuto | `2 - 15 min / Disabilitato` |
| | | |
| **Avvisi Punto di Minimo Avvicinamento (CPA)** | | |
...
| Distanza di avviso CPA | L'imbarcazione è contrassegnata in rosso se la distanza al CPA è inferiore a questo limite | `0.02 - 2 miglia nautiche` |


### Modalità di simulazione AIS {#ais-simulation-mode}

> *Puoi simulare le posizioni delle imbarcazioni AIS utilizzando il [plugin di sviluppo OsmAnd](../plugins/development.md).*
