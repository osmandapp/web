...
- **Nella schermata Configura**. Seleziona un pulsante dall'elenco e tocca il pulsante&nbsp;  "**＋**"  &nbsp;. Devi prima selezionare il [tipo di azione](#action-types), e poi cambiare il suo nome e i [parametri](#actions-in-loop).
...
Per alcune azioni, è possibile assegnare un elenco di livelli che cambieranno in modo circolare ad ogni tocco: *Sorgente mappa*, *Stile mappa*, *Sovrapposizione*, *Sottoposizione*, *Schema colori terreno*, *Aggiungi profilo*.

Il pulsante consente di assegnare un singolo livello o un elenco di livelli che cambiano in modo circolare al tocco o con una finestra di dialogo intermedia.
...
### Cambiare Posizione sulla Mappa {#change-position-on-the-map}
...
- **[Pulsanti personalizzati](#custom-buttons) multipli**. È possibile aggiungere più pulsanti di azione rapida alla schermata della mappa. Questi pulsanti possono anche essere personalizzati e riposizionati individualmente utilizzando lo stesso metodo di trascinamento.
...
### Aspetto del Pulsante di Azione Rapida {#quick-action-button-appearance}
