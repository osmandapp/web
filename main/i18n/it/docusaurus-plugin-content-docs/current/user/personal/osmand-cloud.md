...
OsmAnd Cloud ti consente di **archiviare e sincronizzare impostazioni personali e dati dell'applicazione** su più dispositivi. Puoi accedere a questi dati su qualsiasi dispositivo in cui è installato OsmAnd, inclusa l'app desktop. Tutte le modifiche apportate all'interno dell'app possono essere **sincronizzate tra le piattaforme**, garantendo un accesso senza interruzioni ai dati salvati.

## Multipiataforma {#cross-platform}

OsmAnd Cloud ti consente di eseguire il backup, ripristinare e sincronizzare i tuoi dati su Android, iOS e la [versione web](../web/index.md). Con le funzionalità multipiattaforma, puoi trasferire **impostazioni del profilo**, **tracce**, **preferiti**, **modifiche/note OSM** e altro ancora tra i dispositivi. Inoltre, la **versione web** ti consente di pianificare percorsi e salvarli per un uso successivo sui dispositivi connessi.
...
1. **Registra un [account](#login) su OsmAnd Cloud**:
   ...
   Scopri di più sull'acquisto per [Android](../purchases/android.md#how-to-buy) o [iOS](../purchases/ios.md#how-to-buy).
    ...
    2.1 **Usa il piano gratuito OsmAnd Start** per i backup cloud di preferiti, impostazioni e modifiche/note OSM.
   Il limite di archiviazione gratuito è di **5 MB**.

2. **Usa l'email registrata** come login per attivare [OsmAnd Pro](../purchases/cross.md), [OsmAnd Maps+](../purchases/cross.md) o OsmAnd Start su altre piattaforme.

:::note
**OsmAnd Pro** è un abbonamento multipiattaforma che include tutte le funzionalità di OsmAnd. Puoi acquistarlo in un negozio (ad esempio, Google Play, Huawei AppGallery) e usarlo anche su **iOS**. La tua **[email registrata](#login)** è il tuo login per attivare OsmAnd Pro [su tutte le piattaforme](../purchases/cross.md).
:::
...
Per eseguire il backup e ripristinare i dati, devi aver effettuato l'accesso al tuo account OsmAnd Cloud.
...
- Il tuo account Cloud **deve essere registrato sul dispositivo in cui è stato originariamente effettuato l'acquisto**.

- *Processo di accesso*:
  - Inserisci il tuo indirizzo email e tocca **Continua**.
  - Un codice di verifica verrà inviato alla tua email per la conferma.
  ...
  - Usa questo account per accedere a OsmAnd Cloud su altri dispositivi.
...
:::tip Risoluzione dei problemi di accesso
...
Il piano **OsmAnd Start** fornisce accesso gratuito a **OsmAnd Cloud** con funzionalità di base limitate e 5 MB di spazio di archiviazione per archiviare e sincronizzare i tuoi **Preferiti, Impostazioni e modifiche e note di OpenStreetMap (OSM)**. Questo piano è disponibile per gli utenti di [OsmAnd Free o Maps+](../purchases/index.md).

Funzionalità incluse in OsmAnd Start:

- [Backup gratuito dei preferiti](../personal/favorites.md#free-cloud-backup) – Archivia posizioni e percorsi importanti nei tuoi Preferiti e accedivi da qualsiasi dispositivo connesso a OsmAnd Cloud.
- [Backup gratuito delle impostazioni](../personal/profiles.md#free-cloud-backup) – Salva le impostazioni dell'app OsmAnd su OsmAnd Cloud e ripristinale su altri dispositivi per un'esperienza senza interruzioni.
...
![Funzioni della cartella Preferiti Android](@site/static/img/personal/favorites_free_backup_purch_andr.png)
...
**Hai bisogno di un account OsmAnd Cloud per usare OsmAnd Start**.

Per usare OsmAnd Start, segui questi passaggi:

- **Registrati per un account OsmAnd Cloud** - Vedi la sezione [Accesso](#login) per le istruzioni.
- Apri *Menu → Impostazioni → Acquisti*.
- Seleziona **OsmAnd Start** dalle opzioni disponibili.
- **Crea un backup** per iniziare a memorizzare i tuoi dati in OsmAnd Cloud.

:::note Restrizioni sull'abbonamento
...
- *Modifiche locali*. Mostra il numero totale di modifiche locali non caricate nel cloud.
- *Modifiche cloud*. Il numero totale di modifiche caricate nel cloud da altri dispositivi dall'ultima sincronizzazione.
...
**Sincronizza ora** combina i pulsanti *Carica tutto* e *Scarica tutto* nel menu Modifiche, nelle sezioni [Locale e Cloud](#local-and-cloud). I **conflitti** non vengono sincronizzati in questo modo.

<!--
#### Sincronizzazione delle raccolte GPX {#syncing-gpx-collections}

OsmAnd supporta il raggruppamento delle tracce GPX in **raccolte**, il che consente di organizzare le tracce correlate sotto un'unica etichetta.

Quando si utilizza **OsmAnd Cloud**, le raccolte GPX sono:

- Eseguite il backup insieme ai dati personali.
- Sincronizzate tra i dispositivi.
- Ripristinate con la stessa **struttura della raccolta** (incluso il nome e tutte le tracce associate).

> *Ciò garantisce che l'organizzazione del percorso sia preservata, anche dopo aver reinstallato l'app o essere passati a un nuovo dispositivo.*

Per utilizzare questa funzione:

1. Crea o assegna tracce a una raccolta tramite *Menu → I miei luoghi → Tracce*.
2. Vai a *Menu → Impostazioni → OsmAnd Cloud → Esegui il backup dei dati* e abilita la sincronizzazione.
3. Su un altro dispositivo, accedi con lo stesso account OsmAnd Cloud e ripristina i dati.
-->
...
La funzione **Cestino** ti consente di visualizzare e gestire i file che sono stati eliminati da OsmAnd Cloud. È progettata per prevenire l'eliminazione accidentale di file e la perdita irreversibile dei tuoi dati.

- **Visualizza file**. *Cestino* elenca tutti i file precedentemente eliminati dalla cartella *Modifiche Cloud*. Tutti i file contengono informazioni sulla data e l'ora in cui sono stati eliminati. Sono raggruppati per mese e ordinati dal più recente al più vecchio tra i file rimasti nel Cestino.
- **Svuota Cestino**.

    ![Cestino OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_trash_3_andr.png)

    Per liberare spazio da tutti i file eliminati dal Cloud, puoi usare la funzione *Svuota Cestino*. È disponibile dal *menu a tre punti* nell'angolo in alto a destra della schermata del Cestino. I file vengono eliminati definitivamente, senza possibilità di ripristinarli.

- **Gestione di file separati**.

    ![Cestino OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_trash_4_andr.png)

    - *<Translate ios="true" ids="restore_from_trash"/>*. Con questa funzione, puoi ripristinare i file su OsmAnd Cloud. Dopo aver toccato, il file selezionato viene spostato dal Cestino nella sezione *Modifiche* alla cartella *Cloud*.
    - *<Translate ios="true" ids="download_to_device"/>*. Questa funzione ti consente di scaricare un file sul dispositivo. Se il file selezionato con lo stesso nome è presente nella cartella, viene visualizzata la finestra di dialogo *Il file esiste già*, in cui puoi selezionare un'opzione di azione: *Sostituisci* o *Mantieni entrambi*.
    - *<Translate ios="true" ids="shared_string_delete_immediately"/>*. Questa funzione ti consente di eliminare definitivamente il file selezionato senza poterlo ripristinare. Fai attenzione quando usi questa funzione poiché l'operazione è irreversibile.
...
Questa sezione è dedicata alla modifica e alla gestione del tuo OsmAnd Cloud.

### Seleziona i dati da sottoporre a backup {#select-data-to-back-up}
...
In questo menu, puoi selezionare quali dati e cartelle caricare nel Cloud:

1. **Impostazioni**. Questa scheda include tutte le tue impostazioni, come preferenze generali, profili specifici, azioni rapide, POI e impostazioni di evitamento delle strade.
...
3. **Risorse**. Qui, puoi scegliere di eseguire il backup di risorse come preferiti, informazioni di routing, istruzioni vocali ed elementi aggiuntivi.

Tutte le [mappe](../start-with/download-maps.md) sono memorizzate sul dispositivo su cui sono state precedentemente scaricate. OsmAnd Cloud memorizza solo le informazioni su quali mappe hai e la data dell'ultima modifica.
...
![Schermata di backup dei dati iOS](@site/static/img/personal/osmand-cloud/cloud_ios_13.png)

</TabItem>

</Tabs>

La sezione del menu **<Translate android="true" ids="backup_version_history"/>** (Android) / **<Translate ios="true" ids="manage_storage"/>** (iOS) mostra informazioni su quanto dei 3,15 gigabyte di memoria in *Impostazioni*, *I miei luoghi* e *Risorse* viene utilizzato. Puoi eliminare le modifiche a tipi di dati separati da OsmAnd Cloud, ma in questo caso, l'azione non può essere annullata.

### Account {#account}

![Modifiche OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_9-1.png)

Questa sezione contiene informazioni su quale dei tuoi account è connesso a OsmAnd Cloud e include un pulsante di disconnessione. Per eseguire il backup o ripristinare i dati, dovrai accedere di nuovo.
...
Le azioni che puoi eseguire nella sezione **Zona di pericolo** del menu Impostazioni sono irreversibili e richiedono quindi un'attenzione particolare.
...
2. **Rimuovi vecchie versioni**. Questa opzione ti consente di eliminare la cronologia delle modifiche per i dati precedentemente caricati. La versione corrente dei dati verrà conservata sul server, ma non sarai in grado di tornare alle versioni precedenti.
...
Nella sezione [Ultima sincronizzazione](#last-sync) del menu a discesa, tocca qualsiasi elemento per aprire la sezione Modifiche. Questa sezione contiene tre schede che mostrano tutte le modifiche disponibili per la modifica. Se l'elenco è vuoto, non si sono verificate modifiche dall'ultima sincronizzazione.

- Ogni elemento nell'elenco **Modifiche** include la **data e l'ora esatta** dell'ultima sincronizzazione, insieme alle opzioni per scaricare o caricare la versione corrispondente istantaneamente.

- **Il download delle modifiche dall'archivio cloud sovrascriverà le modifiche locali**. Allo stesso modo, il caricamento dei dati locali rimuove le modifiche corrispondenti dalla versione cloud.

- In ogni scheda, i dati sono **ordinati per tipo e alfabeticamente** per aiutarti a trovare rapidamente elementi specifici, soprattutto quando gestisci più file in OsmAnd Cloud.
...
- **Locale** - Puoi sincronizzare tutte le modifiche locali che non sono ancora nel cloud toccando **Carica tutto**. In alternativa, puoi caricare modifiche specifiche selezionandole dall'elenco e scegliendo **Carica versione locale**.
    ...
    Le modifiche che vengono caricate con successo nel cloud vengono automaticamente rimosse dalla *scheda Modifiche → Locale* dopo un periodo di tempo impostato.

- **Cloud** - La scheda **Cloud** mostra un elenco di modifiche disponibili per il download dall'archivio cloud. Puoi scaricare tutte le modifiche contemporaneamente usando il pulsante nella parte inferiore dello schermo o scaricarle individualmente.
...
![Modifiche OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_ios_7.png)
...
- Toccando un file nella cartella **Conflitti** vengono visualizzati i dettagli sulle versioni **Locale** e **Cloud**, inclusi l'ultima modifica e i tempi di caricamento.
...
<!--
## Guide alle azioni {#action-guides}

- Ripristina i tuoi dati dopo la scadenza degli acquisti.
- È assolutamente fondamentale che il tuo account Cloud sia registrato sul dispositivo in cui è stato originariamente effettuato l'acquisto.
- Accesso per un altro anno dopo la scadenza dell'abbonamento.

:::tip
Se hai problemi con la registrazione del tuo account:

- Controlla se ti stai registrando sul dispositivo su cui è stato effettuato l'acquisto.
- Devi avere una connessione Internet attiva.
