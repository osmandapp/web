...
La funzionalità [Punto di Interesse (POI) di OsmAnd](https://wiki.openstreetmap.org/wiki/Points_of_interest) in OsmAnd offre precisione, visibilità e comodità nel trovare luoghi interessanti, percorsi e servizi vicino alla tua posizione attuale o a un'area selezionata sulla mappa.
...
- Il [pulsante di ricerca](../widgets/map-buttons.md#search) è sempre visualizzato sulla mappa, e toccandolo verrai indirizzato alla [schermata generale](#how-to-use) dello strumento, dove puoi trovare la scheda *Categorie*.
- *Android*: Vai al *Menu principale → Cerca → scheda Categorie*.
- Quando ti prepari a iniziare un percorso, tocca [*Navigazione → Imposta destinazione → campo Cerca → scheda Categorie*](../navigation/setup/route-navigation.md#set-target-point).
- *Android*: Vai al [*Menu principale → Configura mappa → Sovrapposizione POI → Cerca*](../map/point-layers-on-map.md#points-of-interest-pois).
...
- **Ricerca Punti di Interesse (POI):**
    - Consente di cercare:
       - punti di interesse (POI) vicini o di categorie specifiche.
...
- **Ricerca per marchio:**
    - Quando si inserisce il nome di un'attività commerciale o di un marchio (come Audi, Starbucks o Aldi), l'elenco dei risultati della ricerca viene ordinato con priorità per nome del POI e ordinato per distanza dalla posizione corrispondente.

    - Il nome della località viene visualizzato accanto a ogni risultato della ricerca per aiutarti a identificare la posizione corretta tra le altre a una distanza simile dalla tua.

    - Questo tipo di ricerca è conveniente da usare se un punto vendita al dettaglio è specializzato nella vendita di beni o servizi di un marchio o se il marchio stesso è più conosciuto e riconoscibile del nome o della categoria del POI.
...
:::note
Per svolgere alcune di queste attività (individuare indirizzi, POI) dovrai disporre del file della mappa vettoriale offline. Inizialmente, la ricerca si basa sui dati presenti sulla mappa nell'area visibile dello schermo del dispositivo. Se non trovi nulla, OsmAnd propone di aumentare il raggio di ricerca.
:::
...
Lo strumento **Ricerca per categorie** ti consente di trovare rapidamente oggetti, luoghi e percorsi classificati in diverse categorie. Ogni categoria ha un insieme unico di caratteristiche, e questo strumento dispone di filtri che ti consentono di affinare i risultati della ricerca selezionando diversi valori per caratteristiche aggiuntive.
...
- *Personalizzazione dell'utente* - I filtri delle caratteristiche consentono di personalizzare le ricerche per soddisfare le loro esigenze e preferenze specifiche.
- *Risultati raffinati* - I filtri aiutano a raffinare i risultati della ricerca, rendendoli più pertinenti e accurati.
- *Categorie multiple* - Il database OpenStreetMap dispone di un ampio set di categorie POI, che ti consente di trovare siti di diversi tipi e categorie.

<!--
La ricerca per categoria POI ti consente di trovare e selezionare rapidamente luoghi di interesse in base alle tue esigenze. È uno strumento utile per viaggiare, trovare servizi o luoghi di interesse nelle vicinanze e pianificare percorsi basati sulle categorie POI selezionate.

OsmAnd inizia a trovare nomi e categorie di POI in base alle parole inserite. I primi risultati saranno le categorie, i secondi risultati saranno i POI con informazioni aggiuntive, nome completo, nome della categoria, direzione e distanza dal POI, orario di lavoro. Premendo sulla categoria necessaria si apre l'elenco dei POI di questa categoria.
...
Con la funzione **Filtro**, puoi selezionare le caratteristiche necessarie e ottenere come risultato un elenco contenente solo oggetti pertinenti. Per utilizzare il filtro:
...
Un filtro può essere composto da un numero considerevole di elementi, la cui presenza dipende dalla categoria selezionata. Ogni categoria ha un numero di filtri specifici per essa, organizzati in cartelle per tipo. Ci sono in totale *22 categorie predefinite*. Alcune di esse sono elencate qui:

 1. **<Translate android="true" ids="poi_filter_accomodation"/>**. [Alloggi](https://wiki.openstreetmap.org/wiki/Key:building#Accommodation) include strutture che offrono alloggi temporanei come hotel, motel, ostelli, pensioni e campeggi. Questi luoghi offrono servizi e comfort per un soggiorno confortevole durante i viaggi.
 ...
 2. **Caffè e ristorante**. [Caffè](https://wiki.openstreetmap.org/wiki/Tag:amenity%3Dcafe) e [ristoranti](https://wiki.openstreetmap.org/wiki/Tag:amenity%3Drestaurant) rappresentano luoghi di ristorazione.
 ...
 3. **Stazione di ricarica**. Una [stazione di ricarica](https://wiki.openstreetmap.org/wiki/Tag:amenity%3Dcharging_station) è un'infrastruttura dove i proprietari di veicoli elettrici possono ricaricare le loro auto, motociclette o altri veicoli elettrici. Questi punti forniscono caricabatterie e connessioni appropriate per fornire energia alle batterie dei veicoli.
        Include *35* tipi di filtri: **Accesso biciclette** (ha *1* valore), **Accesso autobus** (ha *1* valore), **Accesso HGV** (ha *5* valori), **Accesso autovetture** (ha *1* valore), **Accesso scooter** (ha *1* valore), **Autenticazione tramite app** (ha *1* valore), **Autenticazione tramite chip card** (ha *1* valore), **Autenticazione contactless** (ha *1* valore), **Autenticazione tramite chiave** (ha *1* valore), **Autenticazione richiesta** (ha *1* valore), **Autenticazione tramite chiamata telefonica** (ha *1* valore) **Autenticazione tramite messaggio breve** (ha *1* valore), **Tariffa** (ha *2* valori), **Tipo di accesso a Internet** (ha *3* valori), **Tipo di accesso a Internet** (ha *3* valori), **Accessibilità per sedie a rotelle** (ha *4* valori), Aperto ora / Aperto 24/7, e altri.
 ...
 4. **Minimarket e supermercato**. [Minimarket](https://wiki.openstreetmap.org/wiki/Tag:shop%3Dconvenience) e [supermercato](https://wiki.openstreetmap.org/wiki/Tag:shop%3Dsupermarket) rappresentano strutture di vendita al dettaglio.
        Include *13* tipi di filtri: **Acquisto all'ingrosso** (ha *2* valori), **Prelievo contanti** (ha *5* valori), **Fasciatoio** (ha *3* valori), **Caffè** (ha *2* valori), **Consegna** (ha *1* valore), **Dieta** (ha *8* valori), **Gelato** (ha *1* valore), **Prodotti biologici** (ha *2* valori), **Tipo di pagamento** (più di *10* valori), **Filtro usato** (ha *2* valori), **Self-checkout** (ha *2* valori), **Accessibilità per sedie a rotelle** (ha *4* valori), Aperto ora / Aperto 24/7.

 5. **<Translate android="true" ids="poi_filter_emergency"/>**. [Emergenza](https://wiki.openstreetmap.org/wiki/Key:emergency) include strutture e luoghi associati ai servizi di emergenza come ospedali, stazioni di polizia, caserme dei pompieri e centri medici di primo soccorso.
        Non contiene alcun filtro.

 6. **Stazione di servizio**. Una [stazione di servizio](https://wiki.openstreetmap.org/wiki/Tag:amenity%3Dfuel), nota anche come stazione di rifornimento, stazione di benzina, stazione di gas e garage di benzina. È la struttura di tipo al dettaglio dove i veicoli a motore possono essere riforniti.
 ...
 7. **Finanza**. [Finanza](https://wiki.openstreetmap.org/wiki/Tag:office%3Dfinancial) utilizzata per gli uffici di aziende nel settore finanziario.
 ...
 12. **<Translate android="true" ids="poi_filter_closest_poi"/>**. Include POI che sono vicini a te.
 ...
 22. **Turismo**. [Turismo](https://wiki.openstreetmap.org/wiki/Key:tourism) include luoghi e cose di particolare interesse per i turisti, inclusi luoghi da vedere, luoghi dove soggiornare e cose e luoghi che forniscono informazioni e supporto ai turisti.
        Include *32* tipi di filtri.

 23. **Acqua**. Fonti di [acqua potabile](https://wiki.openstreetmap.org/wiki/Key:drinking_water) create o organizzate dall'uomo.
        Non contiene alcun filtro.
...
- Sposta le categorie trascinandole e rilasciandole nella posizione corretta nell'elenco.
...
- Utilizza il pulsante **Ripristina** predefinito per riportare l'elenco allo stato preimpostato.
...
- Utile nelle città e sulle autostrade con buona copertura Internet.
- La ricerca online è conveniente da usare se non ci sono mappe della regione e se è impossibile scaricarle.
...
Per migliorare i risultati, prima di utilizzare lo strumento di ricerca, ingrandisci la mappa per avvicinarti a dove devi trovare una posizione specifica. E durante la ricerca, puoi **aumentare il raggio di ricerca** per trovare luoghi più lontani dalla tua posizione. Quando cerchi offline, se i risultati richiesti non sono disponibili, OsmAnd suggerisce di utilizzare la ricerca online.
...
*Nominatim* in OsmAnd viene utilizzato come motore di ricerca per i dati di OpenStreetMap, con il quale è possibile cercare per nome (la ricerca per indirizzo è attualmente disabilitata). Ogni risultato ha un link a una pagina di dettaglio per vedere quali dati sull'oggetto sono memorizzati nel database.
...
La ricerca POI personalizzata è una funzionalità di ricerca avanzata che ti consente di trovare POI da diverse categorie e combinarli per una ricerca più precisa e personalizzata. A differenza della ricerca POI standard, che è limitata ad alcune categorie predefinite, la ricerca POI personalizzata offre la flessibilità di selezionare e combinare diversi tipi di POI.

Ad esempio, puoi cercare ristoranti e farmacie contemporaneamente per trovare il posto più vicino dove cenare e poi prendere le tue medicine. Oppure cercare la stazione di servizio e il servizio auto più vicini quando hai un problema con la tua auto.

Questa funzionalità è particolarmente utile in vari casi:

- *Viaggio*. Quando viaggi, utilizzando la ricerca personalizzata puoi trovare rapidamente diversi luoghi di interesse e combinarli per creare un percorso ottimale.
...
- *Ricerca di servizi nelle vicinanze*. La ricerca personalizzata ti consente di combinare diverse categorie POI per una facile ricerca se hai bisogno di trovare diversi tipi di servizi in un determinato quartiere o in un determinato tratto del percorso.
...
<!--
![POI personalizzati iOS](@site/static/img/search/custom_poi_ios.png) ![Tipi di POI personalizzati iOS](@site/static/img/search/custom_poi_types_ios.png)
-->

![Filtro POI personalizzato iOS](@site/static/img/search/custom_poi_filter_1_ios.png) ![Tipi di filtro POI personalizzato iOS](@site/static/img/search/custom_poi_filter_2_ios.png)

</TabItem>

</Tabs>

Il *filtro POI personalizzato* include insiemi di categorie di servizi composte da valori pertinenti. OsmAnd ti consente di raccogliere e combinare i tipi di POI richiesti da una o più categorie suggerite.

Per creare filtri personalizzati per trovare luoghi sulla mappa, seleziona le categorie e sottocategorie pertinenti di interesse. Puoi [salvare](#save-new-custom-filters) questi filtri per un uso successivo o [modificarli](#edit-an-existing-filter) se necessario. In alternativa, puoi visualizzare i dati filtrati senza salvare toccando il pulsante *Mostra*. Tuttavia, nota che se non salvi i filtri, tutte le selezioni verranno annullate una volta che cambi ricerca o esci dallo strumento di ricerca.

- Ogni categoria di servizi contiene un elenco di valori generici disponibili per i tipi di POI.
...
1. **<Translate android="true" ids="amenity_type_administrative"/>**. Include 25 tipi: *Allotments, Atoll, Borough, City, City block, Country, Courthouse, Customs, Diplomatic office, Farm, Government, Hamlet, Island, Islet, Isolated dwelling, Locality, Neighbourhood, Police, Prison, Quarter, Residential area, Suburb, Town, Townhall, Village*.
...
3. **<Translate android="true" ids="amenity_type_education"/>**. Include 16 tipi: *Allotments, Atoll, Borough, City, City block, Country, Courthouse, Customs, Diplomatic office, Farm, Government, Hamlet, Island, Islet, Isolated dwelling, Locality, Neighbourhood, Police, Prison, Quarter, Residential area, Suburb, Town, Townhall, Village*.
...
12. **<Translate android="true" ids="amenity_type_military"/>**. Include 7 tipi: *Area di pericolo, Bunker militare, Base navale militare, Ufficio militare, Campo militare, Zona militare, Sito di esplosione nucleare*.
...
- I POI religiosi basati sul tag `amenity=place_of_worship` sono divisi in luoghi specifici religiosi come chiese, moschee o sinagoghe.
- I servizi minori come "panchina", "panchina per giovani" (`amenity=bench`) sono disponibili nell'elenco delle categorie.
:::
-->
...
- Seleziona *[categoria](#poi-search-by-categories) → [icona filtro](#types-of-filters) →* seleziona *tipi* di caratteristiche *→ menu a tre puntini → Salva filtro*.
...
- Tocca il *menu a tre puntini*.
- Nel menu a discesa, seleziona **Modifica filtro**.
- Per salvare una ricerca POI personalizzata dopo la modifica:
    - Seleziona le categorie POI e i tipi di filtro per esse *→* tocca *Mostra → menu a tre puntini → Salva come →* inserisci un nuovo nome per il filtro o *salva* le modifiche a quello corrente.
...
- Vai alla scheda [Categorie](#poi-search-by-categories) e seleziona quella richiesta.
- Nel campo accanto a *Mostra sulla mappa*, tocca l'icona che rappresenta il *filtro*.
- Nel menu *Filtri*, tocca il *menu a tre puntini* ( &#8285; ) situato nell'angolo in alto a destra dello schermo.
