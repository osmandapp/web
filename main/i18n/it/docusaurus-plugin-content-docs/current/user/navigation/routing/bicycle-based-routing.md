...
La navigazione per bicicletta considera fattori come la disponibilità di piste ciclabili, sentieri e strade sicure e legali per i ciclisti. OsmAnd utilizza sempre i dati di elevazione per calcolare il tempo stimato di arrivo (tempo per coprire la distanza) secondo la [regola di Naismith](https://en.wikipedia.org/wiki/Naismith%27s_rule#Scarf's_equivalence_between_distance_and_climb). Per impostazione predefinita, viene fornito il percorso più veloce. La navigazione per bicicletta considera anche fattori aggiuntivi come la [larghezza](../guidance/vehicle-parameters.md#limits) della tua bicicletta per aiutare a evitare piste ciclabili strette.  
...
![Impostazioni di navigazione per bicicletta](@site/static/img/navigation/routing/cycling_routing_andr.webp)
