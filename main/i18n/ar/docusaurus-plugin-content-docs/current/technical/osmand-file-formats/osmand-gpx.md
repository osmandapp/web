...
GPX (تنسيق تبادل GPS) هو معيار قائم على XML شائع الاستخدام لتخزين بيانات GPS، بما في ذلك المسارات والطرق ونقاط الطريق. يدعم OsmAnd تنسيق GPX لاستيراد وتصدير وتخصيص بيانات GPS للملاحة، مما يمكّن المستخدمين المبتدئين والمتقدمين من تحسين تجربتهم.
...
1. كيفية تخصيص المسارات ونقاط الطريق باستخدام وسوم GPX.
2. الميزات المتقدمة مثل الأوصاف بتنسيق HTML وبيانات المستشعرات وأنواع الأنشطة.
3. كيفية تصدير المسارات المحسوبة والحفاظ على وظائفها الكاملة.
4. تحويل ملفات GPX إلى تنسيق OBF للتخزين المحسن وإمكانيات البحث المتقدمة.
...
عند استيراد ملف GPX، يقوم OsmAnd بإنشاء كائن مسار واحد بناءً على الملف:
- يُشتق اسم المسار المستورد من اسم ملف GPX. لا يُستخدم عنصر `<name>` داخل `<trk>` كاسم المسار الأساسي أثناء الاستيراد.
- إذا كان ملف GPX يحتوي على عدة عناصر `<trk>`، يستورد OsmAnd هذه العناصر كمقاطع لنفس المسار. في هذه الحالة، قد تُستخدم قيم `<name>` كتسميات للمقاطع المقابلة داخل المسار.
...
| `<vertical_exaggeration_scale>` | - مضاعف لتغيير حجم قيمة السمة `line_3d_visualization_by_type`. <br/> - *عائم:* الافتراضي: 1.0 |
| `<elevation_meters>` | - يحدد ارتفاعًا ثابتًا بالأمتار لـ `"fixed_height"` في `<line_3d_visualization_by_type>`. <br/> - *عائم:* الافتراضي: `1000` |
...
1. سيصنف هذا المسار ضمن مجموعة **ركوب الدراجات النارية** بنوع **ركوب الدراجات النارية على الطرق الوعرة (Dirt Biking)**.
