...
| `<split_type>` | - يحدد نوع تقسيم المسار. <br/> - *سلسلة نصية:* `"no_split"`، `"distance"`، `"time"` |
| `<split_interval>` | - يحدد الفاصل الزمني لتقسيم المسار بناءً على النوع المحدد. <br/> - *عدد عشري:* عدد صحيح (أمتار لـ `"distance"`، ثوانٍ لـ `"time"`) |
...
| `<vertical_exaggeration_scale>` | - مضاعف لتوسيع قيمة السمة `line_3d_visualization_by_type`. <br/> - *عدد عشري:* الافتراضي: 1.0 |
...
| `<color_palette>` | - يحدد نظام الألوان للمسار. <br/> - *سلسلة نصية:* `"default"` / [أنظمة يحددها المستخدم](/docs/user/personal/color-palette-schemes) |
