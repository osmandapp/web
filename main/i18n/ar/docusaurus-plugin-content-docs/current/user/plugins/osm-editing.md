...
[OpenStreetMap](https://www.openstreetmap.org/about) (OSM) هو مجتمع عالمي يهدف إلى إنشاء خريطة جغرافية مجانية مفصلة للعالم وتوفير بيانات مفتوحة المصدر ومحدثة لكل مستخدم. يتيح لك مكون OpenStreetMap الإضافي للمساهمة في المجتمع.

باستخدام OsmAnd ومكون تحرير OSM الإضافي، يمكنك بسهولة المساهمة بمعلوماتك الخاصة في OpenStreetMap.org، مثل إنشاء أو تعديل [نقاط الاهتمام (POIs)](#create--modify-poi)، إضافة أو التعليق على [الملاحظات](#create--modify-osm-note)، وتحميل [المسارات](#upload-gps-track) المسجلة بصيغة GPX.

## معلمات الإعداد المطلوبة {#required-setup-parameters}

لاستخدام مكون *تحرير OpenStreetMap* الإضافي، يجب عليك إجراء الإعدادات التالية:

1. تمكين [المكون الإضافي](../plugins/index.md#enable--disable).
2. تعيين تحرير OpenStreetMap في أي [ملف تعريف](../personal/profiles.md).
...
![إعدادات مكون OpenStretMap الإضافي للتحرير أندرويد](@site/static/img/plugins/osm-editing/osm_plugin_settings_android-2.png)
...
![إعدادات مكون OpenStretMap الإضافي للتحرير أندرويد](@site/static/img/plugins/osm-editing/osm_plugin_settings_ios-00.png)
...
- [تسجيل الدخول إلى OpenStreetMap](#authorization). تسجيل الدخول لتحميل التغييرات الجديدة أو المعدلة. استخدم طريقة *OAuth 2.0* الآمنة أو [سجل الدخول على موقع OSM](https://www.openstreetmap.org/login). بالنسبة لنظام iOS، يتوفر زر طريقة OAuth فقط للمستخدمين الذين لديهم إصدار مدعوم من iOS، 16.4
...
- **<Translate android="true" ids="use_dev_url"/>** *(أندرويد فقط)*. يتيح لك تمكين [dev.openstreetmap.org](https://dev.openstreetmap.org/) بدلاً من [openstreetmap.org](http://openstreetmap.org/) لاختبار تحميل ملاحظات OSM أو نقاط الاهتمام (POI) أو مسارات GPS. عند التمكين، يتم تسجيل خروجك تلقائيًا من OpenStreetMap.org. هذا الإعداد متاح فقط في القائمة عند تمكين *مكون OsmAnd الإضافي للتطوير*.
- [<Translate android="true" ids="map_updates_for_mappers"/>](#free-map-updates-for-mappers).
- **<Translate android="true" ids="layer_osm_edits"/>**. انقر لفتح [*القائمة ← أماكني ← تعديلات OSM*](../personal/myplaces.md). تسرد هذه علامة التبويب ملاحظات OSM غير المحررة. الإجراءات التالية متاحة مع الملاحظة: *تحميل*، *إظهار على الخريطة*، *تعديل ملاحظة OSM*، *حذف*. لا يتم عرض الملاحظات المحملة أو المحذوفة في القائمة بعد الآن.
...
يتيح لك مكون OpenStreetMap الإضافي للتحرير إنشاء كائنات جديدة على الخريطة تسمى [نقاط الاهتمام أو POI](../map/point-layers-on-map.md#points-of-interest-pois). يمكنك رسم خريطة لمتجر جديد، معلم مفضل، مقعد، أو مأوى حتى يتمكن الناس من العثور عليها.
...
2. اختر [<Translate android="true" ids="context_menu_item_create_poi"/>](../map/map-context-menu.md#-create--modify-poi)، وأضف اسمها ومعلومات أخرى مثل ساعات العمل، الموقع الإلكتروني، إلخ. عندما تحدد نقطة اهتمام لم يتم تحميلها، تتغير *إنشاء نقطة اهتمام* إلى *تعديل نقطة اهتمام*.
...
يمكنك بسهولة إضافة [علامات](https://wiki.openstreetmap.org/wiki/Tags) OSM إلى نقطة الاهتمام (POI). عندما تدخل عدة أحرف في شريط البحث، يقترح البرنامج علامات محتملة.

**تعديلات دون اتصال.**
يمكنك عرض نقاط الاهتمام (POIs) أو تحميلها إلى OSM أو حذفها في *أماكني ← تعديلات OSM*. يمكنك أيضًا تصدير تغييرات نقاط الاهتمام (POIs) الخاصة بك إلى [ملف OSC](https://wiki.openstreetmap.org/wiki/OsmChange) واستيرادها إلى [JOSM](https://wiki.openstreetmap.org/wiki/JOSM).
...
5. انقر على *إضافة علامة* لإضافة نقطة الاهتمام الجديدة إلى قاعدة البيانات الخاصة بك.
...
- يمكنك تحميل التغييرات بمجرد توفر اتصال بالإنترنت. يمكنك أيضًا التراجع عن التغييرات قبل تحميلها.
...
![مكون OpenStretMap الإضافي لتحرير GPX إلى OSM أندرويد](@site/static/img/plugins/osm-editing/osm_plugin_gpx_to_osm_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![مكون OpenStretMap الإضافي لتحرير GPX إلى OSM iOS](@site/static/img/plugins/osm-editing/osm_plugin_gpx_to_osm_ios.png)
...
- **<Translate android="true" ids="shared_string_description"/>**. يتيح لك إضافة وصف للمسار. ينطبق الوصف على جميع المسارات المحددة. إذا لم تدخل شيئًا في الحقل، يتم استخدام أسماء المسارات لكل مسار منفصل كأوصاف.
- **<Translate android="true" ids="gpx_tags_txt"/>**. يتيح لك هذا العنصر في قسم الإعدادات إضافة علامات لتحديد المسار. العلامة الافتراضية هي "osmand"، يمكنك إدخال علامات إضافية مفصولة بفواصل.
- **<Translate android="true" ids="gpx_visibility_txt"/>**. رؤية المسار لمستخدمي OSM:
   - *عام* يعني أن المسار متاح للجمهور ويتم عرضه في شكله الخام في مسارات GPS الخاصة بك، وقوائم مسارات GPS، وقوائم المسارات ذات الطوابع الزمنية. البيانات المرسلة عبر واجهة برمجة التطبيقات (API) لا يتم الإشارة إليها بصفحة المسار الخاصة بك. الطوابع الزمنية لنقاط التتبع غير متاحة عبر واجهة برمجة تطبيقات GPS العامة ولا يتم ترتيب نقاط التتبع زمنيًا.
   - *قابل للتعريف* يعني أن المسار سيتم عرضه للجمهور في نقاط مسار GPS الخاصة بك وقوائم مسار GPS العامة، مما يعني أن المستخدمين الآخرين سيكونون قادرين على تنزيل المسار الخام وربطه باسم المستخدم الخاص بك. بيانات نقاط المسار ذات الطوابع الزمنية العامة من واجهة برمجة تطبيقات GPS التي تمر عبر واجهة برمجة تطبيقات نقاط المسار سترتبط بصفحة المسار الأصلية الخاصة بك.
...
يمكنك إنشاء ملف OBF باستخدام [OsmAndMapCreator](../../technical/map-creation/create-offline-maps-yourself.md#osmandmapcreator) إذا كان لديك قاعدة بيانات كبيرة من مسارات GPX. هذه مجموعة من المسارات من osmand.net بعلامة 'gpsies'، ومجموعة من المسارات التي تم تنزيلها من OpenStreetMap أو مجموعتك الخاصة.
