...
sidebar_position: 4
title:  الملاحظات الصوتية / الفيديو
---
...
يمكن أن تكون تسجيلاتك الصوتية والفيديو مفيدة في المستقبل، مثل ذكريات للأماكن التي زرتها. وهي متاحة في قائمة [أماكني](../personal/myplaces.md) وتكون مرئية عند تمكين إضافة الملاحظات الصوتية/الفيديو.

توسع إضافة الملاحظات الصوتية/الفيديو من وظائف OsmAnd من خلال السماح لك بإنشاء ملاحظاتك الخاصة بتنسيقات مختلفة، مثل الصور أو الفيديو أو الصوت، وربطها بموقعها الجغرافي أو موقعك الحالي. يمكنك إجراء التسجيلات فقط عند تشغيل الإضافة، باستخدام [ويدجت الملاحظات الصوتية/الفيديو](../widgets/info-widgets.md#audiovideo-notes-widget) و/أو [قائمة سياق الخريطة](../map/map-context-menu.md#-audiovideo-note). يتم حفظ البيانات المستلمة تلقائيًا في [أماكني](../personal/myplaces.md)، حيث يمكن إدارتها ومشاركتها بسهولة.  

تشكل جميع الملاحظات الصوتية والفيديو التي تم إنشاؤها طبقة التسجيل. عند تمكينها على الخريطة، فإنها تغير عرض البيانات المسجلة وهناك المزيد من الإعدادات لإدارة التسجيلات. في طبقة التسجيل، يتم وضع الملاحظات الصوتية والفيديو كنقاط اهتمام (POIs) على الخريطة، مما ينشئ سردًا للمستخدم لا يراه إلا مالك الجهاز.

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![مقدمة الملاحظات الصوتية والفيديو](@site/static/img/plugins/audio-video-notes/widgets_view.png)
...
يتم توفير الملاحظات الصوتية/الفيديو مع إضافة الملاحظات الصوتية/الفيديو، وتتطلب الإعداد التالي:

1. تمكين [إضافة الملاحظات الصوتية/الفيديو](../plugins/index.md#enable--disable) في قسم الإضافات من *القائمة الرئيسية*.  
...
| **<Translate android="true" ids="notes"/>** | يعيد هذا الخيار توجيهك إلى [الملاحظات الصوتية/الفيديو في أماكني](../personal/myplaces.md) - التخزين المركزي لجميع الملاحظات الصوتية/الفيديو التي تم إنشاؤها في التطبيق. |
| **<Translate android="true" ids="reset_plugin_to_default"/>** | يمكنك تعيين القيم الافتراضية لجميع إعدادات الملاحظات الصوتية/الفيديو.  |
| **<Translate android="true" ids="copy_from_other_profile"/>** | يفتح مربع حوار لتحديد ملف شخصي لنسخ تكوين تسجيل الملاحظات الصوتية/الفيديو منه، وتعيينه للملف الشخصي الحالي. |
...
بالنسبة *للملاحظات الصوتية والفيديو*، يتم عرض مسجل الصوت أو الفيديو على التوالي. يمكنك:
...
يقوم [ويدجت الملاحظات الصوتية/الفيديو](../widgets/info-widgets.md#audiovideo-notes-widget) بإنشاء الملاحظات والصور وملفات الصوت والفيديو وربطها بموقعك الحالي. يحتوي الويدجت على عدة خيارات يمكن تعيينها كإجراء افتراضي يقوم به الويدجت عند لمس الشاشة.  
...
إذا كنت تستخدم [جهاز إدخال خارجي](https://osmand.net/docs/user/map/interact-with-map#external-input-devices) (*أندرويد*)، يمكنك بدء وإيقاف الملاحظات الصوتية/الفيديو دون لمس الشاشة. للقيام بذلك، قم بتعيين إجراء "إضافة ملاحظة صوتية" أو "إضافة ملاحظة فيديو" إلى مفتاح:<br/>
...
:::note
إذا تم تصدير المسار، فلا يمكن عرض نقاط الطريق أو تشغيلها، لأن الملاحظات الصوتية/الفيديو/الصور يتم تخزينها على الجهاز فقط، ومتاحة لك فقط على هذا الجهاز.
:::
...
2. **يدويًا**، يتم إنشاء نقطة طريق عبر [قائمة السياق](../map/map-context-menu#-audiovideo-note)، على النحو التالي:
    ...
    - حدد الملاحظة الصوتية/الفيديو/الصورة المطلوبة على الخريطة.
...
- تتوفر نقطة الطريق للعرض كنقطة طريق على الخريطة وفي قائمة [نقاط الطريق لمسار معين](../map/tracks/track-context-menu.md#points--waypoints). لا يؤدي حذف نقطة الطريق إلى حذف الملاحظة الصوتية/الفيديو/الصورة المقابلة.

- تتوفر ملاحظة صوتية أو فيديو أو صورة للعرض على الخريطة وفي قائمة الملاحظات الصوتية/الفيديو في [أماكني](../plugins/audio-video-notes#view-in-my-places). لا يؤدي حذف الملاحظة الصوتية أو الفيديو أو الصورة إلى حذف نقطة الطريق المقابلة.
...
إذا كنت تعرض ملف GPX للمسار المضاف مع نقطة طريق من ملاحظة صوتية/فيديو/صورة، فستكون نقطة الطريق كما في المثال أدناه، مع الإحداثيات، والطابع الزمني، واسم الملاحظة المقابلة، والرابط القصير.
...
![الملاحظات الصوتية والفيديو في Josm](@site/static/img/plugins/audio-video-notes/josm-track-points.png)  
