...
- [استكشاف الأخطاء وإصلاحها](../troubleshooting/track-recording-issues.md#recorded-tracks-have-gaps) — **أجهزة iOS** أحيانًا **تعلق أو توقف التطبيقات** مؤقتًا عن العمل في الخلفية، مما قد يقطع تسجيل المسار.
...
تتيح لك الودجات عرض المعلومات الرئيسية حول تسجيل المسار مباشرة على الشاشة، مثل *المسافة* و*المدة* و*الصعود* و*النزول* و*السرعة القصوى* و*متوسط الميل*. في iOS، يمكنك أيضًا إضافة ودجة *وقت الحركة*.
...
![إضافة ودجة المسافة/بدء-إيقاف في iOS](@site/static/img/plugins/trip-recording/add_recording_widgets_ios_moving_time.png)
...
لتخصيص واجهتك، يمكنك إضافة أو إزالة ودجة تسجيل الرحلة و[الودجات الأخرى](../plugins/trip-recording#additional-widgets) من خلال قائمة تكوين الشاشة.
...
![أوضاع العرض](@site/static/img/plugins/trip-recording/trip_rec_widgets_mode.png) ![أوضاع العرض](@site/static/img/plugins/trip-recording/average_slope_mode.png)

</TabItem>

<TabItem value="ios" label="iOS">

![أوضاع العرض](@site/static/img/widgets/tr_rec_wid_conf_scr_2_new.png) ![أوضاع العرض](@site/static/img/plugins/trip-recording/average_slope_mode_ios.png)

</TabItem>

</Tabs>

تدعم بعض ودجات تسجيل الرحلة أوضاع عرض متعددة. تتيح الأوضاع التبديل بين قيم الرحلة الإجمالية وقياسات لأحدث قسم صاعد أو نازل في الرحلة المسجلة حاليًا.

يمكن أن تكون الأوضاع التالية متاحة اعتمادًا على الودجة.
1. **متوسط الميل**:
- آخر نزول  
- آخر صعود  

2. **المسافة (بدء-إيقاف)**:
- المسافة الإجمالية (افتراضي)
- آخر نزول
- آخر صعود
...
- الصعود / النزول / السرعة القصوى / متوسط الميل / وقت الحركة — انقر على الودجة على الخريطة للتبديل إلى وضعها.
- المسافة (بدء-إيقاف) — تدعم ودجة المسافة أوضاع عرض متعددة (حددها في إعدادات الودجة)، لكن النقر عليها يفتح دائمًا حوار تسجيل الرحلة، حيث يمكنك بدء وإيقاف وعرض معلومات مفصلة حول مسارك. 

إذا لم يحتوِ التسجيل الحالي بعد على قسم صاعد أو نازل، فإن الودجات في وضع آخر صعود أو آخر نزول تعرض 0 أو — (لا توجد بيانات).

**ملاحظات:** تنطبق الأوضاع على الرحلة المسجلة حاليًا وتُحدَّث مع استمرار التسجيل.
...
### الودجات الإضافية {#additional-widgets}

| |
|------------|
|**متوسط الميل**. يعرض متوسط الميل لآخر قسم صاعد أو نازل في الرحلة الحالية، اعتمادًا على الوضع المحدد. |
|![ودجات](@site/static/img/widgets/tr_rec_wid_average_slope.png)|
|**النزول**. يشير إلى الإجمالي النازل أو قسم النزول الأخير، اعتمادًا على الوضع المحدد. |
|![ودجات](@site/static/img/widgets/tr_rec_wid_dow_new.png)|
|**المدة**. تعرض الوقت الإجمالي لتسجيل الرحلة الحالي بالساعات والدقائق. |
|![ودجات](@site/static/img/widgets/tr_rec_wid_dur_new.png)|
|**السرعة القصوى**. يعرض السرعة القصوى لرحلة التسجيل الحالية في الوضع المحدد. |
|![ودجات](@site/static/img/widgets/tr_rec_wid_max_speed.png)|
|**وقت الحركة** (*iOS فقط*). يعرض وقت الحركة لرحلة التسجيل الحالية، أو الوقت لآخر صعود ونزول، اعتمادًا على الوضع المحدد. |
|![ودجات](@site/static/img/widgets/tr_rec_wid_moving_time.png)|  
