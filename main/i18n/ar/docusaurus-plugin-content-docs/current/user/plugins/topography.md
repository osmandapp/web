...
للحصول على عمل مستقر وتوفير الموارد، يمكن تنزيل خريطة لمنطقة معينة من البلد بدلاً من البلد بأكمله، إذا كانت هذه المناطق متوفرة في التطبيق. يتم سرد معلومات حول حجم كل نوع من الخرائط تحت اسمها.
...
تحتاج إلى تنزيل خرائط **خريطة التضاريس (ثلاثية الأبعاد)** لعرض تظليل التلال والميل والتضاريس ثلاثية الأبعاد. بعد تنزيل الخرائط، يمكن عرض **خطوط الكونتور** و**التضاريس** باستخدام قسم [تخصيص الخريطة](../map/configure-map-menu.md) من *القائمة الرئيسية*.
...
![قائمة التضاريس آي أو إس](@site/static/img/plugins/contour-lines/topography_plugin_terrain_menu_1_ios.png)   ![قائمة التضاريس آي أو إس](@site/static/img/plugins/contour-lines/topography_plugin_terrain_menu_2_ios_new.png)

</TabItem>

</Tabs>  

خيار **التضاريس** يُمكّن ويسمح لك بتخصيص ثلاث ميزات مثل *تظليل التلال*، و*الميل*، و*الارتفاع*. في نظام iOS، تتضمن قائمة التصور أيضًا *ظلال التضاريس*، وهي وضع تظليل تضاريس فوري يعتمد على التضاريس ثلاثية الأبعاد.  
ميزات محددة:  

- يمكن تمكين خيار واحد فقط في نفس الوقت، إما تظليل التلال أو الميل أو الارتفاع أو ظلال التضاريس.
- إذا لم تلاحظ أي تغييرات بعد تنزيل وتمكين الخريطة المقابلة، فمن المستحسن إعادة تشغيل التطبيق.

تتضمن قائمة **التضاريس** اختيار [نظام الألوان](#default-color-scheme) (**التصور** على iOS) مع خيار [تعديله](#modify-color-scheme) (لـ [مشتركي Pro](../../user/purchases/index.md))، والقدرة على تغيير شفافية الطبقة على الخريطة ([الرؤية](#visibility))، واختيار [مستوى التقريب](#zoom-levels) لعرضها، ومعلومات حول حجم [البيانات المخزنة مؤقتًا](#cache-size)، وقائمة بـ [الخرائط](../../user/personal/maps-resources.md) اللازمة لعرض الطبقة.
...
### ظلال التضاريس (iOS) {#terrain-shadows}

![ظلال التضاريس على iOS](@site/static/img/plugins/contour-lines/terrain_shadows_ios.png)

توفر **ظلال التضاريس** تظليلًا ديناميكيًا فوريًا يعتمد على هندسة التضاريس ثلاثية الأبعاد. على عكس خرائط تظليل التلال النقطية، يتم إنشاء تأثير التظليل مباشرة على الجهاز باستخدام بيانات التضاريس ويتبع منظور الخريطة الحالي تلقائيًا.

تتطلب ظلال التضاريس التضاريس ثلاثية الأبعاد وتفعلها تلقائيًا عند تحديدها. صُممت هذه الميزة لتحسين قراءة التضاريس مع الحفاظ على تأثير منخفض على الأداء.
...
![تعديل نظام الألوان](@site/static/img/plugins/contour-lines/modify_color_scheme_palette_ios.png)   ![تعديل نظام الألوان](@site/static/img/plugins/contour-lines/modify_color_scheme_palette_ios_2.png)
...
تُستخدم وظيفة *الرؤية* لضبط شفافية الظلال لتظليل التلال والألوان المستخدمة لتمثيل الزاوية في معلمة الميل. بالنسبة لظلال التضاريس، تتحكم الرؤية في قوة تأثير تظليل التضاريس.
...
لا تتوفر مستويات التقريب لظلال التضاريس لأن هذا التصور يُعرض ديناميكيًا ولا يعتمد على نطاقات رؤية الطبقة النقطية.
...
![المباني ثلاثية الأبعاد](@site/static/img/map/3d_buildings_4_new.png) ![المباني ثلاثية الأبعاد](@site/static/img/map/3d_buildings_3_new.png)
...
![المباني ثلاثية الأبعاد](@site/static/img/map/3d_buildings_1_ios_new.png) ![المباني ثلاثية الأبعاد](@site/static/img/map/3d_buildings_highlight_ios.png)
...
استخدم المفتاح الرئيسي لتمكين أو تعطيل عرض المباني ثلاثية الأبعاد. عند التفعيل، يعرض الإعداد أيضًا [مستوى التفاصيل الحالي](#performance) (منخفض أو عالي) تحت المفتاح الرئيسي (*أندرويد فقط*). لعرض المباني ثلاثية الأبعاد، قم بإمالة الخريطة بوضع إصبعين على الشاشة وسحبهما لأعلى. في هذا العرض، قد تغطي المباني جزئيًا الطرق أو تسميات الخريطة اعتمادًا على إعداد الرؤية.

على أندرويد، هذا الخيار متاح فقط عند تمكين إضافة التضاريس.  
...
![المباني ثلاثية الأبعاد](@site/static/img/map/3d_buildings_color_ios.png)

</TabItem>

</Tabs>

يتحكم **<Translate android="true" ids="shared_string_appearance"/>** في كيفية ظهور المباني ثلاثية الأبعاد على الخريطة. ويشمل إعدادين: اللون والرؤية. 

**<Translate android="true" ids="shared_string_color"/>** يسمح لك بتغيير لون المبنى. عند النقر على اللون، يفتح OsmAnd شاشة معاينة منفصلة حيث يمكنك رؤية الخريطة أثناء ضبط الإعداد. تعرض شاشة المعاينة خريطة حية بحيث يمكنك رؤية كيفية تأثير اللون المحدد على المباني فورًا.
...
**<Translate android="true" ids="gpx_visibility_txt"/>** يتحكم في عدم الشفافية (الشفافية) للمباني ثلاثية الأبعاد. استخدم الشريط المنزلق لتعيين الرؤية كنسبة مئوية. يسمح الشريط المنزلق بقيم من ١٠٪ إلى ١٠٠٪، مع ٥٠٪ كقيمة افتراضية. القيم المنخفضة تجعل المباني أكثر شفافية وتساعد الطرق/التسميات على البقاء قابلة للقراءة. القيم الأعلى تجعل المباني تبدو أكثر صلابة ومهيمنة بصريًا. النقر على الرؤية يفتح أيضًا شاشة معاينة منفصلة مع الشريط المنزلق.

في شاشات المعاينة (اللون / الرؤية)، يمكنك استخدام إعادة تعيين إلى الافتراضي من شريط التطبيق لاستعادة القيمة الافتراضية.

### الأداء {#performance}

يتحكم **<Translate android="true" ids="performance"/>** في كيفية عرض المباني ثلاثية الأبعاد. ويشمل إعدادين: مستوى التفاصيل ومسافة العرض.

**<Translate android="true" ids="level_of_details"/>** يحدد تعقيد هندسة المبنى ثلاثي الأبعاد:
- منخفض (افتراضي) — هندسة أبسط.
- عالي — هندسة أكثر تفصيلاً. تأثير التلاشي للمباني ثلاثية الأبعاد متاح فقط مع هذا الإعداد.

**<Translate android="true" ids="view_distance"/>** يتحكم في مدى بعد الكاميرا يتم عرض المباني ثلاثية الأبعاد:
...
- يسمح الجمع بين طبقات **التضاريس ثلاثية الأبعاد** و**تظليل التلال** بالحصول على تمثيل أكثر واقعية ومرئية للتضاريس والنقوش وتفاصيل المناظر الطبيعية. هذا المزيج مناسب بشكل خاص للتضاريس الجبلية والتلية.
