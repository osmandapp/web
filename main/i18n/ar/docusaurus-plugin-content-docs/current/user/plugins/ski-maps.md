...
يعرض ملحق عرض خريطة التزلج في OsmAnd المنحدرات الشهيرة والشائعة، ومناطق التزلج المعتمدة رسميًا، وخرائط المسارات لمعظم منتجعات التزلج. [OpenStreetMap](https://www.openstreetmap.org/#map=16/51.5110/0.0550) هي مصدر البيانات لخرائط التزلج في OsmAnd. من خلال دمج الخرائط المتجهية مع الخرائط النقطية، تكون خريطة التزلج في OsmAnd دقيقة جدًا وستكون مفيدة للمدربين، والمتسلقين، وعشاق الرياضات الشتوية.
...
1. تمكين [ملحق عرض خريطة التزلج](../plugins/index.md#enable--disable).  
2. تعيين نمط خريطة [الشتاء والتزلج](#set-winter-and-ski-map-style) [للملف الشخصي](../personal/profiles.md) المطلوب.  
...
3. مرر لأسفل إلى نمط الخريطة/نوع الخريطة، افتحه، وحدد **الشتاء والتزلج**.
...
ملحق عارض خريطة التزلج هو تطبيق منفصل، وتعطيل عرضه لا يزيله. إذا كنت بحاجة إلى إلغاء تثبيته، فانتقل إلى إعدادات أندرويد باستخي *فتح الإعدادات* في قائمة الملحق.  
:::
...
تختلف مسارات التزلج حسب النوع. يمثل النوع خصائص مختلفة للمسار لتلبية متطلبات الأنشطة المختلفة المتعلقة بالشتاء: التزلج الألبي، والتزلج الكلاسيكي، والتزلج على الجليد، وركوب الزلاجات، والتزلج في المناطق النائية، وجولات التزلج، وتزلج الموغول، والتزلج الليلي، وما إلى ذلك. يساعد فهم [نوع مسار التزلج](https://wiki.openstreetmap.org/wiki/Key:piste:type) على الخريطة في بناء مسار ملاحة مثالي.
...
لمعرفة أي مسارات خارج منتجع التزلج يمكن التنقل فيها، تحتاج إلى تمكين خيار **منحدرات التزلج ومساراتها** في قائمة [**<Translate android="true" ids="rendering_category_routes"/>**](../map/routes/types-of-routes.md)، التي يتم فتحها من قائمة [تكوين الخريطة](../map/configure-map-menu.md).  

مع تمكين ملف التزلج الشخصي، وملاحة التزلج، و[نمط خريطة الشتاء والتزلج](../map/map-styles.md#winter-and-ski)، تعرض [منحدرات التزلج ومساراتها](../map/routes/types-of-routes.md#ski-slopes-and-routes) جميع أنواع المسارات التي يمكن لملاحة التزلج استخدامها لبناء مسارات الملاحة على طولها. يتم تحديد مسارات منحدرات التزلج هذه، إذا تم تمكينها، باللون البنفسجي في الغالب خارج حدود منتجع التزلج.  
عند تمكين الخيار [منحدرات التزلج ومساراتها](../map/routes/types-of-routes.md#ski-slopes-and-routes) لنمط خريطة غير الشتاء والتزلج، يتم عرض المنحدرات فقط. قد لا تظهر الميزات الأخرى المتعلقة بالتزلج في طبقة التصميم هذه.  
_[منحدرات التزلج ومساراتها](_Ski slopes and routes_) [قابلة للنقر](../map/routes/index.md#actions-with-routes).
...
يمكن استخدام نمط **الشتاء والتزلج** مع [الوضع الليلي أو وضع شروق/غروب الشمس](../map/vector-maps.md#map-mode). وهو خيار مريح لأولئك الذين يفضلون الألوان الداكنة، أو تعتيم الشاشة في الليل، وكذلك لأولئك الذين يمارسون التزلج الليلي. إلى جانب الوضع الليلي، يمكن لخرائط التزلج في OsmAnd أيضًا إظهار المسارات التي بها إضاءة باستخي خيار [إضاءة الشوارع](../map/vector-maps.md#details).  
