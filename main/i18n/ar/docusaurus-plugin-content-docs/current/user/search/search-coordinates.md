...
**البحث بالإحداثيات في OsmAnd** يتيح لك العثور على موقع محدد عن طريق خطوط الطول والعرض. يساعد هذا النوع من البحث في تحديد موقع بدقة. وهو مفيد بشكل خاص للمناطق التي لا تحتوي على عناوين دقيقة، أو للمهام الجغرافية المحددة مثل إنشاء سياجات جغرافية أو تحديد المواقع بدقة في المساحات المفتوحة.
...
يمكن إدخال الإحداثيات بأي تنسيق متاح، لكن يُعرض الموقع الذي تم العثور عليه بالتنسيق المحدد في *تكوين الملف الشخصي ← الإعدادات العامة ← الوحدات والتنسيقات*.
...
### الإحداثيات العشرية {#decimal-coordinates}

![البحث بالإحداثيات في أندرويد](@site/static/img/search/coordinates_search_degrees_andr.png)  

**<Translate android="true" ids="navigate_point_format_D"/>**. في هذا التنسيق، تُكتب إحداثيات خطوط الطول والعرض بالتنسيق العشري، على سبيل المثال، ٥٢.٣٧٠٢٢ درجة لخط العرض و ٤.٨٩٥١٧ درجة لخط الطول لأمستردام. طريقة الإدخال هذه سهلة الاستخدام وهي الأكثر شيوعًا. يمكنك قراءة المزيد عن تنسيق الإحداثيات العشرية [هنا](https://en.wikipedia.org/wiki/Decimal_degrees).  
...
### الدرجات والدقائق {#degrees-and-minutes}

![البحث بالإحداثيات في أندرويد](@site/static/img/search/coordinates_search_DM_andr.png)  

**<Translate android="true" ids="navigate_point_format_DM"/>**. في هذا التنسيق، تُسجل إحداثيات خطوط الطول والعرض باستخدام الدرجات والدقائق. على سبيل المثال، ٣٧:٤٦.٢٩ لخط العرض و -١٢٢:٢٥.١٠ لخط الطول لسان فرانسيسكو. قد يكون هذا التنسيق أقل دقة وقد يستغرق وقتًا أطول في الإدخال.
...
### الشبكة السويسرية (أندرويد فقط) {#swiss-grid}
...
- *الأكثر استخدامًا*. يُستخدم تنسيقا الشبكة السويسرية (CH1903) والشبكة السويسرية (CH1903+) في سويسرا للقيام بقياسات جيوديسية ورسم الخرائط وأنظمة المعلومات الجغرافية والبناء والهندسة. توفر هذه التنسيقات نظام إحداثيات محلي خاص بسويسرا.

### نظام محدد موقع ميدينهيد (أندرويد فقط) {#maidenhead-locator-system}

![البحث بالإحداثيات في أندرويد](@site/static/img/search/coordinates_search_maidenhead.webp)

**ميدينهيد**. هذا نظام ترميز جغرافي يُستخدم بشكل أساسي من قبل مشغلي الراديو الهواة. يقوم بترميز المواقع الجغرافية إلى تسلسل قصير من الحروف والأرقام، مما يجعله ملائمًا لتحديد المواقع وتبادل معلومات الموقع. مثال على الإحداثيات بتنسيق ميدينهيد هو KN29AS 19CX. يحدد الزوج الأول من الحروف حقلاً كبيرًا، يليه أرقام وأزواج حروف إضافية تزيد من دقة الموقع. يمكنك قراءة المزيد عن نظام محدد موقع ميدينهيد [هنا](https://en.wikipedia.org/wiki/Maidenhead_Locator_System).  
- *قواعد الإدخال*. تتكون الإحداثيات من أزواج متناوبة من الحروف والأرقام. توفر المحددات الأطول دقة موضعية أعلى.  
- *الأكثر استخدامًا*. يُستخدم نظام محدد موقع ميدينهيد على نطاق واسع من قبل مشغلي الراديو الهواة لتحديد موقع المحطة والاتصالات اللاسلكية والمسابقات.
...
## تخصيص تنسيق الإحداثيات (أندرويد فقط) {#customize-coordinate-format}

![البحث بالإحداثيات في أندرويد](@site/static/img/search/customising_format.webp) ![البحث بالإحداثيات في أندرويد](@site/static/img/search/searching_format.webp)

يمكنك تخصيص قائمة تنسيقات الإحداثيات المتاحة للملف الشخصي الحالي.  
اذهب إلى: *<Translate android="true" ids="shared_string_menu,configure_profile,general_settings_2,coordinates_format"/>*

في هذه الشاشة يمكنك:  
- النقر على زر *تعديل* لتعيين تنسيق الإحداثيات الأساسي، أو إعادة ترتيب تنسيقات الإحداثيات أو إزالتها.
- النقر على + لإضافة تنسيق إحداثيات آخر.
- فتح قائمة ⋮ لاستعادة القائمة الافتراضية لتنسيقات الإحداثيات أو نسخ تنسيقات الإحداثيات من ملف شخصي آخر.

عند البحث بالإحداثيات، يمكنك تحديد تنسيق إحداثيات من قائمة الحديثة أو النقر على *تحديد تنسيق آخر* لتصفح جميع أنظمة الإحداثيات المتاحة. يمكنك أيضًا البحث باسم نظام الإحداثيات أو رمز EPSG.
...
- [إدخال الإحداثيات](../plan-route/coordinate-input.md)
- [قائمة سياق الخريطة](../map/map-context-menu.md)
- [الأدوات الإعلامية](../widgets/info-widgets.md)
