...
يدعم OsmAnd العديد من روابط خرائط جوجل المشتركة، بما في ذلك بعض الروابط المختصرة وعناوين المواقع المشتركة. يمكنك أيضًا لصق رابط خرائط جوجل مباشرة في بحث OsmAnd.

**الحل البديل:** *اضغط مطولاً على الرسالة → مشاركة → اختر OsmAnd*. هذا يرسل الرابط إلى OsmAnd، حيث يمكن تحليله وفتحه كموقع.

قد يكون الموقع المفتوح تقريبيًا، وذلك حسب المعلومات المتاحة في رابط خرائط جوجل المشترك.
...
### ما معنى «التحديثات المجانية (طوارئ)»؟ {#what-does-free-updates-emergency-mean}

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![الوصول الطارئ أندرويد](@site/static/img/troubleshooting/emergency_access_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![الوصول الطارئ iOS](@site/static/img/troubleshooting/emergency_access_ios.png)

</TabItem>

</Tabs>

**تحديثات الخرائط الطارئة** قد يتم تمكينها لمناطق معينة أثناء حالات الطوارئ أو الأزمات. في هذه الحالات، تكون الخرائط متاحة مؤقتًا مع تحديثات مجانية وأكثر تكرارًا.

عندما تكون هذه الميزة نشطة، قد يعرض شاشة تنزيل الخريطة الرسالة *التحديثات المجانية (طوارئ) / التنزيلات المجانية*.

هذا يعني أن الخريطة يمكن أن تتلقى تحديثات أكثر تكرارًا من الخرائط القياسية، أحيانًا مرة واحدة أو عدة مرات في اليوم، اعتمادًا على الوضع.
...
### البحث عن الرموز البريدية في المملكة المتحدة باستخدام ملف نقاط اهتمام مخصص {#search-uk-postcodes-using-a-custom-poi-file}

إذا تعذر العثور على الرمز البريدي في المملكة المتحدة باستخدام *القائمة ← بحث ← العنوان*، فقد لا يكون متاحًا من خلال فهرس بحث العناوين المستخدم في OsmAnd.

يتوفر ملف نقاط اهتمام (POI) للرموز البريدية في المملكة المتحدة يديره المجتمع كملف `.obf` مخصص. يحتوي على الرموز البريدية المخزنة كنقاط اهتمام (POIs) مع إحداثيات ويمكن استخدامه للبحث عن الرموز البريدية مباشرة.

كيفية استخدامه:

1. نزّل ملف `UK_postcodes_poi_europe.obf` من [مستودع المجتمع](https://github.com/hvdwolf/OsmAnd-UKpostcodes/releases/tag/202605).
2. إذا تم تنزيل الملف كأرشيف ZIP، فاستخرجه أولاً.
3. انسخ ملف `.obf` إلى مجلد بيانات OsmAnd، أو افتحه باستخدام مدير الملفات على جهازك واختر **OsmAnd** لاستيراده.
4. استخدم وظيفة **البحث** العامة (رمز العدسة المكبرة) بدلاً من بحث **العنوان**.
5. أدخل رمزًا بريديًا (مثل CT14 8EU أو EX34 7EX) في شريط البحث.

**ملاحظة:** قد تظهر بعض الرموز البريدية أكثر من مرة في نتائج البحث لأن بيانات خريطة OsmAnd الافتراضية وملف نقاط الاهتمام المخصص للرموز البريدية قد يحتويان على نفس الرمز البريدي.
