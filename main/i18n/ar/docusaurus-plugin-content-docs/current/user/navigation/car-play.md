...
١. *CarPlay* غير متاح في جميع البلدان. تأكد من أنه مدعوم في [بلدك أو منطقتك](https://www.apple.com/uk/ios/feature-availability/#applecarplay-applecarplay) وأن *CarPlay* مدعوم في [طراز مركبتك](https://www.apple.com/ios/carplay/available-models/).
...
يتوافق موضع الأزرار وويدجت الملاحة في *CarPlay* مع جانب شريط حالة CarPlay. ويعتمد ذلك على إعداد **موضع السائق** (يسار اليد أو يمين اليد) في وحدة الرأس في سيارتك، وليس على أي إعداد داخل CarPlay أو تطبيق OsmAnd.
...
- [موضعي](../widgets/map-buttons.md#my-position-and-zoom). ينقل الجزء المرئي من الخريطة بحيث يكون موقعك الحالي في وسط الشاشة.
- [تكبير](../widgets/map-buttons.md#my-position-and-zoom). يسمح لك بتكبير وتصغير الجزء المرئي من الخريطة.
...
- يجب أن يكون لديك [إصدار مدفوع من OsmAnd أو اشتراك](../purchases/ios#free-and-paid-features). تحقق من ذلك هنا *القائمة ← الإعدادات ← المشتريات*.
...
٤. أثناء القيادة، ستعرض شاشة المركبة معلومات OsmAnd حول موقعك الحالي على الخريطة، واتجاهات القيادة، والمسافة إلى المنعطف التالي، والخطوط، ووقت الوصول والمسافة إلى وجهتك.
...
- يتم عرض الشوارع وأرقام المباني أولاً عند البحث لعنوان. هذا يضمن عرض معلومات العنوان ذات الصلة قبل المواقع الأخرى مثل الطرق أو المحطات.
...
**الخرائط المفقودة أو القديمة**

إذا كانت بعض الخرائط غير المتصلة المطلوبة لحساب المسار مفقودة أو قديمة، يُعرض تنبيه. يمكنك اختيار أحد الخيارات التالية:
- **استخدام الخرائط التي تم تنزيلها**. احسب المسار باستخدام الخرائط المتاحة حاليًا على جهازك.
- **عرض على الهاتف**. افتح شاشة تنزيل الخرائط على iPhone الخاص بك لتنزيل أو تحديث الخرائط المطلوبة. بعد تنزيل الخرائط، يمكنك بدء الملاحة مرة أخرى.
- **إلغاء**. أغلق التنبيه دون بدء الملاحة.

**تحذير الوصول الخاص**

إذا كان المسار يتطلب استخدام طريق خاص للوصول إلى وجهتك، يُعرض تحذير: *السماح باستخدام طريق خاص للوصول إلى وجهتك؟* يمكنك اختيار:
- **السماح**. تابع المسار باستخدام الطريق الخاص.
- **إلغاء**. أغلق التنبيه وارجع إلى الشاشة السابقة، حيث يمكنك تغيير الوجهة أو متابعة إجراء آخر.
...
ويدجت **عداد السرعة** هو عنصر واجهة متكامل يعرض *السرعة الحالية* باستخدام بيانات GPS و*حدود السرعة* من [قاعدة بيانات OSM](https://wiki.openstreetmap.org/wiki/Key:maxspeed) و[إعدادات OsmAnd](../navigation/guidance/voice-navigation.md#speed-limit) على شاشة نظام الوسائط المتعددة في المركبة. عندما تقترب سرعتك الحالية من تحمل حد السرعة المُعد أو تتجاوز حد السرعة، يغير الويدجت مظهره (الألوان) لتقديم تحذير. قد يكون تغيير المظهر متحركًا.
...
![إنهاء الملاحة](@site/static/img/navigation/auto-car/finish_navigation_carplay_new.png)
...
- إذا كانت السرعة الحالية أقل من ١ م/ث (على سبيل المثال، عند التوقف في محطة وقود)، يتم إيقاف الملاحة مؤقتًا. يستأنف الملاحة تلقائيًا عند إعادة الاتصال بـ CarPlay.
...
- حدد الموضع المفضل لديك:
    ...
    - **الوسط**. يتم وضع المؤشر في وسط الشاشة، وهو مناسب للملاحة القيادة.
...
١. أسئلة شائعة حول شاشة الاتصال:
    - *لماذا لا يمكنني التحكم في التطبيق من هاتي عند الاتصال بـ CarPlay؟*  
