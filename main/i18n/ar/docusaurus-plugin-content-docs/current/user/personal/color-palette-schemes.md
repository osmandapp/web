...
تُستخدم أنظمة الألوان في [تلوين الطرق](#tracks-routes)، و[تضاريس الخريطة](#terrain)، و[خريطة الطقس](#weather)، وتُمثل كجزء منفصل من المعلومات التي يمكن تصديرها واستيرادها وتعديلها لتغيير نظام ألوان الخريطة وتصور البيانات.
...
إذا كنت بحاجة إلى تغيير ملف لوحة ألوان موجود داخل OsmAnd (بدلاً من إضافة ملف TXT جديد)، يمكنك تعديله من خلال نسخ احتياطي مصدر (.osf).
- قم بتصدير بيانات لوحة الألوان كنسخة احتياطية .osf (انظر قسم [الاستيراد / التصدير](#import--export) أدناه).
- فك ضغط الـ .osf المصدر. ملف .osf هو أرشيف .zip معاد تسميته. قم بعمل نسخة من ملف .osf الأصلي، ثم قم بفك ضغطه باستخدام أي أداة أرشيف.
- ابحث عن ملف TXT للوحة الألوان وقم بتعديله. داخل المجلدات المستخرجة، حدد ملف لوحة الألوان المطلوب (على سبيل المثال: height_altitude_&lt;type_name&gt;.txt أو route_speed_&lt;type_name&gt;.txt) وقم بتعديل قيمه في محرر نصوص.
- ضغط مرة أخرى وإعادة تسمية إلى .osf. قم بضغط المحتوى المستخرج مرة أخرى إلى أرشيف .zip، ثم أعد تسميته إلى .osf. احتفظ بنفس هيكل المجلد.
- استورد الـ .osf المحدث إلى OsmAnd (انظر قسم **الاستيراد / التصدير** أدناه) وأعد تشغيل التطبيق لتطبيق التغييرات.
