...
يحتوي تطبيق OsmAnd على العديد من الميزات القوية لعرض [المسارات](#type-of-routes) المختلفة على الخريطة. بشكل افتراضي، تكون هذه المسارات جزءًا من [خرائط المتجهات](./vector-maps.md#routes) (بيانات OpenStreetMap)، ولكن يتم توفير وظائف مماثلة بواسطة [المسارات](./tracks/index.md) التي يمكن إنشاؤها باستخدام [تخطيط مسار](../plan-route/create-route.md)، أو استيرادها [كمسارات GPX](#save-as-a-track)، أو تسجيلها باستخدام [مكون تسجيل الرحلات الإضافي](../plugins/trip-recording.md).


## أنواع المسارات {#type-of-routes}

تمثل المسارات أنشطة خارجية مختلفة باستخدام بيانات من [OpenStreetMap](https://wiki.openstreetmap.org/wiki/Relation:route). سيؤدي النقر على **أيقونة الدرع التي تحتوي على رقم المسار أو علامة المسار** على الخريطة إلى فتح قائمة سياقية توفر معلومات إضافية حول المسار وتسمح باستخدامه كجزء من التنقل.
...
تتميز مسارات الدراجات الجبلية (MTB) بتخطيط محدد في [OpenStreetMap](https://wiki.openstreetmap.org/wiki/Tag:route%3Dmtb) ويمكن اختيارها بشكل منفصل عن مسارات ركوب الدراجات.
...
- ***<Translate android="true" ids="mtb_imba"/>***.  يعرض مسارات الدراجات الجبلية وفقًا للاتحاد الدولي للدراجات الجبلية، [MTB IMBA](https://www.imba.com/). مزيد من المعلومات موجودة في مقال [دليل الخريطة](../map-legend/index.md).  
...
- ***<Translate android="true" ids="rendering_value_walkingRoutesOSMC_name"/>***.  تلوين المسارات وفقًا [للونها المحلي](https://wiki.openstreetmap.org/wiki/Key:osmc:symbol#Maps_that_show_osmc:symbol) (إذا كان متاحًا في OpenStreetMap) ورمز الدرع السياحي.  
![Map routes - hiking osmc](@site/static/img/map/map-routes-hiking-osmc.png)

- ***<Translate android="true" ids="rendering_value_walkingRoutesScopeOSMC_name"/>***.  تلوين حسب [الانتماء إلى الشبكة](https://wiki.openstreetmap.org/wiki/Key:osmc:symbol#Maps_that_show_osmc:symbol).  
![Map routes - hiking network](@site/static/img/map/map-routes-hiking-network.png)

- ***<Translate android="true" ids="rendering_value_walkingRoutesOSMCNodes_name"/>***.  تلوين المسارات وفقًا لنوع [شبكة العقد](https://wiki.openstreetmap.org/wiki/Node_Networks) (دولية، إقليمية، أو محلية).  
...
يمكنك تفعيل هذه الميزة في *القائمة ← تكوين الخريطة ← المسارات ← مسارات الدراجات الترابية*. يعمل النظام بشكل مشابه لمقياس MTB، حيث يوفر تمثيلاً مرئيًا للمسارات المصممة خصيصًا لركوب الدراجات الترابية — يمكن العثور على مزيد من المعلومات حول وسم [`dirtbike` هنا](https://wiki.openstreetmap.org/wiki/Key:dirtbike:scale).
...
- تشير **صخور التسلق** إلى **تشكيلات صخرية محددة** أو جدران مناسبة للتسلق.
...
مسارات التزلج هي مجموعة محددة من [المسارات](https://wiki.openstreetmap.org/wiki/Tag:route%3Dski) في OpenStreetMap. عادةً، يمكن استخدام هذه المسارات مع نمط خريطة [الشتاء والتزلج](../map/vector-maps.md#winter-and-ski) مع جميع [المسارات](https://wiki.openstreetmap.org/wiki/Pistes) المرئية.
...
تُعرض [مسارات الخيول](https://wiki.openstreetmap.org/wiki/Tag:route%3Dhorse) كمسارات ملونة برموز لركوب الخيل.
...
[مسار اللياقة البدنية](https://wiki.openstreetmap.org/wiki/Tag:route%3Dfitness_trail) هو مسار أو درب مجهز بعوائق أو محطات على طوله لتمرين جسم الإنسان لتحسين الصحة.
...
يمكنك تخصيص عرض بعض ***كتب السفر*** وأنواع ***المسارات***، وعرض المسارات كنقاط، أو ***مجموعات نقاط*** (على سبيل المثال لكتيبات سفر ويكيفويج). تعرف على المزيد حول [أدلة السفر هنا](../plan-route/travel-guides.md).
...
على الخريطة، يمكنك تحديد مسارات [للمشي لمسافات طويلة، وركوب الدراجات، والسفر، والمزيد](#type-of-routes)، والتي يتم تمييزها [برموز OSMC](https://wiki.openstreetmap.org/wiki/Key:osmc:symbol) حيثما ينطبق ذلك.
...
ستكون المعلومات التالية مرئية في قائمة السياق:

- **معلومات المسار**: اسم المسار، رابط OSM (يتطلب مكون OSM Editing الإضافي)، النوع، الوصف، المسافة، الاتجاه، الصعود/النزول، نطاق الارتفاع، الشبكة، المشغل، الولاية، اللون، الرحلة ذهابًا وإيابًا، نقاط البداية/النهاية، والمزيد.
...
- [تحليل المسار على الخريطة](../map/tracks/index.md#analyze-track-on-map). يعرض تحليلًا مفصلًا لبيانات المسار باستخدام الرسوم البيانية والخرائط.

لعرض رسم بياني للارتفاع أو الميل أو تفاصيل الارتفاع لمسار محدد، ما عليك سوى النقر على أي من القيم التالية في قائمة السياق: **المسافة**، **صعودًا**، **نزولًا**، أو **الارتفاع**.
...
لحفظ المسار كـ *ملف GPX*، انقر على زر **حفظ (تنزيل)** تحت [لوحة المعلومات](../map/tracks/track-context-menu.md#info-panel). سيؤدي هذا إلى فتح لوحة أزرار [الإجراءات السريعة](../map/tracks/track-context-menu.md#track-actions) حيث يمكنك الوصول إلى إجراءات المسار مثل تغيير [المظهر](./tracks/appearance.md)، [التنقل](../navigation/setup/route-navigation.md)، أو تحرير [تخطيط مسار](../plan-route/create-route.md).
...
سيؤدي هذا إلى تشغيل وضع [**التنقل حسب المسار**](../navigation/setup/gpx-navigation.md)، مما يتيح لك متابعة المسار بتوجيه صوتي وميزات التنقل.
