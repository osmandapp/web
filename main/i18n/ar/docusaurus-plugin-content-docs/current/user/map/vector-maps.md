...
تهدف خرائط المتجهات إلى استخدامها كمصدر بيانات الخرائط الافتراضي لتطبيق OsmAnd، لذا **تحتاج إلى تنزيلها على جهازك**. تدعم خرائط المتجهات عددًا كبيرًا من أنماط الخرائط لأنشطة مختلفة مثل ركوب الدراجات، والمشي لمسافات طويلة، والقيادة بالسيارة أو عربة الثلج، وغيرها.

يمكن تخصيص كل نمط خريطة لإبراز أو إخفاء كائنات محددة، والتبديل بين وضعي النهار والليل. يمكن تعزيز بيانات خرائط المتجهات ببيانات المتجهات وعرضها بنمط الخريطة الافتراضي، مثل معلومات *خطوط الكنتور*. يمكنك *إنشاء نمط خريطة OsmAnd الخاص بك* لعرض المعلومات المطلوبة.


## حالات الاستخدام {#use-cases}

تعد أنماط الخرائط القابلة للتخصيص إحدى المزايا الرئيسية لتطبيق OsmAnd. يمكنك تخصيص عرض الخريطة ليناسبك وهواياتك، وتعديل عرض أو إخفاء بعض كائنات الخريطة، وأحجام وألوان هذه الكائنات، وتغيير مقياس عرض بعض الكائنات.
...
يوازن نمط عرض الخريطة القياسي في OsmAnd بين التفاصيل والبساطة، مما يجعله مثاليًا للاستكشاف الحضري والخارجي. فهو يوضح تفاصيل المدينة مثل الشوارع والمباني ومحطات النقل مع تبسيط الفوضى البصرية عن طريق جعل الخرائط أكثر وضوحًا.
...
### عرض الرحلات {#touring-view}

![نمط عرض الرحلات](@site/static/img/map/map-style-touring.png)

نمط الرحلات بتباين عالٍ وأقصى تفاصيل. يتضمن جميع خيارات نمط OsmAnd الافتراضي مع عرض أكبر قدر ممكن من التفاصيل، خاصة الطرق والمسارات وطرق السفر الأخرى. تمييز واضح لأنواع الطرق في *أطلس الرحلات*. مناسب للاستخدام النهاري والليلي والخارجي.
...
هذا نمط للملاحة البحرية يتميز بالعوامات، والمنارات، ومسارات الأنهار، وممرات البحر، والعلامات، والموانئ، والعلامات البحرية، وخطوط الكنتور العمقية. اقرأ المزيد في المقال [عرض الخريطة البحرية](../plugins/nautical-charts.md).
...
يمكنك رؤية منحدرات التزلج وتفاصيل أخرى مثل صعوبة المنحدرات وعلامات المصاعد. تشمل الفوائد الرئيسية العرض المريح للمنحدرات، والمصاعد، وميزات التزلج الأخرى. تقليل الكائنات الثانوية المشتتة للانتباه في الخريطة. اقرأ المزيد في المقال [خرائط التزلج](../plugins/ski-maps.md).

### توبو {#topo}

![نمط خريطة توبو](@site/static/img/map/map-style-topo.png)

تم تصميم هذا النمط للمشي لمسافات طويلة والتخييم وركوب الدراجات في الطبيعة. يتميز بطرق وميزات طبيعية متباينة، وأنواع مختلفة من المسارات، وخيارات خطوط كنتور موسعة، وتفاصيل إضافية. يمكن قراءته في الهواء الطلق. يسمح إعداد *سلامة السطح* بتمييز جودة الطريق.

### OSM-carto {#osm-carto}

![نمط خريطة OSM-carto](@site/static/img/map/map-style-osm-carto.png)

يحاكي هذا النمط [نمط OpenStreetMap الافتراضي على الويب](https://www.openstreetmap.org/). يتوفر الكود المصدري لنسخة الويب على [Github](https://github.com/gravitystorm/openstreetmap-carto)، ويتوفر كود OsmAnd على [Github](https://github.com/osmandapp/OsmAnd-resources/blob/master/rendering_styles/osm-carto.render.xml).

### الصحراء {#desert}

![نمط خريطة الصحراء](@site/static/img/map/map-style-desert.png)

مصمم للصحاري والمناطق ذات الكثافة السكانية المنخفضة، ويوفر معلومات خرائط أكثر تفصيلاً.

### الطرق الوعرة {#offroad}

![نمط خريطة الطرق الوعرة](@site/static/img/map/map-style-offroad.png)

صُمم هذا النمط للملاحة على الطرق الوعرة، وهو يعتمد على تخطيط خريطة [توبو](#topo) ويعمل جيدًا مع صور الأقمار الصناعية كطبقة سفلية. يتميز بطرق رئيسية أرق لإبراز المسارات، والدروب، ومسارات الدراجات، وغيرها من مسارات الطرق الوعرة، مما يجعله مثاليًا لاستكشاف الطرق غير المعبدة في المناطق الريفية أو النائية.

### عربة الثلج {#snowmobile}

![نمط خريطة عربة الثلج](@site/static/img/map/map-style-snowmobile.png)

صُمم هذا النمط خصيصًا للملاحة بعربات الثلج، حيث يبرز المسارات والطرق والدروب المناسبة لعربات الثلج. ويسلط الضوء على المسارات المتخصصة في المناطق الثلجية، مما يوفر ملاحة واضحة عبر التضاريس المغطاة بالثلوج حيث قد لا تتوفر الطرق القياسية.


## مفتاح الخريطة {#map-legend}

يعمل مفتاح الخريطة كمفتاح لفهم الرموز المستخدمة في خرائط OsmAnd. يشرح معنى الرموز المختلفة على الخريطة، بما في ذلك النقاط والخطوط والمناطق. على سبيل المثال، تشير الرموز مثل الخطوط المتعرجة الزرقاء إلى الأنهار، بينما قد تمثل الألوان والأشكال المختلفة المباني والمسارات والطرق.
...
بالنسبة للخرائط باللغات الصينية المبسطة/التقليدية، واليابانية، والكورية، قد تظهر أحرف أو رموز غير صحيحة (مثل المربعات) إذا كان جهازك يفتقر إلى الخطوط الضرورية. تحدث هذه المشكلة عند عرض الأسماء المحلية ويمكن حلها عن طريق تنزيل الخطوط المطلوبة عبر OsmAnd.
...
تُعرض خطوط الكنتور في OsmAnd كخطوط ارتفاع على خرائط المتجهات. لاستخدامها، يجب عليك أولاً تمكين [مكون الطبوغرافيا الإضافي](../plugins/topography.md)، وتنزيل البيانات الإقليمية الضرورية، وتكوين العرض. لا يتم تمكين هذه الميزة افتراضيًا وتتطلب [شراء](../purchases/index.md).
...
تمثل خطوط كنتور العمق البحري مناطق ذات عمق متساوٍ، مما يساعد على تصور التغيرات الطبوغرافية تحت الماء. تتوفر هذه الميزة عبر جميع أنماط ووضعيات الخرائط ويمكن تعديلها مع أو بدون تمكين مكون عرض الخرائط البحرية الإضافي.

لمزيد من التفاصيل حول عرض الخرائط البحرية، قم بزيارة صفحة [مكون الخرائط البحرية الإضافي](../plugins/nautical-charts).
...
عند تمكين ميزة *إظهار حدود الخرائط التي تم تنزيلها*، تظهر الخرائط التي تم تنزيلها باللون **الأخضر** في كل من إصدارات OsmAnd لأندرويد و iOS.
...
تُضيف ميزة **شبكة الإحداثيات** شبكة مرجعية على الخريطة، مما يتيح لك تصور خطوط العرض والطول بناءً على أنظمة إحداثيات مختلفة. هذه الميزة مفيدة لتحديد المواقع بدقة والملاحة الجغرافية المكانية.
...
- **MGRS** (نظام الإحداثيات الشبكية العسكرية)
- **UTM** (مركاتور المستعرضة العالمية - نظام شبكي قائم على المناطق)
...
|----------------|-------------|----------------|
| **WGS84** | EPSG:4326 | نظام مرجعي لخطوط العرض/الطول الافتراضي، يستخدم عالميًا. |
| **مركاتور** | EPSG:3857 | يستخدم للخرائط المستندة إلى الويب (خرائط Google، OpenStreetMap، إلخ.). |
| **UTM** | EPSG:6387 | يقسم العالم إلى **60 منطقة** لتحديد المواقع المحلية بدقة. |
| **MGRS** | - | نظام الإحداثيات الشبكية العسكرية (امتداد لنظام UTM). |
...
- **<Translate android="true" ids="daynight_mode_sensor"/>** (*أندرويد فقط*). يستخدم مستشعر الضوء في الجهاز للتبديل تلقائيًا بين المظاهر الفاتحة والداكنة وفقًا لمستويات الإضاءة المحيطة.
...
- **<Translate ios="true" ids="rendering_attr_showSurfaces_name"/>**. يعرض نوع سطح الطريق. يساعد لون الطريق على فهم ماهية سطح الطريق، مثل الأسفلت، أو العشب، أو الرمل. انظر [مفتاح الخريطة](../map-legend/index.md).
...
- **<Translate ios="true" ids="rendering_attr_showAccess_name"/>**. يعرض إمكانية الوصول إلى الطريق: خاص أو مسموح به، للطوارئ فقط، أو طريق برسوم. انظر [مفتاح الخريطة](../map-legend/index.md) للعثور على الطرق المتاحة.
...
- **<Translate ios="true" ids="rendering_attr_OSMMapperAssistant_name"/>**. صُممت هذه الميزة للمخططين، حيث تعرض المراجع والملاحظات والتعليقات من المستخدمين الآخرين على الخريطة.
...
- **<Translate android="true" ids="rendering_attr_natureReserves_name"/>**. عرض لوحة خضراء وعلامات "NR" لـ [أراضي المحميات الطبيعية](https://wiki.openstreetmap.org/wiki/Tag:leisure%3Dnature_reserve). يبرز المناطق المحمية بحدود خضراء وعلامة "NR" لمناطق الحفاظ على الحياة البرية.
...
عند التنقل في المدن، تحتاج إلى عرض أكثر تباينًا لوسائل النقل العام والمحطات. يمكنك النقر على محطة نقل عام وتحديد أحد المسارات أو المحطات من القائمة. يتم عرض مسارات القطارات أيضًا في هذا الإعداد.
...
- **<Translate android="true" ids="rendering_attr_noAdminboundaries_name"/>**. إخفاء الحدود الإقليمية داخل البلدان، ولكن حدود الولايات تظل مرئية.
...
- **<Translate android="true" ids="rendering_attr_showProposed_name"/>**. إخفاء الكائنات المقترحة - تلك الكائنات المخطط لبنائها ولكن لديها مشروع فقط (طرق مقترحة، تقاطعات، مبانٍ، وغيرها).
...
يمكنك اختيار هذا الإعداد عن طريق الضغط مطولاً على زر "+" أو "-" على الشاشة أو:
...
يساعد هذا الإعداد في تغيير تكبير الخريطة. وهو قابل للتطبيق على الخرائط النقطية والمتجهة. بالنسبة للخرائط النقطية، فإنه يطبق تأثير تكبير، بحيث تبدو تسميات النص أكبر أو أصغر. بالنسبة لخرائط المتجهات، فإنه يجعل الخريطة تبدو أكثر أو أقل تفصيلاً، إذا قمت بتعيينها على قيمة منخفضة، فستبدو الخريطة صاخبة/بطيئة.
...
يقوم خيار **لغة الخريطة** بتكوين تهجئة الأسماء على خريطة OsmAnd لمستويات التكبير من 7 إلى 20، شاملة. لمستويات التكبير من 2 إلى 6، يتم عرض الأسماء باللغة المحددة لـ OsmAnd في قائمة إعدادات النظام (انظر قسم *[كيفية تغيير لغة التطبيق](../start-with/first-steps.md#how-to-change-app-language)*).

إذا لم يتم ترجمة الأسماء على الخريطة إلى اللغة المطلوبة من قبل مجتمع OSM، يمكنك استخدام [الأسماء المنقولة حرفيًا](https://en.wikipedia.org/wiki/Transliteration): *<Translate android="true" ids="use_latin_name_if_missing"/>* (أندرويد) أو *<Translate ios="true" ids="translit_names"/>* (iOS).
...
- **استخدم الاسم اللاتيني إذا كان مفقودًا**
    ![لغة الخريطة المنقولة حرفيًا](@site/static/img/map/map-language-transliterate_2.png)
...
إذا كان لديك نمط خريطة مخصص شخصي أو من طرف ثالث تم إنشاؤه وفقًا لـ [المواصفات](../../technical/osmand-file-formats/osmand-rendering-style.md)، فيمكنك تثبيته على جهازك بهذه الطرق:

- انسخ ملف `.render.xml` إلى جهازك وافتحه باستخدام OsmAnd.
- استخدم [حوارات الاستيراد/التصدير القياسية](../personal/import-export.md) لتصدير أو استيراد أنماط العرض. إذا قمت بإنشاء حزمة `.osf`، فإنها تعمل كمكون إضافي يمكنك مشاركته مع الآخرين.
