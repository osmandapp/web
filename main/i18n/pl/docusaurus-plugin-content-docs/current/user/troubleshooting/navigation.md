...
2. Dla urządzeń z wyższej półki można zwiększyć pamięć do 512 MB lub 1024 MB - [Pamięć przydzielona urządzeniom](../plugins/development.md#memory-settings).
...
### Dlaczego niektóre ostrzeżenia o fotoradarach mogą nie być wyzwalane {#why-some-speed-camera-warnings-may-not-be-triggered}
...
OsmAnd obsługuje obie metody. Ostrzeżenia o fotoradarach mogą być wyzwalane dla fotoradarów oznaczonych `highway=speed_camera`, a także dla fotoradarów zdefiniowanych za pomocą relacji `enforcement`.

Jeśli węzeł fotoradaru jest umieszczony bezpośrednio na drodze, tag `highway=speed_camera` jest wystarczający, aby OsmAnd go wykrył i wyświetlił ostrzeżenia.

Jeśli fotoradar jest oznaczony obok drogi, a nie na niej, musi być połączony z drogą za pomocą relacji `enforcement`. W przeciwnym razie OsmAnd może nie powiązać fotoradaru z drogą i ostrzeżenie nie zostanie wyzwolone.
