...
Przyciski mapy, w tym *Powiększenie*, *Wyszukiwanie*, *Wskazówki dojazdu*, *Kompas*, *Moja lokalizacja*, *Tryb 3D* i *Menu*, są podstawowymi elementami sterującymi do interakcji z mapą. Każdy przycisk oferuje specyficzne funkcje, które poprawiają nawigację i korzystanie z mapy.
...
Przycisk **Wskazówki dojazdu** jest niezbędny do planowania i nawigacji:

- [Budowanie trasy](../navigation/index.md). Użyj tego przycisku, aby utworzyć trasę.
- [Rozpocznij nawigację](../navigation/index.md). Rozpocznij nawigację zakręt po zakręcie.
- W trybie nawigacji przycisk *Wskazówki dojazdu* nie jest domyślnie widoczny, ale pojawia się po krótkim dotknięciu mapy.
...
![Przycisk wyszukiwania](@site/static/img/widgets/search_button.png)

Przycisk wyszukiwania zapewnia szybki dostęp do [funkcji wyszukiwania](../search/index.md), umożliwiając wyszukiwanie lokalizacji, punktów POI i innych informacji bezpośrednio z mapy.


## Kompas {#compass}

Przycisk Kompas wskazuje orientację mapy na ekranie urządzenia. Aby uzyskać dodatkowe informacje, zapoznaj się z artykułem [Interakcja z mapą](../map/interact-with-map.md#map-orientation--compass).


### Tryby orientacji mapy {#map-orientation-modes}

- ![Kompas](@site/static/img/widgets/map_butt_manually_ios.png)
**Obrócony ręcznie**. W tym trybie można ręcznie obracać mapę za pomocą [gestu dwoma palcami (dotknięcie dwoma palcami i obrót)](../map/interact-with-map.md#gestures) zgodnie z własnymi preferencjami. Orientacja mapy nie zależy od kierunku podróży ani kompasu urządzenia, ale jest określana przez użytkownika. Tryb ręcznego obrotu jest ustawiony domyślnie.

- ![Kompas](@site/static/img/widgets/map_butt_movem_dir_ios.png)
**Kierunek ruchu**. W tym trybie mapa jest zorientowana zgodnie z kierunkiem ruchu znanym z danych GPS. Na przykład, jeśli skręcisz w prawo, mapa również się obraca, tak aby prawa strona ekranu odpowiadała kierunkowi ruchu. Aby uzyskać szczegółowe informacje, przejdź do [Obróć mapę według kierunku](../map/interact-with-map#rotate-map-by-bearing).
...
**Północ na górze**. W tym trybie mapa jest ustalona (z twardą blokadą) w kierunku północnym, który jest górną krawędzią urządzenia. Niezależnie od kierunku, w którym porusza się urządzenie, mapa pozostaje statyczna, a Ty możesz zobaczyć swoją pozycję w stosunku do otoczenia. Mapa nie ma automatycznego ani ręcznego obrotu.

### Zachowanie przycisku kompasu {#compass-tapping-behavior}
...
- *<Translate android="true" ids="shared_string_hidden"/>*. Jeśli ten tryb jest wybrany dla przycisku, musisz użyć gestu, aby zmienić widok mapy. Dotknij mapy dwoma palcami i przesuń je w górę i w dół, aby dostosować nachylenie mapy.
- *<Translate android="true" ids="shared_string_visible"/>*. Przycisk jest zawsze wyświetlany na ekranie mapy.
- *<Translate android="true" ids="visible_in_3d_mode"/>*. Przycisk jest wyświetlany na ekranie mapy po zmianie widoku mapy za pomocą gestu [*dwoma palcami i przesunięciem*](../map/interact-with-map.md#gestures).
...
2. **Zapisuje kąt**. Po przełączeniu na tryb 3D i ręcznej zmianie kąta za pomocą gestu, kąt jest zapisywany i używany przy następnym włączeniu trybu 3D. Ta opcja jest zapisywana tylko dla profilu wybranego w danym momencie.

3. **Przenoszenie**. Możesz umieścić przycisk w dowolnym miejscu na ekranie urządzenia. Aby to zrobić, przytrzymaj przycisk i nie odrywając palca, przeciągnij go w wymagane miejsce.

4. **Pozycja przycisku**. Pozycja przycisku na mapie na ekranie aplikacji jest zapisywana oddzielnie dla każdego profilu.

5. **Rekomendacja**. Zaleca się pobranie i włączenie [mapy cieniowania wzgórz](../plugins/topography.md#hillshade-slope-and-altitude-layers) dla wybranego regionu.


## Przyciski niestandardowe {#custom-buttons}

[Widżet szybkiej akcji](./quick-action.md) to konfigurowalny przycisk z różnymi typami akcji, które można przypisać. Możliwe jest również posiadanie wielu [przycisków niestandardowych](./quick-action.md#custom-buttons).
...
Ustawienia wyglądu przycisku mapy są dostępne za pośrednictwem następujących menu:
