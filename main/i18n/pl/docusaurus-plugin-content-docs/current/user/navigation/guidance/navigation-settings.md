...
Aby nawigacja była udana, ważne jest, aby odpowiednio skonfigurować ustawienia zgodnie z Twoimi potrzebami i sposobem podróżowania – samochodem, pieszo, konno czy na nartach. Ten artykuł opisuje podstawowe parametry trasy, [komunikaty głosowe](#voice-prompts), [powiadomienia ekranowe](#screen-alerts), [zachowanie mapy podczas nawigacji](#map-during-navigation) oraz [wygląd linii trasy](#customize-route-line). Szczegółowo opisuje również ustawienia pojazdu, takie jak [prędkość domyślna](#default-speed) i [prędkość maksymalna/minimalna](#road-speeds), [typ silnika](#fuel-used-by-motor) do obliczania emisji CO2, [pojemność zbiornika paliwa](#fuel-tank-capacity) oraz [rozmiar/waga](#size-parameters), które mogą wpływać na trasę. Dostosowanie tych ustawień pomoże Ci w pełni wykorzystać możliwości OsmAnd i dotrzeć do celu bez zbędnych opóźnień.
...
Istnieją trzy sposoby dostępu do menu Ustawienia nawigacji.
...
Nawigacja jest ważną częścią podróży, a wybór *odpowiedniego typu* nawigacji może znacznie ułatwić podróż. Typ nawigacji, który wybierzesz, może zależeć od sposobu podróżowania i tego, czy masz połączenie z Internetem.

- **Nawigacja online**
Wykorzystuje głównie specjalny system routingu lub stronę internetową, która umożliwia aplikacji OsmAnd wyznaczenie trasy na podstawie bieżących informacji i innych czynników. O wyborze najlepszej trasy online możesz przeczytać w artykule [Routing online](../routing/online-routing.md). Ten typ nawigacji jest dostępny tylko dla *wersji Android* aplikacji.
...
Typ nawigacji określa, które zasady są używane do obliczania tras. Jeśli potrzebujesz określonego typu nawigacji, możesz **zaimportować własny zmodyfikowany plik routingu *XML***. Więcej o zasadach routingu przeczytasz na naszej [stronie GitHub](https://github.com/osmandapp/OsmAnd-resources/blob/master/routing). Istnieje również osobny artykuł o formacie XML i jego zastosowaniu, [Styl renderowania mapy](../../../technical/osmand-file-formats/osmand-rendering-style.md), który może pomóc w tworzeniu pliku trasy.
...
*[Routing samochodowy (Ciężarówka, Motocykl)](../routing/car-based-routing.md), [Routing rowerowy (MTB)](../routing/bicycle-based-routing.md), [Routing motorowerowy](../routing/moped-routing.md), [Routing pieszy](../routing/pedestrian-routing.md), [Routing transportu publicznego](../routing/public-transport-navigation.md), [Routing konny](../routing/horse-routing.md), [Routing narciarski](../routing/ski-routing.md), [Routing kolejowy](../routing/train-routing.md), [Routing łodzią](../routing/boat-navigation.md), [Routing punkt-punkt](../routing/direct-to-point-routing.md), [Routing prostą linią](../routing/straight-line-routing.md), [Routing online](../routing/online-routing.md), [Routing BRouter](../routing/brouter.md)*.
...
Parametr **Obliczanie trasy** automatycznie zmienia obliczoną trasę, gdy z niej zboczysz lub pojedziesz w przeciwnym kierunku. W obu przypadkach to ustawienie pomaga utrzymać kierunek jazdy i dotrzeć do celu bez marnowania czasu na ręczne wyszukiwanie nowej trasy.
...
| *Przelicz trasę w przypadku odwrotnego kierunku* | Twoja trasa zostanie przeliczona, jeśli wrócisz do punktu początkowego. | Przy wyłączonym tym ustawieniu ruch do punktu początkowego nie jest uważany za odchylenie od ścieżki (o ile podąża za obliczoną trasą). |
...
| Parametr | Opis | Uwaga |
|:------------|:---------------|
| *<Translate android="true" ids="use_live_routing"/>* | Pozwala na używanie [aktualizacji OsmAnd Live](../../personal/maps-resources.md#osmand-live) podczas routingu. Zauważ, że zalecamy używanie tej opcji tylko do celów testowych. |
| *<Translate android="true" ids="use_two_phase_routing"/>* | Dodaje dodatkową fazę do domyślnego algorytmu (A*) w celu poprawy jakości obliczonej trasy. Ta opcja może być przydatna do znajdowania tras w dużych i złożonych sieciach drogowych, chociaż obliczanie trasy może zająć więcej czasu. |
...
- *<Translate ios="true" ids="routing_settings"/> button* *(lub <Translate ios="true" ids="shared_string_menu,shared_string_navigation"/>) → Wybierz profil → <Translate ios="true" ids="shared_string_settings,routing_settings_2,voice_announces"/>*
...
Pierwsza część artykułu *[Komunikaty głosowe / Powiadomienia](./voice-navigation.md)* szczegółowo opisuje, jak ustawić komunikaty głosowe.

Instrukcje audio podczas nawigacji pomagają kierowcy lub pieszemu dotrzeć do celu podczas [nawigowania wybraną trasą](../setup/route-navigation.md). Zawierają informacje o zakrętach, kierunkach jazdy, znakach drogowych, odległościach do celu i innych czynnikach, które mogą wpływać na nawigację.
...
- Jeśli skonfigurujesz [*komunikaty głosowe i tekstowe*](../../navigation/guidance/voice-navigation.md) dostępne w aplikacji OsmAnd, będziesz mieć wygodniejszą i bezpieczniejszą podróż.
...
- **Ograniczenie prędkości** ([informacje OSM Wiki](https://wiki.openstreetmap.org/wiki/Speed_limits)). W ustawieniach alertów ekranowych *Ograniczenie prędkości* jest wyświetlane na ekranie tylko po włączeniu *Ostrzeżeń o ruchu drogowym*. Jeśli chcesz widzieć ograniczenia prędkości podczas nawigacji bez innych ostrzeżeń, użyj [widżetu](../../widgets/nav-widgets.md#speed-limit).
...
- **Fotoradary** ([informacje OSM Wiki](https://wiki.openstreetmap.org/wiki/Tag:highway%3Dspeed_camera)). Więcej informacji na temat ustawienia Fotoradaru w OsmAnd można znaleźć w [Typach alertów](../../widgets/nav-widgets.md#alert-types) oraz w artykule *Ustawienia globalne*, sekcja [Prawne](../../personal/global-settings.md#legal).
- **Tunele** ([informacje OSM Wiki](https://wiki.openstreetmap.org/wiki/Key:hazmat#Tunnel_restrictions)).

:::note
Typy ostrzeżeń mają różny wygląd wizualny, który zależy od regionu podróży. OsmAnd nie ma na celu przedstawienia w 100% identycznych znaków drogowych, ale wskazuje na pewne podobieństwa.
:::
...
1. Ustaw [*prędkość domyślną* lub *prędkości drogowe*](#default-speed--road-speeds), jako [minimalną i maksymalną prędkość](#road-speeds) pojazdu. Pomoże to aplikacji określić czas potrzebny na pokonanie trasy i pozwoli jej wybrać najlepszą trasę, biorąc pod uwagę ograniczenia prędkości na różnych odcinkach drogi.
2. Określ [*typ*](#fuel-used-by-motor) paliwa używanego przez silnik. Pozwoli to aplikacji oszacować emisję CO2.
3. Wprowadź [*pojemność zbiornika paliwa*](#fuel-tank-capacity) (*Android*), aby dokładnie śledzić poziom paliwa i jego zużycie.
...
Poprawne ustawienie parametrów w aplikacji OsmAnd pomoże uniknąć problemów podczas nawigacji po trasie, wybrać najbardziej odpowiednią trasę w zależności od typu pojazdu i ograniczeń drogowych oraz obliczyć czas podróży.
...
2. Ograniczenia dotyczące ruchu pojazdów na określonych odcinkach dróg, takich jak mosty, tunele o ograniczonej przestrzeni, niskie wiadukty, skomplikowane zakręty lub inne konstrukcje.
...
Ustawienie **Paliwo używane przez silnik** jest dostępne tylko w nawigacji opartej na pojazdach, takiej jak *[Samochód, Motocykl i Ciężarówka](../../navigation/routing/car-based-routing.md)*.
...
- Ulepszone planowanie trasy z optymalizacją postojów na tankowanie zgodnie z pojemnością zbiornika.
...
:::note
Te ustawienia w iOS są dostępne w *Menu → Ustawienia → profil aplikacji → Ustawienia nawigacji → Parametry trasy → Prędkości drogowe*.
:::
-->

- **Prędkość minimalna**
Ustawienie to określa minimalną prędkość jazdy dla wszystkich typów dróg na trasie. Zwiększa priorytet dla dróg z zalecaną prędkością niższą niż prędkość minimalna.
- **Prędkość maksymalna**
...
Funkcja **Szczegółowe wskazówki dotyczące ścieżki** poprawia dokładność nawigacji po trasie podczas korzystania z **tras opartych na ścieżkach**. Gdy wybierzesz ścieżkę jako cel, możesz włączyć ustawienie [Dołącz do dróg](../setup/gpx-navigation.md#attach-to-the-roads) z menu [Śledź ścieżkę](../setup/gpx-navigation.md#follow-track-options). Zapewnia to dopasowanie ścieżki do istniejących dróg, co zapewnia płynniejszą i bardziej precyzyjną nawigację.

Istnieją dwie opcje korzystania z *Szczegółowych wskazówek dotyczących ścieżki*:

- **<Translate android="true" ids="ask_every_time"/>** (*ustawienia domyślne*) – Opcja *Dołącz do dróg* pojawi się w sekcji [Szczegóły trasy](../setup/route-details.md) za każdym razem, gdy zostanie obliczona trasa oparta na ścieżce. Pozwala to zdecydować, czy zastosować dołączenie do dróg dla każdej sesji nawigacji.
