...
**Wtyczka Nagrywanie podróży** to niezbędne narzędzie do rejestrowania tras, ruchów i treningów. Funkcja ta nie tylko umożliwia nagrywanie podróży, ale także pozwala na łatwe ponowne wykorzystanie, modyfikowanie, aktualizowanie i udostępnianie śladów innym.

Dzięki *wtyczce Nagrywanie podróży* możesz tworzyć nowe ślady, wyświetlać nagrane bezpośrednio na mapie i zarządzać istniejącymi nagraniami. Oferuje ona cenne informacje o Twoich podróżach, takie jak prędkość, zmiany wysokości, szczegóły terenu i dane z zewnętrznych czujników. Wtyczka rejestruje Twoją trasę, umożliwiając szczegółową analizę i udostępnianie po podróży.

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Nagrana podróż w systemie Android](@site/static/img/plugins/trip-recording/trip_rec_pl_setup_andr_3.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Nagrana podróż w systemie iOS](@site/static/img/plugins/trip-recording/trip_rec_pl_setup_ios_2.png)
...
Dzięki wtyczce Nagrywanie podróży w OsmAnd możesz łatwo rozpocząć nagrywanie śladu, korzystając z różnych wygodnych opcji. Oto jak możesz rozpocząć swoją podróż:

- [Widżet Dystans/Start-Stop](#distance-start-stop) — Służy do aktywowania nagrywania.

- [<Translate android="true" ids="start_recording"/>](../personal/myplaces.md#tracks) — Naciśnij przycisk w *zakładce <Translate android="true" ids="shared_string_menu,shared_string_my_places,shared_string_gpx_files"/>*.

- Przyciski [Szybka akcja](../widgets/quick-action.md#add-and-delete-actions) — Aby rozpocząć nagrywanie podróży, przejdź do *Menu → Konfiguruj ekran → Przyciski niestandardowe → Szybka akcja → Dodaj akcję → Moje miejsca* i wybierz [Start / Pauza: Nagrywanie podróży](../widgets/quick-action.md#my-places).
...
- *Tylko Android*:
    ...
    - [Program uruchamiający](#launcher-android) — Służy do rozpoczęcia nagrywania.

**Rekomendacje:**

- [Precyzyjna lokalizacja](../start-with/first-steps.md#permission-to-access-the-location) — Aby nagrywanie śladu było dokładne, upewnij się, że OsmAnd ma to uprawnienie.

- [Rozwiązywanie problemów](../troubleshooting/track-recording-issues.md#recorded-tracks-have-gaps) — **Urządzenia z systemem iOS** mogą czasami tymczasowo **zawieszać lub zatrzymywać aplikacje** działające w tle, co może przerwać nagrywanie śladu.
...
- **Menu ustawień** (*Android*). Uzyskaj dostęp do tego menu, aby precyzyjnie dostosować preferencje nagrywania. Aby uzyskać szczegółowy przegląd, zapoznaj się z sekcją [Ustawienia nagrywania podróży](#recording-settings).

- **Zapamiętaj mój wybór** (*iOS*). Po włączeniu nagrywanie śladu rozpoczyna się automatycznie, bez monitowania o dodatkowe ustawienia lub potwierdzenie. Jeśli chcesz zmienić ustawienia nagrywania przed rozpoczęciem nowego śladu, uruchom ponownie wtyczkę Nagrywanie podróży, aby menu ustawień pojawiło się ponownie.

### Program uruchamiający (Android) {#launcher-android}
...
- Dotknij opcji **Rozpocznij nagrywanie**, aby uruchomić aplikację OsmAnd z [ustawieniami nagrywania śladu](#overview-screen) wyświetlonymi na ekranie.
...
- (*Tylko Android*) Aby **zatrzymać** nagrywanie, dotknij elementu *Nagrywanie podróży* w menu głównym.

- Aby **zatrzymać** lub **zapisać** nagrywany ślad, przejdź do *<Translate android="true" ids="shared_string_menu,shared_string_my_places"/>* *→* [*<Translate android="true" ids="shared_string_gpx_files"/> tab*](../personal/myplaces.md#tracks) i dotknij odpowiedniego przycisku w polu *Aktualnie nagrywany ślad*.

- Aby **zapisać, wstrzymać, rozpocząć** nowy segment lub **zakończyć** nagrywanie, użyj przycisków [Szybka akcja](../widgets/quick-action.md#add-and-delete-actions). Przejdź do *Menu → Konfiguruj ekran → Przyciski niestandardowe → Szybka akcja → Dodaj akcję → [Moje miejsca](../widgets/quick-action.md#my-places)* i dodaj jeden lub więcej przycisków szybkiej akcji.
...
W OsmAnd, wersje **Android** i **iOS** nieco inaczej obsługują interfejs nagrywania śladów. Na Androidzie znajdziesz dedykowane menu dla kontekstu *Nagrywania śladu*, natomiast na iOS jest to uproszczone do menu kontekstowego *Aktualnie nagrywanego śladu*.

Podczas nagrywania śladu generowane są dynamiczne wykresy, dostarczające wizualnych danych w czasie rzeczywistym o Twojej podróży. Wykresy te mogą odzwierciedlać informacje dla całej trasy lub tylko dla wybranego segmentu, w zależności od poziomu powiększenia.
...
- **Wykres dla kluczowych informacji**:
    - ***Android***. Dla ***osi Y*** na wykresie można wybrać *do dwóch parametrów* ze wszystkich dostępnych danych, takich jak *Wysokość*, *Nachylenie*, *Prędkość* i [informacje z zewnętrznych czujników](../plugins/external-sensors.md#trip-recording). Dla ***osi X*** wybierz wyświetlanie *Dystansu*, *Czasu* lub *Pory dnia*.
...
- **Statystyki**. Dla Androida wyświetla stały zestaw danych statystycznych niezależnie od ustawień wykresu: *Dystans*, *Zakres czasu*, *Podjazd*, *Zjazd*, *Średnia prędkość*. Dla iOS, każda zakładka, Przegląd, Wysokość i Prędkość, ma inny zestaw danych.
...
| |
| ------------- |
|**Zakładka Przegląd** iOS (dla Androida, parametry osi Y to *Wysokość*, *Prędkość*, a parametr osi X to *Dystans*). Ta zakładka wyświetla wykres przedstawiający zmiany prędkości i wysokości wzdłuż śladu oraz kluczowe szczegóły śladu. Dane śladu dla iOS: *Dystans*, *Zakres czasu*, *Czas rozpoczęcia* i *Czas zakończenia*. Możesz zobaczyć, jak to jest wyświetlane w wersjach Android i iOS poniżej. |
...
| **Zakładka Wysokość** iOS (dla Androida, parametry osi Y to *Wysokość*, *Nachylenie*, a parametr osi X to *Dystans*). Ta zakładka koncentruje się na profilu wysokościowym nagranego śladu, dostarczając informacji o zmianach wysokości i nachyleniach terenu. Kluczowe metryki dla iOS obejmują: *Średnią wysokość*, *Zakres wysokości*, *Podjazd* i *Zjazd*. Wykresy poniżej ilustrują różnice między interfejsami Android i iOS. |
...
| **Zakładka Prędkość** iOS (dla Androida, parametr osi Y to *Prędkość*, a parametr osi X to *Dystans*). Zakładka Prędkość wyróżnia dane związane z prędkością na całym śladzie. Dla iOS zawiera informacje takie jak *Średnia prędkość*, *Maksymalna prędkość*, *Czas w ruchu* i *Skorygowany dystans*. Poniższe zrzuty ekranu pokazują, jak te dane są wyświetlane na obu platformach. |
...
| `ele` | **Wysokość** mierzy wysokość punktu śladu nad poziomem morza, rejestrowaną w metrach. Może to być przydatne do śledzenia zmian wysokości podczas Twoich aktywności, takich jak wędrówki czy przejażdżki rowerowe. |
| `time` | **Sygnatura czasowa** rejestruje dokładną datę i godzinę zalogowania punktu śladu. Pozwala to na precyzyjne śledzenie czasu trwania i momentu Twojej aktywności. |
| `hdop` | **Poziomy rozcieńczenie precyzji (HDOP)** to miara dokładności GPS, odzwierciedlająca potencjalny błąd w lokalizacji z powodu pozycji satelitów. W przeciwieństwie do standardowego HDOP, ślady GPX w OsmAnd używają tej wartości do reprezentowania dokładności poziomej w metrach. Należy pamiętać, że ta implementacja różni się od typowych standardów HDOP. [Dowiedz się więcej](https://github.com/osmandapp/Osmand/issues/3445). |
...
| `heading` | **Kierunek** wskazuje kierunek, w którym skierowany jest przód Twojego pojazdu lub urządzenia, mierzony w stopniach. Różni się od kursu, który jest rzeczywistą ścieżką ruchu. Różnice mogą wystąpić z powodu czynników zewnętrznych, takich jak wiatr, prądy lub warunki drogowe. [Więcej o kierunku](https://en.wikipedia.org/wiki/Heading_(navigation)). |
...
Zanim zaczniesz śledzić swoje podróże, musisz odpowiednio skonfigurować **wtyczkę Nagrywanie podróży**, aby zapewnić optymalną wydajność.
...
- **Precyzyjne dane lokalizacyjne**. Aby zapewnić płynne i użyteczne nagrania do przyszłego użytku, możesz dostosować sposób przechwytywania punktów lokalizacyjnych. Może to obejmować dostosowanie częstotliwości nagrywania, formatu danych (np. liczba punktów na jednostkę odległości) lub odstępów między nagranymi punktami.

- **Ustawienia globalne**. Ustawienia nagrywania podróży są konfigurowane dla każdego profilu, którymi możesz zarządzać w globalnych [ustawieniach](../personal/global-settings.md) aplikacji. Zanim dostosujesz wtyczkę Nagrywanie podróży, wybierz profil, który chcesz skonfigurować, z sekcji [profili](../personal/profiles.md). Stamtąd przejdź do ustawień profilu, aby uzyskać dostęp do opcji nagrywania podróży.

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Konfiguracja nagrywania podróży w systemie Android](@site/static/img/plugins/trip-recording/recording_sett_1_andr.png) ![Konfiguracja nagrywania podróży w systemie Android](@site/static/img/plugins/trip-recording/recording_sett_3_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Konfiguracja nagrywania podróży w systemie iOS](@site/static/img/plugins/trip-recording/recording_sett_1_ios.png) ![Konfiguracja nagrywania podróży w systemie iOS](@site/static/img/plugins/trip-recording/recording_sett_2_ios.png)
...
| **Ogólny interwał logowania** | Określa, jak często rejestrowane są punkty lokalizacji. Domyślnie jest to 5 sekund. Jest włączony za pomocą *widżetu Nagrywanie podróży*. |
| **Minimalne przemieszczenie** | Filtr, który zapobiega rejestrowaniu punktów, gdy ruch jest niewielki lub go nie ma. Pomaga zmniejszyć szum danych.<ul><li>*Skutki uboczne*: Okresy odpoczynku mogą nie być rejestrowane, a małe ruchy mogą być ignorowane. Może to zmniejszyć dane po przetworzeniu, ale może również zapobiec rejestrowaniu błędów GPS.</li><li>*Zalecenie*: Ustaw przemieszczenie na 5 metrów, jeśli chcesz mniej drobnych szczegółów w swoich nagraniach.</li></ul> |
| **Minimalna dokładność** | Filtruje punkty lokalizacji poniżej progu minimalnej dokładności, zgłoszonego przez urządzenie.<ul><li>*Skutki uboczne*: Punkty w obszarach o słabym sygnale (pod mostami, drzewami, między budynkami lub w niektórych warunkach pogodowych) mogą być brakujące.</li><li>*Zalecenie*: Jeśli nie masz pewności, lepiej wyłączyć ten filtr, aby uniknąć utraty danych.</li></ul><details><summary>*Uwaga*</summary>Załóżmy, że GPS został wyłączony tuż przed nagrywaniem. W takim przypadku pierwszy zmierzony punkt może mieć zmniejszoną dokładność, więc lepiej poczekać chwilę przed nagraniem punktu lub nagrać najlepszy z 3 kolejnych punktów.</details> |
| **Minimalna prędkość** | Ustawia próg ignorowania punktów zarejestrowanych poniżej określonej prędkości.<ul><li>*Skutki uboczne*: Sekcje, w których prędkość spada poniżej określonego progu, nie będą rejestrowane.</li><li>*Zalecenie*: Zamiast tego użyj filtra *Minimalne przemieszczenie*, ponieważ może on dać lepsze wyniki bez utraty ważnych danych.</li></ul><details><summary>*Uwaga*</summary>Spróbuj najpierw użyć wykrywania ruchu za pomocą filtra minimalnego przemieszczenia logowania (B), może to dać lepsze wyniki, a stracisz mniej danych. Jeśli Twoje ślady pozostają zaszumione przy niskich prędkościach, spróbuj tutaj wartości niezerowych. Należy pamiętać, że niektóre pomiary mogą w ogóle nie zgłaszać wartości prędkości (niektóre metody oparte na sieci), w takim przypadku nic nie zarejestrujesz.<br/><br/>Sprawdzenie prędkości > 0: Większość chipsetów GPS zgłasza wartość prędkości tylko wtedy, gdy algorytm ustali, że jesteś w ruchu, i żadnej, jeśli nie. Stąd użycie ustawienia > 0 w tym filtrze w pewnym sensie wykorzystuje wykrywanie ruchu chipsetu GPS. Ale nawet jeśli nie jest filtrowane tutaj w czasie nagrywania, nadal używamy tej funkcji w naszej analizie GPX do określenia skorygowanego dystansu, tzn. wartość wyświetlana w tym polu to dystans zarejestrowany podczas ruchu.</details> |
| **Automatyczne dzielenie nagrań po przerwie** | Automatycznie dzieli ślady na podstawie przerw czasowych między zarejestrowanymi punktami.<ul><li>Nowy segment rozpoczyna się po 6-minutowej przerwie.</li><li>Nowy ślad rozpoczyna się po 2-godzinnej przerwie.</li><li>Nowy plik rozpoczyna się, gdy zmienia się data.</li><li>Przerwy mogą wynikać z utraty sygnału GPS, niskiej prędkości lub ustawień konfiguracji.</li></ul><details><summary>*Uwaga*</summary>Przerwa jest identyfikowana, gdy nie są rejestrowane żadne punkty. Może się to zdarzyć, ponieważ lokalizacja nie jest wykrywana lub jest wykrywana, ale nie jest rejestrowana. Może to być spowodowane kilkoma czynnikami, w tym słabymi sygnałami GPS z powodu złych warunków pogodowych lub spadkiem prędkości ruchu poniżej skonfigurowanego progu. W takich przypadkach, nawet jeśli urządzenie może wykryć lokalizację, nie rejestruje jej.<br/><br/>Te luki w zarejestrowanych danych mogą wywołać utworzenie nowego segmentu w ramach tego samego śladu, nowego śladu w tym samym pliku lub nowego pliku GPX w ramach jednego nagrania. Jest to zarządzane w ramach jednej sesji nagrywania start/stop.</details> |
...
| **Resetuj ustawienia wtyczki do domyślnych** | Resetuje wszystkie ustawienia nagrywania podróży dla bieżącego profilu do ich domyślnych wartości. |
...
![Powiadomienie o nagrywaniu podróży](@site/static/img/plugins/trip-recording/trip_rec_notific_1_andr.png)
...
To zachowanie jest wymagane przez Androida dla każdej usługi działającej na pierwszym planie, takiej jak nagrywanie podróży, aby pozostała widoczna dla Ciebie.
...
![Powiadomienie o nagrywaniu podróży](@site/static/img/plugins/trip-recording/trip_rec_notification_andr.png)

- W ***Ustawieniach Androida → Powiadomienia i pasek stanu → Powiadomienia na ekranie blokady*** możesz usunąć OsmAnd z listy aplikacji, aby zapobiec wyświetlaniu powiadomień na ekranie blokady, unikając przypadkowego włączenia ekranu. Nie wpłynie to na nagrywanie śladu. Powiadomienia nadal będą pojawiać się w regularnym obszarze powiadomień.
...
![Powiadomienie o nagrywaniu podróży](@site/static/img/plugins/trip-recording/trip_rec_notification_badge_andr.png)
...
![Dodawanie widżetów nagrywania w systemie iOS](@site/static/img/plugins/trip-recording/add_recording_widgets_ios.png)

</TabItem>

</Tabs>

[Widżet Nagrywanie podróży](../widgets/info-widgets.md#trip-recording-widgets) zapewnia łatwy sposób monitorowania stanu nagrywania i szybkiego dostępu do ustawień i szczegółów nagrywania. Ten widżet jest automatycznie dodawany do ekranu po włączeniu wtyczki Nagrywanie podróży.
...
![Widżet nagrywania podróży](@site/static/img/plugins/trip-recording/trip_rec_widgets_andr.png)
...
![Widżet nagrywania podróży](@site/static/img/widgets/tr_rec_wid_conf_scr.png) ![Widżet nagrywania podróży](@site/static/img/widgets/tr_rec_wid_conf_scr_2.png)
...
Widżet *Dystans* wyświetla całkowity dystans bieżącej nagranej podróży i działa jako główny interfejs do zarządzania nagraniami. Dotknięcie go ujawnia [okno dialogowe Nagrywanie podróży](#start-a-dialog), gdzie można rozpocząć, zatrzymać i wyświetlić szczegółowe informacje o śladzie.
...
| ![Widżet nagrywania podróży (REC)](@site/static/img/widgets/tr_rec_wid_rec.png) |
| Aby otworzyć [okno dialogowe Nagrywanie podróży](#start-a-dialog) po dotknięciu nieaktywnego widżetu, włącz opcję *Pokaż okno dialogowe startowe* w ustawieniach wtyczki Nagrywanie podróży. Jeśli opcja jest wyłączona, nagrywanie rozpocznie się natychmiast po dotknięciu widżetu bez otwierania okna dialogowego.|
...
|**Zjazd**. Wskazuje całkowity spadek wysokości zgromadzony podczas bieżącego nagrywania podróży. |
|![widżety](@site/static/img/widgets/tr_rec_wid_dow.png)|

Jeśli masz wybranych wiele widżetów — *Czas trwania*, *Podjazd* lub *Zjazd* — możesz uzyskać dostęp do tego samego okna dialogowego dla każdego z nich bez konieczności przełączania lub zamykania. Ten ujednolicony interfejs ułatwia płynne przeglądanie i zarządzanie wszystkimi powiązanymi informacjami.
...
- Nagrywanie podróży niespodziewanie się zatrzymuje.
    - Sprawdź, czy *ustawienia optymalizacji baterii* Twojego urządzenia nie zatrzymują nagrywania. Niektóre urządzenia mogą wstrzymywać aktywność w tle, aby oszczędzać energię. Przejdź do *Ustawienia urządzenia → Bateria*.
