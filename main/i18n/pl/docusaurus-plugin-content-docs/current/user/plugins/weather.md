...
:::info Funkcja płatna
 <ProFeature/> Wtyczka Pogoda jest [płatną funkcją](../purchases/index.md) aplikacji OsmAnd.
:::

Interaktywne warstwy mapy pogodowej w aplikacji OsmAnd umożliwiają monitorowanie temperatury, ciśnienia atmosferycznego, zachmurzenia, prędkości wiatru i opadów w Twoim mieście lub dowolnej innej lokalizacji na interaktywnej mapie globalnej. Wtyczka Pogoda zapewnia godzinowe prognozy pogody na 7 dni z wyprzedzeniem bezpośrednio na mapie OsmAnd. Informacje pogodowe można również pobrać do użytku offline.
...
**4.** [Wyświetl **warstwy pogodowe**](#customize-weather-layers) na mapie za pomocą elementu menu Pogoda w menu Konfiguruj mapę (pamiętaj, aby oddalić widok).

**5.** [Wyświetl **prognozę pogody**](#weather-forecast-screen) dla wybranego obszaru (to menu pozwala na niezaśmiecanie głównego ekranu mapy OsmAnd warstwami pogodowymi).

**6.** [Pobierz](#offline-forecast) prognozę pogody, jeśli planujesz używać jej offline.

**7.** [Włącz **widżety pogodowe**](#weather-widgets), jeśli chcesz codziennie widzieć aktualne informacje pogodowe.
...
Główne menu boczne zawiera dedykowany element menu **Pogoda**, który zapewnia szybki dostęp do wszystkich narzędzi pogodowych. *Ekran prognozy pogody* wyświetla informacje o *temperaturze*, *ciśnieniu atmosferycznym*, *prędkości wiatru*, *zachmurzeniu* i *opadach*.
...
### Dostosowywanie warstw pogodowych {#customize-weather-layers}
...
Aby wyświetlić warstwy pogodowe na ekranie mapy OsmAnd, należy użyć menu *Konfiguruj mapę*. Przejdź do menu, znajdź element *Pogoda* na liście (dostępny tylko po włączeniu wtyczki) i dotknij go.
...
Ważne jest, aby zdecydować, w jakich jednostkach miary wygodnie jest wyświetlać [dane pogodowe](#weather-layers) na mapie i w sekcji [widżetów pogodowych](#weather-widgets) ekranu. Możesz wybrać odpowiednie jednostki miary dla każdego parametru lub użyć wartości domyślnych (*tylko iOS*). W wersji iOS aplikacji OsmAnd możesz również włączyć opcję prognozy offline, aby uniknąć używania danych mobilnych podczas podróży.

:::note
W wersji OsmAnd na iOS możesz zmienić jednostki miary i poziom przezroczystości w menu, które otwiera się po długim naciśnięciu ikony wymaganej warstwy mapy.
:::

### Warstwy pogodowe {#weather-layers}

Wszystkie dane pogodowe są przedstawione jako oddzielne warstwy mapy. Aby zobaczyć warstwy pogodowe na mapie, musisz oddalić widok, są one dostępne tylko w skalach powiększenia 2-12. Warstwy pogodowe używają [palet pogodowych](../personal/color-palette-schemes.md#weather) do kolorowania mapy, te palety mogą być [zmienione](../personal/color-palette-schemes.md#edit-palette-file) w razie potrzeby.
...
| **Temperatura** wyraża ilościowe postrzeganie ciepła i zimna. Mierzona jest w stopniach. W OsmAnd możesz wybrać między skalą Celsjusza i Fahrenheita. |
|![Temperature sheme](@site/static/img/plugins/weather/temperature.png)|
| **Wiatr** to ruch powietrza spowodowany nierównomiernym nagrzewaniem Ziemi przez słońce i jej własnym obrotem. Wiatry mogą wahać się od lekkich powiewów do klęsk żywiołowych, takich jak huragany i tornada. W OsmAnd prędkość wiatru może być wyświetlana w metrach na sekundę (m/s), kilometrach na godzinę (km/h), milach na godzinę (mph) i węzłach (kt). |
...
| **Ciśnienie atmosferyczne** jest spowodowane ciężarem powietrza nad punktem pomiarowym. Zmniejsza się wraz ze wzrostem wysokości. Ciśnienie atmosferyczne może być wyświetlane w hektopaskalach (hPa), milimetrach słupa rtęci (mmHg) i calach słupa rtęci (inHg). |
...
Aby uzyskać kompleksowe informacje pogodowe, możesz używać warstw mapy oddzielnie lub łatwo je łączyć. Do warstwy pogodowej można również dodać izoliny jednego określonego typu. Wystarczy dotknąć przycisku *Warstwy* lub *Izoliny* na ekranie.
...
Aby przełączyć widoczność warstw pogodowych, możesz użyć przycisków *Szybkich działań* na ekranie mapy. Ogólne ustawienia pokazywania lub ukrywania warstw można znaleźć w oddzielnej [sekcji Pogoda](#weather-forecast-screen) w *Menu głównym*. W artykule [Szybkie działania](../widgets/quick-action.md#configure-map) znajdziesz listę warstw dostępnych do wyświetlenia. Jeśli potrzebujesz szybkiego dostępu do tej konfiguracji mapy, użyj narzędzia *Przycisk niestandardowy*.

- Przejdź do [Dodaj akcję](../widgets/quick-action.md#custom-buttons): *Menu → Konfiguruj ekran → Przyciski niestandardowe → Szybkie działanie → Dodaj akcję → Konfiguruj mapę*.
- Dodaj jeden lub więcej przycisków QA, aby zmienić widoczność określonej warstwy pogodowej.
...
Dla każdego kraju możesz pobrać **prognozę offline** (do 7 dni). Im bliżej aktualnej daty, tym dokładniejsza będzie. Jeśli chcesz korzystać z prognozy pogody offline, powinieneś wcześniej pobrać dane pogodowe.

W menu pobierania prognozy możesz uzyskać informacje i ustawić parametry, aby dane pogodowe były aktualne.

- **Utworzono** (*Android*) / **Zaktualizowano** (*iOS*). Pokazuje, kiedy nastąpiła ostatnia aktualizacja.
...
- Przycisk **Usuń** *kosz* (*Android*) / **Usuń prognozę** (*iOS*). Pozwala usunąć wszystkie dane prognozy dla tego regionu.
...
- Aby aktywować widżety pogodowe, wybierz element menu głównego *Konfiguruj ekran*, panel, na którym mają być umieszczone, oraz wymagane widżety z listy.
