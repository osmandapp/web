...
Ta sekcja zawiera ustawienia motywu aplikacji i przycisku kompasu, jednostek i formatów danych mapy i profilu, zewnętrznych urządzeń wejściowych oraz wiele innych ustawień, aby stworzyć optymalną konfigurację dla wybranego profilu.
...
W sekcji **Jednostki i formaty** można dostosować jednostki miary i formaty, aby dopasować je do osobistych preferencji i standardów regionalnych, zapewniając wygodne i znajome wyświetlanie wszystkich danych.
...
|          | <Translate android="true" ids="shared_string_milliradians"/> | Wszystkie wartości kątowe mają [wartość miliradiana](https://en.wikipedia.org/wiki/Milliradian).  |
|  |  |  |
...
Aby uzyskać dostęp do ustawień wtyczki, musisz najpierw [**włączyć wtyczkę**](../plugins/index.md#enable--disable) w *sekcji Wtyczki w menu głównym*.
:::
...
- **Usuń profil**:
    ...
    - Domyślne profile nie mogą zostać usunięte.
