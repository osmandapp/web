...
Najnowszą bezpłatną wersję OsmAnd można pobrać z wymienionych sklepów z aplikacjami.
...
- Nowy styl mapy morskiej „Marine” z szerokimi opcjami personalizacji.
- Dodano szczegóły ulic i miast do wyników wyszukiwania.
- Nowe widżety nagrywania podróży: Maks. prędkość, Średnie nachylenie oraz ulepszony Podjazdy/Zjazdy.
- Ulepszono prędkość renderowania mapy.
- Podstawowa automatyczna synchronizacja z OsmAnd Cloud.
- Ulepszono łączność OBDII BLE.
- Aktualizacje funkcji „Analizuj według interwału”.
- Ulepszono narzędzie Planuj trasę.
- Dodano możliwość ustawiania jednostek wysokości oddzielnie od jednostek odległości.
- Ulepszono szybką akcję orientacji mapy z wybieranymi trybami kompasu.
- Ulepszenia Android Auto.
- Odwrócone orientacje ekranu.
...
- Wybierz trasy narciarskie i MTB na mapie, aby uzyskać szczegółowe informacje
...
- Android Auto: wyświetlanie ETA i odległości do następnego punktu; nazwy miast pojawiają się teraz w wynikach wyszukiwania
...
- Wprowadzono nowe szybkie akcje do nagrywania podróży i blokady ekranu dotykowego.
...
- Oddzielono parametry widoczności dla opcji koloryzacji terenu.
...
- Edycja OSM: usunięto metodę autoryzacji loginu/hasła.
...
- Poprawiono dostosowanie wyglądu dla grupy ścieżek.
- Dodano filtry według danych z czujników dla ścieżek.
- Dodano obsługę dodatkowych tagów GPX.
- Dostosuj „Dystans podczas nawigacji”: wybierz między precyzyjnymi lub zaokrąglonymi liczbami.
...
- Czujniki BLE/ANT+: przebudowano połączenie, zapis i przeglądanie otrzymanych danych do GPX.
- Bezpłatna kopia zapasowa ulubionych i ustawień do OsmAnd Cloud.
...
- Android Auto: nowe menu startowe dla dostępu do ulubionych, ścieżek i kategorii POI.
...
- Dodano klasy „Materiałów niebezpiecznych” do nawigacji ciężarówek w Ameryce Północnej.
...
- Dodano obsługę tematycznych ikon.
- OsmAnd Cloud: dodano opcję usunięcia konta.
- Dostępność: rozmiar celu dotyku dla przycisków zmieniono na 48 dp; naprawiono problem z widocznością interfejsu mapy dla czytników ekranu.
...
- Nowy, szybszy silnik renderowania mapy w wersji 2 (OpenGL), z widokiem 2.5D
- Płynna animacja zmian lokalizacji podczas ruchu
- Nowe widżety dla współrzędnych środka mapy i wysokości geolokalizacji
...
- Trasy piesze/rowerowe/podróżnicze: dotknij symbolu trasy i uzyskaj pełne informacje o trasie.
...
- Planowanie trasy: po uruchomieniu zostanie użyty wybrany profil
...
- Odległość po dotknięciu przeniesiono z linijki promienia do osobnej opcji
...
- Dodano nowe typy POI, kluby: muzyczne, społeczne, harcerskie, wolnomularskie
...
- Zaktualizowano funkcję „Zaplanuj trasę”: umożliwia używanie różnych typów nawigacji dla każdego segmentu i włączanie ścieżek
...
- Ścieżki są teraz dotykalne, mają menu kontekstowe z podstawowymi informacjami.
...
- Twórz własne filtry/mapy POI z pełną elastycznością
...
- Przebudowano ustawienia wtyczek
- Przebudowano ekran ustawień dla szybkiego dostępu do wszystkich profili
...
- Dodano poprawną transliterację do mapy Japonii
...
- Profile aplikacji: utwórz niestandardowy profil dla własnych potrzeb, z niestandardową ikoną i kolorem
...
- Naprawiono awarię przy uruchomieniu, która występowała z mapami niełacińskimi
...
- Edycje OSM: pokazano ikonę i nazwę kategorii POI, wyświetlono ukończone akcje
...
- Nowe dane Wikipedii uporządkowane według krajów i zawierające znacznie więcej punktów niż wcześniej
...
- Pomarańczowy kolor dla POI, które zostaną zamknięte w ciągu 30 minut
...
- Dodano komunikat głosowy o powrocie na trasę
...
- Lepszy interfejs do konfiguracji mapy i wyglądu ekranu
- Obsługa Android Wear
- Drukuj plan podróży i informacje o skręcie
...
- Szczegółowe statystyki / podział ścieżki GPX według czasu lub odległości i sprawdź różnicę wysokości/prędkość
- Obsługuje import GPX/KML (konwersja z KML do GPX)
...
- Więcej możliwości komunikatów głosowych (ogłaszanie nazw ulic)
...
- Poprawki błędów
- Opcja stylu mapy „Widok turystyczny” z większą liczbą szczegółów mapy, nowymi stylami dróg, większym kontrastem i opcją wędrówek alpejskich (skala SAC)
...
- Obsługa skryptów arabskich, kannada i języków RTL
...
- Wykrywanie kierunku: „Wykonaj zawrót” jest wskazywane i ogłaszane, gdy jedzie się w przeciwnym kierunku lub po pominięciu skrętu
- Optyczne wskazanie zbliżającego się skrętu w odległości 100 m poprzez zmianę koloru strzałki kierunkowej
- Dane Wikipedii offline z artykułami
...
- Poprawa monitorowania GPX (filtrowanie niektórych niedokładnych pozycji i zapisywanie dokładności/wysokości/prędkości)
...
- Ulepszenie nawigacji offline (nadal eksperymentalne)
- Opcje nawigacji GPX