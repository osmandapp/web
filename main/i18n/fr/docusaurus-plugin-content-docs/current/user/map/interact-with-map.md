...
|:------------|:-------------|
| **Déplacer** | Touchez et maintenez la carte avec **un** doigt, puis faites glisser pour vous déplacer. |
| **Glisser** | Balayez la carte avec **un** doigt. |
| **Zoomer** | Touchez deux fois la carte avec **un** doigt. <br/> Touchez deux fois avec **un** doigt et balayez vers le bas. <br/> Pincez avec **deux** doigts pour zoomer. |
| **Dézoomer**| Touchez deux fois avec **deux** doigts. <br/> Touchez deux fois avec **un** doigt et balayez vers le haut. <br/> Pincez avec **deux** doigts pour dézoomer. |
| **Pivoter** | Touchez la carte avec **deux** doigts, puis faites pivoter vos doigts dans un mouvement circulaire. |
| **Incliner (3D)** | Touchez avec **deux** doigts et déplacez-les vers le haut ou vers le bas. <br/> Disponible uniquement avec le [moteur de rendu de carte](../personal/global-settings.md#map-rendering-engine) version 2 (OpenGL). |
...
Les *modes d'orientation de la carte* vous permettent de choisir comment la carte est affichée sur l'écran de l'appareil. OsmAnd propose des modes tels que **Rotation manuelle**, **Direction du mouvement**, **Direction de la boussole** et **Le Nord est en haut**. L'activation de chaque mode modifie la façon dont la carte est orientée selon l'option sélectionnée. Pour plus de détails, consultez l'article [Boutons de carte](../widgets/map-buttons.md#compass).
...
Le bouton boussole indique comment la [carte est orientée](#map-orientation-modes). La *flèche rouge* sur les icônes, ou la direction de la flèche en mode *Direction du mouvement*, indique le Nord. [L'icône sur le bouton boussole](../widgets/map-buttons.md#compass) indique le mode d'orientation de la carte actuel. Le widget du bouton boussole offre trois [options d'interaction](../widgets/map-buttons.md#compass-tapping-behavior) : *Un seul appui* fait pivoter la carte vers le Nord, *Un double appui* alterne entre tous les modes d'orientation de la carte, et *Un appui long* ouvre la liste des modes.


## Faire pivoter la carte par relèvement {#rotate-map-by-bearing}

En mode **faire pivoter la carte par relèvement** ([Direction du mouvement](../widgets/map-buttons.md#compass)), la carte s'aligne automatiquement avec votre direction de mouvement, de sorte que la zone devant vous est affichée en haut de l'écran. Ce mode améliore la navigation en décalant légèrement le centre de la carte vers le bas, montrant plus de l'itinéraire à venir. Si vous êtes immobile, la carte reste fixe.

Vous pouvez activer cette fonction via *Menu → Paramètres → Profils → Paramètres généraux → Apparence → Orientation de la carte* ou en double-tapotant le [bouton Boussole](../widgets/map-buttons.md#compass-tapping-behavior). Pour plus de détails sur le relèvement, voir [ici](../widgets/nav-widgets.md#bearing-widget).
...
Avec le nouveau [moteur de rendu](../personal/global-settings.md#map-rendering-engine) de carte, vous pouvez modifier l'[inclinaison de la caméra](../plugins/development.md#camera-tilt) de 90 (pas d'inclinaison) à 10 degrés. Approximativement à une inclinaison de carte inférieure à 20-22 degrés (ce paramètre dépend du niveau de zoom), la ligne d'horizon imaginaire devient visible. Contrairement à la vraie, l'horizon du programme est toujours droit.
...
Utilisez la fonction **Verrouillage de l'écran tactile** pour éviter les appuis accidentels pendant la navigation. C'est particulièrement utile pour des activités comme le cyclisme ou la randonnée, où des touches involontaires pourraient perturber votre itinéraire.
...
Crée un effet de panoramique de carte fluide par rapport à l'icône *[Ma position](../personal/profiles/#profile-appearance)* lorsque vous êtes en mouvement. L'effet introduit un léger décalage par rapport à la vérité terrain d'environ 1 seconde. L'activation de cette option a été signalée comme créant des problèmes de décalage prononcés dans certaines circonstances, désactivez-la si de tels problèmes surviennent.


## Dispositifs d'entrée externes {#external-input-devices}

Les boutons des dispositifs d'entrée externes offrent un moyen pratique et efficace d'interagir avec la carte et l'interface de l'application OsmAnd. Ils sont situés sur des dispositifs externes, tels que des **claviers Bluetooth ou d'autres types de claviers**, des **boutons spécialisés sur les dispositifs de navigation de véhicules**, ou des contrôleurs [WunderLINQ](https://blackboxembedded.com/) et [Parrot](https://www.parrot.com/en).

L'une des principales fonctions des boutons sur les dispositifs d'entrée externes est de zoomer et dézoomer la carte. Ils vous permettent également de naviguer sur la carte et de modifier son orientation sans avoir à toucher et à faire des gestes sur l'écran. Les boutons des dispositifs d'entrée externes prennent en charge de nombreuses autres fonctions, telles que l'ouverture du *Menu principal*.

:::note
Le clavier reste fonctionnel même lorsque l'option *Dispositifs d'entrée externes* est désactivée et que *Aucun* est sélectionné.
:::

### Dispositif d'entrée préconfiguré {#preconfigured-input-device}
...
![Appareils externes](@site/static/img/map/external_menu_android.png) ![Appareils externes](@site/static/img/map/external_types_android.png)
...
![Appareils externes](@site/static/img/map/external_types_ios.png)
...
- Il existe une **affectation de paquet action-clé définie** différente pour chaque type de périphérique d'entrée externe. Appuyez sur *<Translate android="true" ids="key_assignments"/>* pour voir une liste d'actions et de clés. Le tableau des clés est présenté ci-dessous.
...
|**&#43;** **=**| *Clavier* | Carte - [Zoomer](#my-location-and-zoom) |
|**&#8722;**| *Clavier* | Carte - [Dézoomer](#my-location-and-zoom) |
|**Appuyer sur retour**| *Clavier* | Naviguer – Écran précédent |
|**&#8595;**| *Wunderlinq* | Carte - [Dézoomer](#my-location-and-zoom) |
|**&#8593;**| *Wunderlinq* | Carte - [Zoomer](#my-location-and-zoom) |
...
| **Appui joystick** <br/> (*Android hérité*) | *Clavier* | Déplacer - [Vers ma position](#my-location-and-zoom) |
| **Bouton média** <br/> (*Android uniquement*)| *Clavier* | Afficher / Masquer - [Notes AV](../plugins/audio-video-notes.md#manage-a-single-note) |
| **&#8592;** <br/> (*Android uniquement*)| *Parrot* | Carte - [Dézoomer](#my-location-and-zoom) |
| **&#8594;** <br/> (*Android uniquement*) | *Parrot* | Carte - [Zoomer](#my-location-and-zoom) |
...
![Appareils externes](@site/static/img/map/external_mypilot_android.png) ![Appareils externes](@site/static/img/map/external_mypilot2_android.png)
...
### Ajout d'actions et d'affectations de touches {#add-action--key-asssigments}

![Appareils externes](@site/static/img/map/external_custom_4_andr.png) ![Appareils externes](@site/static/img/map/external_custom_3_andr.png)
...
- Sélectionnez un type d'appareil, puis appuyez sur l'élément **Affectations de touches**.
...
- Si le bouton que vous attribuez est déjà utilisé pour une autre action, vous recevrez une notification avec l'option de réaffecter le bouton ou d'annuler l'affectation.
- Plus tard, vous pourrez modifier les actions et les affectations de touches ou en ajouter d'autres pour les actions déjà créées, il suffit de sélectionner un élément dans la liste des affectations de touches.

:::

### Supprimer l'affectation de clé {#delete-key-assignment}

![Appareils externes](@site/static/img/map/external_custom_1_andr.png)

Vous pouvez supprimer plusieurs actions inutiles à la fois à l'aide du **bouton Modifier** (en forme de crayon) dans le coin supérieur droit de l'écran :

- ***Supprimer une action*** par action, avec le bouton &nbsp; "**−**" &nbsp; dans le champ de l'élément.
- ***Supprimer toutes les affectations de touches*** pour le type sélectionné en appuyant sur le bouton dans le coin supérieur droit de l'écran à côté de *Modifier le nom*.
