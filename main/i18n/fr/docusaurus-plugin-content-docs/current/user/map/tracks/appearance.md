...
La fonction **Apparence des traces** permet aux utilisateurs de personnaliser la façon dont les traces sont affichées sur la carte. Cela inclut l'ajustement de la couleur, de la largeur, des indicateurs de direction et d'autres éléments visuels. La personnalisation de l'apparence des traces permet de différencier plusieurs traces, de mettre en évidence des itinéraires spécifiques et d'améliorer la lisibilité de la carte.

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Aperçu de l'apparence des traces Android](@site/static/img/map/track-appear-and-1.png) ![Menu Apparence des traces Android](@site/static/img/map/track_appearence_1_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu Apparence des traces iOS](@site/static/img/map/track_appearence_1_ios.png) ![Menu Apparence des traces iOS](@site/static/img/map/track_appearence_2_ios.png)
...
Vous pouvez modifier l'apparence des traces en utilisant l'une des méthodes suivantes :

- **Depuis Mes lieux**. Ouvrez *Menu → Mes lieux → Traces*, sélectionnez une trace, appuyez sur l'*icône Apparence* dans le [menu contextuel des traces](../../map/tracks/track-context-menu.md#overview).
- **Depuis la carte**. Appuyez sur une trace directement sur la carte, puis appuyez sur l'*icône Apparence* dans la *section Aperçu*.
- **Depuis l'enregistrement de trace**. Modifiez l'apparence de la trace actuellement enregistrée via le [menu contextuel d'enregistrement de trace](../../plugins/trip-recording.md#сurrent-track-recording).
...
## Paramètres d'apparence des traces {#track-appearance-settings}
...
Vous pouvez définir la couleur de la trace manuellement ou utiliser la **visualisation des données basée sur la couleur** (par exemple, vitesse, altitude, pente). Si une trace manque de certaines données, OsmAnd affiche les sections manquantes en gris.
...
- **Paramètres gratuits** : *<Translate android="true" ids="track_coloring_solid"/> couleur*, *<Translate android="true" ids="shared_string_speed"/>* (si enregistré) et *<Translate android="true" ids="altitude"/>* (si enregistré).
    ...
    ![Menu Apparence des traces Couleur de la trace Android](@site/static/img/map/track_appearance_menu_track_color_android.png) ![Apparence Couleur de la trace Android](@site/static/img/map/track_appearance_menu_track_color_ios-2.png)
...
![Menu Apparence des traces Épaisseur de la trace Android](@site/static/img/map/track_appearance_menu_track_thickness_android.png) ![Apparence des traces](@site/static/img/map/track_appearance_width_andr.png)
...
![Menu Apparence des traces Intervalle de fractionnement](@site/static/img/map/track_appearance_menu_split_interval_android.png) ![Intervalle de fractionnement](@site/static/img/map/track_appearance_menu_split_interval_ios.png)

Sélectionnez si vous souhaitez fractionner l'intervalle sur la trace **par distance ou par temps**.

### Flèches de direction {#direction-arrows}

![Menu Apparence des traces Flèches de direction Android](@site/static/img/map/track_appearance_menu_direction_arrows_android.png)

Ajoute des **marqueurs de flèche** le long de la trace pour indiquer la direction du mouvement.

### Icônes de début et de fin {#start-and-finish-icons}

![Menu Apparence des traces Icônes de début et de fin Android](@site/static/img/map/track_appearance_menu_sf_icons_android.png)
...
Connecte les **segments de trace** qui ont été interrompus en raison d'une perte GPS ou d'une pause dans l'enregistrement. La fonction **<Translate android="true" ids="join_segments"/>** (*Android*) ou **<Translate ios="true" ids="gpx_join_gaps"/>** (*iOS*) vous permet de connecter ces lacunes dans la trace GPX sélectionnée à l'aide de lignes droites.

### Réinitialiser à l'original {#reset-to-original}

Rétablit tous les **paramètres d'apparence des traces** à leurs valeurs par défaut.


## Trace 3D {#3d-track}

:::info Fonctionnalité payante
La **Trace 3D** est une fonctionnalité payante d'**OsmAnd Pro** pour [iOS](../../purchases/ios.md#pro-features) et [Android](../../purchases/android.md#pro-features) <ProFeature />.
:::
...
![Menu Apparence des traces Visualisation 3D](@site/static/img/map/3d_track_appearance_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu Apparence des traces Visualisation 3D](@site/static/img/map/3d_track_appearance_ios.png)

</TabItem>

</Tabs>

La fonction **Trace 3D** génère une visualisation tridimensionnelle en intégrant des données d'élévation dans des traces 2D standard. Cette fonctionnalité crée une représentation améliorée de la profondeur, permettant une analyse précise de la trace par rapport au terrain. En incorporant les détails d'élévation, la trace 3D offre une vue plus complète de la topographie et du contexte géographique de l'itinéraire.
...
Options incluses : *Aucun*, &nbsp;*Solide*, &nbsp;*Dégradé descendant*, &nbsp;*Dégradé ascendant*, &nbsp;*Altitude*, &nbsp;*Pente*, &nbsp;*Vitesse*.

**<Translate android="true" ids="track_line"/>**
Ajoute une [ligne surlignée](#color) en haut ou en bas de la trace 3D.
Options incluses : *Haut*, &nbsp;*Bas*, &nbsp;*Haut et bas*.

> **REMARQUE** : *La **version Android** de l'application nécessite que le moteur de rendu de la carte [Version 2 (OpenGL)](../../personal/global-settings.md#map-rendering-engine) soit activé pour utiliser la visualisation de trace 3D.*


### Cas d'utilisation {#use-cases}

La fonction de trace 3D est très utile pour des activités comme le parapente, où la visualisation de votre trajectoire de vol peut offrir des informations plus approfondies. En rendant la trace en 3D, vous pouvez utiliser des fonctions telles que la **Hauteur du mur** et la **Couleur du mur** pour extraire des informations visuelles précieuses pour l'analyse.
...
2. **Visualiser par capteur cardiaque avec relief 3D**.
    Une autre application intéressante est la combinaison des données du capteur de fréquence cardiaque avec des cartes en relief 3D. Cela vous permet de cartographier les fluctuations de la fréquence cardiaque directement sur le terrain, vous donnant une vue complète de la façon dont les changements d'altitude ont pu influencer votre effort physique.
...
Options de personnalisation de l'apparence en vrac :
...
- **Couleur** – Appliquer un [coloriage](#color) manuel ou basé sur les données.

- **Largeur** – Ajuster la [largeur de la ligne de trace](#width) à l'aide de préréglages ou manuellement.

- **Intervalle de fractionnement** – [Définir l'intervalle](#split-interval) pour les marqueurs de distance / temps.
    États : *Inchangé*, &nbsp;*Original*, &nbsp;*Sélectionner* : *Temps* ou *Distance*.

Original et inchangé :

- **Original** - Restaure les paramètres originaux du fichier de trace.

- **Inchangé** - Conserve les paramètres actuels pendant la session d'édition.


## Couleurs des traces dans les fichiers GPX {#track-colors-in-gpx-files}

OsmAnd prend en charge les **couleurs de trace personnalisées** dans les fichiers GPX à l'aide de la balise `<osmand:color>` dans la section `<extensions>` d'un élément `<trk>`. Cela permet aux utilisateurs de définir des couleurs pour des traces individuelles, améliorant la différenciation visuelle lorsque plusieurs traces sont affichées sur la carte.
...
2. **Affichage unicolore pour les traces fusionnées** :
...
- Ajustez manuellement les couleurs des traces dans les **Paramètres d'apparence des traces** :
