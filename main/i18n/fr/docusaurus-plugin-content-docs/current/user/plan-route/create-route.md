...
L'outil *Planifier un itinéraire* permet de modifier une trace GPX existante ainsi qu'une [trace importée](../personal/tracks/manage-tracks.md#import). Cependant, si vous devez exclure un grand nombre de points basés sur des critères génériques, [<Translate android="true" ids="shared_string_gps_filter"/>](../map/tracks/track-context-menu.md#gps-filter) pourrait être un outil plus approprié.
...
- [Utiliser les routes à proximité](#attach-track-to-roads). Ce mode utilise des cartes hors ligne pour trouver les routes les plus proches afin de construire une trace, de sorte que les données d'élévation seront récupérées des routes attachées. La géométrie de la trace peut être ajustée.
...
- Correction des informations de virage pour la [navigation par trace](../navigation/setup/gpx-navigation.md#how-to-follow-the-track) (surtout aux ronds-points).
- Noms de rues et informations sur les voies.
- Données d'élévation.
- [Attributs de route](../navigation/setup/route-details.md#road-attributes).

Vous pouvez sélectionner une [valeur seuil](../navigation/setup/gpx-navigation.md#attach-to-the-roads) pour la distance que les points de trace simplifiés peuvent avoir par rapport aux points de trace originaux.
...
- De fusionner des segments : l'option [Joindre des segments](#point-context-menu) supprime l'espace aux segments précédemment séparés.
...
Chaque point de votre itinéraire a son menu contextuel. Il affiche le *numéro de séquence* du point, la *distance depuis le début de l'itinéraire*, la fonction pour le déplacer sur la carte et le supprimer, et un ensemble d'actions avec des sections de l'itinéraire avant et après ce point. Pour modifier, appuyez sur le point requis dans la [liste des points](#adding-points) ou directement sur la carte.

- ***Informations sur le point***. Le nombre de points dans la liste. La distance par rapport au premier point est basée sur le type de routage défini, la distance peut changer lors du recalcul vers le nouveau type si le paramètre *Itinéraire entre les points* est modifié.
...
- ***<Translate ios="true" ids="change_route_type_before"/>***. Vous pouvez modifier le type de routage configuré pour un profil disponible, entre les points sur le segment précédent, ou pour tous les segments du début de l'itinéraire au point sélectionné.

- ***<Translate ios="true" ids="change_route_type_after"/>***. Vous pouvez modifier le type de routage entre les points sur le segment suivant, ou pour tous les segments du point sélectionné au dernier point ajouté.

- ***<Translate ios="true" ids="delete_point"/>***. Supprimer le point sélectionné de l'itinéraire. Similaire à la suppression d'un point dans une liste.
...
En utilisant l'outil *Planifier un itinéraire* et l'option [Itinéraire entre les points](#route-between-points), vous pouvez créer des itinéraires multimodaux où, par exemple, la première partie peut être un itinéraire *cyclable*, la deuxième partie un itinéraire en *ligne droite* puis un itinéraire *piéton*. Notez que la navigation sur les itinéraires multimodaux n'est pas disponible, vous devez donc sélectionner l'un des profils les plus adaptés pour pouvoir suivre les instructions virage par virage.
...
- ***Enregistrement rapide***. Le bouton ***Terminé*** / ***Enregistrer*** (pour les traces existantes) en haut à droite permet d'enregistrer rapidement les modifications et de quitter l'outil *Planifier un itinéraire*. Le nom est généré en fonction de la date actuelle.
...
- [<Translate android="true" ids="route_between_points"/>](#route-between-points). Affiche le profil d'application sélectionné (la valeur par défaut est une ligne droite). Appuyer sur cette action équivaut à appuyer sur le bouton *Profil*, qui ouvre le [menu de profil](../personal/profiles.md) de l'application.
...
- [<Translate android="true" ids="save_as_new_track"/>](#save-route). Enregistre votre itinéraire comme la trace GPX.
...
- **<Translate android="true" ids="reverse_route"/>**. Vous échangez le point de *départ* de l'itinéraire et le dernier point ajouté. Les paramètres de segment d'itinéraire ne changent pas lorsque l'inversion est appliquée.
...
- ***<Translate android="true" ids="shared_string_clear_all"/>***. Il archive complètement toutes vos actions. Un "artefact" reste sur la carte - les lignes pointillées de l'itinéraire juste effacé. Il disparaît à l'ajout suivant de nouveaux points. Vous pouvez annuler la fonction Effacer tout avec le bouton Annuler l'action. La fonction n'affecte pas les parties inchangées des itinéraires ouverts dans l'outil.
