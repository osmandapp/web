...
| *Routage direct vers un point* | Utilise également les cartes hors ligne et propose un routage en ligne droite (ou navigation *point à point*). | Types de navigation : *[Ligne droite](./straight-line-routing.md)*, *[Direct vers un point](./direct-to-point-routing.md)*. |
...
- *[Routage cyclomoteur](./moped-routing.md)* - est basé sur le routage vélo, bien qu'il possède ses propres caractéristiques.
- *[Routage transports en commun](./public-transport-navigation.md)* - peut vous aider lors de vos déplacements en ville.
- *[Routage équestre](./horse-routing.md)* - fournit aux cavaliers les outils dont ils ont besoin pour planifier et profiter de leurs activités d'équitation.
...
- *[Routage en ligne droite](./straight-line-routing.md)* - propose un routage en ligne droite (ou navigation "point à point") qui est recalculé lorsque votre position change.
- *[Routage direct vers un point](./direct-to-point-routing.md)* - propose un routage en ligne droite (ou navigation "point à point") qui reste stable pendant toute la navigation, contrairement au routage en **Ligne droite** (le recalcul d'itinéraire est désactivé).
...
- La fonction Éviter certaines routes fonctionne incorrectement avec l'option [Navigation par trace](../setup/gpx-navigation.md) et n'a pas d'impact sur la [Navigation par marqueurs](../setup/markers-navigation.md#use-markers-in-navigation) simplifiée.
...
Dans ce menu, vous pouvez annuler la sélection précédente de la route qui devrait être évitée. Pour ce faire, appuyez sur une croix (Android) ou sur un **"-"** rouge en face de la route que vous avez l'intention d'utiliser pour le calcul d'itinéraire.
...
Selon les paramètres de votre appareil et les applications disponibles, toute votre liste de routes à éviter ou seulement une partie d'entre elles peut être [exportée](../../personal/import-export.md#export) au format `.osf` vers différents emplacements. Sélectionnez les routes que vous souhaitez exporter dans la liste suggérée.
