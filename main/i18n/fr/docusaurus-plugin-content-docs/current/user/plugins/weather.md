...
**1.** [Acheter un **abonnement OsmAnd Pro**](../purchases/).
...
**7.** [Activer les **widgets Météo**](#weather-widgets) si vous souhaitez voir des informations météorologiques à jour quotidiennement.
...
Pour basculer la visibilité des couches météo, vous pouvez utiliser les boutons *Action rapide* sur l'écran de la carte. Les paramètres généraux pour afficher ou masquer les couches se trouvent dans la [section Météo](#weather-forecast-screen) distincte du *Menu principal*. Dans l'article [Action rapide](../widgets/quick-action.md#configure-map), vous trouverez une liste des couches disponibles pour l'affichage. Si vous avez besoin d'un accès rapide à cette configuration de carte, utilisez l'outil *Bouton personnalisé*.
