...
- [Dépannage](../troubleshooting/track-recording-issues.md#recorded-tracks-have-gaps) — Les **appareils iOS** peuvent parfois **suspendre ou arrêter temporairement les applications** fonctionnant en arrière-plan, ce qui pourrait interrompre l'enregistrement de trace.
...
### Démarrer un dialogue {#start-a-dialog}

<Tabs groupId="operating-systems">

<TabItem value="android" label="Android">

Le dialogue de démarrage s'ouvre si l'option **<Translate android="true" ids="show_start_dialog"/>** est activée dans la section des paramètres de la boîte de dialogue de démarrage.
...
Le dialogue de démarrage s'ouvre si l'option **<Translate ios="true" ids="track_interval_remember"/>** n'est pas activée.
...
- **Intervalle d'enregistrement** (*Android/iOS*). Ce paramètre contrôle la fréquence à laquelle l'application demande des données au capteur GPS pour marquer de nouveaux points sur votre trace. L'ajustement de l'intervalle vous permet d'équilibrer entre le détail de la trace et la consommation de la batterie. Un intervalle plus court se traduira par des traces plus détaillées mais une consommation de batterie plus élevée, tandis qu'un intervalle plus long économise la batterie mais réduit la précision de la trace.
...
- **Menu Paramètres** (*Android*). Accédez à ce menu pour affiner vos préférences d'enregistrement. Pour un aperçu détaillé, consultez la section [Paramètres d'enregistrement de trajet](#recording-settings).

- **Se souvenir de mon choix** (*iOS*). Lorsque cette option est activée, l'enregistrement de trace démarre automatiquement, sans demander de paramètres supplémentaires ni de confirmation. Si vous souhaitez modifier les paramètres d'enregistrement avant de démarrer une nouvelle trace, redémarrez le plugin d'enregistrement de trajet pour faire réapparaître le menu des paramètres.
...
Dans OsmAnd, les versions **Android** et **iOS** gèrent l'interface d'enregistrement de trace légèrement différemment. Sur Android, vous trouverez un menu dédié pour le contexte *Enregistrement de trace*, tandis que sur iOS, cela est intégré dans le menu contextuel *Trace en cours d'enregistrement*.
...
- **Valeurs de données**. Sur le côté droit du graphique se trouvent les valeurs ***les plus élevées***, ***les plus basses*** et ***moyennes*** pour la section visible de la trace.

- **Graphique pour les informations clés** :
    - ***Android***. Pour l'***axe Y*** du graphique, vous pouvez sélectionner *jusqu'à deux paramètres* parmi toutes les données disponibles telles que *Altitude*, *Pente*, *Vitesse* et [informations des capteurs externes](../plugins/external-sensors.md#trip-recording). Pour l'***axe X***, sélectionnez pour afficher soit *Distance*, *Temps* ou *Heure du jour*.
...
- Les **points** représentent des moments individuels de votre chemin enregistré, chacun marqué par des attributs comme les coordonnées, la vitesse, l'altitude et le cap.
...
|-----------------|----------------|
| `trkpt` | **Point de trace** représente une position géographique spécifique détectée pendant votre activité. Chaque point de trace comprend des coordonnées de latitude et de longitude, formant la base de votre chemin enregistré. |
...
| `power` | **Puissance cycliste** quantifie la puissance générée lors du pédalage, capturée à partir de [capteurs externes](../plugins/external-sensors.md). C'est une métrique essentielle pour les cyclistes sérieux qui souhaitent évaluer leurs performances. [En savoir plus](https://docs.osmand.net/docs/technical/osmand-file-formats/osmand-gpx#tags-name-for-sensor-data).   |
...
| `hr` | **Fréquence cardiaque** surveille vos battements par minute, en utilisant les données des [capteurs](../plugins/external-sensors.md) connectés. Cette information peut être vitale pour suivre vos performances cardiovasculaires pendant les entraînements. [En savoir plus](https://docs.osmand.net/docs/technical/osmand-file-formats/osmand-gpx#tags-name-for-sensor-data).  |
...
| **Afficher le dialogue de démarrage** (*Android*) | Active un dialogue où vous pouvez configurer les paramètres avant de commencer l'enregistrement. Si désactivé, l'enregistrement démarre automatiquement. |
...
- Pour désactiver les badges dans les paramètres Android, allez dans les *Paramètres de l'appareil → Applications → OsmAnd → Notifications* et désactivez l'option d'affichage du badge pour cette application.
...
Lorsque les widgets sont affichés sur la carte, appuyer sur l'un d'eux révèle des détails supplémentaires sur la trace et permet l'interaction avec l'enregistrement.
Le widget *Distance* affiche la distance totale de votre trajet enregistré actuel et sert d'interface principale pour gérer vos enregistrements. En appuyant dessus, le [dialogue d'enregistrement de trajet](#start-a-dialog) s'ouvre, où vous pouvez démarrer, arrêter et afficher des informations détaillées sur votre trace.
...
- Si l'option *Afficher le dialogue de démarrage* est désactivée dans les paramètres du plugin d'enregistrement de trajet, appuyer sur le widget actif ouvrira toujours la boîte de dialogue *Enregistrement de trajet*, vous permettant d'accéder à d'autres options et informations.
...
| Pour ouvrir le [dialogue d'enregistrement de trajet](#start-a-dialog) en appuyant sur un widget inactif, activez l'option *Afficher le dialogue de démarrage* dans les paramètres du plugin d'enregistrement de trajet. Si l'option est désactivée, l'enregistrement démarrera immédiatement après avoir appuyé sur le widget sans ouvrir le dialogue. |
...
Si vous avez plusieurs widgets sélectionnés — *Durée*, *Montée* ou *Descente* — vous pouvez accéder au même dialogue pour chacun sans avoir besoin de changer ou de le fermer. Cette interface unifiée facilite la visualisation et la gestion de toutes les informations connexes de manière transparente.
...
- La distance enregistrée ne correspond pas à la distance réelle.
    ...
    - Réduisez le seuil de précision GPS : *Menu → Configurer le profil → Enregistrement de trajet → Précision minimum*.
