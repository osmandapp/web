...
Les **paramètres généraux** sont des [paramètres](../personal/profiles.md) généraux pour tous les profils de l'application OsmAnd. Ce sont les paramètres de base du système OsmAnd, et toute modification apportée affectera l'ensemble de l'application.
...
OsmAnd utilise le profil par défaut, *Parcourir la carte*, comme premier profil pour les nouveaux utilisateurs, et si ce profil a été sélectionné par vous, il sera utilisé chaque fois que l'application sera redémarrée. Vous pouvez sélectionner n'importe quel profil de la liste par défaut, y compris *Dernier utilisé* et *Parcourir la carte*.
...
2. Dans le **<Translate android="true" ids="change_data_storage_folder"/>**, vous pouvez sélectionner un dossier pour enregistrer les données OsmAnd sur votre appareil.  
    ...
    - *Stockage externe 1*. Seule 1 application OsmAnd a accès et les applications Administrateur et USB.
...
:::note Android 12+ (changer le dossier de stockage)
...
Aller à : *Appareil iOS → Réglages → Général → Stockage iPhone → Cartes OsmAnd*  
...
Aller à : *Fichiers → Sur mon iPhone → Cartes OsmAnd*
...
| Affichage de la carte | L'ensemble de la carte est rendu sous forme d'un ensemble de tuiles, et les marqueurs, lignes et textes sont déjà à l'intérieur de ces tuiles. | Tout d'abord, l'ensemble des tuiles est rendu, puis les symboles disponibles sont appliqués à la carte, [couche par couche](../../technical/algorithms/map-rendering-layers.md). |
...
- Vous pouvez supprimer un ou plusieurs éléments de la liste d'historique ou les partager sous forme de fichier (pour accéder à ces fonctions *sous iOS*, appuyez sur le bouton ***Modifier***).  
...
- *Message de démarrage* inclut les promotions, les quiz, les événements, et plus encore.
...
- [*Boîte de dialogue de téléchargement de carte*](../start-with/download-maps.md#download) vous informe que des cartes spécifiques peuvent être téléchargées.
...
- OsmAnd reçoit des données des ressources *Google Fused Location Providers* et *Android GPS and Network Providers*. Vous pouvez basculer **<Translate android="true" id="location_source" />** dans : *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_settings,location_source" />*.

***iOS*** :

- Le système iOS utilise l'*API iOS* pour déterminer la localisation.  
- Tous les paramètres de localisation d'OsmAnd se trouvent dans les paramètres de l'appareil iOS : *Réglages → Cartes OsmAnd → Localisation*  
...
- Par défaut, vos *paramètres OsmAnd* et vos marqueurs de carte [*Favoris*](../personal/favorites.md#automatic-favorites-backup) sont automatiquement sauvegardés par le système Android. Cela vous permet de les restaurer sur de nouveaux appareils ou après une réinstallation de l'application.
...
- Pour trouver les fichiers de sauvegarde, utilisez une application de gestion de fichiers tierce.  
    *REMARQUE : Vous pouvez trouver les fichiers de sauvegarde à l'aide du gestionnaire de fichiers Android : Android → data → net.osmand → files → backup, mais sachez que dans les versions récentes d'Android, l'accès aux répertoires système est limité.*  
...
Ce paramètre vous permet d'activer ou de désactiver les POI avec radars, vous devez redémarrer l'application OsmAnd pour appliquer les modifications.
  
Dans certains pays ou régions, l'utilisation d'applications d'avertissement de radars est illégale. Vous devez faire un choix en fonction des lois de votre pays. Sélectionnez **Garder actif**, et vous recevrez des alertes et des notifications de radars. Sélectionnez **Désinstaller** et toutes les données liées aux radars, telles que les avertissements, les notifications et les POI, seront supprimées jusqu'à ce que vous réinstalliez complètement OsmAnd.  
