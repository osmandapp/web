...
![Paramètres généraux Profil par défaut Android](@site/static/img/personal/profiles/default_profile_ios.png)
...
2.  Dans **<Translate android="true" ids="change_data_storage_folder"/>**, vous pouvez sélectionner un dossier pour enregistrer les données OsmAnd sur votre appareil.  
...
![Paramètres généraux de stockage iOS](@site/static/img/personal/profiles/general_settings_storage_ios.png)  
...
<!-- ![App Fichiers iOS](@site/static/img/personal/storage/files_app_ios.png) ![App Fichiers iOS](@site/static/img/personal/storage/files_app_1_ios.png)  -->

![Paramètres généraux de stockage iOS](@site/static/img/personal/profiles/files-1.png) ![Paramètres généraux de stockage iOS](@site/static/img/personal/profiles/files-2.png)
...
|:---|:--- |:--- |
| Multiplateforme | Non utilisée dans la version iOS. | Convient aux deux versions. |
| Dépendance GPU / CPU | Recommandé pour les appareils peu performants ou obsolètes. | Moteur de rendu puissant. <br /> Disponible uniquement pour [OsmAnd 4.3](https://docs.osmand.net/blog/osmand-android-4-3-released) et les versions ultérieures, et en mode test pour [OsmAnd 4.2](https://docs.osmand.net/blog/osmand-android-4-2-released). |
...
| Affichage de la carte | La carte entière est rendue sous forme d'un ensemble de tuiles, et les marqueurs, les lignes et le texte sont déjà à l'intérieur de ces tuiles. | D'abord, l'ensemble des tuiles est rendu, puis les symboles disponibles sont appliqués à la carte, [couche par couche](../../technical/algorithms/map-rendering-layers.md). |
...
L'image pour la vue 3D de la carte est chargée tuile par tuile, puis tous les symboles. La vitesse de rendu de la carte dépend du nombre d'objets graphiques 2D et 3D qui s'y trouvent, et directement des performances de votre matériel. Par conséquent, la Version 2 (OpenGL) ne peut pas être utilisée sur des appareils peu performants.
...
OsmAnd vous permet de choisir si vous souhaitez *[partager votre activité dans l'application](#analytics)* et vos [identifiants](#identifiers) (*Android uniquement*) dans l'application, la possibilité de modifier votre *[historique](#history)* et d'utiliser un *[proxy](#proxy)* (*Android uniquement*) de votre choix.  
...
- Un UUID aléatoire est utilisé pour télécharger les cartes hors ligne depuis les serveurs OsmAnd afin de contrôler l'utilisation rationnelle des ressources du serveur, de prévoir l'utilisation du trafic et de fournir des rapports mensuels généraux sur les téléchargements de cartes.
- L'UUID change tous les 3 mois.  
- À partir de la version 4.3 d'OsmAnd, vous pouvez désactiver la collecte des données utilisateur lors de l'utilisation de l'application OsmAnd. L'UUID ne sera alors envoyé avec aucune demande de téléchargement.  
...
![Paramètres généraux Dialogues iOS](@site/static/img/personal/profiles/general_settings_dialogs_2_ios.png)  
...
-   *Message de démarrage* inclut des promotions, des quiz, des événements, etc.
-   *<Translate ios="true" ids="do_not_show_discount"/>* (*iOS*) supprime l'affichage des réductions d'application et des messages d'événements locaux spéciaux.
...
-   Vous pouvez choisir les services *Google Play* ou *Android API* qu'OsmAnd utilisera pour déterminer votre position. Cela peut être utile pour les appareils sans *Google Play Services* ou en cas de données de position ou d'altitude imprécises lors de l'enregistrement de traces.
...
-   Tous les paramètres de localisation d'OsmAnd se trouvent dans les réglages de l'appareil iOS : *Réglages → OsmAnd maps → Localisation*  
...
-   Pour trouver les fichiers de sauvegarde, utilisez une application de gestion de fichiers tierce.  
    *NOTE : Vous pouvez trouver les fichiers de sauvegarde en utilisant le gestionnaire de fichiers Android : Android → data → net.osmand → files → backup, mais sachez que dans les versions récentes d'Android, l'accès aux répertoires système est limité.*  
...
![Paramètres généraux Radars Android](@site/static/img/personal/profiles/uninstall_speed_camera_1_ios.png)   ![Paramètres généraux Radars Android](@site/static/img/personal/profiles/uninstall_speed_camera_2_ios.png)

</TabItem>

</Tabs>

Ce paramètre vous permet d'activer ou de désactiver les POI avec radars, vous devez redémarrer l'application OsmAnd pour que les modifications prennent effet.
...
Lisez les alertes de radars sur votre itinéraire dans l'article Widgets de navigation, section *[Widget d'alerte](../widgets/nav-widgets.md#alert-widget)*.
