...
2.  Dans le **<Translate android="true" ids="change_data_storage_folder"/>**, vous pouvez sélectionner un dossier pour enregistrer les données OsmAnd sur votre appareil.
    ...
    -   *Mémoire interne de l'application*. Seule l'application OsmAnd a accès à ses données, aucune des applications externes.
    -   *Mémoire partagée*. N'est plus disponible.
    -   *Stockage externe 1*. Seule 1 application OsmAnd a accès, ainsi que les applications Admin et USB.
...
| [Terrain 3D](../map/raster-maps.md#3d-relief) | Non pris en charge. | Cette fonctionnalité visualise le terrain sur la carte en ajoutant des informations d'élévation à une carte 2D normale, ce qui crée un effet 3D et de profondeur. |
...
L'image pour la vue 3D de la carte est chargée tuile par tuile, puis tous les symboles. La vitesse de rendu de la carte dépend du nombre d'objets graphiques 2D et 3D présents, et directement des performances de votre matériel. Par conséquent, la Version 2 (OpenGL) ne peut pas être utilisée sur les appareils peu performants.
...
OsmAnd vous permet de choisir si vous souhaitez *[partager votre activité dans l'application](#analytics)* et vos [identifiants](#identifiers) (*Android uniquement*), la possibilité de modifier votre *[historique](#history)* et d'utiliser un *[proxy](#proxy)* (*Android uniquement*) de votre choix.
...
-   Tous les paramètres de localisation OsmAnd se trouvent dans les réglages de l'appareil iOS : *Réglages → OsmAnd maps → Localisation*
...
Lisez les alertes pour les radars sur votre itinéraire dans l'article Widgets de navigation, section *[Widget d'alerte](../widgets/nav-widgets.md#alert-widget)*.
