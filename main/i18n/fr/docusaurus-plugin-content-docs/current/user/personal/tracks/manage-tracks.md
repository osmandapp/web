...
Toutes les traces enregistrées, créées ou importées sont automatiquement affichées dans le dossier [Mes lieux](../../personal/myplaces.md) dans l'onglet *Traces*. Elles sont organisées par dossier ou affichées dans une liste en dessous.
...
Chaque trace, comme les dossiers, dispose d'un menu qui vous permet de gérer les traces efficacement. Voici comment y accéder :
...
- [Trier par](#sort-by) pour une meilleure recherche si vous ne vous souvenez plus du titre.
...
- **Afficher sur la carte** — Grâce à cette fonctionnalité, vous pouvez afficher les traces sélectionnées sur la carte pour mieux comprendre leur emplacement et leur relation les unes par rapport aux autres.
...
- [Changer d'activité](../../map/tracks/track-context-menu.md#track-information-activity) — Cette action vous permet de changer le type d'activité, par exemple en *Voiture*, *Sports motorisés d'aventure*, *Randonnée* ou autres, pour la trace sélectionnée.
...
- **Durée la plus longue en premier** — Trie les traces par durée, la plus longue en haut.

- **Durée la plus courte en premier** — Trie les traces par durée, la plus courte en haut.
...
L'option de tri sélectionnée s'applique non seulement au sous-dossier actuel, mais aussi à tous les dossiers imbriqués et à toutes les traces à tous les niveaux. Cela garantit une structure cohérente de vos données enregistrées.
...
![importation de plusieurs traces](@site/static/img/personal/tracks/tracks_folder_menu.png)
...
- **Afficher toutes les traces sur la carte** (*Android uniquement*) — Affiche toutes les traces du dossier sur la carte —  
    *REMARQUE : S'il y a beaucoup de traces, cela peut entraîner un blocage de l'appareil.*
...
- *Somme de la durée* — Affiche le temps total nécessaire pour parcourir toutes les traces du dossier sélectionné.
...
- [Sélectionner](#selection-mode) — Appuyez sur les dossiers et traces requis pour appliquer les actions du menu en *mode sélection*.

- [Trier les sous-dossiers](#sort-by) — Applique un ordre de tri (par exemple, par nom, par date) à tous les sous-dossiers et traces à tous les niveaux.
...
2. **Importation via le menu *Mes lieux* :**
    ...
    *Options supplémentaires :*
    ...
    - **Fichiers GPX à trace unique** - importent la trace entière directement.
...
![importation de plusieurs traces](@site/static/img/personal/tracks/import_multitrack.png) ![importation de plusieurs traces](@site/static/img/personal/tracks/import_multitrack_1.png)
...
| Exporter | Importer |
|:------------|:---------------|
| ![importation de plusieurs traces](@site/static/img/personal/tracks/import_gpx_ios_1.png) <details><summary> Exporter </summary> ![importation de plusieurs traces](@site/static/img/personal/tracks/import_gpx_ios.png) </details> | ![importation de plusieurs traces](@site/static/img/personal/tracks/import_gpx_ios_3.png) <details><summary> Exporter </summary> ![importation de plusieurs traces](@site/static/img/personal/tracks/import_gpx_ios_2.png) </details> |
...
3. Par [Fichier de sauvegarde local](../../personal/import-export.md#export-to-file). Exportation de traces GPX sous forme de fichiers `.osf`.
