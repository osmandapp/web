...
- Découvrez les POI les mieux notés avec le nouveau mode Explorer
- Tous les itinéraires OSM, maintenant consultables ! Randonnée, cyclisme, VTT, et plus encore
- Nouveau widget de navigation combinant flèche de virage et instructions de navigation
...
- Possibilité de sélectionner la taille du widget pour les panneaux gauche et droit
- Ajout d'une "Grille de coordonnées" avec des coordonnées géographiques
- Ajout de la possibilité de personnaliser l'apparence par défaut des groupes de favoris
- Android Auto : affiche l'heure d'arrivée estimée et la distance jusqu'au prochain point de passage ; les noms de villes apparaissent maintenant dans les résultats de recherche
- Les sommets proéminents sont maintenant découvrables via la carte et la recherche
- Hauteurs des sommets de montagne en pieds
- Ajout : Falaises et zones d'escalade sur la carte
- La recherche d'adresses aux États-Unis a été améliorée
- Ajout de la synthèse vocale en hébreu pour la navigation
...
- Ajout d'une galerie plein écran pour les images Wikimedia.
...
- Correction d'un problème de coordonnées inversées en RTL.
- Correction de données de capteur manquantes pour la trace en cours d'enregistrement.
...
- Les actions rapides peuvent maintenant être attribuées à des claviers externes.
...
- Correction des statistiques de traces, du tri et de la recherche.
- Édition OSM : suppression de la méthode d'autorisation par login/mot de passe.
...
- Écran de démarrage : ajout d'une option pour restaurer l'état d'OsmAnd à partir d'un fichier.
...
- Accessibilité : la taille de la cible tactile pour les boutons a été modifiée à 48 dp ; correction d'un problème d'interface utilisateur de carte visible pour les lecteurs d'écran.
...
- Ajout d'une option pour partager l'itinéraire par lien
...
- Ajout d'une option pour désactiver le téléchargement UUID
...
- Itinéraires de randonnée / cyclisme / voyage : appuyez sur le symbole de l'itinéraire pour obtenir des informations complètes sur l'itinéraire.
- Groupes de favoris : définissez l'apparence par défaut pour les nouveaux points du groupe
- Corrections : mise à jour automatique des cartes en ligne
...
- Ajout d'une option pour télécharger les courbes de niveau en pieds
...
- Planifier l'itinéraire paysage : ajout d'onglets pour basculer entre les points ou les graphiques
...
- Boîte de dialogue repensée pour sélectionner la langue des instructions de navigation
- Mes lieux : le dernier onglet ouvert est maintenant mémorisé et sera ouvert la prochaine fois
- Divers correctifs pour le RTL
- Correction d'un problème d'unités de mesure
- Correction d'un problème de partage de plus de cent points favoris
...
- Planifier l'itinéraire : graphiques pour les segments de trace avec itinéraire, et ajout de la possibilité de créer et d'éditer plusieurs segments de trace.
...
- Profils : vous pouvez maintenant changer l'ordre, définir l'icône pour la carte, changer tous les paramètres pour les profils de base et les restaurer aux valeurs par défaut
...
- Ajout d'une translittération correcte sur la carte du Japon
...
- Correction du comportement du bouton de boussole pendant la navigation
- Corrections de bugs
...
- Nouvel écran Directions : affiche les boutons de destination Domicile et Travail, le raccourci de l'itinéraire précédent, la liste des traces actives, l'historique de recherche
- Informations supplémentaires dans *Détails de l'itinéraire* : types de routes, surface, pente, fluidité
...
- Édition OSM : Possibilité d'éditer les balises pour les objets et les voies non ponctuels, correction des commentaires manquants sur les notes, sauvegarde des modifications
...
- Interface utilisateur Ouvrir la trace : affiche les groupes de points de passage
- Marqueurs de carte : importation de groupes sélectionnés à partir de fichiers GPX, nouveau look de la saisie de coordonnées
...
- Mise à jour du menu contextuel : affiche quand le POI ouvre/ferme
...
- Modifications OSM : affiche une icône et un nom de la catégorie de POI, affiche les actions terminées
- Nouvel écran de saisie rapide de coordonnées pour la création rapide de marqueurs
...
- OSM Live beta. Un nouveau système encourageant les contributeurs et fournissant des mises à jour constantes
...
- Géocodage inverse affichant l'adresse d'un lieu en appuyant sur la carte
...
- Nouvelle interface utilisateur contextuelle pour appuyer sur les emplacements sur la carte et sur d'autres écrans
...
- Structure et interface de téléchargement de données de carte retravaillées
...
- Styles de carte nouveaux et mis à jour, y compris OsmAnd, LightRS, UniRS
...
- Masquer automatiquement les boutons en mode navigation
- Calculer l'itinéraire hors ligne vers le premier point de la trace GPX (option 'passer toute la trace')
- Simulation active dans les tunnels
- Paramètre d'accessibilité du débit vocal
...
- Nouvelle carte de base mondiale avec les routes principales, les chemins de fer et les grandes caractéristiques zonales comme les forêts.
- Fonction de loupe de carte (clic long sur le bouton de zoom) - Utile à la fois pour les cartes en tuiles et vectorielles
- Définir/éditer des sources de tuiles en ligne personnalisées (et spécifier le temps d'expiration)
...
- Avertissements vocaux sur les limites de vitesse et les radars
...
- Plugin de courbes de niveau restructuré et amélioré
- Paquets d'ombrage pour le plugin de courbes de niveau
...
- Corrections de bugs
- Option de style de carte 'Vue touristique' avec plus de détails de carte, de nouveaux styles de route, plus de contraste et option de randonnée alpine (échelle SAC)
...
- Introduction des cartes uniquement routières.
...
- Plugin Parking ('Où est garée ma voiture ?')
- Points de passage
- Guidage d'itinéraire amélioré
- Paramètre d'évitement des autoroutes pendant la navigation
- Widgets de carte dynamiques
- Bouton de verrouillage d'écran avec services de journalisation
- Prise en charge des scripts arabes, kannada et des langues RTL
- Recalcul d'itinéraire intelligent
- Direction par voies
- Informations sur les limites de vitesse, les radars et les ralentisseurs
...
- Afficher la trace actuellement enregistrée sur la carte
- Détection de direction : 'Faire demi-tour' est indiqué et annoncé lorsque vous vous dirigez dans la direction opposée ou après avoir manqué un virage
- Indication optique d'un virage imminent à moins de 100 m par la flèche de direction changeant de couleur
...
- Implémenter la recherche de POI par nom dans de grandes zones (pays)
...
- Explorateur GPX dans l'application (afficher les détails de la trace, la distance, la vitesse moyenne, l'altitude, etc.)
- Navigation GPX (plus d'options, continuer la navigation après un plantage, prendre en charge les ronds-points)
- Fonction pour enregistrer l'itinéraire sous forme de trace GPX pour une utilisation hors ligne
- Améliorer la surveillance GPX (filtrer certaines positions inexactes et enregistrer la précision/altitude/vitesse)
- [Télécharger GPX](https://download.osmand.net/gpx/) vers la communauté OSM
- Moteur Text To Speech pour la navigation vocale
- Carte de base pour le monde entier fournie avec l'application
- Ajuster la navigation hors ligne (toujours expérimentale)
