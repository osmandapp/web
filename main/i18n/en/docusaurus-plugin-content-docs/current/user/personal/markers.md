...
Markers are special points marked on the map as flags that could be quickly created as a single tap without specifying name and other information in contrast with [Favorite points](./favorites.md). Also markers provide additional functionality with [widgets](../widgets/markers.md), **arrows** (Show arrows on the map) and **distance indication** (Show direction).
...
- **Marker** / **Edit map marker**. Puts a new marker on the selected point.
...
- &nbsp;Add to map markers  or Remove from map markers.
...
Go to: *Menu, Map Markers, More, Appearance on the map*
...
Go to: *Menu, Map Markers, Appearance on Map*
...
*Menu, Map Markers, More, Plan route*
...
- **Sort by**. Markers can be sort in list by name, proximity, adding time.
- **Appearance on the map**  or **Appearance**. [Settings for Map markers](#appearance-on-the-map).
- **Coordinate input**. Adds Markers by [coordinate input](../plan-route/coordinate-input.md).
- **Plan route**. Builds route [for navigation](../navigation/setup/markers-navigation.md) using chosen markers.
- **Save as track**. Saves active markers in a GPX file.
- **Move all to history**. Deactivates all markers and move them to [History](#history).
