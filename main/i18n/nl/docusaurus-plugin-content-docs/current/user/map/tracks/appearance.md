...
De functie **Uiterlijk van track** stelt gebruikers in staat om aan te passen hoe tracks op de kaart worden weergegeven. Dit omvat het aanpassen van kleur, breedte, richtingaanwijzers en andere visuele elementen. Het aanpassen van het uiterlijk van tracks helpt bij het onderscheiden van meerdere tracks, het markeren van specifieke routes en het verbeteren van de leesbaarheid van de kaart.
...
U kunt het uiterlijk van de track wijzigen met behulp van een van de volgende methoden:
...
- **Vanaf de kaart**. Tik rechtstreeks op een track op de kaart en tik vervolgens op het *Uiterlijk-pictogram* in het *Overzicht-gedeelte*.
- **Vanaf trackopname**. Wijzig het uiterlijk van de momenteel opgenomen track via het [trackopnamecontextmenu](../../plugins/trip-recording.md#сurrent-track-recording).  
...
## Instellingen voor het uiterlijk van tracks {#track-appearance-settings}
...
Voor meer details over het aanpassen van het uiterlijk van de routelijn, zie [Kaartscherm tijdens navigatie](../../navigation/guidance/map-during-navigation.md#color) en over het selecteren en maken van een aangepaste kleur, zie [Kleurenschema's](../../personal/color-palette-schemes.md#routes).
...
U kunt het uiterlijk van de track aanpassen in verschillende menu's van de applicatie:
...
- [*Mijn plaatsen → Tracks-tabblad → elke map → Standaard uiterlijk*](../../personal/tracks/manage-tracks.md#track-folder).

Pas de trackbreedte aan om de zichtbaarheid op de kaart te verbeteren.

- **Vooraf ingestelde opties**: **Dun, Gemiddeld,** en **Vet**.

- **Aangepaste breedte**: Instelbaar van 1 tot 24 px met behulp van een schuifregelaar.
...
- **<Translate android="true" ids="vertical_exaggeration"/>**. Indien gevisualiseerd op *Hoogte* of *Snelheid*, kunt u 3D-gegevens schalen om een prominentere visualisatie voor de 3D-track te krijgen in het bereik van *Geen* tot *x3.0*.
...
- U kunt dit combineren met [3D-reliëf](../../plugins/topography.md#3d-relief). Als de bron van de 3D-gegevens geen hoogte is, zal de 3D-weergave een combinatie zijn van reliëfhoogte en de geselecteerde gegevens, zoals snelheid of hartslag.

**<Translate android="true" ids="wall_color"/>**  
Past de **3D-trackkleur** aan met behulp van gradiënten of datagebaseerde visualisatie.  
Opties zijn: *Geen*, &nbsp;*Effen*, &nbsp;*Aflopend verloop*, &nbsp;*Oplopend verloop*, &nbsp;*Hoogte*, &nbsp;*Helling*, &nbsp;*Snelheid*.
...
De 3D-trackfunctie is zeer nuttig voor activiteiten zoals paragliding, waarbij het visualiseren van uw vliegroute diepere inzichten kan bieden. Door de track in 3D weer te geven, kunt u functies zoals **Muurhoogte** en **Muurkleur** gebruiken om waardevolle visuele informatie voor analyse te extraheren.
...
1. **Visualiseren op snelheid met 3D**.  
    ...
    Met Muurkleur op basis van de Helling kunt u observeren hoe het terrein, met name heuvels en hellingen, uw snelheid beïnvloedde. Deze combinatie van snelheids- en hellingsvisualisatie biedt een gedetailleerd inzicht in de prestaties gedurende de activiteit.

2. **Visualiseren op hartslagsensor met 3D-reliëf**.  
...
3. **Visualiseer trackhoogte en hellingskleurlijn op een platte aardweergave.**  
    Bij het visualiseren van hoogtedata wordt aanbevolen om een platte aardweergave te gebruiken om problemen zoals GPS-fouten te voorkomen, die ervoor kunnen zorgen dat delen van de track ondergronds verschijnen.  

    In dit scenario kunnen hellingsveranderingen afzonderlijk langs de tracklijn worden gevisualiseerd, terwijl Muurkleur wordt gebruikt om de hoogte weer te geven. Deze methode zorgt ervoor dat u, zelfs met mogelijke data-onjuistheden, nog steeds een duidelijke en nauwkeurige visualisatie van helling en hoogte ontvangt.

## Uiterlijk wijzigen voor meerdere tracks {#change-appearance-for-multiple-tracks}
...
Het tabblad [Tracks](../../personal/tracks/manage-tracks.md) toont automatisch alle ooit opgenomen, gemaakte of geïmporteerde tracks. Ze zijn georganiseerd per map of worden als lijst eronder weergegeven.  
...
- [Selectiemodus](../../personal/tracks/manage-tracks.md#selection-mode) - Wijzigt het uiterlijk van specifieke tracks in de geselecteerde map of de gehele lijst in het tabblad Tracks.
    - Selecteer het *drie-stippenmenu* in de rechterbovenhoek van het tabblad Tracks en tik op *Selecteren*.
    - Tik vervolgens op het *drie-stippenmenu* bovenaan het scherm en selecteer *Uiterlijk wijzigen*.  

Opties voor bulk aanpassing van het uiterlijk:

- **Richtingspijlen** - Voegt [bewegingsindicatoren](#direction-arrows) toe.  
    Statussen: *Ongewijzigd*, &nbsp;*Origineel*, &nbsp;*Aan*, &nbsp;*Uit*.

- **Start- en eindpictogrammen weergeven** - [Toont markeringen](#start-and-finish-icons) voor start- en eindpunten van tracks.  
    Statussen: *Ongewijzigd*, &nbsp;*Origineel*, &nbsp;*Aan*, &nbsp;*Uit*.
...
OsmAnd ondersteunt **aangepaste trackkleuren** in GPX-bestanden met behulp van de `<osmand:color>`-tag binnen de `<extensions>`-sectie van een `<trk>`-element. Hierdoor kunnen gebruikers kleuren definiëren voor individuele tracks, wat de visuele differentiatie verbetert wanneer meerdere tracks op de kaart worden weergegeven.

Om een kleur voor een track te specificeren, gebruikt u het volgende formaat:  
...
1. **Standaard kleurtoewijzing**:  
    ...
    - Wanneer een **GPX-track als één track wordt geïmporteerd** (optie ***Importeren als één track***), krijgt de hele track de **standaard GPX-kleur** (rood) toegewezen.  
    - In OsmAnd **4.9.10 en later** is dit probleem opgelost – individuele trackkleuren blijven nu behouden bij import.

2. **Eénkleurige weergave voor samengevoegde tracks:**  
...
3. **Prioriteit van uiterlijkinstellingen:**  
    ...
    - OsmAnd ondersteunt momenteel **kleur- en breedte-instellingen alleen voor de hele track**.
    - Deze uiterlijkinstellingen worden opgeslagen in de **top-level GPX-extensies**.
