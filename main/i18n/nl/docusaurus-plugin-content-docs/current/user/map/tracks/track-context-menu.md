...
Het *Track contextmenu* biedt informatie over het *[Track](../../personal/tracks/index.md)* in zowel tekst- als [grafische](#altitude--speed-graphs) vormen. Hiermee kunt u informatie toevoegen, wijzigingen aanbrengen, bewerken en diverse andere acties uitvoeren met de track. U kunt het *track contextmenu* openen door simpelweg op de gewenste track op de kaart te tikken of via het menu *[Mijn plaatsen](../../personal/myplaces.md)* (*<Translate android="true" ids="shared_string_menu,shared_string_my_places,shared_string_gpx_files"/>*). Mogelijk moet u de trackmap openen en de gewenste track selecteren door erop te tikken.
...
Het infopaneel biedt ook informatie over het volgende:
...
- **<Translate android="true" ids="shared_string_appearance"/>** – Opent het [Uiterlijkmenu](../tracks/appearance.md) om de kleur, breedte en transparantie van de track aan te passen.
- **<Translate android="true" ids="follow_track"/>** (*Android*) / **<Translate ios="true" ids="shared_string_navigation"/>** (*iOS*) – Start [navigatie per track](../../navigation/setup/gpx-navigation.md)., waarbij uw route wordt uitgelijnd met de geselecteerde track.

<br/>

- **<Translate android="true" ids="join_segments"/>** (*Alleen Android*) – Voegt tracksegmenten samen om gaten op te vullen.
- **<Translate android="true" ids="analyze_on_map"/>**. Opent de tool [Analyseren op kaart](../tracks/index.md#analyze-track-on-map-analyze-track-on-map) om de hoogte, snelheid en afstand van de track te inspecteren.
- **<Translate android="true" ids="analyze_by_intervals"/>** (*Alleen Android*) - Analyseert de track per tijd- of afstand [intervallen](./track-context-menu.md#split-interval).
...
- **<Translate android="true" ids="rename_track"/>** – Maakt het mogelijk om de tracknaam te wijzigen.
- **<Translate android="true" ids="change_folder"/>** – Verplaatst de track naar een andere map of maakt een nieuwe aan.

<br/>

- **<Translate android="true" ids="shared_string_gps_filter"/>** (*Alleen Android*) - Maakt het mogelijk om [punten van de track te filteren](#gps-filter) om de nauwkeurigheid te verfijnen.
...
- **Aanvullend**. Alle tags uit de ***extensions*** sectie:&nbsp; `address`,&nbsp; `icon`,&nbsp; `background`,&nbsp; `color`,&nbsp; `country`,&nbsp; `state`,&nbsp; `telephone`,&nbsp; `postcode`,&nbsp; `start_date`,&nbsp; `desc`.
...
- [Activiteitstags voor GPX-tracks](../../plugins/trip-recording.md#recording-settings). Bij het opnemen van een GPX-track kunt u een activiteitstype toewijzen. Deze activiteitstagging helpt u om tracks in eerste instantie te categoriseren.
- [Activiteitsfilter](../../personal/tracks/smart-folder.md#search-filter). U kunt de opgenomen GPX-tracks filteren op activiteit, waardoor u zich kunt richten op het vinden van specifieke soorten opnamen, zoals alle fiets- of wandeltracks.
...
- **<Translate android="true" ids="analyze_on_map"/>** knop opent [Analyseer trackmenu](../tracks/index.md#analyze-track-on-map) voor de track.
...
Dit tabblad is alleen zichtbaar op opgenomen tracks. Punten op dit type track hebben **[`snelheid`](../../plugins/trip-recording#recorded-gpx-file)** tags.
:::
...
De grafiek **Snelheid** toont statistieken zoals:
...
De grafiek **Hoogte** toont statistieken zoals:

- *<Translate android="true" ids="average_altitude"/>*. Geeft de gemiddelde hoogte boven zeeniveau op de route aan.
- *<Translate android="true" ids="altitude_range"/>*. De hoogste en laagste hoogte die op de route is geregistreerd.
...
De offline functie voor hoogteberekening is alleen beschikbaar voor [**OsmAnd Pro-abonnees**](../../purchases/android.md#pro-features) <ProFeature />.
:::
...
Waypoints zijn een van de soorten punten die beschikbaar zijn op de kaart. Over het algemeen kunnen het trackpunten zijn die deel uitmaken van GPX-bestanden. Een gedetailleerde beschrijving is beschikbaar in het artikel [punten op de kaart](../../map/point-layers-on-map.md).
...
Waypoints kunnen worden toegevoegd aan de momenteel opgenomen track of aan elke track uit de lijst *[Mijn plaatsen → Trackslijst*](../../personal/tracks/manage-tracks.md):
...
- Voeg een waypoint toe als een [Snelactie](../../widgets/quick-action.md#my-places) knop.
...
3. **Uiterlijk** instelling:
    ...
    - Selecteer een [**icoon**](../../personal/favorites.md#favorite-icons) uit de lijst met alle POI-categorie-iconen.
    - Selecteer **Kleur** van het icoon.
    - Selecteer vervolgens de **Vorm** van het icoon: *Cirkel, Achthoek, Vierkant*.
...
Het *drie-stippenmenu* ( &#8285; ) opent het waypoint *Groepsmenu*.

**Acties:**

- **<Translate android="true" ids="shared_string_show_on_map"/>** - Hiermee kunt u groepswaypoints wel of niet op de kaart weergeven. Of u kunt de [schermknop](#points--waypoints) gebruiken om groepswaypoints op de kaart weer te geven of weg te laten.
...
| --- | --- |
| `osmand:icon` | **Icoon** is de naam van het door u geselecteerde icoon. |
...
Waypoints worden samen met de track opgenomen in het [GPX-bestand](../../../technical/osmand-file-formats/osmand-gpx.md). Elk van deze waypoints heeft zijn eigen set tags voor beschrijving. Alle tags die direct verband houden met de beschrijving van waypoints en die direct in de OsmAnd-applicatie kunnen worden bewerkt, staan in dit gedeelte vermeld.
Om de waypointbeschrijving te wijzigen, kunt u een van de volgende manieren volgen:
...
3. Tik op het gewenste waypoint als het [zichtbaar](../../personal/tracks/manage-tracks.md#track-menu) is op de kaart.
    ...
    Vervolgens tikt u in het [Tracks Contextmenu](./track-context-menu.md) op *punten*, het veld van de gewenste map met beschikbare waypoints en het waypoint dat u zoekt. Selecteer menu *Acties →* **GPX-waypoint bewerken** en breng wijzigingen aan. U kunt de aanmaakdatum van een punt, *tijdstempel*, niet wijzigen in de applicatie.

**Voorzieningstags** die aanvullende informatie beschrijven, zoals openingstijden of telefoonnummer, specifiek voor de POI, halen die informatie uit OpenStreetMap-gegevens of gegevens die u hebt ingevoerd bij het maken van de POI. U kunt dergelijke tags en [aangepaste tags](#display-custom-gpx-tags) alleen bewerken in de code van het GPX-bestand, die u met elke teksteditor op uw apparaat kunt openen.
...
Splits de track op afstand of tijdinterval.
*<Translate android="true" ids="shared_string_options,analyze_by_intervals"/>*

![Track splitsingsinterval scherm Android](@site/static/img/personal/tracks/track_split_interval_android.png) ![Track splitsingsinterval scherm tijd Android](@site/static/img/personal/tracks/track_split_interval_time_android.png)
...
Met dit filter kunt u uw trackstatistieken verbeteren door onnodige of onjuiste gegevens uit te sluiten. U kunt trackpunten filteren die niet passen bij uw trackparameters, en zo een nauwkeurigere grafiek en visuele routelijn krijgen zonder vervorming of opnameruis. U kunt wijzigingen aanbrengen met filters zoals *Vloeiend maken*, *Snelheid*, *Hoogte* en *GPS-precisie*, die gefilterde punten verbergen voor de huidige track. Bovendien kunt u in het menu *Statistieken* controleren hoe uw wijzigingen worden weergegeven op de grafiek voordat u ze opslaat. U kunt ook *Terugzetten naar origineel* en *Opslaan als kopie* van uw track in dit filter zonder de originele op te slaan.
...
|![GPS-filter hoogte nummers Android](@site/static/img/personal/tracks/gps_filter_altitude_android.png) |
|*GPS-precisie*. Stelt de maximaal toegestane waarde voor HDOP in. Punten met een hogere waarde worden verborgen.|
|![GPS-filter precisie nummers Android](@site/static/img/personal/tracks/gps_filter_precision_android.png) |
...
- Het *Gegevens*-gedeelte toont statistieken vergelijkbaar met het [Filtermenu](#filter-menu).
- Het *Grafiek*-gedeelte toont drie soorten grafieken per parameter: Overzicht, Hoogte en Snelheid. Deze functionaliteit is een kopie van [trackgrafiek](#altitude--speed-graphs).
