...
Tikken op een [GPX-track](../map/tracks/index.md) op de kaart opent het [trackcontextmenu](../map/tracks/track-context-menu.md), waar u toegang hebt tot aanvullende trackgerelateerde opties.
...
Het paneel **Objectinfo** geeft aanvullende details over het geselecteerde object. U kunt op een willekeurige invoer tikken om de informatie naar het klembord te kopiëren.
...
- [Openbaar vervoerroutes](../map/map-context-menu.md#public-transport-routes) (voor haltes)
...
Functies van de weergave van alternatieve namen:

- Aanvullende vertalingen zijn toegankelijk via een *uitklaplijst*.
...
- Andere categorieën zijn onder meer *regiospecifieke vertaling* of *standaardinstellingen*.
...
### Polygone informatie {#polygon-information}

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Polygone informatie Android](@site/static/img/map/polygon_information_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Polygone informatie Android](@site/static/img/map/polygon_information_ios.png)

</TabItem>

</Tabs>

De functie **Polygone informatie** biedt gedetailleerde gegevens over nabijgelegen **polygonen** die op de kaart worden weergegeven. Dit omvat geografische kenmerken, administratieve grenzen en andere in kaart gebrachte gebieden.

**Binnen sectie**:

- Toont een *lijst met omringende polygonen*, gesorteerd *van kleinste naar grootste* op basis van oppervlakte.

- Elke vermelding toont het *polygone type en de naam*, waardoor regio's en grenzen eenvoudig kunnen worden geïdentificeerd.

**Aanvullende details**:

- Bij het selecteren van een punt op de kaart of een POI, toont het paneel **polygone informatie** een lijst van alle polygonen die de geselecteerde locatie bevatten.
...
Tikken op *POI's in de buurt / Wikipedia-artikelen* opent een puntenlijst (POI's of Wikipedia). Een tik op een van deze punten uit een puntenlijst verplaatst de kaart naar dit punt (POI's of Wikipedia) met het openen van het kaartcontextmenu van het punt.

> **OPMERKING**. *[<Translate android="true" ids="wiki_around"/>](../plugins/wikipedia.wikipedia.md) verschijnt alleen als u eerder speciale kaarten met [Wikipedia-artikelen voor dit gebied](../plugins/wikipedia.md#download-wikipedia-packages-download-wikipedia-packages) hebt gedownload*.

### Openbaar vervoerroutes {#public-transport-routes}

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Openbaar vervoerroutes Android](@site/static/img/map/pt_routes_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Openbaar vervoerroutes iOS](@site/static/img/map/pt_routes_ios.png)

</TabItem>

</Tabs>

Getoonde informatie over openbaar vervoerroutes voor de gekozen halte. Informatie over het contextmenu van het openbaar vervoer en acties hiermee, leest u in het [artikel](../map/public-transport.md#transport-routes-context-menu).
...
Voor **lineaire objecten**, zoals *barrières, trappen, poorten, banken, hellingen, stuwen, paaltjes of coupures*, toont het contextmenu aanvullende details. Deze omvatten het POI-type en het bijbehorende adres, of het objecttype als er geen naam is toegewezen.

### Klimgebied en rotswanddetails {#climbing-area-and-crag-details}

![Favorietenlijst](@site/static/img/map/climbing_andr.png)

Bij het selecteren van een [klimgebied of rotswand](../map/routes.md#climbing-routes) biedt OsmAnd een gedetailleerde samenvatting van de klimlocatie, inclusief: naam en locatie, moeilijkheidsgraad (UIAA, Frans, YDS, enz.), rotssoort, hoogte en routelengte, klimkwaliteit en oppervlaktecondities.

### Skipistes, MTB-routes en crossmotorpaden {#ski-slopes-mtb-routes-and-dirt-bike-trails}

![Skipistes en MTB-routes](@site/static/img/map/ski_mtb.png) ![Skipistes en MTB-routes](@site/static/img/map/ski_mtb_2.png)

U kunt op kaartelementen zoals **skipistes**, **mountainbikeroutes (MTB)** en **crossmotorpaden** tikken om hun contextmenu met gedetailleerde informatie te openen. Beschikbare informatie omvat: route- of pistenaam (indien beschikbaar in OSM-gegevens), OSM-object-ID of relatie-ID, moeilijkheidsgraad, padlengte, terreintype en meer.

**Ondersteunde tags:** `piste:type=*` (skipistes), `route=mtb`, `route=atv` of `route=dirt_bike` (crossmotor off-road paden)
...
![Beschrijving lijst](@site/static/img/map/description_list_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Beschrijving lijst](@site/static/img/map/description_list_ios.png)
...
Wanneer u op een populaire plaats op de kaart tikt, toont de sectie **Online foto's** in het *POI's contextmenu* voorbeelden van afbeeldingen afkomstig van [Wikipedia](https://www.wikipedia.org/) en [Wikimedia Commons](https://www.wikimedia.org/).

Tikken op een foto opent de **Galerij**-weergave, waar u:

- Maximaal 100 afbeeldingen per plaats kunt bekijken
...
Zie voor meer informatie: [Online foto's](https://osmand.net/docs/user/map/popular_places#online-photos) en [Galerijweergave](https://osmand.net/docs/user/map/popular_places#gallery)
...
OsmAnd ondersteunt integratie met [Mapillary](https://www.mapillary.com/), een wereldwijd platform voor crowdsourced street-level imagery. Met deze functie kunt u fotosequenties uit de echte wereld direct op de kaart bekijken en verkennen.
...
> *Meer informatie over het activeren en gebruiken van deze functie vindt u in de [Mapillary-plug-ingids](../plugins/mapillary.md).*
...
Het is een reeks specifieke manipulaties die op een punt of object kunnen worden uitgevoerd. Dit menu is verdeeld in twee delen: het zichtbare gedeelte bestaat uit maximaal 3 acties en andere acties zijn toegankelijk via de knop *Acties*. U kunt (*Android*) de volgorde van de acties aanpassen in [Algemene instellingen](#customize).
...
In het contextmenu zijn er opties om het geselecteerde punt/object **toe te voegen** of te **bewerken** in de [favorietenlijst](../personal/myplaces.md).
...
- Om informatie over een favoriet punt te **bewerken**, moet u *Op kaart tonen* inschakelen (*Menu → Mijn plaatsen → Favorieten*), er vervolgens op drukken en in het contextmenu verschijnt in plaats van het *Ster*-pictogram een *Potlood*-pictogram (met handtekening Bewerken).
...
Het is mogelijk om een punt of object te markeren om de navigatieplanning te vergemakkelijken. U hoeft alleen maar op het **vlag**-pictogram in het menu (*Android*) te tikken, **pijl** (*iOS*) - toont de richting en afstand tot het geselecteerde punt vanaf uw huidige locatie.
...
- **Markeer gepasseerd** (*Android*) / *Verwijder* (*iOS*). Deactiveert de markering en plaatst deze in de geschiedenis.
- **Actief maken** (*Android*). Verplaatst de markering naar de bovenste positie (op het bovenste paneel).
- **Markering herstellen** (*Android*). Verplaatst de markering van de geschiedenis naar de actieve lijst.
...
- ***Verzenden***. Verzendt tekst via apps die beschikbaar zijn op uw apparaat. Bevat alle informatie over de locatie: Naam van mijn locatie of de naam van de geselecteerde POI, adres, coördinaten en geolocatie als link.
- ***Kopiëren***. Kopieert alle locatie-informatie (Naam van mijn locatie of naam van de geselecteerde POI, adres, coördinaten en geolocatie als link) naar het klembord.
...
- ***Toewijzen aan contact*** (*iOS*) - maakt een pictogram voor gekozen contact vanuit de kaartscreenshot.
...
Om navigatie of routeplanning te starten, moet u eerst een bestemming selecteren met [<Translate android="true" ids="get_directions"/>](../widgets/map-buttons.md#directions). Als u al een bestemmingspunt hebt, zal het menu u voorstellen om het bestemmingspunt te vervangen of als een ander tussen- of startpunt in te voegen.
...
Als u op **Start navigatie** tikt, wordt het *Van*-punt verwijderd omdat de applicatie de navigatiemodus ingaat. Om een route te behouden, tikt u niet op *Start navigatie* en [**veegt u het menu *Routevoorbereiding* naar beneden**](#hide-context-menu).
:::
...
Met deze contextmenu-actie is het mogelijk om te zoeken rond een specifieke locatie op de kaart.
...
Met behulp van het kaartcontextmenu kunt u een specifieke weg toevoegen aan de lijst met niet-te-gebruiken wegen voor routering. Dit heeft invloed op de [routevoorbereiding](../navigation/setup/route-navigation.md).
Om eerder geselecteerde wegen uit de *[Wegen vermijden-lijst](../navigation/routing/osmand-routing.md#avoid-roads-menu)* te verwijderen, tikt u op een teken, kiest u *Verwijderen* (**Android**) of *Verwijderen* (**iOS**).
...
Bijna elk door de gebruiker gemaakt object is verplaatsbaar, d.w.z. Marker, Favoriet, Gemaakte POI, Audio/Video-notitie of Track Waypoint. Selecteer eerst een object op de kaart en gebruik vervolgens het menu *<Translate android="true" ids="change_markers_position"/>* om het naar een nieuwe locatie te verplaatsen.

### Een route plannen {#plan-a-route}

![Actie Plan een route Android](@site/static/img/map/action_plan_route_android.png)
...
:::note
Om parkeerpunten toe te voegen/verwijderen, schakelt u de OsmAnd [**Parkeerpositie-plug-in**](../plugins/parking.md) in.
:::
...
![Track selecteren om Waypoint toe te voegen Android](@site/static/img/map/action_select_track_to_add_waypoint_android.png)
...
![Track selecteren om Waypoint toe te voegen iOS](@site/static/img/map/action_select_track_to_add_waypoint_ios.png)
...
- Kies in het *kaartcontextmenu* dat wordt geopend *Acties → Track waypoint toevoegen*.
...
U kunt **kaartdatafouten** op een specifieke locatie melden aan de [OpenStreetMap-gemeenschap](https://wiki.openstreetmap.org/wiki/Join_the_community). Volg de [richtlijnen](https://wiki.openstreetmap.org/wiki/Notes#Adding_notes) en voeg de juiste opmerkingen toe aan een probleem.
...
Neemt een medianotitie op of maakt deze op een geselecteerd punt op de kaart.
