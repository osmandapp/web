...
- **Zoomen**. Hiermee kunt u het zoomniveau van de kaart aanpassen om meer of minder details weer te geven.
...
- [Route opbouwen](../navigation/index.md). Gebruik deze knop om een route aan te maken.
...
- Het *grijze standaardpictogram* geeft aan dat de route nog niet is aangemaakt. Door op dit pictogram te tikken, wordt de functionaliteit voor [route-instellingen](../navigation/setup/route-navigation.md) geopend.
- Het *blauwe standaardpictogram* geeft aan dat de route is opgebouwd, maar de navigatie nog niet is gestart. Door op dit pictogram te tikken, wordt de functionaliteit voor [route-instellingen](../navigation/setup/route-navigation.md#start--stop-navigation) geopend.
- Het *blauwe pijlpictogram* geeft aan dat navigatie actief is. Door op dit pictogram te tikken, worden de [details van de huidige route](../navigation/setup/route-details.md) geopend.
...
De knop **Kaart configureren** geeft toegang tot het menu [Kaart configureren](../map/configure-map-menu.md). Het pictogram geeft ook het [Huidige app-profiel](../personal/profiles.md) weer, zodat u verschillende profielen kunt identificeren en ertussen kunt schakelen.
...
### Tikgedrag kompas {#compass-tapping-behavior}

De kompas-knop biedt meerdere acties om de kaartoriëntatie te regelen:

- **Enkele tik**. Een *enkele tik* op de [Kompas-knop](../widgets/map-buttons.md#compass) (linksboven in het scherm wanneer [zichtbaar](../widgets/map-buttons.md#display-options)) heroriënteert de kaartoriëntatie direct *naar het Noorden* in alle kaartoriëntatiemodi. Zelfs als de kaart in de *Kompasrichting*-modus staat, roteert deze nog even en keert dan terug naar de dynamische oriëntatie van die modus.

- **Dubbele tik**. Om snel te schakelen tussen kaartoriëntatiemodi, zoals het volgen van de GPS-richting of het draaien met het apparaat, *dubbeltikt* u op de [Kompas-knop](../widgets/map-buttons.md#compass) (wanneer [zichtbaar](../widgets/map-buttons.md#display-options)).
...
- *<Translate android="true" ids="shared_string_hidden"/>*. Als deze modus voor een knop is geselecteerd, moet u een gebaar gebruiken om de kaartweergave te wijzigen. Tik met twee vingers op de kaart en beweeg ze omhoog en omlaag om de kanteling van de kaart aan te passen.  
...
1. **Knop weergeven**. (*Voor Android*) De instelling voor de *3D-modus* knop verschijnt alleen in de lijst van het Configureerscherm als de kaartrendering-engine [Versie 2 (OpenGL)](../personal/global-settings.md#map-rendering-engine) is geselecteerd.
...
4. **Knop positie**. De knop positie op de kaart op het toepassingsscherm wordt afzonderlijk opgeslagen voor elk profiel.
...
[Widget voor snelle actie](./quick-action.md) is een configureerbare knop met verschillende actietypes die kunnen worden toegewezen. Het is ook mogelijk om meerdere [Aangepaste knoppen](./quick-action.md#custom-buttons) te hebben.


## Uiterlijk kaartknop {#map-button-appearance}
...
Instellingen voor het uiterlijk van de kaartknop zijn beschikbaar via de volgende menu's:

- *Menu → Scherm configureren → Knoppen → Standaardknoppen*
- *Menu → Scherm configureren → Aangepaste knoppen → Snelle actie → drie-puntjes menu → Uiterlijk*
...
- [Radius-liniaal en Liniaal](./radius-ruler.md)
- [Markeerwidgets](./markers.md)
