...
**Zoeken** is een handig hulpmiddel voor het snel vinden van locaties. U kunt Zoeken gebruiken om een locatie te vinden op adres, coördinaten, nuttige plaatsen (POI) of eerdere zoekopdrachten. Met Zoek [Adres](#search-address) kunt u het adres invoeren om een exacte locatie te krijgen. Zoeken op [Coördinaten](#search-coordinates) werkt met geografische coördinaten zoals breedtegraad en lengtegraad. Met [POI](#search-poi) zoeken kunt u zoeken naar plaatsen in de buurt in specifieke categorieën, zoals cafés, hotels of tankstations. Zoek [Geschiedenis](#search-history) slaat eerdere zoekopdrachten op voor snelle toegang tot eerder gevonden plaatsen. De zoekfunctie maakt het gemakkelijk om reizen te plannen, onbekend terrein te navigeren en POI's in de buurt te vinden.
...
- Aanvankelijk **is de zoekopdracht gebaseerd op de gegevens op de kaart in het zichtbare gebied van het apparaatscherm**, maar als daar niets wordt gevonden, biedt OsmAnd aan om de zoekradius te vergroten.  
...
- URL-zoekopdracht (Google en OsmAnd).
- Zoekgeschiedenis.
...
- Het sorteerproces wordt **automatisch beheerd** door OsmAnd, op basis van de hierboven vermelde criteria.
...
OsmAnd stelt u in staat om te zoeken naar punten die op een kaart op het scherm van uw apparaat worden weergegeven. Om dit te doen:

- Gebruik [een van de manieren](#how-to-use) om naar de zoektool te gaan.
...
![Zoek Android](@site/static/img/search/search_all_near_location_andr.png)
...
![Zoek Android](@site/static/img/search/search_address_2_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Zoek iOS](@site/static/img/search/street_search_ios.png)  

</TabItem>

</Tabs>

**Adres zoeken** maakt gebruik van OpenStreetMap-gegevens. Dit type zoekopdracht stelt u in staat om de locatie en route naar een specifiek adres te vinden uit een reeds gesorteerde lijst. Lees meer in het artikel [Adres zoeken](./search-address.md).
...
- OsmAnd biedt een [Online zoeken](./search-poi.md#online-search) dat in realtime werkt, waardoor u snel toegang krijgt tot zoekresultaten.  
...
Coördinaten zoeken biedt een nauwkeurige locatie. U kunt precieze coördinaten invoeren die bijvoorbeeld zijn verkregen uit andere bronnen zoals een kaart, GPS-apparaat of online services, wat bijzonder handig is als het gaat om plaatsen die geen exact adres hebben. Voor meer informatie, zie het artikel [Coördinaten zoeken](./search-coordinates.md).
