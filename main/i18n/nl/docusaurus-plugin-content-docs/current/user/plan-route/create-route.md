...
De tool **Route plannen** (*Menu → Route plannen*) is een krachtige functie van de OsmAnd-applicatie waarmee u [nieuwe routes kunt maken](#create-new-route) als GPX-tracks, [nieuwe segmenten kunt bewerken en toevoegen](#segments) aan reeds opgeslagen tracks, [afstanden op de kaart kunt meten](#distance-measurement), en [tracksegmenten kunt koppelen](#attach-track-to-roads) aan de dichtstbijzijnde beschikbare weg met behulp van verschillende navigatieprofielen. De functie is ontworpen om in *offline modus* te werken.

Een route bestaat uit een reeks segmenten tussen gespecificeerde punten. De segmenten kunnen rechte lijnen zijn of routes die zijn aangepast aan het geselecteerde profiel. De route kan worden opgeslagen als GPX voor later importeren, bewerken en [navigatie](../navigation/setup/gpx-navigation.md).

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Route plannen android](@site/static/img/plan-route/plan_route_overview_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios](@site/static/img/plan-route/plan_route_overview_ios.png)
...
![Route plannen android](@site/static/img/plan-route/plan-route-menu-andr.png)
...
![Route plannen ios](@site/static/img/plan-route/plan-route-menu-ios.png)
...
Om een nieuwe track in GPX-formaat te maken, gebruikt u de hoofdfunctie van de tool *Route plannen*. Het vooraf maken van een route heeft veel voordelen, in tegenstelling tot het opnemen van de huidige track met *[Trip recording plugin](../plugins/trip-recording.md)*. U kunt zoveel punten als u wilt [toevoegen](#adding-points) aan de route, [deze verwijderen en verplaatsen](#point-context-menu), routetypes wijzigen per [segment](#route-between-points), en [gedetailleerde informatie over de route](#graph) krijgen.  
...
De tool *Route plannen* maakt het mogelijk om bestaande GPX-tracks te wijzigen, evenals een [geïmporteerde track](../personal/tracks/manage-tracks.md#import). Echter, in het geval dat u een grote hoeveelheid punten wilt uitsluiten op basis van generieke criteria, is [<Translate android="true" ids="shared_string_gps_filter"/>](../map/tracks/track-context-menu.md#gps-filter) mogelijk een geschiktere tool.


### Afstandsmeting {#distance-measurement}

![Route plannen android](@site/static/img/plan-route/plan_route_lines_andr.png)  

*Route plannen* is een snelle en eenvoudige manier om de afstand tussen punten te meten.

- Selecteer de methode *Rechte lijn* voor routeplanning. Er wordt een stippellijn getrokken tussen de punten.
- [Voeg](#adding-points) het eerste punt op de kaart toe van waaruit de lijn wordt getrokken.
...
:::note
De *Rechte lijn* is nodig en wordt gebruikt voor gebieden die niet worden gedekt door routeringsgegevens, zoals off-road en off-track gebieden.
:::
...
![Route plannen android](@site/static/img/plan-route/plan_route_graph_4_andr.png)  
...
- [Gebruik terreinkaarten](../map/tracks/track-context-menu.md#calculating-missing-elevation). ([OsmAnd Pro](../purchases/android.md#pro-features)) De modus berekent de hoogte op basis van terreinkaartgegevens (3D). Het verschil tussen de door uw apparaat opgenomen hoogtes kan worden gebruikt voor hoogtemeting. De geometrie van de track blijft ongewijzigd.
...
![Route plannen snap-road-ios](@site/static/img/plan-route/plan_route-snap_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen snap-road-ios](@site/static/img/plan-route/plan_route-snap_ios.png)

</TabItem>

</Tabs>  

Met de instelling **Koppelen aan wegen** kunt u een opgenomen of geïmporteerde track koppelen aan de wegen van OsmAnd offline kaarten om aanvullende informatie te verkrijgen:
...
U kunt een [drempelwaarde](../navigation/setup/gpx-navigation.md#attach-to-the-roads) selecteren voor de afstand die vereenvoudigde trackpunten kunnen hebben ten opzichte van de originele trackpunten.
...
![Route plannen android](@site/static/img/plan-route/plan_route_points_list_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios](@site/static/img/plan-route/plan_route_points_list_ios.png)

</TabItem>

</Tabs>  

Om een afstand te meten of een reis te plannen, voegt u één voor één punten toe op de *Aanwijzer* locatie en tikt u op de knop *Punt toevoegen*. Door de **puntenlijst** hieronder te openen, kunt u punten opnieuw rangschikken, verwijderen of een specifiek [puntcontextmenu](#point-context-menu) openen.
...
2. Tik op het *profielpictogram* linksonder in het kaartscherm. Niet op het bovenste pictogram, dit opent het menu Kaart configureren.
...
![Route plannen android](@site/static/img/plan-route/plan_route_between_points_andr.png) ![Route plannen android](@site/static/img/plan-route/plan_route_change-route-type_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios](@site/static/img/plan-route/plan_route_between_points_ios.png) ![Route plannen ios](@site/static/img/plan-route/plan_route_change-route-type_ios.png)
...
- *Routetype wijzigen voor/na punt*. In het *puntcontextmenu* kunt u de manier wijzigen waarop de route wordt berekend voor het gedeelte van dit punt tot het dichtstbijzijnde of tot het randpunt. De instelling geeft informatie over de afstand van dit punt tot het begin of het einde van de route, of tot het volgende/vorige punt.
- *Routes herberekenen*. U kunt routeherberekening gebruiken zonder het profieltype te wijzigen. Het profielpictogram dat op de geplande route in de tool wordt weergegeven, verandert niet, maar het routetype komt overeen met het geselecteerde type. Dit kan nodig zijn om alternatieve routes te vinden.  
...
![Route plannen android](@site/static/img/plan-route/plan_route_points_menu_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios](@site/static/img/plan-route/plan_route_points_menu_ios.png)
...
- ***Informatie over het punt***. Het aantal punten in de lijst. De afstand tot het eerste punt is gebaseerd op het ingestelde routeringstype, de afstand kan veranderen wanneer deze opnieuw wordt berekend naar het nieuwe type als de parameter *Route tussen punten* wordt gewijzigd.  
...
- ***<Translate ios="true" ids="trim_before"/>***. U kunt het hele gedeelte van de route, punten en segmenten, vanaf het begin tot het geselecteerde punt inkorten. De in te korten afstand wordt onder de optienaam aangegeven.  

- ***<Translate ios="true" ids="trim_after"/>***. U kunt een route, punten en segmenten inkorten, vanaf het laatst toegevoegde punt tot het geselecteerde punt. De in te korten afstand wordt onder de optienaam aangegeven.  

- ***<Translate ios="true" ids="plan_route_split_before"/>***. Hiermee kan de route vóór het geselecteerde punt worden gesplitst in afzonderlijke, niet-verbonden segmenten.
...
![Route plannen android-routeline](@site/static/img/plan-route/plan-route-routeline-android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios-screen](@site/static/img/plan-route/plan-route-routeline-ios.png)

</TabItem>

</Tabs>

Met de tool *Route plannen* en de optie [Route tussen punten](#route-between-points) kunt u multimodale routes maken waarbij bijvoorbeeld het eerste deel een *fietsroute* kan zijn, het tweede deel een *rechte lijn* en vervolgens een *voetgangersroute*. Houd er rekening mee dat navigatie op multimodale routes niet beschikbaar is, dus u moet een van de meest geschikte profielen selecteren om de turn-by-turn instructies te kunnen volgen.
...
![Route plannen android](@site/static/img/plan-route/plan_route_save_changes_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios](@site/static/img/plan-route/plan_route_save_changes_ios.png)
...
Nadat u ten minste één punt aan de kaart hebt [toegevoegd](#adding-points), kunt u de opslagoptie gebruiken. Alle routes die zijn opgeslagen in *Route plannen* zijn te vinden in het hoofdmenu *<Translate android="true" ids="shared_string_menu"/> → <Translate android="true" ids="shared_string_my_places"/> →* *[<Translate android="true" ids="show_gpx"/>](../personal/tracks/manage-tracks.md)*.  

Er zijn vier manieren om op te slaan:

- ***Snel opslaan***. De knop rechtsboven ***Klaar*** / ***Opslaan*** (voor bestaande tracks) maakt snel opslaan van wijzigingen en het afsluiten van de tool *Route plannen* mogelijk. De naam wordt gegenereerd op basis van de huidige datum.
- ***Wijzigingen opslaan*** in [Opties menu](#options) stelt u in staat wijzigingen in een bestand op te slaan en door te gaan met het plannen van de route.
- ***Opslaan als nieuwe track*** in het [Opties menu](#options) opent een dialoogvenster waarin u de naam van de track en de map opgeeft waar de route wordt opgeslagen.
...
![Route plannen android-options](@site/static/img/plan-route/plan_route_menu_options_3_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Route plannen ios-options](@site/static/img/plan-route/plan_route_menu_options_ios.png)
...
- **<Translate ios="true" ids="gpx_start_new_segment"/>** (*iOS*) of **<Translate android="true" ids="plan_route_add_new_segment"/>** (*Android*). Tekent nieuwe routesegmenten die niet zijn verbonden met het vorige segment.
...
- **<Translate android="true" ids="reverse_route"/>**. U wisselt het *Startpunt* van de route en het laatst toegevoegde punt om. De segmentinstellingen van de route veranderen niet wanneer Omkeren wordt toegepast.
...
- [<Translate android="true" ids="shared_string_gps_filter"/>](../map/tracks/track-context-menu.md#gps-filter) (alleen Android). U kunt routepunten filteren die niet overeenkomen met het geselecteerde routeringstype, onnodige gegevens verwijderen of onnauwkeurige gegevens corrigeren. Het GPS-filter werkt alleen als het routeringstype is opgegeven als *Rechte lijn*. <!-- Android only(No!!!) with Straight line routing. **?How to use?** **When are additional details needed to calculate a route when switching to another type of routing?** -->
...
![Route plannen android](@site/static/img/plan-route/plan_route_graph_5_andr.png)
...
Tijdens de routeplanning toont de [grafiek](../navigation/setup/route-details.md#elevation-graph) de hoogte en het hellingspercentage van de route, en u kunt overal op de grafiek tikken om een aanwijzer met specifieke gegevens weer te geven.  

- Voor **Android** wordt de grafiek weergegeven in het tabblad *Grafiek* bij het maken of openen van een track, en bij [Navigatie](../navigation/setup/gpx-navigation.md) met behulp van *Route plannen*.
- Voor **iOS** is de grafiek alleen beschikbaar via het menu *[Opties](#options) → Navigatie*.

Bij het berekenen van een route voor navigatie in *Route plannen* kunt u aanvullende trackinformatie vinden, zoals [Hoogte-informatie](../navigation/setup/route-details.md#elevation-info) en [Wegkenmerken](../navigation/setup/route-details.md#road-attributes), en de tool [Analyseren op kaart](../navigation/setup/route-details.md#analyse-on-map) gebruiken. Tik op de knop *Details* onder de grafiek.  
