...
![Reisgidsenweergave Algemeen](@site/static/img/guides/travel_guides_view_android.png)
...
- Blader door [artikelen](#browse-articles) over een land, regio, stad, attractie.
    - Als alternatief, [toon reisgidsartikelen en/of punten op de kaart](#travel-routes) met behulp van het menu Kaart configureren.
...
- [Bookmark](#explore-and-bookmark) artikelen om ze later te lezen.
...
:::note
*Reisgidsen*-artikelen worden in OsmAnd opgeslagen als GPX-tracks met Waypoints. Nadat u deze tracks aan de kaart hebt toegevoegd (zie *[Punten](#points)* of *[Reisroutes](#travel-routes)* secties van dit artikel), kunt u ze beheren via het menu *[Mijn plaatsen](../personal/myplaces.md)*.
:::
...
Door de benodigde regio's op uw apparaat op te slaan, kunt u de artikelen met OsmAnd bekijken, zelfs als u in het buitenland bent of buiten bereik van uw mobiele netwerk.
...
Wanneer u *Reisgidsen* opent (*<Translate android="true" ids="shared_string_menu,shared_string_travel_guides"/>*), toont OsmAnd u artikelen over interessante plaatsen die zich bevinden nabij de geografische coördinaten van het huidige midden van de kaart. Wanneer u op het item in de lijst tikt, wordt het [Reisgidsartikel](#travel-article) geopend. Onderaan het scherm bevindt zich een schakelaar naar het tabblad *Bladwijzers*.
...
Als u op de knop *Bladwijzer* tikt, wordt het artikel opgeslagen om later te lezen en is het beschikbaar op het tabblad Bladwijzers. Op het tabblad *Gebladwijzerde artikelen* zijn artikelen gerangschikt zoals ze zijn opgeslagen, met de nieuwste artikelen bovenaan.
...
In het Opties-menu kunt u kiezen of u afbeeldingen in de bekeken artikelen wilt opslaan om ze offline te gebruiken. U kunt ook de bestaande afbeeldingscache wissen of de zoekgeschiedenis verwijderen (werkt alleen bij zoeken in *Reisgidsen*).
...
- Met de &#8942; knop kunt u het artikel delen met uw vrienden. De link opent in de OsmAnd-app, het juiste reisgidsbestand moet op het apparaat van de ontvanger worden gedownload of de [Wikivoyage](https://www.wikivoyage.org/) site, als de ontvanger de OsmAnd-app niet heeft of voor iOS-apparaten.
...
- [Bladwijzer](#explore-and-bookmark). Hiermee kunt u een artikel toevoegen aan bladwijzers om later te lezen.
...
Als u op de pijl in de linkerbovenhoek van het scherm tikt, gaat u direct naar het tabblad *Verkennen* of *Gebladwijzerde artikelen* (afhankelijk van welk tabblad u eerder hebt geopend).
...
- &nbsp;*Erheen*. Details over hoe je op bepaalde plaatsen komt en hoeveel deze route zal kosten.
...
- &nbsp;*Verder*. Dichtstbijzijnde plaatsen van Wikivoyage.
...
*Reisgidsartikelen* bevatten vaak toeristische attracties gegroepeerd per onderwerp. Door eenmaal op de knop *Punten* onderaan het *[Artikel](#travel-article)* te tikken, worden alle attracties die gerelateerd zijn aan het onderwerp (eetgelegenheden, accommodatiemogelijkheden, luchthavens, winkels, historische monumenten, enzovoort) aan de kaart toegevoegd en kunnen ze *[beheerd worden als een Track](#manage-as-gpx-track)*.

Standaard zijn attractiegroepen gemarkeerd met bepaalde pictogrammen: *Eten en Drinken* categorieën met een mes en vork in een rode cirkel, *Doen en Zien* met een camera in een groene cirkel, en andere. U kunt groepen punten beheren met &#8942; als een *[Waypointmap](../map/tracks/track-context-menu.md#points--waypoints)*.
...
Nadat u de weergave van *Reisroutes* in het menu *Kaart configureren* hebt ingeschakeld, verschijnen er cirkels van verschillende kleuren op de kaart. De oranje cirkels vertegenwoordigen routetracks, routeartikelen en enkele waypoints. Tik op een specifieke route, artikel of punt en tik vervolgens op de knop *Downloaden* (&#9047;), en u kunt ze [beheren als een track](#manage-as-gpx-track).

### Aangepaste reisgidsen {#custom-travel-guides}

Wij raden u aan nieuwe artikelen of bewerkingen toe te voegen aan [Wikivoyage](https://nl.wikivoyage.org/). Wij actualiseren de reisgidsen ongeveer elke 6 maanden, en u kunt nieuwe artikelen downloaden. U kunt ook uw eigen *Reisboek* maken. Hoe u dit doet, wordt beschreven in de technische documentatie (zie artikel *[Aangepaste reisgids maken](../../technical/map-creation/create_travel_guide.md)*).
...
In de *Reisgidsartikelen* vindt u nuttige informatie over toeristische attracties (contactinformatie, weblinks, prijzen, enzovoort). Veel artikelen hebben een *Wikipedia*-knop waarmee een *[Wikipedia-artikel](../plugins/wikipedia.md)* (online of offline) over de bezienswaardigheid kan worden geopend.

In sommige gevallen zal het tabblad *[Bladwijzers](#explore-and-bookmark)* van OsmAnd u vragen om [Wikipedia-kaartlagen](../plugins/wikipedia.md#download-wikipedia-packages-download-wikipedia-packages) te downloaden om gerelateerde artikelen offline te bekijken.

![Reisgidsen downloaden Wikipedia](@site/static/img/guides/travel_guides_wikipedia_download.png)
