...
We kozen [Telegram](https://telegram.org/) als het meest open sociale platform omdat het een [Open API](https://core.telegram.org/api), Open SDK heeft en bovendien uiteindelijk een Open Server-implementatie (Blockchain) zal hebben.
...
De applicatie stuurt Live Locatie-berichten naar geselecteerde chats voor de door u ingestelde tijd, toont een lijst met uw contacten en groepen, en controleert chats op berichten met uw locatie, die vervolgens op de kaart in OsmAnd worden weergegeven.
...
*[Mijn locatie](#my-location-screen) scherm → scherm omlaag bewegen (contactenlijst) → ⋮ → Instellingen*
of
...
**Mijn locatie verzenden** — hiermee kunt u het interval instellen voor het verzenden van uw positie, van eens per seconde (voor hogere nauwkeurigheid) tot eens in de 5 minuten (voor lager stroomverbruik).
...
**Locatie verzenden als** - hiermee kunt u een categorie kiezen voor het verzenden van berichten over uw locatie in Telegram (3 typen verzending): tekst, kaart, tekst en kaart.

**Buffervervaldatum** - hiermee kunt u de tijd kiezen voor het opslaan van punten in de buffer.

**Achtergrondwerk** - hiermee kunt u de batterijoptimalisatie-instellingen wijzigen om het delen van locaties te stabiliseren.
...
Op dit scherm ziet u uw Telegram-contacten. U kunt een of meer van hen kiezen, deelopties instellen en uw locatie beginnen te delen.
...
Wanneer u begint met delen, ziet u een lijst met voorgestelde contacten en een *Terug naar OsmAnd*-knop op het scherm *Mijn locatie*.
...
Dit is het derde tabblad in de applicatie. Hier kunt u een dag selecteren om de geschiedenis van locaties te bekijken die met u zijn gedeeld. Het selecteren van een dag toont de contacten die hun locatie op die dag hebben getoond.

Monitoring stelt u in staat om informatie op de achtergrond te verzamelen wanneer uw contacten deze delen. Om dit te doen, moet u het inschakelen.
...
U kunt de online beweging van uw contacten op de OsmAnd-kaart zien met behulp van de online GPS-track. Om dit te doen, moet u *Live* inschakelen bij het contact in de *Tijdlijn*. Als er op dit moment veel gemeenschappelijke contacten zijn, kiest de OsmAnd-app willekeurig de kleur van de tracks.
...
2. Ten tweede, wanneer u uw locatie begint te delen, verandert de widget van uiterlijk.
...
3. Zodra de internetverbinding is hersteld, begint het met het uploaden van locatiepunten uit de buffer. De OsmAnd-widget verandert van uiterlijk en het aantal minuten erin begint af te nemen. Wanneer alle locatiepunten uit de buffer zijn geüpload naar Telegram, verschijnt het emoji *OK*-pictogram op de widget.
...
4. Als u de widget als *OK* emoji-pictogram ziet, betekent dit dat alles in orde is. Het uitwisselingsproces verloopt zonder problemen.


## OsmAnd Assistent {#osmand-assistant}

De OsmAnd-app heeft een eigen Telegram-bot genaamd "OsmAnd Assistent", die u helpt OsmAnd Telegram te integreren met trackers van derden of API's. U kunt niet alleen uw locatie delen, maar ook de locatie van uw fiets of een groep mensen die via de API beschikbaar zijn. OsmAnd Telegram geeft u een mooie presentatie van uw locaties op de kaart in OsmAnd zelf.
