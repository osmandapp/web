...
De **AIS Scheepstracker** plug-in toont [Automatic Identification System (AIS)](https://nl.wikipedia.org/wiki/Automatic_Identification_System) posities en gedetailleerde informatie over schepen in de buurt. De AIS-gegevens worden via een netwerkverbinding van een externe AIS-ontvanger ontvangen.
...
1. [Schakel](../plugins/index.md#enable--disable) de plug-in **AIS scheepstracker** in via *Hoofdmenu → Plug-ins → AIS scheepstracker*.
...
AIS werkt op *VHF-frequenties* (161,975 MHz en 162,025 MHz) en heeft een beperkt signaalbereik vanwege zichtlijnvoortplanting.
...
Indien correct ingesteld, verschijnen de posities van schepen op de kaart. Belangrijkste kenmerken:

- Schepen bewegen op basis van real-time snelheid en koers.
- Nieuwe schepen komen en verlaten het zichtbare gebied dynamisch.
...
2. Dynamische informatie (real-time gegevens).
    *Verzonden met verschillende intervallen, afhankelijk van snelheid en manoeuvreren.*
    ...
    - **Scheepscoördinaten (breedtegraad en lengtegraad)**
...
De *AIS scheepstracker* plug-in biedt verschillende instellingen om navigatie en interactie te personaliseren voor gebruikers met een handicap. Deze instellingen worden toegepast voor alle [profielen](../personal/profiles.md) in OsmAnd.
...
| Timeout voor scheepszichtbaarheid | Scheepsiconen veranderen van status als er geen signaal wordt ontvangen | `2 - 15 min / Uitgeschakeld` |
| | | |
| **Waarschuwingen voor dichtstbijzijnde punt van nadering (CPA)** | | |
