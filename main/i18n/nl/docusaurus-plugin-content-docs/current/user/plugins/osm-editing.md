...
- [Inloggen bij OpenStreetMap](#authorization). Log in om nieuwe of gewijzigde wijzigingen te uploaden. Gebruik de veilige *OAuth 2.0-methode* of [log in op de OSM-site](https://www.openstreetmap.org/login). Voor iOS is de knop voor de OAuth-methode alleen beschikbaar voor gebruikers met een ondersteunde versie van iOS, 16.4
...
De instelling <Translate android="true" ids="map_updates_for_mappers"/> stelt u in staat om **[live updates](../personal/maps-resources.md#free-for-osm-mappers)** in te schakelen als u een actieve bijdrager aan OpenStreetMap bent. Op deze manier kunt u genieten van gratis uurlijkse kaartupdates, in tegenstelling tot de standaard maandelijkse updates. Actieve bijdragers aan OpenStreetMap.org kunnen recht hebben op onbeperkte uurlijkse kaartupdates. Meld u aan bij OpenStreetMap om te controleren of u in aanmerking komt voor onbeperkte maandelijkse en uurlijkse kaartupdates.
...
2. Tik op het *POI-type*.
3. Voer de sleutel *amenity* en de waarde *freeshops* in de betreffende velden in.
...
Om OSM-notities te bewerken, te becommentariëren of te sluiten, schakelt u de weergave ervan op de kaart in via het menu [Kaart configureren](../map/configure-map-menu.md).

- De functie OSM-notities stelt u in staat om notities aan de kaart toe te voegen. In deze notities kunt u fouten beschrijven of schrijven over ontbrekende informatie in de OpenStreetMap-gegevens. U en andere gebruikers kunnen opmerkingen achterlaten bij uw notities.
...
- Lees [hier](https://wiki.openstreetmap.org/wiki/Notes) meer over de OpenStreetMap-notities.

Om een notitie aan te maken, hoeft u niet in te loggen op OpenStreetMap.org, u kunt deze anoniem verzenden.

- Tik op de kaart waar de nieuwe OSM-notitie moet worden geplaatst (of tik op een nog niet geüploade notitie) en selecteer [Acties](../map/map-context-menu.md#actions).
- Als u informatie wilt toevoegen aan een bestaande notitie of een nog niet geüploade notitie, een opmerking wilt bewerken of een notitie wilt sluiten, tikt u op de OSM-notitie op de kaart en selecteert u de [vereiste actie](../map/map-context-menu.md#-comment--close-osm-note).
...
:::info
U kunt meer dan één track selecteren om naar OSM te uploaden. Tik hiervoor op het pictogram *Uploaden naar OpenStreetMap* onderaan het scherm, selecteer tracks met het selectievakje, tik op *Uploaden* en vervolgens op *Doorgaan*.
:::
