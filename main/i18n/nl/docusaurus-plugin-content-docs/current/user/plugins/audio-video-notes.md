...
![Intro audio-/videonotities](@site/static/img/plugins/audio-video-notes/widgets_view.png)
...
1. Schakel de [plug-in Audio-/videonotities](../plugins/index.md#enable--disable) in de sectie Plug-ins van het *Hoofdmenu* in.
...
![Instellingen plug-in audio-/video Android-foto](@site/static/img/plugins/audio-video-notes/settings_av_plugin.png)
...
| --- | --- |
| **<Translate android="true" ids="camera_app"/>** | U kunt kiezen of u de systeem-app of de OsmAnd-camera wilt gebruiken om <Translate android="true" ids="photo_notes"/> te maken. |
...
| **<Translate android="true" ids="multimedia_rec_split_title"/>** | Indien ingeschakeld, worden oude video-opnamen automatisch overschreven en vervangen door nieuwe video-opnamen als de opslagruimte de limiet bereikt. |
| **<Translate android="true" ids="rec_split_clip_length"/>** | Dit bepaalt de bovengrens voor opgenomen videoclips. Er zijn opties met een bereik van 1 minuut tot 30 minuten. |
| **<Translate android="true" ids="rec_split_storage_size"/>** | Dit stelt de grootte van de opslag in die bedoeld is voor opgenomen video. Er zijn opties met een bereik van 1024 MB tot 62 GB. Een specifiek bereik van opties en de standaardoptie worden bepaald door de parameters van een specifiek apparaat. Een systeemmelding herinnert u eraan dat de opslagruimte de ingestelde limiet bereikt. |
...
| **<Translate android="true" ids="copy_from_other_profile"/>** | Opent een dialoogvenster om een profiel te selecteren waaruit de configuratie van de audio-/video-opname moet worden gekopieerd en deze in te stellen voor het huidige profiel. |


## Eén notitie beheren {#manage-a-single-note}

U kunt uw informatie, gedachten, ideeën, enz. die aan een geografische plaats zijn gekoppeld, als volgt beheren:

- Een notitie maken.
- Alle notities als een lijst bekijken in [Mijn plaatsen](../personal/myplaces.md).
...
![Widget plug-in audio-/video Android](@site/static/img/plugins/audio-video-notes/audio_video_plugin_widget_choice_android.png)

</TabItem>

</Tabs>

De <Translate android="true" ids="map_widget_av_notes"/>-widget wordt gebruikt om een notitie te maken die aan uw huidige positie is gekoppeld, en zo te voorkomen dat u op de kaart naar een geschikt punt voor een notitie hoeft te zoeken. Tik eenvoudig op de widget en maak de notitie.

De functies die de <Translate android="true" ids="map_widget_av_notes"/>-widget biedt, zijn afhankelijk van hoe deze is geconfigureerd in het menu [Scherm configureren](../widgets/info-widgets.md#-audio-video-notes-widget-android). De widget opent ofwel onmiddellijk de camera om een notitie te maken, volgens de ingestelde instellingen, of vraagt eerst in welk formaat een notitie moet worden gemaakt en opent vervolgens het betreffende opnamedialoogvenster.
...
Om een notitie te maken die aan een geselecteerd punt op de kaart is gekoppeld, gebruikt u het [Contextmenu](../map/map-context-menu.md#-record-av-note-android) van het punt:
...
:::info Opmerking
Automatisch worden alle foto-, audio- en video-opnamen opgeslagen in: tabblad <Translate android="true" ids="shared_string_menu,shared_string_my_places,notes"/>.
:::
...
![Plug-in audio-/video Mijn plaatsen-menu](@site/static/img/plugins/audio-video-notes/audio_video_notes_myplaces_menu.png)
...
![Plug-in audio-/video Mijn plaatsen-menu Context](@site/static/img/plugins/audio-video-notes/audio_video_notes_myplaces_menu_context.png)
...
![Plug-in audio-/video Mijn plaatsen-menu acties](@site/static/img/plugins/audio-video-notes/audio_video_notes_myplaces_menu_actions.png)
...
De sectie [Details](../map/map-context-menu#-audiovideo-note-android) biedt meer gegevens over de notities, zoals breedtegraad en lengtegraad, de datum en tijd van de notitie en foto's.
...
De [widget Audio-/videonotities](../widgets/info-widgets.md#audiovideo-notes-widget) maakt notities, foto's, audio- en videobestanden en koppelt deze aan uw huidige positie. De widget heeft verschillende opties die kunnen worden ingesteld als de standaardactie die door de widget wordt uitgevoerd wanneer het scherm wordt aangeraakt.

Het toevoegen van een widget aan het scherm is noodzakelijk als u wilt voorkomen dat u op de kaart naar geolocatie zoekt en notities wilt koppelen aan uw huidige standaardlocatie. Volg de onderstaande stappen om een widget toe te voegen of aan te passen:
...
![Plug-in audio-/video Mijn plaatsen-menu Drie acties](@site/static/img/plugins/audio-video-notes/audio_video_notes_myplaces_menu_three_actions.png)
...
### Alles op de kaart weergeven {#show-all-on-the-map}
...
Gebruik de kaartlaag [Opname](../map/point-layers-on-map#-audio--video-points-android) om alle notities weer te geven of te verbergen. Als u deze inschakelt in het menu [Kaart configureren](../map/configure-map-menu), toont de opnamelaag alle notities op de kaart.
...
:::note
Als een track wordt geëxporteerd, kunnen de waypoints niet worden bekeken of afgespeeld, omdat audio-/video-/fotonotities alleen op het apparaat worden opgeslagen en alleen beschikbaar zijn voor de eigenaar van dat apparaat.
:::
...
1. **Automatisch** wordt een waypoint gemaakt, in het geval u een audio-/video-/fotonotitie maakt terwijl de [Ritopname](../plugins/trip-recording) bezig is, ongeacht de geolocatie of deze is gekoppeld aan de huidige positie of een andere plaats, en dus, ongeacht de gebruikte middelen daarvoor: of het de [Widget](../widgets/info-widgets#-audio-video-notes-widget-android) is, of het [Contextmenu](../map/map-context-menu#-audiovideo-note-android).
...
2. **Handmatig** wordt een waypoint gemaakt via het [Contextmenu](../map/map-context-menu#-audiovideo-note-android), als volgt:
    ...
    - Het dialoogvenster *<Translate android="true" ids="quick_action_add_gpx"/>* wordt geopend en stelt u in staat om enkele eigenschappen in te stellen, zoals de naam van het waypoint, een pictogram, kleur en andere. Tik na voltooiing op **Opslaan**.
...
- Op de kaart kunnen een waypoint en een audio-, video- of fotonotitie al dan niet zichtbaar zijn op de kaart. Dit wordt bepaald door welke laag momenteel is ingeschakeld voor het profiel.

- Wanneer noch de Tracks- noch de Opnamelaag is ingeschakeld, kunnen zowel waypoints als notities worden weergegeven door erop te tikken in Mijn plaatsen. In dit geval toont de kaart de locaties van de getapte notitie, of respectievelijk de locaties van alle waypoints van de getapte track.
...
De <Translate android="true" ids="audionotes_plugin_name"/>-plug-in genereert audio-/video-/fotobestanden in het volgende formaat:
    ...
    `{SHORTLINK_LOCATION}_Beschrijving.{avi,mp3,jpg}`
