...
Met de interactieve weerkaartlagen in de OsmAnd-app kunt u temperatuur, atmosferische druk, bewolking, windsnelheid en neerslag in uw stad of elke andere locatie volgen op een interactieve wereldkaart. De Weer-plugin biedt uurlijkse weersvoorspellingen voor 7 dagen vooruit, direct op de OsmAnd-kaart. Weerinformatie kan ook worden gedownload voor offline gebruik.
...
**2.** [Schakel](../plugins/#enable--disable) de Weer-plugin in via de sectie *Plugins* van het *Hoofdmenu*.

**3.** [Selecteer de **meeteenheden**](#weather-settings) voor de weersverschijnselen.

**4.** [Geef de **Weerlagen**](#customize-weather-layers) weer op de kaart met behulp van het menu-item Weer in het menu Kaart configureren (vergeet niet uit te zoomen).
...
:::caution Vereiste weergave
De Weer-plugin is alleen beschikbaar met de kaartweergave-engine [Versie 2](../personal/global-settings/#map-rendering-engine) (OpenGL).
:::
...
Om weerlagen op het OsmAnd-kaartscherm weer te geven, moet u het *Kaart configureren-menu* gebruiken. Ga naar het menu, zoek het item *Weer* in de lijst (alleen beschikbaar als de plugin is ingeschakeld) en tik erop.

In dit menu kunt u de **zichtbaarheid** en [instellingen](#weather-settings) voor elke laag wijzigen, het type contourlijnen selecteren en de [offline 7-daagse weersvoorspelling](#offline-forecast) downloaden (*alleen hier beschikbaar voor Android*).
...
| **Bewolking** is de gemiddelde fractie van de hemel die bedekt is met wolken, waargenomen vanaf een bepaalde locatie. Het wordt gemeten in %. |
...
Om de zichtbaarheid van weerlagen te schakelen, kunt u de *Snelle actie*-knoppen op het kaartscherm gebruiken. De algemene instellingen voor het tonen of verbergen van lagen vindt u in de aparte [Weersectie](#weather-forecast-screen) van het *Hoofdmenu*. In het artikel [Snelle actie](../widgets/quick-action.md#configure-map) vindt u een lijst met de lagen die beschikbaar zijn voor weergave. Als u snel toegang nodig heeft tot deze kaartconfiguratie, gebruikt u de tool *Aangepaste knop*.
...
Wanneer het tabblad Weer is geopend, wordt, tenzij anders aangegeven, de vereiste weersinformatie van het netwerk gedownload. U kunt de vooraf geladen [Weersvoorspelling](#download-forecast) op uw apparaten gebruiken. De weersinformatie wordt automatisch in de cache opgeslagen na het downloaden en kan offline worden gebruikt totdat de voorspelling verloopt.

:::tip
Op Android-apparaten kunt u controleren wanneer weergegevens zijn gedownload door op de *Weer-widget* te tikken. De OsmAnd Development-plugin moet zijn ingeschakeld.
...
Voor elk land kunt u **Offline Voorspelling** downloaden (tot 7 dagen). Hoe dichter bij de huidige datum, hoe nauwkeuriger deze zal zijn. Als u de weersvoorspelling offline wilt gebruiken, moet u de weergegevens van tevoren downloaden.

In het downloadvoorspellingsmenu kunt u informatie verkrijgen en parameters instellen om de weergegevens up-to-date te houden.
...
- *Alleen via Wi-Fi bijwerken* (*iOS*). Schakel deze optie in als u geen mobiele gegevens wilt gebruiken voor downloads.
...
![Weer-widgets op Android](@site/static/img/plugins/weather/weather_widgets_1_android.png) ![Weer-widgets op Android](@site/static/img/plugins/weather/weather_widgets_2_android.png)
...
![Weer-widgets op iOS](@site/static/img/plugins/weather/weather_widgets_1_ios.png)

</TabItem>

</Tabs>

[Weer-widgets](../widgets/info-widgets.md#weather-widgets) zoals Temperatuur, Luchtdruk, Wind, Wolken en Neerslag tonen de huidige weergegevens voor het **centrale punt op de kaart**.

- Weer-widgets worden alleen weergegeven wanneer het [Weerscherm](#weather-forecast-screen) is ingeschakeld.
- Om weer-widgets te activeren, selecteert u het hoofdmenu-item *Scherm configureren*, het paneel waarop u ze wilt plaatsen en de gewenste widgets uit de lijst.
