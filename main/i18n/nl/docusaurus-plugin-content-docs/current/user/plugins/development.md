...
Testfuncties van de app zijn verzameld in de OsmAnd-ontwikkelingsplug-in. Gebruik ze op eigen risico.

Met de OsmAnd-ontwikkelingsplug-in kunt u nieuwe functies van de app uitproberen of OsmAnd configureren voor testen: navigatieroutes simuleren, de schermweergaveprestaties controleren, enz. De plug-in is ontworpen voor *ontwikkelaars en ervaren gebruikers* en is niet vereist voor dagelijks gebruik van de applicatie.
...
1. Schakel de OsmAnd-ontwikkelingsplug-in in via het gedeelte Plug-ins van het *Hoofdmenu*.
2. Voer de nodige instellingen uit in het [Instellingenmenu](#plugin-settings) van de plug-in.
...
:::info info
OsmAnd-ontwikkelingsplug-ininstellingen zijn globaal en gelden voor alle profielen.
:::

Gebruik een van de volgende paden om de plug-ininstellingen te openen:

- *Hoofdmenu → Plug-ins → OsmAnd-ontwikkeling → Instellingen*.
- *Hoofdmenu → [Algemene instellingen](../personal/global-settings.md) → Profiel → OsmAnd-ontwikkelingsplug-ininstellingen*.
...
- **Gebruik raster [SQLIte-formaat](../../technical/osmand-file-formats/osmand-sqlite) voor schaduw en helling** (*Android*). Laad oude formaat rasterkaarten.
...
- **Batterijbesparingsmodus** (*Android* / *iOS*). Deze instelling vermindert de [schermverversingssnelheid](#map-rendering-fps-widget) met 20 frames per seconde om batterijvermogen te besparen tijdens het gebruik van applicaties.
...
- **<Translate android="true" ids="write_bearing"/>** (*Android*). <Translate android="true" ids="write_bearing_description"/>. Peiling is de kompasrichting van uw huidige positie naar uw beoogde bestemming. Heeft invloed op *[trackopname](../plugins/trip-recording.md)*.
- **<Translate android="true" ids="write_heading"/>** (*Android*). <Translate android="true" ids="write_heading_description"/>. Koers is de kompasrichting van uw huidige positie naar het ware noorden. Deze optie is standaard ingeschakeld. Heeft invloed op *[trackopname](../plugins/trip-recording.md)*.
...
- **Simuleer eerste app-start**. Stelt de vlag in die de eerste app-start aangeeft, behoudt alle andere instellingen ongewijzigd.
- **Test steminstructies**. Selecteer een stem en test door aankondigingen af te spelen.
...
- De optie **Toegewezen geheugen voor routering** stelt u in staat om de hoeveelheid geheugen te bepalen die is toegewezen voor het berekenen van lange routes. U kunt deze optie gebruiken als de [Standaard routering A*](../navigation/guidance/navigation-settings.md#development-settings) is geselecteerd in de *Navigatie-instellingen*.
...
- **Plug-ininstellingen terugzetten naar standaard**. Zet alle plug-ininstellingen terug naar hun standaardwaarden.
...
:::info Belangrijk
OsmAnd-ontwikkelaarswidgets kunnen alleen aan het scherm worden toegevoegd wanneer de OsmAnd-ontwikkelingsplug-in is ingeschakeld.
:::
...
U kunt [Camerapositie-widgets](../widgets/info-widgets.md#camera-widgets) gebruiken om de OsmAnd-kaart een realistische uitstraling te geven en foto's van de aarde, zowel van boven als vanuit de ruimte genomen, te matchen.
...
De Camerakantelwidget toont de helling tussen de centrale kijkvector van de camera (kijker) en een denkbeeldige vlakke grond van de aarde. De standaardwaarde is 90 graden (geen kanteling).
...
De Camera-elevatiewidget toont de camera-elevatie boven het oppervlakteniveau. Camera-elevatie wordt aangegeven in meters / kilometers.
...
- **<Translate android="true" ids="map_widget_map_scale"/>**. Toont de huidige verhouding tussen een afstand op een kaart en de overeenkomstige afstand op de aarde. Voorbeelden: "1 : 3 000", "1 : 3,3 M" "1: 340 K".

:::note

- Het initiële zoomniveau (0) is het niveau waarop het gehele oppervlak van de aarde (de volledige kaart) op het scherm wordt weergegeven, en de grootte is 256 bij 256 pixels.
...
Deze widget is speciaal voor u ontworpen bij het gebruik van OsmAnd om een ​​handiger en informatiever interactie met de beschikbare RAM op uw apparaat te bieden. Hiermee kunt u eenvoudig en snel de beschikbare RAM controleren, wat vooral handig is bij het gebruik van OsmAnd voor navigatie. Lees meer over de widget in het artikel [Informatiewidgets](../widgets/info-widgets.md#available-ram).


## GPX-tracksimulatie {#gpx-track-simulation}

OsmAnd biedt de mogelijkheid om de locatie en beweging van uw apparaat te simuleren met behulp van een GPX-track. Gebruik hiervoor de optie *<Translate android="true" ids="simulate_location_by_gpx"/>* in de OsmAnd-ontwikkelingsplug-in [instellingen](#plugin-settings):
...
- U kunt ook beweging langs een GPX-track simuleren vanuit het [navigatiemenu](../navigation/setup/route-navigation.md#simulated-navigation) zonder de OsmAnd-ontwikkelingsplug-in in te schakelen. In dat geval wordt uw locatie niet gesynchroniseerd met de track.
