...
De stemnavigatiefunctie van OsmAnd biedt real-time gesproken instructies en waarschuwingen die u helpen geïnformeerd en veilig te blijven terwijl u onderweg bent. U kunt de begeleiding afstemmen op uw voorkeuren en behoeften. Om dit te doen, selecteert u een geschikt stemprofiel, ofwel een [Tekst-naar-Spraak (TTS) engine](#tts-text-to-speech) die dynamisch straatnamen, afstanden en snelheidslimieten kan aankondigen, of een [vooraf opgenomen stem](#recorded-voice-prompts) die beknopte instructies geeft.

Naast afslag-voor-afslag aanwijzingen kan de stemnavigatie van OsmAnd u op de hoogte stellen van aankomende nuttige plaatsen, veranderingen in verkeersomstandigheden en wanneer u een vooraf ingestelde snelheidslimiet overschrijdt. Voor meer informatie over het aanpassen van deze waarschuwingen, zie [Instellingen voor aankondigingstijd](#announcement-time) en [Waarschuwingen voor snelheidslimiet](#speed-limit).
...
OsmAnd biedt verschillende opties voor het regelen van steminstructies, zodat u uw route gemakkelijk kunt volgen. Om deze instellingen te configureren, begint u in de juiste applicatiesecties.
...
- Schakel steminstructies in of uit via *Menu → Navigatie →* tik op *Geluidsicoon*,
    of *Menu → Navigatie →* Instellingenicoon *→ Geluid →* aan/uit zetten.
...
**Aankondiging**. Hiermee kunt u de volgende soorten prompts configureren:

- *Straatnamen (TTS), Afslagnummers, Verkeerswaarschuwingen, Voetgangersoversteekplaatsen* en *Tunnels.*
...
- Schakel steminstructies in voor de vooraf ingestelde en toegevoegde [Waypoints](../../map/tracks/track-context-menu.md#add-waypoint-to-a-track), [Favorieten](../../personal/favorites.md) of [POI's](../../map/point-layers-on-map.md#points-of-interest-pois). Tijdens het rijden worden de geselecteerde punten aangekondigd wanneer u deze nadert of passeert.

| Type prompt | Aanlooptijd [s]:<br/>Corresponderende<br/>Aanloopafstand @ Standaardsnelheid [m] | Limiet |
| :- | :- | :- |
| Naderend | **60 s:**<br/>Rijden: 750 m<br/>Fietsen: 167 m<br/>Lopen: 67 m | Niet meer dan 1 punt tegelijk |
...
- *Aankondigen GPS-signaalverlies en herstel*. OsmAnd kondigt aan of het GPS-signaal op het apparaat verloren is gegaan.
- *Aankondigen routeherberekening*. OsmAnd meldt routeherberekening in geval van een [afwijking of beweging in omgekeerde richting](./navigation-settings.md#recalculate-route).
- *Aankondigen afwijking van de route*. U ontvangt informatie over afwijkingen van de route volgens de [ingestelde parameters](./navigation-settings.md#recalculate-route).

**Opties**:

- *Navigatie-instructies herhalen*. Hiermee kunt u de navigatie-instructies met regelmatige tussenpozen van 1 minuut tot 30 minuten herhalen. Of handmatig - als u een steminstructie mist, kunt u deze opnieuw beluisteren door simpelweg op [de huidige afslagpijl](../../widgets/nav-widgets.md#next-turn) op het applicatiescherm te tikken.
- *[Aankondigingstijd](#announcement-time)*.

**Uitvoer** (*alleen Android*):

- *[Spraakbegeleidingsuitvoer](#voice-guidance-output)*.
...
Om te voorkomen dat audio tegelijkertijd in dezelfde uitvoerstroom wordt afgespeeld, is de audiofocus geïmplementeerd in Android. OsmAnd zal de geselecteerde luidspreker uit de lijst in deze instelling gebruiken voor audio-uitvoer. Andere applicaties zullen de weergave pauzeren of het volume verlagen om het u gemakkelijker te maken de steminstructies van OsmAnd te horen.
...
### Audio-problemen oplossen {#solving-audio-issues}

Als u geen **steminstructies** of [snelheidslimietwaarschuwingen](#speed-limit) hoort:

- Zorg ervoor dat het volume van uw apparaat aan en hoog staat.
...
- Controleer welke [spraakbegeleiding](#voice-prompt-language) is geselecteerd en zorg ervoor dat u een TTS of opgenomen stem hebt die snelheidslimietwaarschuwingen kan aankondigen.
- Controleer of audio niet naar een onbedoeld uitvoerapparaat wordt geleid (bijv. een losgekoppelde Bluetooth-headset).
...
OsmAnd ondersteunt **Tekst-naar-Spraak (TTS)** stemmen, die dynamische en gedetailleerde spraakbegeleiding bieden, inclusief afslaginstructies, straatnamen en POI-aankondigingen. TTS-stemmen zijn inbegrepen in de applicatie, maar vereisen een [Tekst-naar-spraak engine](https://nl.wikipedia.org/wiki/Spraaksynthese) die op het apparaat is geïnstalleerd.
...
1. Open Instellingen.
2. Navigeer naar *Toegankelijkheid → Tekst-naar-spraakuitvoer*.
...
6. Tik op **Luister naar voorbeeld** om de spraakuitvoer te testen.

Voor meer details, raadpleeg:
[Google Support – Android Tekst-naar-spraakinstellingen](https://support.google.com/accessibility/android/answer/6006983).
...
### Steminstructietaal {#voice-prompt-language}
...
- **<Translate android="true" ids="tts_title"/>**
    ...
    - Kan worden aangepast met instellingen voor spreeksnelheid, toonhoogte en uitspraak.
...
1. Open apparaat *Instellingen → Toegankelijkheid → Tekst-naar-spraakuitvoer*.
...
> *Voor aanvullende stemmen, download taalpakketten via de systeeminstellingen.*

#### Beschikbare TTS-talen {#available-tts-languages}

Er zijn momenteel in totaal 45 talen. Niet alle hieronder genoemde talen worden mogelijk door elke TTS-engine ondersteund. Zie [hier](https://accessibleandroid.com/list-of-languages-with-available-tts-engines-on-android/).
...
Het gebruik van opgenomen stemmen in OsmAnd zou alleen een terugval moeten zijn. Ze zijn vrij beperkt en kunnen geen straatnamen, plaatsnamen, enz. uitspreken. U kunt steminstructies uit twee verschillende sets uit de lijst downloaden.
...
- **Complex**. Geschikt voor wielrennen, het complexe patroon gebruikt pieptonen van verschillende lengtes en toonhoogtes om u te informeren over aankomende afslagen.
    - Een lage pieptoon betekent een linker afslag, terwijl een hoge pieptoon een rechter betekent. Een reeks pieptonen met gemiddelde toonhoogte vertegenwoordigt de afslag die u moet nemen op een rotonde (verkeersplein). Alle drie de toonhoogtes achter elkaar vertegenwoordigen een U-bocht. In al deze gevallen betekenen korte pieptonen dat u zich moet voorbereiden om iets te doen, terwijl lange pieptonen betekenen dat u nu iets moet doen.

## Tekstmeldingen {#text-notifications}

Zodra u een route start, kunt u de informatie bekijken in het vervolgkeuzemenu van het systeem in de lijst met meldingen. De stille meldingen van OsmAnd bevatten informatie zoals afslag-voor-afslag instructies, afslagpijlen, aankomsttijd en resterende tijd, huidige snelheid en afstand tot bestemming.

![Navigatie route Melding Android](@site/static/img/navigation/route/navigation_notifications_android.png)
...
1. **<Translate android="true" ids="system_screen_timeout"/>**. Het scherm wordt uitgeschakeld afhankelijk van de systeeminstellingen van uw apparaat. De knop *Instellingen wijzigen* biedt snelle toegang tot het systeemmenu zoals *Scherm en helderheid* waar u de schermtime-out kunt aanpassen.
...
4. [TTS werkt niet goed? Volg deze stappen om het te verhelpen.](../../troubleshooting/navigation.md#tts-does-not-function-properly-follow-these-steps-to-fix-it)
