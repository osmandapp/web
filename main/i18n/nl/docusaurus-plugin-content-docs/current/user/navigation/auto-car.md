...
Terwijl uw apparaat is verbonden met *Android Auto*, is de kaart op het app-scherm vergrendeld. Dit is een opzettelijke veiligheidsfunctie om ervoor te zorgen dat navigatie uitsluitend vanaf het autodashboard wordt bediend, waardoor afleiding tijdens het rijden wordt geminimaliseerd. Raadpleeg de [veelgestelde vragen](#common-issues-and-solutions) over het verbindingsscherm.
...
- [Zoeken](#search). Tikken op de knop opent een scherm met toegang tot de zoekfuncties.
...
- [Zoomen](../widgets/map-buttons.md#my-location-and-zoom). Hiermee kunt u in- en uitzoomen op het zichtbare deel van de kaart.
...
1. **Rijveiligheid.** Verschillende soorten transport vereisen verschillende informatie en instellingen tijdens het rijden. Vrachtwagenchauffeurs kunnen bijvoorbeeld beperkt zijn in de hoogte, het gewicht en de breedte van hun voertuigen, dus het *Vrachtwagen*-profiel biedt hen de juiste beperkingen en richtlijnen. Terwijl fietsers en motorrijders informatie nodig hebben over fietspaden en routes, zonder gegevens voor vrachtwagens.

2. **Bruikbaarheid en efficiëntie.** Navigatie moet worden aangepast aan de behoeften van een specifieke transportmodus. Hierdoor kunnen gebruikers de tijd die nodig is om optimale routes te vinden verkorten en onnodige verkeersbeperkingen vermijden.

3. **Fouten vermijden.** Overschakelen naar het *Rij*-profiel wanneer er geen specifiek profiel is voor de geselecteerde transportmodus, biedt u constante toegang tot de minimaal noodzakelijke instellingen en informatie, waardoor mogelijke fouten en misverstanden tijdens het rijden worden voorkomen.  
...
1. Als u een bepaalde locatie op de kaart als eindpunt wilt, selecteert u de juiste categorie punten uit de lijst, [POI's](#poi-categories), [Favorieten](#favorites) of [Kaartmarkeringen](#map-markers).  
    ...
    - Alle categorieën kunnen niet meer lijstitems bevatten dan door uw multimediasysteem in het voertuig worden geleverd. Dit betekent dat de lijst in *Android Auto* onvolledig kan zijn, en alle andere mappen zijn alleen beschikbaar in de OsmAnd-app op uw apparaat.
...
In de categorie Geschiedenis kunt u een bestemming selecteren uit een lijst met eindpunten. Deze zijn gesorteerd op meest recent gebruik, beginnend met de nieuwste. Dit kan handig zijn voor snelle toegang tot veelgebruikte routes. Lees het artikel [Zoekgeschiedenis](../search/search-history.md) om te leren hoe u een bestemming kunt gebruiken of verwijderen.  
...
2. Tikken op een categorie opent het volgende scherm met een lijst met POI's.
3. Tikken op een item in de lijst opent het navigatiescherm.
...
1. Tikken op een map opent het volgende scherm met een lijst met favoriete mappen.
2. Tikken op een item in de lijst opent de navigatie.
...
Om uw eindbestemming als Kaartmarkering te definiëren, moet u eerst het vereiste aantal markeringen aanmaken in de OsmAnd-app op uw apparaat voordat u deze met het systeem verbindt. Markeringen moeten beschikbaar zijn in de lijst *Menu → Kaartmarkering*. Dit wordt uitgebreider beschreven in het artikel [Markeringen](../personal/markers.md).
...
Selecteer uit de lijst met tracks in het *Menu → Mijn plaatsen → [Tracks](../personal/tracks/manage-tracks.md)* de gewenste track voordat u verbinding maakt met het multimediasysteem van het voertuig, en de route zal ofwel worden opgebouwd tot het begin van de track of de huidige richting langs de track weergeven.
...
Naar Zoeken gaan geeft u snel toegang tot een lijst met recente [bestemmingsgeschiedenis](#history). Evenals bij het [Navigatiemenu](../navigation/setup/route-navigation.md#navigation-menu), waar alle OsmAnd-bestemmingsmethoden zoals een adres, POI's of coördinaten zijn verzameld.
...
- [POI](../search/search-poi.md) en [Aangepaste POI](../search/search-poi.md) - de lijst met vergelijkbare namen in de buurt zal ook POI's of hun categorieën weergeven.
...
De **snelheidsmeterwidget** is een geïntegreerd interface-element dat de *huidige snelheid* weergeeft met behulp van GPS-gegevens en de *snelheidslimiet* uit de [OSM-database](https://wiki.openstreetmap.org/wiki/Key:maxspeed) en [OsmAnd-instellingen](../navigation/guidance/voice-navigation.md#speed-limit) op het scherm van het multimediasysteem van het voertuig.

- De **snelheidsmeterwidget** is [*profielafhankelijk*](../personal/profiles.md), dus als u de instellingen voor het ene profiel wijzigt, worden deze niet toegepast op een ander profiel.
- Er is geen optie om de weergave van de **snelheidsmeterwidget** rechtstreeks in *Android Auto* aan te passen, u moet deze configureren in de OsmAnd-app voordat u de navigatie start en uw apparaat met het voertuig verbindt.
- Voor meer informatie over het toevoegen en configureren van de *snelheidsmeter*-widget op het scherm, leest u het artikel [Informatiewidgets](../widgets/info-widgets.md#speedometer).
...
In de OsmAnd-app kunt u één of meerdere [tussenpunten](../navigation/setup/route-navigation.md#intermediate-destinations) voor de navigatieroute selecteren.
...
OsmAnd heeft [kaartmodus](../map/vector-maps.md#map-mode) opties, maar wanneer u de app in *Android Auto* gebruikt, worden deze opties genegeerd. In plaats daarvan wordt de modus gebruikt die u hebt geselecteerd in de *Android Auto*-systeeminstellingen voor kaarten.

- Selecteer Dagmodus of Nachtmodus om de kaartweergave ongewijzigd te laten.
- Als u wilt dat de kaartweergavemodus schakelt afhankelijk van het tijdstip van de dag, selecteert u de Automatische modus.  
...
De OsmAnd-app kan worden geopend en gebruikt om te navigeren in *Android Auto* op het multimediasysteem van het voertuig, tegelijkertijd met muziek-, bericht- of andere meldingsapps. Het instellen van de gesplitste schermweergave op een telefoon die is verbonden met *Android Auto* of op het scherm in uw voertuigsysteem.  

1. Tik op het startscherm op Apps.
2. Tik op Instellingen.
3. Zoek Meerdere vensters en kies schakelaar.
...
- **Lay-out van het startscherm:**  
  Ga naar *Android Auto-instellingen → Lay-out wijzigen → Locatie van bestuurdersstoel wijzigen* om de lay-out aan te passen op basis van de positie van de bestuurdersstoel.
...
4. Veelgestelde vragen over het verbindingsscherm:
    - *Waarom kan ik de app niet bedienen vanaf mijn telefoon wanneer deze is verbonden met Android Auto?*  
        Om veiligheidsredenen is het OsmAnd-app-scherm op uw telefoon vergrendeld wanneer deze is verbonden met Android Auto. U kunt alleen via het autodashboard met de app communiceren.
    ...
    - *Wat moet ik doen als mijn telefoon en het autodashboard losgekoppeld lijken?*  
