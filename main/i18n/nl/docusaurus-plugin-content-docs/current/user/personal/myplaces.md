...
**Mijn Plaatsen** is de centrale hub in de OsmAnd-app voor het beheren en aanpassen van alle persoonlijke gegevens. U kunt deze sectie gebruiken om [Favoriete punten](#favorites) te organiseren die als belangrijk of veelbezocht zijn gemarkeerd. Het tabblad [Tracks](#tracks) stelt u in staat om GPX-bestanden te bekijken, importeren, opnemen en maken, zodat u een gedetailleerde geschiedenis van uw routes en reizen kunt bijhouden. U kunt ook uw [OpenStreetMap Bewerkingen](#openstreetmap-edits) beheren, waardoor het eenvoudig wordt om bij te dragen aan kaartverbeteringen en -updates. De [Audio / Video Notities](#audiovideo-notes) plug-in en widgets stellen Android-gebruikers in staat om multimedia-notities te maken en op te slaan die gerelateerd zijn aan specifieke locaties, waardoor context aan hun reizen wordt toegevoegd.
...
- [***Kaarten & Bronnen***](../personal/maps-resources.md):
    ...
    Het menu-item *Kaarten & Bronnen* in het *Hoofdmenu* van de OsmAnd-applicatie biedt toegang tot gegevensbeheer vanuit de sectie *Mijn Plaatsen*. Het tabblad [*Lokaal*](../personal/maps-resources.md#local) toont u hoeveel ruimte alle bestaande OsmAnd-gegevens op uw apparaat innemen, en de sectie *Mijn Plaatsen* in het bijzonder. U kunt het gebruiken om gedetailleerde informatie over uw gegevens te krijgen en toegang te krijgen om deze te beheren. De sectie kan alleen items bevatten waarvoor gegevens zijn gedownload.
...
**Favorieten** stellen u in staat belangrijke of veelbezochte locaties te markeren. Deze favoriete punten zijn georganiseerd in mappen en kunnen worden aangepast met verschillende kleuren, vormen en pictogrammen. U kunt snel naar elke favoriete plaats navigeren via het menu **Mijn Plaatsen** zonder deze herhaaldelijk te hoeven zoeken.
...
1. ***Actieknoppen***. *Actieknoppen* onderaan het scherm Mijn Plaatsen stellen u in staat de lijst met mappen met uw favorieten te beheren:
    ...
    - **Importeren** (*Toevoegen* knop) — Stelt u in staat om *favorite.gpx* bestanden te importeren vanuit de opslag van uw apparaat.
    ...
    - **Verwijderen** (*op iOS bevindt deze optie zich in het menu Bewerken*) — Verwijdert favoriete punten één voor één of geselecteerde favoriete mappen en alle daarin opgenomen punten.
...
**Tracks** bieden krachtige tools voor het opnemen, creëren en beheren van routes binnen OsmAnd. Ze kunnen worden gebruikt voor [navigatie](../navigation/setup/gpx-navigation.md), [reisregistratie](../plugins/trip-recording.md), of het [integreren](../personal/tracks/manage-tracks.md#import) van externe GPX-bestanden.

- **Tracks tabblad** — Alle ooit opgenomen, gemaakte of geïmporteerde tracks worden automatisch weergegeven in de map *Mijn Plaatsen* op het tabblad *Tracks*. Ze zijn georganiseerd per map of worden daaronder in een lijst weergegeven.

- **Een track maken** — Begin met opnemen via het **Tracks Tabblad** of de [Reisregistratie plug-in](../plugins/trip-recording.md).

- **Bekijken en Bewerken** — Toegang tot de lijst met tracks via *Mijn Plaatsen* en beheer ze via het *drie-puntjes menu* voor [mappen](../personal/tracks/manage-tracks.md#track-folder) of het menu voor [één track](../personal/tracks/manage-tracks.md#single-track-menu).
...
- **Uiterlijk en Analyse** — [Pas](../map/tracks/appearance.md) de visuele stijl van tracks aan en [analyseer](../map/tracks/index.md#analyze-track-on-map) ze met OsmAnd's tool [Een route plannen](../plan-route/create-route.md).
...
2. ***Drie-puntjes menu***. U kunt specifieke POI's of notities beheren met behulp van het *drie-puntjes menu* naast elke notitie:
    ...
    - **Tonen op kaart** — Geef de specifieke locatie van de POI of notitie op de kaart weer.

    - **OSM-wijziging/notitie wijzigen** — Voer verdere bewerkingen uit aan de POI of notitie die al aan OpenStreetMap zijn toegevoegd.
...
2. ***Drie-puntjes menu***. U kunt specifieke audio-, video- of fotonotities beheren met behulp van het *drie-puntjes menu* naast elke notitie:
