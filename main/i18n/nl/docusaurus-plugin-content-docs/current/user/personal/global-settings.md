...
**Algemene instellingen** zijn algemene [instellingen](../personal/profiles.md) voor alle profielen in de OsmAnd-applicatie. Dit zijn de basisinstellingen voor het OsmAnd-systeem, en alle wijzigingen die worden aangebracht, hebben invloed op de gehele applicatie.
...
Dit gedeelte bevat de basisinstellingen van OsmAnd, zoals het selecteren van een [profiel](#default-profile) bij het laden van de applicatie, het selecteren van de [rendering-engine](#map-rendering-engine) en de [map](#data-storage-folder) waarin de gegevens worden opgeslagen.  
...
- **<Translate android="true" ids="settings_preset"/>**. U selecteert het profiel dat moet worden gebruikt bij het starten van OsmAnd. Dit kan worden gewijzigd in de [applicatie-instellingen](../personal/profiles.md).  
...
Het [**<Translate ios="true" ids="carplay_profile"/>**](../navigation/car-play.md) wordt gebruikt wanneer verbonden met de [CarPlay-software](https://support.apple.com/en-gb/HT205634) van uw voertuig en helpt u het meeste te halen uit de geoptimaliseerde versie van de OsmAnd-app door gebruik te maken van de audio- en videosystemen om gegevens op uw telefoon weer te geven.


### Gegevensopslagmap {#data-storage-folder}
...
1. **Apparaatgeheugen** toont hoeveel vrije ruimte er nog op uw apparaat is. Tik op een veld om toegang te krijgen tot het menu [*Kaarten en bronnen*](../personal/maps-resources.md#maps--resources), waar u gedetailleerde informatie over uw OsmAnd-gegevens kunt bekijken en beheren.
...
2. In de **<Translate android="true" ids="change_data_storage_folder"/>**, kunt u een map selecteren om OsmAnd-gegevens op uw apparaat op te slaan.  
    ...
    - *Multiuser-opslag*. Slechts 1 OsmAnd-app heeft toegang, maar deze wordt gedeeld tussen meerdere Android-gebruikers.
...
| Transparante symbolen overlay / underlay | Ondersteunt alle instellingen. | Het ondersteunt geen rasterlaag bovenop vector tekst. |
...
| [Online kaart](../plugins/online-map.md) | Het ondersteunt de weergave van hoogtelijnen niet als de kaartbron een online tegelkaart is. | Wanneer de kaartbron een online kaart is, worden ook 3D-kaarten gebruikt, en de weergave van hoogtelijnen op de kaart wordt ondersteund. |
| [Weer-plugin](../plugins/weather.md) | U kunt deze plugin niet gebruiken. | Handig te gebruiken met de plugin. |  

De afbeelding voor de 3D-weergave van de kaart wordt tegel voor tegel geladen, daarna alle symbolen. De renderingsnelheid van de kaart hangt af van het aantal 2D- en 3D-grafische objecten erop, en direct van de prestaties van uw hardware. Daarom kan versie 2 (OpenGL) niet worden gebruikt op apparaten met lage prestaties.
...
OsmAnd laat u kiezen of u uw *[in-app-activiteit](#analytics)* en [identificatoren](#identifiers) (*alleen Android*) in de app wilt delen, de mogelijkheid om uw *[geschiedenis](#history)* te bewerken en een *[proxy](#proxy)* (*alleen Android*) naar keuze te gebruiken.  

:::note
Meer informatie vindt u in [*OsmAnd Privacy- en beveiligingsbeleid*](../../legal/privacy-policy.md)
:::
...
Een **UUID** (*Unieke Gebruikersidentificator*) wordt gegenereerd voor elke installatie van de OsmAnd-applicatie en naar de servers gestuurd tijdens het laden van de kaart.
...
- Vanaf OsmAnd versie 4.3 kunt u het verzamelen van gebruikersgegevens uitschakelen bij het gebruik van de OsmAnd-app. Dan wordt de UUID niet meegestuurd met downloadverzoeken.  
...
- [*Dialoogvenster kaart downloaden*](../start-with/download-maps.md#download) informeert u dat specifieke kaarten kunnen worden gedownload.
...
- OsmAnd ontvangt gegevens van de bronnen *Google Fused Location Providers* en *Android GPS en Netwerk Providers*. U kunt **<Translate android="true" id="location_source" />** schakelen in: *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_settings,location_source" />*.

***iOS***:

- Het iOS-systeem gebruikt de *iOS API* om de locatie te bepalen.  
- Alle instellingen voor OsmAnd Locatie zijn te vinden in de iOS-apparaatinstellingen: *Instellingen → OsmAnd maps → Locatie*  
...
- Om de back-upbestanden te vinden, gebruikt u een app voor bestandsbeheer van derden.  
...
**Als u [*OsmAnd Cloud*](../personal/osmand-cloud.md) gebruikt, is het aanbevolen om deze standaard back-upoptie te deactiveren om verwarring tijdens herinstallatie te voorkomen.**
...
### Flitsers deïnstalleren {#uninstall-speed-cameras}
...
Deze instelling stelt u in staat om POI's met flitsers te activeren of deactiveren, u moet de OsmAnd-applicatie opnieuw opstarten om wijzigingen aan te brengen.
  
In sommige landen of regio's is het gebruik van waarschuwingsapplicaties voor flitsers illegaal. U moet een keuze maken afhankelijk van de wetten in uw land. Selecteer **Actief houden**, en u ontvangt waarschuwingen en meldingen voor flitsers. Selecteer **Deïnstalleren** en alle gegevens met betrekking tot flitsers, zoals waarschuwingen, meldingen en POI's, worden verwijderd totdat u OsmAnd volledig opnieuw installeert.  
