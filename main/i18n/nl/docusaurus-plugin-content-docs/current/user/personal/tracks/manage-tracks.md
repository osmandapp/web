...
Alle routes in OsmAnd worden opgeslagen in een tabblad in de *<Translate android="true" ids="shared_string_menu"/> → <Translate android="true" ids="shared_string_my_places"/> → <Translate android="true" ids="shared_string_gpx_tracks"/>*. OsmAnd verwerkt het routebestand in [GPX-formaat](https://en.wikipedia.org/wiki/GPS_Exchange_Format), maar [KML](https://en.wikipedia.org/wiki/Keyhole_Markup_Language) & [KMZ](https://en.wikipedia.org/wiki/Keyhole_Markup_Language) kunnen worden geïmporteerd en geconverteerd naar GPX. Lees meer over verschillende soorten routes in het artikel [Routes op de kaart](../../map/tracks/index.md#types-of-tracks).

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">

![Beheer route Overzicht Android](@site/static/img/personal/tracks/track_on_map_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Beheer route Overzicht iOS](@site/static/img/personal/tracks/track_on_map_ios.png)
...
![Plan een route wijzig-route-android](@site/static/img/plan-route/plan-route-modify-track-android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Plan een route wijzig-route-ios](@site/static/img/plan-route/plan-route-modify-track-ios.png)
...
2. Maak een nieuwe route in de applicatie met behulp van de tool **[Plan een route](../../plan-route/create-route.md)**.
3. Neem een route op in de applicatie met behulp van de plugin **[Ritopname](../../plugins/trip-recording.md)**.
4. Voer meerdere punten in op coördinaten en sla ze op als een route met behulp van de tool **[Coördinaatinvoer](../../plan-route/coordinate-input.md)**.
5. U kunt ook **Waypoints** toevoegen aan een nieuwe route via het **[Contextmenu](../../map/map-context-menu.md#-add--edit-track-waypoint--add--edit-track-waypoint)**.
...
Alle ooit opgenomen, gemaakte of geïmporteerde routes worden automatisch weergegeven in de map [Mijn plaatsen](../../personal/myplaces.md) in het tabblad *Routes*. Ze zijn georganiseerd per map of worden daaronder in een lijst weergegeven.
...
Elke route, net als mappen, heeft een menu, waarmee u routes efficiënt kunt beheren. Hier leest u hoe u er toegang toe krijgt:
...
- [Openen](../../map/tracks/index.md#configure-map) (*iOS*) — Deze instelling is beschikbaar in het menu Kaart configureren. Gelegen in de *<Translate ios="true" ids="shared_string_menu,configure_map,shared_string_gpx_tracks"/>*, kunt u toegang krijgen tot het [contextmenu van de geselecteerde route](../../map/tracks/track-context-menu.md) wanneer u erop tikt.
...
- **Analyseren op kaart** (*Android*) / **Analyseren** (*iOS*) — Opent de tool [Analyseren op kaart](../../navigation/setup/route-details.md#analyze-on-map) binnen Route Details voor verdere analyse.

- **Delen** — Maakt het delen van de geselecteerde route met anderen mogelijk.

- **Wijzigingen uploaden naar OSM** (*iOS*) — [Upload de wijzigingen](../../plugins/osm-editing.md#gps-track) die u aan de route hebt aangebracht direct naar *OpenStreetMap*.

- **Bewerken** (*iOS*) — Gebruik de tool [Route plannen](../../plan-route/create-route.md#modify-gpx-track) om routes te wijzigen.
...
Tik op de &#x1F50D; knop bovenaan het scherm van het apparaat om het sorteermenu van de routelijst te openen.  

Beschikbaar:

- Zoeken op routenaam.
- [Sorteren op](#sort-by) voor een betere zoekopdracht als u de titel niet meer weet.
...
- [Activiteit wijzigen](../../map/tracks/track-context-menu.md#track-information-activity) — Met deze actie kunt u het activiteitstype wijzigen, bijvoorbeeld naar *Auto*, *Avontuurlijke Motorsport*, *Backpacken* of andere, voor de geselecteerde route.
...
OsmAnd biedt een eenvoudige manier om uw routes te sorteren, zodat u een grote verzameling kunt beheren of snel specifieke functies kunt vinden. Tik op **Sorteren op** bovenaan de routelijst om het sorteermenu te openen.
...
- **Nieuwste datum eerst** — Geeft routes weer op aanmaakdatum, met de nieuwste bovenaan.

- **Oudste datum eerst** — Geeft routes weer op aanmaakdatum, met de oudste bovenaan.
...
- **Alle routes op de kaart tonen** (*alleen Android*) — Toont alle routes in de map op de kaart —  
...
- [Verplaatsen](#track-folder) — Hiermee kunt u de map verplaatsen naar een andere bestaande map of een nieuwe maken.
...
Onderaan de lijst in het tabblad *Routes*, en onder de lijst met routes **in elk van uw mappen**, bevindt zich een informatiesectie waarmee u de algemene samenvattende statistieken voor de routes in de geselecteerde map kunt bekijken. Alle waarden worden weergegeven in de [eenheden](../../personal/profiles.md#units--formats) die u hebt geselecteerd.

- *Aantal **routes** in de map* — Toont het aantal routes in de geselecteerde map.

- *Totale som van* **Afstanden** — De totale lengte van alle routes in de geselecteerde map, gemeten in de eenheden die u hebt geselecteerd, wordt weergegeven.

- *Som van* **Stijgingen** — De gegevens geven de totale verticale stijging over alle routes in de geselecteerde map aan.

- *Som van* **Dalingen** — Deze parameter is de som van verticale dalingen op alle routes in de geselecteerde map.

- *Som van* **Duur** *tijd* — Dit toont de totale tijd die nodig is om alle routes in de geselecteerde map te voltooien.
...
- Tik op het *drie-stippenmenu* in de rechterbovenhoek om het mapactiesmenu te openen.
...
- [Submappen sorteren](#sort-by) — Pas een sorteervolgorde toe (bijv. op naam, datum) op alle submappen en routes op elk niveau.
...
- **Nieuwe map toevoegen** — Om uw routes en routes in logische categorieën te organiseren, plaatst u ze in verschillende mappen. Dit helpt bij het structureren van uw navigatie-informatie.
...
U kunt een map met alle routes die deze bevat verwijderen. Om dit te doen:
...
Met de importtool kunt u routes uit externe bestanden toevoegen voor offline navigatie en analyse. Methoden voor het importeren van routes:
...
1. Via [*Routemenu → Delen*](../../map/tracks/track-context-menu.md#options).

2. Via *Mijn plaatsen menu → Routes → Map / Routemenu*:

    - Om [**een enkele route te exporteren**](#track-menu), selecteert u het menu van de route.
...
- [Routemenu](../../map/tracks/track-context-menu.md)
