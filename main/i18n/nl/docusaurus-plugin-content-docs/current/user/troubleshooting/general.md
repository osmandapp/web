...
Er zijn gebruikersrapporten geweest van OsmAnd die buitensporig veel batterijvermogen verbruikt, met gevallen waarin OsmAnd verantwoordelijk was voor tot wel 90% van het batterijverbruik. We hebben echter veel tests uitgevoerd en konden dergelijke resultaten niet reproduceren.
...
Verschillende factoren beïnvloeden het batterijverbruik: de leeftijd van het apparaat, het aantal laadcycli, de batterijcapaciteit, de temperatuur en meer. Hieronder staan typische batterijverbruikspercentages, gebaseerd op tests uitgevoerd op een mid-level apparaat:
...
Het totale stroomverbruik moet het resultaat zijn van het optellen van deze componenten, afhankelijk van hoe u OsmAnd gebruikt. Meld het als u significante afwijkingen van deze typische waarden ziet.
...
- **Automatisch zoomen uitschakelen**. Automatisch zoomen tijdens navigatie kan leiden tot frequent opnieuw tekenen van de kaart, wat het batterijverbruik verhoogt. Het uitschakelen van deze functie kan helpen het stroomverbruik te verminderen. Navigeer naar *<Translate android="true" ids="shared_string_menu,shared_string_settings,application_profiles,routing_settings_2,map_during_navigation"/>* om automatisch zoomen uit te schakelen. Voor meer details, bezoek [Kaart tijdens navigatie](../navigation/guidance/map-during-navigation.md).

- **Instellingen voor het opnemen van tracks optimaliseren**. Zorg ervoor dat u de GPS-nauwkeurigheid en het schermgebruik tijdens het opnemen van tracks aanpast om batterij te besparen. Bezoek de sectie [Probleemoplossing voor het opnemen van tracks](../troubleshooting/track-recording-issues.md) voor meer begeleiding.
...
Voor meer gedetailleerde instructies over de zoekfunctionaliteit, kunt u deze handleiding raadplegen: [Zoekgeschiedenis](../search/search-history.md).
...
### E-mailmismatch-probleem tijdens OsmAnd Pro-aankoop {#email-mismatch-issue-during-osmand-pro-purchase}

<!-- ???
of deze titel:
### Oplossen van synchronisatieproblemen met betaalaccount en app-e-mail in OsmAnd {#resolving-payment-account-and-app-email-sync-issues-in-osmand}
-->
...
1. **Controleer Google Play- en Pay-accounts**. Zorg ervoor dat het juiste e-mailadres is gekoppeld aan uw Google Play Store- en Pay-accounts. Het betalingsproces wordt afgehandeld door deze services, en de e-mailmismatch kan optreden doordat een oud e-mailadres nog steeds is gekoppeld aan uw betalingsprofiel.
...
5. **OsmAnd opnieuw installeren**. Als het wissen van de cache het probleem niet oplost, probeer dan de app te verwijderen en opnieuw te installeren. Zorg ervoor dat u, voordat u dit doet, een back-up maakt van belangrijke gegevens (tracks, favorieten) door naar *Menu → Instellingen → Exporteren naar bestand* te gaan. Het opnieuw installeren van de app zorgt ervoor dat alle accountgegevens correct worden vernieuwd.
