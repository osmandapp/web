...
1. Als de app na 7-8 minuten rekentijd geen route toont, overweeg dan [waypoints te plaatsen](../navigation/setup/route-navigation.md#route-recalculation) (kies bijvoorbeeld plaatsen op snelwegen). 3-4 waypoints zijn voldoende om zelfs routes van 1000 km te berekenen.

2. Voor top-end apparaten kunt u het geheugen vergroten tot 512 MB of 1024 MB - [Geheugen toegewezen apparaten](../plugins/development.md#memory-allocated-for-routing).
...
Als u het **Wandel**-profiel in OsmAnd gebruikt, kan de applicatie crashen bij het berekenen van routes van meer dan 50 km. Dit probleem doet zich specifiek voor wanneer de [**Standaard Routering A***](../navigation/guidance/navigation-settings.md#development-settings) is geselecteerd in de navigatie-instellingen. Verschillende factoren kunnen bijdragen aan dit probleem:

- De routelengte is langer dan 50 km.
...
Om problemen met verkeerde of suboptimale routes op te sporen, kunt u een nieuwe post openen in [Github discussions](https://github.com/osmandapp/OsmAnd/discussions) of [Github issue](https://github.com/osmandapp/Osmand/issues) en de volgende informatie zo gedetailleerd mogelijk specificeren:
...
- Vertel ons of u OsmAnd's in-app offline routering hebt gebruikt, of een online routeringsprovider zoals YOURS, OpenRouteService of OSRM.
- Welk routeringsprofiel is gekozen in de OsmAnd-app (auto, fiets of voetganger)?
- Specificeer zo precies mogelijk het begin- en eindpunt van uw route. Vertel ons indien mogelijk de stadsnaam en straatnaam voor elk. Ook een [Permalink](https://wiki.openstreetmap.org/wiki/Permalink) van openstreetmap.org kan nuttig zijn.
...
Vanwege de geodata die afkomstig zijn van het OpenStreetMap-project, zijn er momenteel twee methoden hoe flitspalen in de ruwe OSM-data zijn geïntegreerd:
...
Momenteel kan OsmAnd alleen gebruikmaken van de elementen die uit een enkele node bestaan. Het analyseren van relaties zal in een toekomstige release beschikbaar komen.
...
*Voordelen van TTS boven opgenomen stemmen:*
...
Voor gedetailleerde instructies over het instellen en aanpassen van gesproken aanwijzingen, raadpleeg: [Gids voor stemnavigatie-instellingen](../navigation/guidance/voice-navigation.md).
...
- *Navigatie simuleren*. Tik op *Navigatieknop → Instellingen → Navigatie simuleren* om te controleren of stembegeleiding wordt afgespeeld.
