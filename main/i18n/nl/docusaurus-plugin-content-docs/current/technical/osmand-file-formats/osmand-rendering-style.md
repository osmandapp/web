...
## Kaartstijlsecties {#map-style-sections}
...
| **Parameters** | Gedefinieerd als `<renderingProperty>` heeft `attr` dat in de stijl als parameternaam zal worden gebruikt en `name`, `description` zullen aan de gebruiker worden getoond. |
...
- Zoek de meest innerlijke `<case>` die overeenkomt met de zoekinvoerparameters `tag/value/zoom`, d.w.z. voor `highway=primary` wordt `<case tag="highway" value="primary">` gevonden.
...
- Alle regels worden sequentieel geëvalueerd en uitvoerparameters kunnen worden overschreven door volgende regels
...
## Attributen (speciaal) & Constanten {#attributes-special--constanten}
Weergaveconstanten en weergaveattributen zijn uitwisselbaar en kunnen worden gebruikt om de weergavestijl te vereenvoudigen en kopiëren/plakken van waarden of codeblokken te voorkomen. Als een attribuut zoals `color` een enkele waarde is, is het **voorkeur** om **weergaveconstante** te gebruiken, omdat dit de prestaties van de weergavestijl aanzienlijk versnelt. 

Weergaveconstanten kunnen alleen worden gebruikt met één gegeven waarde: `<renderingConstant name="motorwayShadowRadius" value="1.6"/>` en later in functieselectoren als `<apply_if shadowRadius="$motorwayShadowRadius"/>`. 

Weergaveattributen kunnen een ingebedde structuur hebben met extra selectoren en een van de volgende attributen uitvoeren (`attrColorValue`, `attrBoolValue`, `attrFloatValue`, `attrIntValue`, `attrStringValue`). Voorbeeld:
```
...
Speciale attributen zijn `<renderingAttribute>` die niet door selectoren worden gebruikt, maar rechtstreeks door de code worden gebruikt om specifieke functies op te vragen die in de applicatie worden getekend, zoals een navigatieroute, liniaal, gpx-track, enz.
...
| `publicTransportLine` | Een openbaarvervoersnavigatieroute weergeven | 
| `walkingRouteLine` | Een openbaarvervoersnavigatie weergeven: voetgangersroute tussen haltes | 
...
||| 
| `routeInfo_*` | Voorbeeld: `routeInfo_surface`, `routeInfo_roadClass` produceren klassen van wegen die moeten worden weergegeven in de legenda van de route. | 
|||
...
```
<renderingProperty attr="baseAppMode" name="Standaard Weergavemodus" description="Kaartoptimalisatie voor respectievelijk Gebruikersprofiel gebaseerd op basis (ouder) profiel"
		type="string" possibleValues="default, car, bicycle, pedestrian, public_transport, boat, ski"/>
```

Later in de selectiestijl kunt u aangeven wanneer deze selector kan worden toegepast, d.w.z. voor welke toepassingsmodus die door de gebruiker is opgegeven
```
