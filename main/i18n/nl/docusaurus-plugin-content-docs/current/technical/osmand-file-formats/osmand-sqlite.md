...
# Tiles SQLite-formaat - .sqlite {#tiles-sqlite-format---sqlite}
...
||"rule"|Tekenreeks. Ondersteund sjabloon:1 (hetzelfde als leeg), wms_tile, yandex_traffic. Voegt specifieke algoritmen toe hoe tegels worden gedownload. wms_tile gebruikt een proxyserver zoals whoots.mapwarper.net om met WMS-bronnen om te gaan|
||"useragent"|Tekenreeks. HTTP User Agent. Zoals gebruikt voor downloaden.|
||"minzoom"|Integer. Min zoomniveau. Respectievelijke integer. (Ook omgekeerd in het geval van BigPlanet).|
||"maxzoom"|Integer. Max zoomniveau. Respectievelijke integer. (Ook omgekeerd in het geval van BigPlanet).|
...
||"expireminutes"|Integer. Specificeert of tegels moeten verlopen na het opgegeven aantal minuten. Ze zouden nog steeds worden weergegeven, maar ook opnieuw worden gedownload.|
||"tilenumbering"|Tekenreeks "" of "BigPlanet". Als "BigPlanet", wordt de zoom omgekeerd en berekend als z = 17 - zoom.|
||"tilesize"| Integer. Leeg of 256 of 512, optioneel, het wordt gebruikt om te beschrijven wat de tegelgrootte is|
|"tiles"|"x", "y", "z"|Integer. Geeft tegel Mercator-coördinaten aan. Merk op dat de zoom kan worden omgekeerd voor het BigPlanet-geval.|
||"image"|Blob van afbeeldingsbytes.|
||"time"|Integer. Tijdstempel wanneer de afbeelding is gedownload.|

Dit formaat wordt ook gebruikt door Hillshade- en Slope-bestanden die door de OsmAnd-applicatie worden verspreid.
