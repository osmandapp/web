...
OsmAnd breidt het standaard GPX 1.1-formaat uit door een eigen `osmand:` XML-naamruimte te introduceren. Deze naamruimte maakt het opslaan van aanvullende gegevens mogelijk, zoals:
...
3. Hoe u berekende routes exporteert en hun volledige functionaliteit behoudt.
...
- `<rte>` - vertegenwoordigt routes, inclusief waypoints en sleutelpunten.
...
Deze sectie beschrijft hoe OsmAnd tracks op de kaart weergeeft en de beschikbare aanpassingsopties om hun uiterlijk aan te passen. De hieronder beschreven parameters worden toegepast binnen de `<gpx>`-tag en beïnvloeden alle tracks die in een GPX-bestand zijn opgenomen.

| Tagnaam | Beschrijving / Waarden |
|:----------------------------------------|:---------|
| `<color>` | - Definieert de kleur van de tracklijn op de kaart. <br/> - *Tekenreeks:* HEX-kleurcode `#RRGGBB` of `#AARRGGBB` |
| `<width>` | - Specificeert de breedte van de tracklijn. <br/> - *Tekenreeks:* `"thin"`, `"medium"`, `"bold"` (gedefinieerd door het attribuut `"currentTrackWidth"`), of geheel getal (1-24) |
| `<show_arrows>` | - Schakelt richtingpijlen langs de track in of uit. <br/> - *Booleaans:* `"true"` / `"false"` |
| `<show_start_finish>` | - Toont of verbergt markeringen voor het begin en einde van de track. <br/> - *Booleaans:* `"true"` / `"false"` |
| `<split_type>` | - Specificeert het type segmentatie voor de track. <br/> - *Tekenreeks:* `"no_split"`, `"distance"`, `"time"` |
| `<split_interval>` | - Stelt het interval in voor tracksegmentatie op basis van het geselecteerde type. <br/> - *Dubbel:* Geheel getal (meters voor `"distance"`, seconden voor `"time"`) |
| `<line_3d_visualization_by_type>` | - Specificeert het type 3D-visualisatie voor de track. <br/> - *Tekenreeks:* `"none"`, `"altitude"`, `"shared_string_speed"`, `"map_widget_ant_heart_rate"`, `"map_widget_ant_bicycle_cadence"`, `"map_widget_ant_bicycle_power"`, `"shared_string_temperature"`, `"shared_string_speed"`, `"fixed_height"` |
| `<line_3d_visualization_wall_color_type>` | - Definieert het type muurkleur voor 3D-visualisatie. <br/> - *Tekenreeks:* `"none"`, `"solid"`, `"downward_gradient"`, `"upward_gradient"`, `"altitude"`, `"slope"`, `"speed"` |
| `<line_3d_visualization_position_type>` | - Stelt de positie van de 3D-visualisatie in ten opzichte van de track. <br/> - *Tekenreeks:* `"top"`, `"bottom"`, `"top_bottom"`|
| `<vertical_exaggeration_scale>` | - Vermenigvuldiger om de waarde van het attribuut `line_3d_visualization_by_type` te schalen. <br/> - *Float:* Standaard: 1.0 |
...
| `<coloring_type>` | - Bepaalt de kleurmethode voor de track. <br/> - *Tekenreeks:* `"solid"`, `"speed"`, `"altitude"`, `"slope"`, `"routeInfo_roadClass`, `"routeInfo_surface"`, `"routeInfo_smoothness"` |
| `<color_palette>` | - Specificeert het kleurenschema voor de track. <br/> - *Tekenreeks:* `"default"` / [door de gebruiker gedefinieerde schema's](/docs/user/personal/color-palette-schemes) |
...
De `<coloring_type>`-tag in OsmAnd stelt gebruikers in staat om trackkleuring aan te passen op basis van specifieke gegevenskenmerken, waardoor een visuele manier wordt geboden om belangrijke informatie langs de track te interpreteren.

| Type | Beschrijving | Gebruiksscenario |
|:-----------------------|:--------|:--------|
| `<solid>` | De gehele track wordt weergegeven met een enkele, effen kleur gespecificeerd door de `color`-tag. | Gebruiken wanneer een uniforme kleur nodig is voor duidelijke zichtbaarheid. |
...
| `<altitude>` | De track wordt gekleurd volgens de hoogtegegevens bij elke `<trkpt>`. Een gradiënt wordt toegepast, wat verschillende hoogtewaarden aangeeft. | Handig voor wandel- of bergroutes om hoogteverschillen te markeren. |
| `<slope>` | De track wordt gekleurd op basis van de helling/daling tussen opeenvolgende trackpunten. Positieve gradiënten geven hellingen bergop aan, terwijl negatieve gradiënten bergafwaartse hellingen aangeven. | Geschikt voor fietsers of wandelaars die de moeilijkheidsgraad van de track analyseren. |
...
| `<routeInfo_surface>` | Kleurt de tracksegmenten op basis van het OSM-oppervlaktetype (bijv. verhard, grind, aarde). | Handig voor het bepalen van trackcondities tijdens activiteiten zoals off-road rijden. |
| `<routeInfo_smoothness>` | Kleurt de track volgens OSM-vloeiendheidsbeoordelingen, wat de ruwheid of gladheid van het pad aangeeft (bijv. uitstekend, slecht). | Handig voor het evalueren van de navigeerbaarheid van de track voor specifieke voertuigen. |
...
Zowel `<metadata>` als `<gpx>` `<extensions>` worden weergegeven.
...
Deze sectie legt uit hoe OsmAnd aanpassing van waypoints in GPX-bestanden mogelijk maakt door het gebruik van pictogrammen, kleuren en groepering.
...
### Trackpuntgegevens {#track-point-details}
...
In **OsmAnd** kunt u een route berekenen en alle gegevens opslaan in GPX, zodat later alle navigatiefuncties beschikbaar zijn als GPX-navigatie, dus **OsmAnd** GPX zal routesegmenten, afslagen, wegnamen, wegtypen, beperkingen, enz. bevatten. De route kan volledig worden hersteld alsof deze zojuist is gebouwd, zelfs bij afwezigheid van de respectievelijke offline kaarten.

Een GPX-bestand kan meerdere routes bevatten. Elk daarvan bevindt zich in een specifiek segment onder **trkseg** / **extensions**. Een GPX-bestand wordt in deze vorm opgeslagen bij het exporteren van een geconstrueerde route of bij het opslaan van een track die uit meerdere afzonderlijke segmenten bestaat via de functionaliteit [**Plan een route**](../../user/plan-route/create-route.md).

[**Plan een route**](../../user/plan-route/create-route.md) voegt ook één (of meerdere, in overeenstemming met het aantal opgenomen afzonderlijke segmenten / tracks) **rte**-blokken toe aan het GPX-bestand, die route-sleutelpunten (**rtept**) bevatten.
...
<rte>
  <rtept ... />
    // Voor routes die zijn gebouwd met "Plan route", worden de parameters van sleutelpunten opgeslagen.
    // Als rtept niet de eerste en laatste is, zal trkpt ervoor (met dezelfde idx) dezelfde gegevens hebben.
    <extensions>
      // Routeprofieltype voor volgend segment (auto, fiets, voetganger, etc.).
      <profile>...</profile>
...
* Er is een uitzondering wanneer aangrenzende route **segmenten** elkaar niet overlappen (delen niet hetzelfde **trkpt**). Dit gebeurt wanneer er een **rtept** "tussen" route **segmenten** is. Het einde van het vorige route **segment** is één **trkpt**, en het begin van het volgende route **segment** is een ander **rtept**. Maar deze twee **trkpt**s zijn totaal gelijk qua lat, lon en andere parameters.
* Overlapping van route **segmenten** kan worden gedetecteerd via **length** en **startTrkptIdx** (de laatste wordt alleen gebruikt voor het gemak van menselijk lezen):
  - Als de som van **startTrkptIdx** en **length** van het vorige route **segment** gelijk is aan **startTrkptIdx** van het volgende route **segment**, overlappen de route **segmenten** elkaar niet
  - Als de som één minder is, dan overlappen de route **segmenten** elkaar
...
### Waypoints weergeven {#waypoints-display}
...
- `octagon` - behandeld als een cirkelvormige achtergrond in OsmAnd.
...
- U kunt geformatteerde beschrijvingen aan waypoints toevoegen met behulp van [HTML-code](#html-in-descriptions). Hiermee kunt u opgemaakte tekst, links of zelfs afbeeldingen toevoegen.
...
<gpx>
  ...
  <wpt lat="35.948477" lon="14.3806796">
    ...
    <extensions>
      <osmand:name_-_en>De Romeinse Bijenstal</osmand:name_-_en>
    </extensions>
  </wpt>
</gpx>
```
...
| `min_ele`, `avg_ele`, `max_ele` | Minimale, gemiddelde en maximale hoogte-analyses. |
| `diff_ele_up`, `diff_ele_down` | Totale hoogtewinst en -verlies over de track. |
| `max_speed`, `avg_speed`, `min_speed` | Track-snelheidsanalyses, inclusief maximale, gemiddelde en minimale snelheid. |
| `time_span`, `time_span_no_gaps` | Totale tijd voor de track, met en zonder rekening te houden met hiaten. |
| `time_moving`, `time_moving_no_gaps` | Totale bewegingstijd, met en zonder rekening te houden met hiaten. |
