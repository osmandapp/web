...
Target SDK-versie: [29](https://github.com/osmandapp/OsmAnd/blob/master/OsmAnd/build-common.gradle#L6)
...
- legacy - legacy / opengl / opengldebug - definieert of de geproduceerde apk native bibliotheken zal hebben om opengl-kaartweergave te gebruiken of niet. Meer informatie in OsmAnd-ontwikkelingsplug-in.
...
|:--------|:---------------|:---------------|
| nightlyFree | net.osmand.dev | Nightly gratis build die naast beide GPlay-versies kan worden geïnstalleerd voor test- en verkenningsdoeleinden
| androidFull | net.osmand.plus | Volledige OsmAnd~-versie kan worden gebruikt voor F-Droid-builds. Maakt geen gebruik van Google Play-services.
| gplayFree | net.osmand | Google Play OsmAnd-versie - maakt gebruik van Google Play-services.
| gplayFull | net.osmand.plus | Google Play OsmAnd+-versie - maakt gebruik van Google Play-services.
| huawei | net.osmand | Speciale build voor Huawei-markt - maakt gebruik van Huawei-services maar niet Google Play

### Debuggen {#debug}

#### NDK-debug inschakelen voor legacy native bibliotheek (OsmAnd-core-legacy) {#enable-ndk-debug-for-legacy-native-library-osmand-core-legacy}
...
APP_DEBUG := true
```
Voeg debug-symbolen toe in de projectconfiguratie van Android Studio. Ga hiervoor naar Bewerken Configuratie / selecteer in het linkermenu Android App "OsmAnd" / selecteer aan de rechterkant het tabblad "Debugger". En voeg OsmAnd/obj/local toe waar bibliotheken worden gebouwd voor verschillende configuraties (arm64-v8a, armeabi-v7a, x86, x86_64).


## OpenGL-bibliotheek compileren {#compile-opengl-library}

OpenGL-bibliotheek is nodig voor targets zoals assembleNightlyFree**Opengl**FatDebug en wordt standaard gedownload van [OsmAnd ivy repository](https://builder.osmand.net/ivy/net.osmand/) - [gradle task](https://github.com/osmandapp/OsmAnd/blob/master/OsmAnd/build.gradle#L187). Voor lokale ontwikkeling moet u mogelijk die bibliotheek compileren en publiceren naar de lokale ivy-repo.
...
3. **Gradle** (opdrachtregel):
    ...
    export ANDROID_HOME=/[uw-pad]/Android/sdk
    ```
    - Compileren met [gradle build task](https://github.com/osmandapp/OsmAnd-core/blob/master/wrappers/android/build.gradle)
    ```
...
Wordt aangevuld: hoe Android NDK-debug in te stellen.
...
- [OsmAnd-map-sample](https://github.com/osmandapp/osmand-api-demo/tree/master/OsmAnd-map-sample) - [een voorbeeldtoepassing](https://download.osmand.net/latest-night-build/OsmAnd-map-sample.apk) heeft OsmAnd volledig geïntegreerd als SDK. Hiermee kunnen alle interne API's worden gebruikt.
