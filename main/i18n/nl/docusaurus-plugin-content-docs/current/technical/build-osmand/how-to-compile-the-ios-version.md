...
# De iOS-versie compileren {#how-to-compile-the-ios-version}
...
4. Log in op het Xcode-account (optioneel)
  ...
  Voorkeuren -> Accounts
  ```
  Druk op de `+` knop. Je kunt inloggen met je AppleID (gebruikersnaam en wachtwoord van je iOS/macOS-apparaten). Volg de Xcode-instructies.
  Voor OsmAnd-teamleden: stuur je AppleID-login, zodat je wordt toegevoegd aan de ontwikkelaarslijst. Wanneer je een e-mail met een uitnodigingsbericht ontvangt, activeer deze dan.
  Sluit Xcode.
...
#
# Voor meer details over het configureren van uw bouwomgeving, bezoek {#for-more-details-on-how-to-configure-your-build-environment-visit}
...
#
# Specificeert de JVM-argumenten die worden gebruikt voor het daemon-proces. {#specifies-the-jvm-arguments-used-for-the-daemon-process}
...
#
# Indien geconfigureerd, zal Gradle in de incuberende parallelle modus draaien. {#when-configured-gradle-will-run-in-incubating-parallel-mode}
...
#Vrijdag 8 april 18:47:31 EEST 2016
...
8. Voer `prepare.sh` uit om de Qt-bibliotheek te compileren en externe afhankelijkheden te downloaden
  ...
  Oplossing: schakel XcodeCommandLineTools naar de Xcode-app, bevestig de licentie en schakel deze terug.
  ```
  ...
  Oplossing 2: controleer of xcrun beschikbaar is: ``` /usr/bin/xcrun -find xcrun ```. Als je krijgt: ``` xcrun: fout: kan hulpprogramma "xcrun" niet vinden, geen ontwikkelaarstool of in PATH ```. Open dan Xcode > Voorkeuren > Locaties en selecteer in het veld "Command Line Tools" je command line tools "Xcode XX.X" En voer `$ ./prepare.sh` opnieuw uit.

  Oplossing 3: Fout: `CMake Fout ... iphoneos is geen iOS SDK`.
  ```
  $ sudo xcode-select -s /Applications/Xcode.app/Contents/Developer
  ```

  Oplossing 4: Als je een fout krijgt zoals deze: ``` CMake Fout bij CMakeLists.txt:1 (cmake_minimum_required): CMake 3.21.2 of hoger is vereist. Je gebruikt versie 3.11.2 ```. Download dan het dmg-installatieprogramma van de CMake-website en voer een handmatige installatie uit. En voer `$ ./prepare.sh` opnieuw uit.
  ```
...
10. Eerste build.
  Stel het build-doel in op `OsmAnd Maps`. (Bij de afspeel-/stopknoppen). Selecteer als doel je apparaat of een van de iOS-simulators. Maar gebruik niet de standaard 'Any iOS Device (arm64)'. Bouw het project (afspeelknop).
...
11. XCode-bestemmingen en Sandbox
 ...
 - MacOS kan u vragen om in te loggen met Apple ID wanneer OsmAnd actief is
 - Gebruik uw Sandbox-account als Apple ID om herhaalde inlogverzoeken te voorkomen
 - Gebruik OsmAnd Cloud met uw Promo-abonnement om onbeperkte kaarten te krijgen

 - Vraag uw teamleider om Sandbox-accounts en Promo-abonnementen

## Probleemoplossing {#troubleshooting}
### Tijdelijke bestanden opschonen. {#cleaning-temp-files}
  - In geval van buildfouten kunt u in Xcode drukken op: ```Product -> Clean build folder```
  - Sluit Xcode.
  ...
  - Herstart uw computer. (Ja, dit kan helpen).
...
### M1 mac {#m1-mac}
  ...
  - Voeg een stringveld van het type ```Any IOS Simulator SDK``` toe met de waarde ```arm64```. (u moet dit na elke prepare.sh-uitvoering toevoegen)
  - Bouw project. In geval van fouten, voer alle instructies van ```11. Probleemoplossing - tijdelijke bestanden opschonen.``` en ```prepare.sh``` opnieuw uit.
...
## Qt debug-add-on (optioneel). Als u Qt-waarden in de debug-modus wilt zien, voert u dit uit: {#qt-debug-addon-optional-if-you-want-to-see-qt-values-in-debug-mode-run-this}
```
$ mkdir -p ~/qtlldb
...
Na het instellen van deze parameters start u uw computer opnieuw op.