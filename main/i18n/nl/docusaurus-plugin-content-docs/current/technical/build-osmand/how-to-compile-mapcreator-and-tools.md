...
2. **Gradle** (commandoregel):
    - Compileer met de commandoregel
    ```
    cd tools/java-tools && ../gradlew build
    ```
    - OsmAndMapCreator.zip wordt geproduceerd met alle scripts erin
    **Opmerking**: een Android-project is vereist om tools te bouwen (het gebruikt het OsmAnd-java-project).
3. **Eclipse**:
    - Preconfigureer Eclipse-projecten
    ```
...
4. **IntelliJ IDEA**
    - 'Bestand' > 'Nieuw' > 'Project vanuit bestaande bronnen...' en navigeer naar `java-tools`
    - Selecteer 'Project importeren vanuit bestaand model' en kies 'gradle' uit de lijst
...
Native renderingbibliotheek kan worden gebruikt om Offline Rendering of Offline Native Routing te testen. Het is erg handig om een renderingstijl of een routingconfiguratie te debuggen en te testen.

1. Stel eerst de **ontwikkelomgeving** in, zie [ontwikkelomgeving](./setup-the-dev-environment).
2. Download externe afhankelijkheden
 ```
...
3. Specificeer JAVA_HOME globaal via $PATH of in de commandoregel
  ...
  export JAVA_HOME=<>
  ```
4. Kies een specifiek [doel](https://github.com/osmandapp/OsmAnd-core/tree/legacy_core/targets) voor OS en compileer een specifieke versie 'debug', 'release' of '' (standaard). Voorbeeld
  ```
  ...
  make -j4
  ```
5. Binaries zijn beschikbaar op core-legacy/binaries/.
Voorbeeld:
  ```
...
```
3. Compileer bibliotheek release
```
