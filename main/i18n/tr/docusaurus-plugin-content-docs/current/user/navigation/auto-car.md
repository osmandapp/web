...
:::tip Satın Alma
Android Auto, [ücretli bir özelliktir](../purchases/index.md).  
:::
...
**Hız göstergesi** widget'ı, GPS verilerini kullanarak *mevcut hızı* ve [OSM veritabanından](https://wiki.openstreetmap.org/wiki/Key:maxspeed) ve [OsmAnd ayarlarından](../navigation/guidance/voice-navigation.md#speed-limit) *hız sınırını* aracın multimedya sistemi ekranında görüntüleyen entegre bir arayüz öğesidir. Mevcut hızınız yapılandırılmış Hız sınırı toleransına yaklaştığında veya hız sınırını aştığında, widget görünümünü (renkleri) değiştirerek uyarı sağlar. Görünüm değişikliği animasyonlu olabilir.
