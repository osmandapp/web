...
- **CarPlay** kullanırken, *Favorileri* ekleyemez veya değiştiremezsiniz, yalnızca mevcut olanları kullanabilirsiniz.
...
*Menü → Yerlerim → [İzler](../personal/tracks/manage-tracks.md)* bölümündeki izler listesinden, araç multimedya sistemine bağlanmadan önce gerekli olanı seçin ve rota ya iz başlangıcına kadar oluşturulacak ya da iz boyunca mevcut yönü gösterecektir.  
...
- *CarPlay*'de uyarıların görüntülenmesini ayarlamak mümkün değildir, navigasyonu başlatmadan ve cihazınızı araca bağlamadan önce uygulamada widget'ı yapılandırmanız gerekir.
...
Mevcut eylemler:
- **<Translate ios="true" ids="context_menu_item_add_parking_point"/>**. Mevcut konumunuzu park yeri olarak kaydeder ve harita üzerinde bir park işaretleyicisi yerleştirir.
- **<Translate ios="true" ids="find_parking"/>**. Yakındaki park seçeneklerini bulmanıza yardımcı olmak için Park kategorisi önceden seçilmiş olarak Arama ekranını açar.
- **<Translate ios="true" ids="recalculate_route"/>**. Navigasyona devam etmek istiyorsanız orijinal hedefe yeni bir rota oluşturur.
- **<Translate ios="true" ids="finish_navigation"/>**. Mevcut rotayı sonlandırır ve OsmAnd'ı varsayılan harita durumuna döndürür.
