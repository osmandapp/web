...
Bisiklet rotası, bisikletçiler için güvenli ve yasal olan bisiklet yollarının, patikaların ve yolların mevcudiyeti gibi faktörleri dikkate alır. OsmAnd, tahmini varış süresini (mesafeyi kat etme süresi) hesaplarken [Naismith kuralına](https://en.wikipedia.org/wiki/Naismith%27s_rule#Scarf's_equivalence_between_distance_and_climb) göre her zaman yükseklik verilerini kullanır. Varsayılan olarak en hızlı rota sağlanır. Bisiklet rotası ayrıca bisikletinizin [genişliği](../guidance/vehicle-parameters.md#limits) gibi ek faktörleri dikkate alarak dar bisiklet yollarından kaçınmaya yardımcı olur.  
