...
![Düğmeler için Izgara_v_android](@site/static/img/widgets/grid_buttons_v_and.png) ![Düğmeler için Izgara_h_android](@site/static/img/widgets/grid_buttons_h_and.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Düğmeler için Izgara_v_ios](@site/static/img/widgets/grid_buttons_v_ios.png) ![Düğmeler için Izgara_h_ios](@site/static/img/widgets/grid_buttons_h_ios.png)

</TabItem>

</Tabs>

[Harita düğmeleri](../widgets/map-buttons.md#overview) otomatik olarak bir ızgara düzeni kullanılarak yerleştirilir. Her düğme ekranın dört köşesinden (*sol üst*, *sağ üst*, *sol alt*, *sağ alt*) birine atanır. Atanan köşede yeterli alan yoksa, düğmeler ekran yönüne bağlı olarak yatay veya dikey olarak hizalanır.

Bu düzen, düğmelerin görsel olarak tutarlı kalmasına yardımcı olur ve örtüşmeyi önler. Örneğin, yatay bir ekranda, sağ köşedeki düğmeler ekranın alt kısmında yatay olarak sıralanırken, dikey bir ekranda gerektiğinde yukarı veya aşağı doğru dikey olarak genişler.
...
Hızlı Eylem aracının işlevselliğini kullanarak, gerekli eylemleri harici giriş cihazınızın tuşlarına atayabilirsiniz. Açıklama [Harita ile Etkileşim](../map/interact-with-map.md#custom-input-device-type) makalesinde bulunabilir.
