...
[Bisiklet rotaları](https://wiki.openstreetmap.org/wiki/Cycle_routes) üç türde bulunur: *yerel* (`lcn`), *bölgesel* (`rcn`), *ulusal* (`ncn`), *uluslararası* (`icn`). Ayrıca, bisiklet rotaları [Düğüm ağlarının](https://wiki.openstreetmap.org/wiki/Tag:network:type%3Dnode_network) bir parçası olabilir. Rota segmentlerine renk atama şeklini seçebilirsiniz:
...
Daha fazla bilgiyi [Harita Açıklamaları](../map-legend/index.md) makalesinde bulabilirsiniz.
...
| | | |
|------------|------------|------------|
| ***<Translate android="true" ids="mtb_scale"/>*** | MTB ölçeğine göre parkurları gösterir. Daha fazla bilgi [Harita Açıklamaları](../map-legend/index.md) makalesinde bulunmaktadır. | ![Harita rotaları - MTB parkurları](@site/static/img/map/map-routes-mtb-trails.png) |
| ***<Translate android="true" ids="mtb_imba"/>*** | Uluslararası Dağ Bisikleti Birliği, [MTB IMBA](https://www.imba.com/)'ya göre MTB parkurlarını gösterir. Daha fazla bilgi [Harita açıklamaları](../map-legend/index.md) makalesinde bulunmaktadır. | ![Harita rotaları - MTB parkurları](@site/static/img/map/map-routes-mtb_imba-trails.png) |
-->

- ***<Translate android="true" ids="mtb_scale"/>***.  MTB ölçeğine göre parkurları gösterir. Daha fazla bilgi [Harita Açıklamaları](../map-legend/index.md) makalesinde bulunmaktadır.  
![Harita rotaları - MTB parkurları](@site/static/img/map/map-routes-mtb-trails.png)  

- ***<Translate android="true" ids="mtb_imba"/>***.  Uluslararası Dağ Bisikleti Birliği, [MTB IMBA](https://www.imba.com/)'ya göre MTB parkurlarını gösterir. Daha fazla bilgi [Harita açıklamaları](../map-legend/index.md) makalesinde bulunmaktadır.  
...
[Yürüyüş rotaları](https://wiki.openstreetmap.org/wiki/Walking_Routes) üç türde bulunur: *yerel* (`lwn`), *bölgesel* (`rwn`), *ulusal* (`nwn`), *uluslararası* (`iwn`). Ayrıca, yürüyüş rotaları [Düğüm ağlarının](https://wiki.openstreetmap.org/wiki/Tag:network:type%3Dnode_network) bir parçası olabilir. OsmAnd, yürüyüş ve trekking rotalarını ayırmaz ve bunları 1 grupta birleştirir. Rota segmentlerine renk atama şeklini seçebilirsiniz:
...
**Kir Bisikleti Parkurları** özelliği, `dirtbike:scale` etiketine göre harita üzerinde kir bisikletine özel rotaları görüntülemenizi sağlar. Bu ölçek, kir bisikleti için uygun parkurları vurgular ve zorluk seviyelerini gösteren renkli bir kaplama ile birlikte gelir.

Bu özelliği *Menü → Haritayı Yapılandır → Rotalar → Kir bisikleti rotaları* bölümünden etkinleştirebilirsiniz. Sistem, MTB ölçeğine benzer şekilde çalışır ve kir bisikleti için özel olarak tasarlanmış rotaların görsel bir temsilini sunar — `dirtbike` etiketi hakkında daha fazla bilgiyi [burada](https://wiki.openstreetmap.org/wiki/Key:dirtbike:scale) bulabilirsiniz.
...
- Bu konumlar, aşağıdaki gibi **ayrıntılı özellikler** içerir:
    ...
    - *Kaya tipi* (örn. kireçtaşı, granit).
    - *Rota uzunluğu* (metre cinsinden).
    - *Tırmanma kalitesi* (sağlam, gevşek, karışık).
    - *Zirve defteri kullanılabilirliği*.

Haritada tırmanma konumlarını etkinleştirmek için: *Menü → Haritayı Yapılandır → Rotalar → Tırmanma Rotaları* yolunu izleyin.
...
Bazı yol segmentleri birçok rotanın bir parçası olabilir, ancak dağlık alanlardaki segmentin zorluğunu temsil etmek için ayrı ayrı renklendirilebilirler. Şu anda OsmAnd, İtalyan ve İsviçre sınıflandırmalarını desteklemektedir ve her birinin bu segmente erişmek için belirli bir ekipman gereksinimleri listesi vardır.
...
Kayak rotaları, OpenStreetMap'te [rotaların](https://wiki.openstreetmap.org/wiki/Tag:route%3Dski) belirli bir grubudur. Tipik olarak, bu rotalar [Kış ve Kayak](../map/vector-maps.md#winter-and-ski) harita stiliyle tüm görünür [Pistlerle](https://wiki.openstreetmap.org/wiki/Pistes) birlikte kullanılabilir.
...
### Beyaz su sporları {#whitewater-sports}

![Harita rotaları - beyaz su sporu](@site/static/img/map/map-routes-whitewater-sport.png)

[Beyaz su rotaları](https://wiki.openstreetmap.org/wiki/Tag:route%3Dcanoe), [beyaz su turizmi](https://wiki.openstreetmap.org/wiki/Whitewater_sports#Whitewater_Map) için erişim simgeleri ve tehlike bölgeleriyle birlikte gösterilir.
...
Belirli ***Seyahat Kitaplarının*** ve ***parkur*** türlerinin görüntülenmesini özelleştirebilir, parkurları nokta olarak veya ***Nokta grupları*** olarak görüntüleyebilirsiniz (örneğin Wikivoyage seyahat kitapları için). [Seyahat rehberleri hakkında daha fazla bilgiyi burada bulabilirsiniz](../plan-route/travel-guides.md).
...
- Gerekli rotaları haritada görüntülemek için, [Haritayı Yapılandır](../map/configure-map-menu.md) menüsünün *Rotalar listesinde* etkinleştirin.
- OsmAnd, [OpenStreetMap'te bulunan rotaları](https://wiki.openstreetmap.org/wiki/Relation:route) vurgulayabilir. Bunlar, [rotadaki sembole](#save-as-a-track) dokunarak seçilebilir ve görünür rota kümesi doğru şekilde yapılandırılmışsa, rengi ve simgeleri takip edebilirsiniz.
...
## Rotalarla Eylemler {#actions-with-routes}
...
Haritada, uygun olduğu yerlerde [OSMC sembolleriyle](https://wiki.openstreetmap.org/wiki/Key:osmc:symbol) işaretlenmiş [yürüyüş, bisiklet, seyahat ve daha fazlası](#type-of-routes) için rotaları seçebilirsiniz.
...
- **Rota bilgisi**: Rota adı, OSM bağlantısı (OSM Düzenleme eklentisi gerekli), Tür, Açıklama, Mesafe, Yön, Yokuş yukarı/aşağı, Rakım aralığı, Ağ, Operatör, Durum, Renk, Gidiş-dönüş, Başlangıç/Bitiş noktaları ve daha fazlası.
...
- [Haritada Parkuru Analiz Et](../map/tracks/index.md#analyze-track-on-map). Grafikler ve haritalar kullanarak parkur verilerinin ayrıntılı analizini görüntüler.
...
Rotayı bir *GPX dosyası* olarak kaydetmek için, [Bilgi paneli](../map/tracks/track-context-menu.md#info-panel) altındaki **Kaydet (İndir)** düğmesine dokunun. Bu, [Görünümünü](./tracks/appearance.md) değiştirme, [Navigasyon](../navigation/setup/route-navigation.md) veya [Rota planlama](../plan-route/create-route.md) gibi parkur eylemlerine erişebileceğiniz [Hızlı eylemler](../map/tracks/track-context-menu.md#track-actions) düğme panelini açacaktır.
...
[Hızlı Eylem](../widgets/quick-action.md#configure-map) widget'ını kullanarak rotaların açılıp kapanmasını özelleştirebilirsiniz.


## Rota Arama {#routes-search}

[Arama işlevini](../search/index.md) kullanarak rota adıyla veya [Kategoriler bölümünde](../search/search-poi.md#) "Rotalar"ı seçerek rotaları bulun.
...
İstenen rotaları bulmak için *<Translate android="true" ids="search_button,search_categories,poi_routes"/>* adresine gidin. Rotalar ad, etkinlik türü, uzunluk, konum ve en yakın noktaya olan mesafeyi içerir.
