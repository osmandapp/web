...
:::tip subscribe  
&nbsp;<ProFeature/> Tüm OsmAnd Cloud özelliklerine erişmek için [Android](../purchases/android.md#osmand-pro) veya [iOS](../purchases/ios.md#osmand-pro) için **OsmAnd Pro**'ya abone olun.  
:::  
...
1. **OsmAnd Cloud'da bir [hesap](#login) kaydedin**:
   ...
   [Android](../purchases/android.md#how-to-buy) veya [iOS](../purchases/ios.md#how-to-buy) için satın alma hakkında daha fazla bilgi edinin.
    ...
    2.1 Favorilerin, ayarların ve OSM düzenlemelerinin/notlarının bulut yedeklemeleri için **ücretsiz OsmAnd Başlangıç planını kullanın**.  
   Ücretsiz depolama sınırı **5 MB**'tır.

2. Diğer platformlarda [OsmAnd Pro](../purchases/cross.md), [OsmAnd Maps+](../purchases/cross.md) veya OsmAnd Başlangıç'ı etkinleştirmek için **kayıtlı e-postayı** giriş bilgisi olarak kullanın.

:::note  
**OsmAnd Pro**, tüm OsmAnd özelliklerini içeren çapraz platform bir aboneliktir. Bir mağazadan (örn. Google Play, Huawei AppGallery) satın alabilir ve **iOS**'ta da kullanabilirsiniz. **[Kayıtlı e-postanız](#login)**, OsmAnd Pro'yu [platformlar arası](../purchases/cross.md) etkinleştirmek için giriş bilgilerinizdir. 
:::
...
- Kayıt olmak veya giriş yapmak için aktif bir [OsmAnd Pro](#cross-platform) veya [OsmAnd Başlangıç](#osmand-start) aboneliğinizin olması gerekir. Abonelik durumunuzu OsmAnd uygulamasında kontrol edebilirsiniz:  
  *Menü → Ayarlar → Satın Almalar*.

- Cloud hesabınız **satın almanın orijinal olarak yapıldığı cihazda kayıtlı olmalıdır**.
...
- Bir **OsmAnd Pro** aboneliğiniz varsa, aboneliğinizin sona ermesinden sonra **bir yıl daha** OsmAnd Cloud hesabınıza erişmeye devam edebilirsiniz.  

:::tip Giriş Sorunlarını Giderme
Kayıt sırasında sorun yaşıyorsanız:

- Satın almanın yapıldığı **aynı cihazda** kayıt olduğunuzdan emin olun.  
...
- *Şimdi senkronize et*. Bu seçeneği seçerek, bu cihazda ve bulutta değiştirdiğiniz [ayarlar](#settings) menüsündeki tüm seçili veri bölümlerini senkronize edin.  

**Şimdi senkronize et**, Değişiklikler menüsündündeki [Yerel ve Bulut](#local-and-cloud) bölümlerindeki *Tümünü Yükle* ve *Tümünü İndir* düğmelerini birleştirir. **Çakışmalar** bu şekilde senkronize edilmez.
...
**Çöp Kutusu** özelliği, OsmAnd Cloud'dan silinen dosyaları görüntülemenize ve yönetmenize olanak tanır. Bu özellik, dosyaların yanlışlıkla silinmesini ve verilerinizin geri dönülemez şekilde kaybolmasını önlemek için tasarlanmıştır.

- **Dosyaları görüntüle**. *Çöp Kutusu*, *Bulut Değişiklikleri* klasöründen daha önce silinen tüm dosyaları listeler. Tüm dosyalar, silindikleri tarih ve saat hakkında bilgi içerir. Aya göre gruplandırılır ve Çöp Kutusu'nda kalan en son silinen dosyadan en eski dosyalara doğru sıralanır.
- **Çöp Kutusunu Boşalt**.  

    ![Trash OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_trash_3_andr.png)  

    Buluttan silinen tüm dosyalardan yer açmak için *Çöp Kutusunu Boşalt* özelliğini kullanabilirsiniz. Bu özellik, Çöp Kutusu ekranının sağ üst köşesindeki *üç nokta menüsünden* erişilebilir. Dosyalar kalıcı olarak silinir ve geri yükleme imkanı yoktur.

- **Ayrı dosyaları yönetme**.  

    ![Trash OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_trash_4_andr.png)  

    - *<Translate ios="true" ids="restore_from_trash"/>*. Bu özellik ile dosyaları OsmAnd Cloud'a geri yükleyebilirsiniz. Dokunduktan sonra, seçilen dosya Çöp Kutusundan *Değişiklikler* bölümündeki *Bulut* klasörüne taşınır.
    ...
    - *<Translate ios="true" ids="shared_string_delete_immediately"/>*. Bu özellik, seçilen dosyayı geri yükleyemeden kalıcı olarak silmenizi sağlar. Bu işlemi geri alınamaz olduğu için bu özelliği kullanırken dikkatli olun.
...
3. **Kaynaklar**. Burada, favoriler, rota bilgileri, sesli komutlar ve ek öğeler gibi kaynakları yedeklemeyi seçebilirsiniz.
...
**<Translate android="true" ids="backup_version_history"/>** (Android) / **<Translate ios="true" ids="manage_storage"/>** (iOS) menü bölümü, *Ayarlar*, *Yerlerim* ve *Kaynaklar*'daki 3.15 gigabayt belleğin ne kadarının kullanıldığını gösterir. OsmAnd Cloud'dan ayrı veri türlerine yapılan değişiklikleri silebilirsiniz, ancak bu durumda işlem geri alınamaz.  
...
Ayarlar menüsünün **Tehlike bölgesi** bölümünde yapabileceğiniz eylemler geri alınamaz ve bu nedenle özel dikkat gerektirir.

1. **Tüm verilerimi sil**. Bu seçenek, daha önce OsmAnd Cloud'a yüklenen tüm verileri, tüm sürüm geçmişi de dahil olmak üzere kaldırmanıza olanak tanır. Silindikten sonra bu verilerin geri yüklenemeyeceğini lütfen unutmayın.
...
Açılır menünün [Son Senkronizasyon](#last-sync) bölümünde, Değişiklikler bölümünü açmak için herhangi bir öğeye dokunun. Bu bölüm, düzenleme için mevcut tüm değişiklikleri gösteren üç sekme içerir. Liste boşsa, son senkronizasyondan bu yana hiçbir değişiklik olmamıştır.
...
- **Bulut depolamadan değişiklikleri indirmek, yerel değişikliklerin üzerine yazacaktır**. Benzer şekilde, yerel verileri yüklemek, ilgili değişiklikleri bulut sürümünden kaldırır.
...
- **Bulut** - **Bulut** sekmesi, bulut depolamadan indirilebilecek değişikliklerin bir listesini görüntüler. Ekranın altındaki düğmeyi kullanarak tüm değişiklikleri aynı anda indirebilir veya tek tek indirebilirsiniz.  
...
- **Çakışmalar** klasöründeki bir dosyaya dokunmak, son değiştirme ve yükleme zamanları da dahil olmak üzere **Yerel** ve **Bulut** sürümleri hakkındaki ayrıntıları görüntüler.
