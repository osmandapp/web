...
Wikipedia eklentisi, gerektiğinde etkinleştirilebilen/devre dışı bırakılabilen ayrı bir özelliktir. Etkinleştirildikten sonra, her coğrafi bölge için [indirilen](../personal/maps-resources.md#download-maps-maps) Wikipedia verilerini yüklemenize olanak tanır. Harita, Wikipedia'dan yüklenen iki tür bilgiye sahiptir: kısa bilgi ve tam makale.


Kısa bilgiler Wikipedia İÇN'leri (İlgi Çekici Nokta kısaltması) tarafından sağlanır. Wikipedia seçeneği menüde açık konumdaysa, haritada bir Wikipedia İÇN'si görünür. Bir ilgi çekici noktaya dokunduğunuzda, Wikipedia'dan kısa bilgiler ve tam bir makale açma yeteneği sağlar. Wikipedia İÇN'si ve ilgili makaleler çevrimdışı olarak kullanılabilir. Bunları gerektiği gibi okuyabilir, dilleri değiştirebilir ve Wikipedia'da başka bilgiler arayabilirsiniz.


[Wikipedia](https://en.wikipedia.org/wiki/Wikipedia), wiki tabanlı bir düzenleme sistemi kullanarak gönüllü editörlerden oluşan bir topluluk tarafından oluşturulan ve sürdürülen ücretsiz, çok dilli, açık işbirlikçi bir çevrimiçi ansiklopedidir.
...
Halihazırda hangi verilerin yüklendiğini görmek için *<Translate android="true" ids="shared_string_menu,download_tab_local,download_wikipedia_maps"/>* adresine gidin.
...
Halihazırda hangi verilerin yüklendiğini görmek için *<Translate ios="true" ids="shared_string_menu,res_mapsres,download_tab_local"/>* adresine gidin.
...
Gerekli bölgeler için Wikipedia verilerini yükleyerek, bu verilerin haritadaki görünürlüğünü kontrol edebilirsiniz. Görünürlük, [Android için](../map/point-layers-on-map.md#-wikipedia) Popüler Yerler (Wikipedia) / [iOS için](../map/point-layers-on-map.md#-wikipedia) Wikipedia İÇN'leri tarafından kontrol edilir ve belirli bir [profil](../personal/profiles.md) (örneğin, sürüş için bir profil, bisiklet için bir profil, toplu taşımayı göstermek için başka bir profil ve diğerleri) tarafından tüm yüklenen bölgeler için gösterilebilir veya gizlenebilir.
...
- Gerekirse, arama panelinin üst kısmındaki **Wikipedia'yı haritada göster** seçeneğine dokunun ve Wikipedia kategorisinden arama sonuçları haritada görünecektir.
...
![İngilizce'de Fransızca Wikipedia İÇN'si](@site/static/img/plugins/wikipedia/Andr-french-wikipedia-in-eng1.png)

</TabItem>

<TabItem value="ios" label="iOS">

![iOS'ta İngilizce'de Fransızca Wikipedia İÇN'si](@site/static/img/plugins/wikipedia/ios_wiki_language2.png)
...
[Makaleleri](#wikipedia-article) okumayı tercih ettiğiniz dili (veya dilleri) ayarlayabilir ve ayrıca her [profil](../personal/profiles.md) için [görsel indirme seçeneklerini](#download-images) yapılandırabilirsiniz.
...
4. Açılan listeden belirli bir dil(ler)i seçin.
...
Wikipedia'dan görselleri cihazınıza indirip indirmemeyi *Wikipedia makalesi* içinde seçebilirsiniz:
