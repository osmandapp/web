...
## Імпорт треків {#import-tracks}

Ви можете імпортувати GPX-треки у Веб-планувальник за допомогою кнопки Імпорт або перетягуванням GPX-файлів безпосередньо на мапу.

Щоб імпортувати трек перетягуванням:
- Відкрийте розділ Треки.
- Перетягніть один або кілька GPX-файлів з комп'ютера.
- Скиньте файли на мапу або в конкретну папку треків.

Коли файл перетягується над мапою, доступна зона скидання виділяється. Якщо скинути файл безпосередньо на мапу, він імпортується в папку Import. Якщо скинути його в наявну папку, він імпортується саме в цю папку.

Після завершення імпорту трек з'являється у списку треків, синхронізується з [OsmAnd Cloud](../personal/osmand-cloud.md) і стає доступним на всіх пристроях, підключених до того самого облікового запису.
...
У меню з трьома крапками (⋮) доступні додаткові дії для Smart Folder. Ви можете *Download as OSF*, *Download as OBF collection*, *Rename* або *Delete* папку.
...
Ви також можете використовувати кнопку **Focus**, щоб приховати всі інші обрані місця та треки на мапі, що полегшує перегляд вибраного треку. Вимкніть режим Focus, щоб відновити видимість інших об'єктів мапи.
