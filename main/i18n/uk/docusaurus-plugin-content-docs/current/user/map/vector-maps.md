...
- **<Translate ios="true" ids="rendering_attr_showSurfaces_name"/>**. Показує тип дорожнього покриття. Колір дороги допомагає зрозуміти, яке покриття має дорога, наприклад, асфальт, трава або пісанд. Дивіться [Легенду карти](../map-legend/index.md).  
...
Опція **Мова карти** налаштовує написання назв на карті OsmAnd для рівнів масштабу від 7 до 20 включно. Для рівнів масштабу 2-6 назви відображаються мовою, встановленою для OsmAnd у меню системних налаштувань (див. розділ *[Як змінити мову програми](../start-with/first-steps.md#how-to-change-app-language)*).  
...
Опція Рельєф дозволяє налаштувати три функції, такі як тінь від пагорбів, нахил та висота. Для отримання детальнішої інформації зверніться до статті [Топографія](../plugins/topography.md#terrain).
...
![3D Будівлі](@site/static/img/map/3d_buildings_1_ios_new.png) ![3D Будівлі](@site/static/img/map/3d_buildings_2_ios.png)

</TabItem>

</Tabs>

Функція **3D Будівлі** відображає будівлі як об'ємні 3D-моделі замість плоских фігур. Будівлі генеруються з [даних OpenStreetMap](https://wiki.openstreetmap.org/wiki/Simple_3D_Buildings).

На Android ця опція доступна лише коли увімкнено плагін Топографія. Детальніше дивіться в статті [плагін Топографія](../plugins/topography.md#3d-buildings).

### 3D Рельєф {#3d-relief}

Перейдіть до: *<Translate android="true" ids="shared_string_menu,configure_map,srtm_plugin_name,relief_3d"/>*

Функція 3D Рельєф створює рельєфне зображення та забезпечує тривимірне представлення ландшафту. 3D Рельєф працює офлайн і може використовуватися з [векторними картами OsmAnd](../map/vector-maps.md) або будь-якими [растровими картами](../map/raster-maps.md#layers) як [Джерело карти](../map/raster-maps.md#main) або як [Підкладка/Накладка](../map/raster-maps.md#overlay). Детальнішу інформацію можна знайти [тут](../plugins/topography.md#3d-relief).
...
![Глобальний вид](@site/static/img/map/globe_view_1.png) ![Глобальний вид](@site/static/img/map/globe_view_2.png)
...
![Глобальний вид](@site/static/img/map/globe_view_1_ios.png) ![Глобальний вид](@site/static/img/map/globe_view_2_ios_new.png)

</TabItem>

</Tabs>

**Глобальний вид** дозволяє відображати карту як сферичну Землю замість плоскої проекції. Цей режим змінює геометрію поверхні карти та адаптує взаємодію з картою до сферичної навігації. Для отримання додаткової інформації дивіться розділ [Глобальний вид](../map/interact-with-map.md#globe-view) у статті Взаємодія з картою.
...
![Меню сітки координат](@site/static/img/map/coordinates_grid_settings_ios.png)
