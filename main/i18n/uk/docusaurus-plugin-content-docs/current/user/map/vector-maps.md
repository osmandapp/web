...
Основні переваги включають картографування маршрутів, якість покриття, обмеження доступу, дорожні знаки, візуалізацію шляхів за шкалою SAC та топографічні деталі, такі як лінії контуру.
...
- **Рівні масштабування:** встановіть мінімальний та максимальний рівні масштабування (2 - 22), на яких сітка видима.
- **Позиція міток:** виберіть між *Краї* (за замовчуванням) або *Центр* для міток сітки.
- **Колір сітки:** доступно окремо для денного/нічного режиму. Налаштування кольору сітки є платною функцією.
- **Формат координат:** виберіть з кількох доступних форматів (див. список нижче).
...
[Швидка дія](../widgets/quick-action.md#overview): Ви також можете додати швидкий перемикач *Показати/Приховати сітку координат* до групи [Налаштувати карту](../widgets/quick-action.md#configure-map) для швидкого доступу.
...
- **<Translate android="true" ids="rendering_attr_natureReserves_name"/>**. Показує зелені межі та мітки "NR" для [території природного заповідника](https://wiki.openstreetmap.org/wiki/Tag:leisure%3Dnature_reserve). Виділяє заповідні зони зеленою межею та міткою "NR" для зон збереження дикої природи.  
