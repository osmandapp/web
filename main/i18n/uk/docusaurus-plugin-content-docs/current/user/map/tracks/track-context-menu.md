---
source-hash: 42c32f8ba134c0c58b04f7ab2f813db95152ab60ce1df97644abbc8b6005df50
sidebar_position: 4
...
:::note
Якщо ваш трек був створений в OsmAnd або будь-якій іншій програмі для відстеження (тому його точки мають теги [`time` та `speed`](../../plugins/trip-recording#recorded-gpx-file)), інформаційна панель також міститиме інформацію про *<Translate android="true" ids="average_speed"/>*, *<Translate android="true" ids="max_speed"/>*, *<Translate android="true" ids="shared_string_time_span"/>* (*Android*) або *<Translate ios="true" ids="total_time"/>* (*iOS*) (тривалість часу між початковою та кінцевою точками треку), *<Translate android="true" ids="shared_string_time_moving"/>* (сума часу під час руху).
:::
...
- Кнопки **Показати / приховати**. Змінює видимість треку на мапі.
...
- **<Translate android="true" ids="analyze_on_map"/>**. Відкриває інструмент [Аналізувати на мапі](../tracks/index.md#analyze-track-on-map-analyze-track-on-map) для перевірки висоти, швидкості та відстані треку.
...
- **<Translate android="true" ids="altitude_correction"/>** (*лише для Android*) - Щоб [отримати дані про висоту](#calculate-missing-elevation), [прив'яжіть свій трек до доріг](../../navigation/setup/gpx-navigation.md#attach-to-the-roads) або розрахуйте його офлайн (потрібні [Мапи рельєфу](../../plugins/topography.md#download-maps)).
...
- **Загальне**. Містить інформацію про *розмір GPX-файлу*, *папку файлу* та *дату створення* з тегом `time`.

- **Інформація**. Теги: *Ключові слова* - `keywords`, &nbsp;*Посилання* - `link`, &nbsp;*Діяльність* - `activities`.

- **Автор**. Теги: *Автор* - `author`, &nbsp;*Ім'я* - `name`, &nbsp;*Електронна пошта* - `email`, &nbsp;*Посилання* - `link`.
...
<summary>Приклад XML-коду GPX-файлу</summary>
...
- [Теги діяльності для GPX-треків](../../plugins/trip-recording.md#recording-settings). Під час запису GPX-треку ви можете призначити тип діяльності. Це тегування діяльності допомагає спочатку класифікувати треки.
...
- **Список діяльності**. Категорії та групи діяльності визначені в ресурсах OsmAnd. Для розробників та учасників список діяльності підтримується в структурованому форматі за посиланням [activities.json](https://github.com/osmandapp/OsmAnd-resources/blob/master/poi/activities.json), де детально описані доступні групи та типи діяльності.
...
- ***Розмістити точку на графіку***. Якщо ви хочете побачити інформацію в певній точці треку, ви можете торкнутися будь-де на графіку, і з'явиться покажчик зі значеннями. Одночасно на мапі з'явиться синій маркер, що вказує на це місце на треку.
...
Ця вкладка може бути видимою лише для записаних треків. Точки на цьому типі треку мають теги **[`speed`](../../plugins/trip-recording#recorded-gpx-file)**.
:::
...
- Кнопка *Розрахувати висоту → [Використовувати найближчі дороги](../../plan-route/create-route.md#get-elevation-data)*. Використовує офлайн-мапу для пошуку найближчих доріг та даних про висоту. Дозволяє налаштувати геометрію треку.
- [Використовувати мапи рельєфу](#calculate-missing-elevation). Використовує мапи рельєфу для отримання даних про висоту. Геометрія треку залишається незмінною.
...
Ця функція дозволяє розрахувати профіль висот для GPX-треку офлайн для будь-якої місцевості між 70 градусами північної широти та 70 градусами південної широти, на основі [даних мапи рельєфу](../../plugins/topography.md#download-maps). *Мапи рельєфу (3D) повинні бути попередньо завантажені*.

1. Завантажте [Мапи рельєфу (3D)](../../plugins/topography.md#download-maps) для потрібного регіону.
2. Якщо трек не містить даних про висоту, перейдіть до контекстного меню треку (*Контекстне меню треку → <Translate android="true" ids="shared_string_gpx_track,altitude,calculate_altitude"/>*), виберіть *Використовувати мапи рельєфу*, і після розрахунку ви отримаєте графік треку на основі даних рельєфу.
...
3. Якщо ви створюєте маршрут за допомогою інструменту [Планування маршруту](../../plan-route/create-route.md#graph) методом *Пряма лінія* і на графіку немає даних про висоту, вам потрібно:
    - Торкніться *Отримати дані про висоту*, потім виберіть *Використовувати мапи рельєфу*.
...
Маршрутні точки — це один з типів точок, доступних на мапі. Загалом, це можуть бути точки треку, які є частиною GPX-файлів. Детальний опис доступний у статті [точки на мапі](../../map/point-layers-on-map.md).
...
Маршрутні точки можна додати до поточного записаного треку або до будь-якого треку зі списку [*Мої місця → Треки*](../../personal/tracks/manage-tracks.md):

- Використовуйте [контекстне меню мапи](../../map/map-context-menu.md#-add--edit-track-waypoint--add--edit-track-waypoint).
  - Маршрутні точки автоматично відображаються, якщо вибраний трек увімкнено на мапі.
  - [Довге натискання](../../map/map-context-menu.md#select-any-point-long-tap) на мапі в будь-якому вибраному місці для створення маршрутної точки.
  - Ви можете встановити інформацію про маршрутну точку у відкритому [контекстному меню мапи](../../map/map-context-menu.md#-add--edit-track-waypoint).
...
Торкніться кнопки *Додати маршрутну точку*, щоб відкрити екран створення маршрутної точки (цей екран схожий на [Контекстне меню Вибраного](../../personal/favorites.md#create)).
...
2. **Виберіть** Групу.
    ...
    - **Іконка**, **колір іконки** та **форма** будуть використовуватися для всіх нових маршрутних точок, доданих до групи.
...
- **<Translate android="true" ids="shared_string_show_on_map"/>** - Дозволяє відображати або не відображати групові маршрутні точки на мапі. Або ви можете використовувати [екранну кнопку](#points--waypoints), щоб відображати або приховувати групові маршрутні точки на мапі.
...
- **<Translate android="true" ids="add_group_to_markers"/>** або **Видалити** (*лише для Android*) - Перемістити групові маршрутні точки до списку [Маркерів мапи](../../personal/markers.md).
- **<Translate android="true" ids="copy_to_map_favorites"/>** (*лише для Android*) - Перемістити групові маршрутні точки до [Вибраного](../../personal/favorites.md).
...
Керуйте видимістю груп на мапі: якщо ви приховаєте один трек, вся група також буде прихована.

Ви можете вмикати або вимикати групові маршрутні точки на мапі за допомогою кнопки у верхньому правому куті контекстного меню треку на мапі. Торкніться її та виберіть потрібну дію.
...
Маршрутні точки записуються разом з треком у [GPX-файл](../../../technical/osmand-file-formats/osmand-gpx.md). Кожна з цих маршрутних точок має свій набір тегів для опису. Усі теги, що безпосередньо стосуються опису маршрутних точок, які можна редагувати безпосередньо в застосунку OsmAnd, перелічені в цьому розділі.
...
3. Торкніться потрібної маршрутної точки, якщо вона [видима](../../personal/tracks/manage-tracks.md#track-menu) на мапі.

    Потім у [Контекстному меню треків](./track-context-menu.md) торкніться *точок*, поля потрібної папки з доступними маршрутними точками та потрібної маршрутної точки. Виберіть меню *Дії →* **Редагувати GPX-маршрутну точку** та внесіть зміни. Ви не можете змінити дату створення точки, *мітку часу*, у застосунку.

**Теги об'єктів** (Amenity tags), що описують додаткову інформацію, таку як години роботи або номер телефону, специфічні для POI, беруть цю інформацію з даних OpenStreetMap або даних, які ви ввели під час створення POI. Ви можете редагувати такі теги та [користувацькі теги](#display-custom-gpx-tags) лише в коді GPX-файлу, який можна відкрити будь-яким текстовим редактором на вашому пристрої.
...
Сторінка програми Android GPS Filter включає наступне:
...
- [Аналіз на мапі](../index.md#analyze-track-on-map)