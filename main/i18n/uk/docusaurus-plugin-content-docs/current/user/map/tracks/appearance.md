...
- **Показати іконки старту та фінішу** - [Відображає маркери](#start-and-finish-icons) для початкової та кінцевої точок треку.  
    Стани: *Без змін*, &nbsp;*Оригінал*, &nbsp;*Увімк.*, &nbsp;*Вимк.*
...
OsmAnd підтримує **користувацькі кольори треків** у файлах GPX за допомогою тегу `<osmand:color>` у розділі `<extensions>` елемента `<trk>`. Це дозволяє користувачам визначати кольори для окремих треків, покращуючи візуальну диференціацію, коли на мапі відображається кілька треків.
...
1. **Призначення кольору за замовчуванням**:  
    ...
    - В OsmAnd **4.9.10 і пізніших версіях** цю проблему було вирішено — індивідуальні кольори треків тепер зберігаються при імпорті.
...
3. **Пріоритет налаштувань зовнішнього вигляду:**  
    ...
    - Ці налаштування зовнішнього вигляду зберігаються у **розширеннях GPX верхнього рівня**.
