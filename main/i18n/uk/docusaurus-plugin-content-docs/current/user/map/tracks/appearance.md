...
Функція **Вигляд треку** дозволяє користувачам налаштовувати відображення треків на мапі. Це включає налаштування кольору, ширини, індикаторів напрямку та інших візуальних елементів. Налаштування вигляду треку допомагає розрізняти декілька треків, виділяти певні маршрути та покращувати читабельність мапи.
...
- **З Моїх місць**. Відкрийте *Меню → Мої місця → Треки*, виберіть трек, торкніться *піктограми вигляду* в [контекстному меню треку](../../map/tracks/track-context-menu.md#overview).
- **З мапи**. Торкніться треку безпосередньо на мапі, потім торкніться *піктограми вигляду* в розділі *Огляд*.
- **З запису треку**. Змініть вигляд поточного записаного треку за допомогою [контекстного меню запису треку](../../plugins/trip-recording.md#сurrent-track-recording).  
...
Ви можете встановити колір треку вручну або використовувати **візуалізацію даних на основі кольору** (наприклад, швидкість, висота, нахил). Якщо треку бракує певних даних, OsmAnd відображає відсутні ділянки сірим кольором.

**Доступні варіанти кольорів:**
...
Виберіть, чи розділяти інтервал на треку **за відстанню або за часом**.
...
### Піктограми початку та кінця {#start-and-finish-icons}

![Track menu Appearance start and finish icons Android](@site/static/img/map/track_appearance_menu_sf_icons_android.png)  

Дозволяє вибрати, чи відображати **піктограми початку та кінця** для сегментів треку.

### З'єднати прогалини {#join-gaps}

З'єднує **сегменти треку**, які були перервані через втрату GPS або паузу в записі. Функція **<Translate android="true" ids="join_segments"/>** (*Android*) або **<Translate ios="true" ids="gpx_join_gaps"/>** (*iOS*) дозволяє з'єднати ці прогалини у вибраному треку GPX за допомогою прямих ліній.

### Скинути до початкового {#reset-to-original}
...
Функція **3D Трек** генерує тривимірну візуалізацію шляхом інтеграції даних про висоту в стандартні 2D треки. Ця функція створює представлення з покращеною глибиною, що дозволяє точно аналізувати трек відносно місцевості. Включивши деталі висоти, 3D трек пропонує більш повний огляд топографії маршруту та географічного контексту.
...
- **<Translate android="true" ids="visualized_by"/>**: &nbsp;*Висота*, &nbsp;*Швидкість*, &nbsp;*Фіксована висота*. Дані 3D-сенсора: &nbsp;*Пульс*, &nbsp;*Каденс велосипеда*, &nbsp;*Потужність велосипеда*, &nbsp;*Температура*, &nbsp;*Швидкість велосипеда*.
...
- Ви можете поєднати його з [3D Рельєфом](../../plugins/topography.md#3d-relief). Якщо джерелом 3D-даних є не висота, 3D-представлення буде комбінацією висоти рельєфу та вибраних даних, таких як швидкість або пульс.

**<Translate android="true" ids="wall_color"/>**  
Налаштовує **колір 3D-треку** за допомогою градієнтів або візуалізації на основі даних.  
Варіанти включають: *Немає*, &nbsp;*Суцільний*, &nbsp;*Низхідний градієнт*, &nbsp;*Висхідний градієнт*, &nbsp;*Висота*, &nbsp;*Нахил*, &nbsp;*Швидкість*.

**<Translate android="true" ids="track_line"/>**  
Додає [виділену лінію](#color) до верхньої або нижньої частини 3D-треку.  
Варіанти включають: *Верх*, &nbsp;*Низ*, &nbsp;*Верх і низ*.

> **ПРИМІТКА**: *Версія програми для **Android** вимагає, щоб для використання візуалізації 3D-треку був увімкнений механізм рендерингу мапи [Версія 2 (OpenGL)](../../personal/global-settings.md#map-rendering-engine).*


### Варіанти використання {#use-cases}

Функція 3D-треку дуже корисна для таких видів діяльності, як парапланеризм, де візуалізація вашого маршруту польоту може надати глибші відомості. Відображаючи трек у 3D, ви можете використовувати такі функції, як **Висота стіни** та **Колір стіни**, щоб отримати цінну візуальну інформацію для аналізу.

1. **Візуалізація за швидкістю в 3D**.  
    Одним з поширених варіантів використання є використання 3D-треків для візуалізації змін швидкості. Це дозволяє точно визначити конкретні ділянки треку, де ви сповільнювалися або прискорювалися.  

    Завдяки кольору стіни, що базується на нахилі, ви можете спостерігати, як рельєф, зокрема пагорби та схили, впливали на вашу швидкість. Це поєднання візуалізації швидкості та нахилу забезпечує детальне розуміння продуктивності протягом усього заняття.

2. **Візуалізація за датчиком серця з 3D рельєфом**.  
    Ще одне цікаве застосування — поєднання даних датчика пульсу з 3D-картами рельєфу. Це дозволяє відображати коливання пульсу безпосередньо на місцевості, надаючи вам повний огляд того, як зміни висоти могли вплинути на ваші фізичні зусилля.  

    Функція 3D-рельєфу, яка перебільшує висоту, працює незалежно від 3D-виду треку. Два параметри вертикального перебільшення можна налаштувати, щоб краще зрозуміти взаємозв'язок між пульсом і висотою місцевості.

3. **Візуалізація висоти треку та кольорової лінії нахилу на пласкому вигляді землі.**  
    При візуалізації даних про висоту рекомендується використовувати плаский вигляд землі, щоб уникнути таких проблем, як помилки GPS, які можуть призвести до того, що ділянки треку будуть виглядати під землею.  

    У цьому сценарії зміни нахилу можуть бути візуалізовані окремо вздовж лінії треку, тоді як колір стіни використовується для представлення висоти. Цей метод гарантує, що навіть за потенційних неточностей даних ви все одно отримаєте чітку та точну візуалізацію нахилу та висоти.
...
Вкладка [Треки](../../personal/tracks/manage-tracks.md) автоматично відображає всі коли-небудь записані, створені або імпортовані треки. Вони організовані за папками або відображаються у вигляді списку під ними.  

Змінити вигляд кількох треків у *Моїх місцях → Треки*:
...
- **Скинути до початкового** — Відновлює індивідуальні налаштування треку.

- [Режим вибору](../../personal/tracks/manage-tracks.md#selection-mode) — Змінює вигляд конкретних треків у вибраній папці або всього списку на вкладці Треки.
...
Параметри налаштування масового вигляду:

- **Стрілки напрямку** — Додає [індикатори руху](#direction-arrows).  
    Стани: *Без змін*, &nbsp;*Початковий*, &nbsp;*Увімкнено*, &nbsp;*Вимкнено*.

- **Показати піктограми початку та кінця** — [Відображає маркери](#start-and-finish-icons) для початкової та кінцевої точок треку.  
    Стани: *Без змін*, &nbsp;*Початковий*, &nbsp;*Увімкнено*, &nbsp;*Вимкнено*.

- **Колір** — Застосувати ручне або засноване на даних [забарвлення](#color).
...
- **Інтервал поділу** — [Встановіть інтервал](#split-interval) для маркерів відстані / часу.  
    Стани: *Без змін*, &nbsp;*Початковий*, &nbsp;*Вибрати*: *Час* або *Відстань*.

Оригінальний та без змін:

- **Оригінальний** — Відновлює початкові параметри з файлу треку.
...
2. **Одноколірне відображення для об'єднаних треків:**  
    ...
    - Об'єднаний трек буде відображатися з використанням **основного кольору GPX** (за замовчуванням: червоний), навіть якщо окремі сегменти треку спочатку мали різні кольори.
...
- Це гарантує, що кожен трек збереже своє **призначене колірне представлення**, навіть якщо файл GPX спочатку містив кілька сегментів з різними кольорами.
