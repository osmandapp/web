...
Для отримання додаткової інформації про налаштування зовнішнього вигляду лінії маршруту див. [Екран мапи під час навігації](../../navigation/guidance/map-during-navigation.md#color), а про вибір і створення власного кольору див. [Колірні схеми](../../personal/color-palette-schemes.md#tracks-routes).
...
Екран *Усі кольори* відображає доступні палітри. Торкніться меню з трьома крапками поруч із палітрою, щоб:
- Перейменувати.
- Редагувати.
- Дублювати.
- Видалити. Видалення палітри вимагає підтвердження.

Торкніться кнопки + на екрані Усі кольори. Доступні два типи палітр:
- **Відносна** — кольори масштабуються автоматично на основі мінімальних і максимальних значень треку.
- **Фіксовані значення** — кольори призначаються конкретним абсолютним числам (наприклад, 50 км/год). Значення можна редагувати вручну. Одиниці відповідають [налаштуванням системи](../../personal/profiles.md#units--formats). Масштаб кольорів не перемасштабовується автоматично при застосуванні до різних треків.

:::info Pro feature
Створення власних палітр є [платною функцією *OsmAnd Pro*](../../purchases/index.md) <ProFeature />.
:::

Щоб дізнатися більше про колірні схеми палітр як формат даних та їх використання в треках і маршрутах, див. статтю [Колірні схеми палітр](../../personal/color-palette-schemes.md#tracks-routes).

Під час створення або редагування палітри екран редактора включає:
- Попередній перегляд шкали кольорів у верхній частині екрана.
- Вибірні кнопки значень під шкалою кольорів (наприклад, 50, 100, 150, Без даних).
- Кнопку + для додавання нового кроку.
- Поле Значення.
- Варіанти вибору кольору. Для отримання додаткової інформації про файли палітр.
- Опцію Видалити крок для вибраного значення.
- Кнопку Зберегти.

Використовуйте іконку скасування в панелі додатків, щоб скасувати останню дію. Торкніться Закрити, щоб вийти з редактора. Якщо були внесені зміни, перед закриттям з'являється діалог підтвердження.
...
Дозволяє вибрати, чи показувати **іконки старту та фінішу** для сегментів треку. Для треків з великою кількістю сегментів ці іконки можуть бути автоматично вимкнені для покращення продуктивності рендерингу мапи. У цьому випадку ви можете увімкнути їх вручну, якщо потрібно.
...
:::tip Purchase
Візуалізація 3D-треку є [платною функцією](../../purchases/index.md).  
:::
...
1. **Візуалізація за швидкістю з 3D**.  
