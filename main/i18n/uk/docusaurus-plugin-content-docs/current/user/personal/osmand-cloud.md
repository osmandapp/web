...
1. **Зареєструйте [обліковий запис](#login) в OsmAnd Cloud**:
   ...
   Дізнайтеся більше про покупку для [Android](../purchases/android.md#how-to-buy) або [iOS](../purchases/ios.md#how-to-buy).
    ...
    2.1 **Використовуйте безплатний план OsmAnd Start** для хмарного резервного копіювання улюблених місць, налаштувань та редагувань/нотаток OSM.  
...
- Якщо у вас є підписка **OsmAnd Pro**, ви можете продовжувати користуватися своїм обліковим записом OsmAnd Cloud ще **один рік** після закінчення терміну дії підписки.  
...
<!--
#### Синхронізація колекцій GPX {#syncing-gpx-collections}

OsmAnd підтримує групування треків GPX у **колекції**, що дозволяє організовувати пов'язані треки під однією міткою.

При використанні **OsmAnd Cloud** колекції GPX:

- Резервуються разом з вашими особистими даними.
- Синхронізуються між пристроями.
- Відновлюються з тією ж **структурою колекції** (включаючи назву та всі пов'язані треки).

> *Це гарантує, що організація ваших маршрутів буде збережена навіть після перевстановлення програми або переходу на новий пристрій.*

Щоб скористатися цією функцією:

1. Створіть або призначте треки до колекції через *Меню → Мої місця → Треки*.
2. Перейдіть до *Меню → Налаштування → OsmAnd Cloud → Резервне копіювання даних* та увімкніть синхронізацію.
3. На іншому пристрої увійдіть в той самий обліковий запис OsmAnd Cloud і відновіть дані.
-->
...
![Зміни в OsmAnd Cloud](@site/static/img/personal/osmand-cloud/deletion_option_new.png)  
...
3. **Ресурси**. Тут ви можете вибрати резервне копіювання таких ресурсів, як улюблені місця, інформація про маршрути, голосові підказки та додаткові елементи.
...
Вимкнення резервного копіювання для певного типу даних просто припиняє його синхронізацію з хмарою. Ваші наявні дані залишаються збереженими та можуть бути знову зарезервовані пізніше за потреби.
...
Розділ меню **<Translate android="true" ids="backup_version_history"/>** (Android) / **<Translate ios="true" ids="manage_storage"/>** (iOS) відображає інформацію про те, скільки з 3.15 гігабайтів пам'яті в *Налаштуваннях*, *Моїх місцях* та *Ресурсах* використовується. Ви можете переглянути розмір сховища для кожного типу даних та видалити його з OsmAnd Cloud за потреби. 
...
Перейдіть до: *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_cloud,shared_string_settings,auto_backup_title"/>*

![Автоматичне резервне копіювання Android](@site/static/img/personal/osmand-cloud/auto_backup_android.png)

Автоматичне резервне копіювання автоматично зберігає вибрані дані OsmAnd у хмару через регулярні інтервали, доки додаток активний. Це допомагає захищати ваші дані та тримати їх синхронізованими між пристроями з одним обліковим записом. Екран автоматичного резервного копіювання поділений на три розширювані розділи: *Налаштування*, *Мої місця* та *Ресурси*.

Поведінка автоматичного резервного копіювання:

- Автоматичне резервне копіювання запускається приблизно щогодини, доки OsmAnd працює на передньому плані. Цей інтервал наразі не можна налаштувати в додатку. Його можна змінити шляхом імпорту файлу .osf.
- Автоматичне резервне копіювання також може початися, коли ви відкриваєте або повертаєтеся до екрана карти.
- Крім запланованих запусків, автоматичне резервне копіювання також може початися після змін у деяких типах даних (наприклад, Улюблені), використовуючи увімкнені типи резервного копіювання в налаштуваннях автоматичного резервного копіювання.
- Ви можете увімкнути/вимкнути кожен елемент або цілі розділи незалежно.
- Ваші дані залишаються доступними локально, навіть якщо автоматичне резервне копіювання вимкнено.
...
<!--
## Посібники з дій {#action-guides}

- Відновіть свої дані після закінчення терміну дії покупок.
- Абсолютно важливо, щоб ваш обліковий запис у хмарі був зареєстрований на пристрої, на якому була здійснена початкова покупка.
- Доступ ще на один рік після закінчення підписки.

:::tip
Якщо у вас виникли проблеми з реєстрацією облікового запису:

- Перевірте, чи ви реєструєтесь на пристрої, на якому була здійснена покупка.
- Вам потрібне активне підключення до Інтернету.
