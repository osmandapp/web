...
OsmAnd використовує профіль за замовчуванням, *Огляд мапи*, як перший профіль для нових користувачів, і якщо ви обрали цей профіль, він використовуватиметься щоразу при перезапуску застосунку. Ви можете обрати будь-який профіль зі списку за замовчуванням, включно з *Останній використаний* та *Огляд мапи*.

- **<Translate android="true" ids="shared_string_last_used"/>**. Дозволяє обрати профіль, який використовувався останнім.
- **<Translate android="true" ids="settings_preset"/>**. Ви обираєте профіль, який використовуватиметься під час запуску OsmAnd. Його можна змінити в [налаштуваннях](../personal/profiles.md) застосунку.
...
[**<Translate ios="true" ids="carplay_profile"/>**](../navigation/car-play.md) використовується при підключенні до програмного забезпечення [CarPlay](https://support.apple.com/en-gb/HT205634) вашого автомобіля та допомагає максимально ефективно використовувати оптимізовану версію застосунку OsmAnd, використовуючи аудіо- та відеосистеми для відображення даних з телефону.
...
1. **Пам'ять пристрою** показує, скільки вільного місця залишилося на вашому пристрої. Торкніться поля, щоб отримати доступ до меню [*Мапи та ресурси*](../personal/maps-resources.md#maps--resources), де ви можете переглядати та керувати детальною інформацією про ваші дані OsmAnd.

2. У **<Translate android="true" ids="change_data_storage_folder"/>** ви можете обрати папку для збереження даних OsmAnd на своєму пристрої.

    - *Внутрішня пам'ять застосунку*. Лише застосунок OsmAnd має доступ до своїх даних, а зовнішні застосунки — ні.
    - *Спільна пам'ять*. Більше недоступна.
    - *Зовнішнє сховище 1*. Лише 1 застосунок OsmAnd має доступ, а також застосунки адміністратора та USB.
    - *Багатокористувацьке сховище*. Лише 1 застосунок OsmAnd має доступ, але він спільно використовується кількома користувачами Android.
...
- У версії застосунку для **Android** ви можете перемикатися між візуалізацією мапи Версії 1, яка була спочатку реалізована в застосунку OsmAnd, та Версією 2, яка використовує програмний інтерфейс OpenGL (Open Graphics Library) для візуалізації дво- та тривимірних графічних об'єктів.
...
| 3D режим | Можна переглядати мапу у 2D, і ви можете змінити кут огляду, щоб отримати [3D вигляд](../widgets/map-buttons.md#3d-mode). | Можна переглядати мапу у 2D та [3D](../widgets/map-buttons.md#3d-mode). |
| Відображення мапи | Вся мапа візуалізується як набір плиток, а маркери, лінії та текст вже знаходяться всередині цих плиток. | Спочатку візуалізується весь пакет плиток, а потім доступні символи застосовуються до мапи [шар за шаром](../../technical/algorithms/map-rendering-layers.md). |
...
Зображення для 3D вигляду мапи завантажується плитка за плиткою, потім усі символи. Швидкість візуалізації мапи залежить від кількості 2D та 3D графічних об'єктів на ній, і безпосередньо від продуктивності вашого обладнання. Тому Версія 2 (OpenGL) не може використовуватися на пристроях з низькою продуктивністю.
...
У цьому розділі налаштувань OsmAnd ви можете налаштувати сповіщення, які ви отримуєте, службу визначення місцезнаходження та спосіб збереження налаштувань застосунку.
...
- *<Translate ios="true" ids="do_not_show_discount"/>* (*iOS*) пригнічує відображення знижок на застосунок та спеціальних місцевих повідомлень про події.
...
- Конфіденційність служб місцезнаходження: *Налаштування → Приватність → Служби місцезнаходження*
...
- За замовчуванням ваші *Налаштування OsmAnd* та маркери мапи [*Обраного*](../personal/favorites.md#automatic-favorites-backup) автоматично резервуються системою Android. Це дозволяє відновити їх на нових пристроях або після переінсталяції застосунку.
...
**Якщо ви використовуєте [*OsmAnd Cloud*](../personal/osmand-cloud.md), рекомендується деактивувати цю опцію резервного копіювання за замовчуванням, щоб уникнути плутанини під час переінсталяції.**


## Юридична інформація {#legal}

OsmAnd може [попереджати вас про камери контролю швидкості](../navigation/guidance/navigation-settings.md#screen-alerts), але лише якщо це дозволено законодавством вашої країни.
...
У деяких країнах або регіонах використання застосунків для попередження про камери контролю швидкості є незаконним. Ви повинні зробити вибір залежно від законодавства вашої країни. Виберіть **Залишити активним**, і ви отримуватимете попередження та сповіщення про камери контролю швидкості. Виберіть **Видалити**, і всі дані, пов'язані з камерами контролю швидкості, такі як попередження, сповіщення та POI, будуть видалені до повної переінсталяції OsmAnd.

Прочитайте про попередження про камери контролю швидкості на вашому маршруті у статті Віджети навігації в розділі *[Віджет попереджень](../widgets/nav-widgets.md#alert-widget)*.
...
- **Android 12+ (зміна папки зберігання)**. Нові рекомендації щодо доступу до сховища були реалізовані у версіях Android 11-12. Ці зміни особливо впливають на зберігання мап на SD-картах, що призводить до значно повільнішої продуктивності доступу до файлів та суворіших обмежень доступу до папок на SD-карті на Android 11 та 12. Щоб дізнатися, як вирішити цю проблему, дивіться розділ [**Вирішення проблем**](../troubleshooting/maps-data.md#maps-slowly-loading-on-android-11-12-sd-card).

- **Джерело місцезнаходження для проблем з висотою**. Google Play змінив свою політику, і для відповідності OsmAnd, починаючи з версії 3.9, повинен використовувати Google Play Services для отримання фіксації місцезнаходження під час роботи у фоновому режимі. Детальніше читайте у розділі [Вирішення проблем](../troubleshooting/track-recording-issues.md#osmand-39-altitude-issues-when-using-google-play-services).
