...
Вибраний параметр сортування застосовується не тільки до поточного підтеки, але й до всіх вкладених тек і треків на всіх рівнях. Це забезпечує узгоджену структуру ваших збережених даних.
...
2. **Імпорт за допомогою меню *Мої місця*:**
    ...
    *Додаткові опції:*
    ...
    Коли файл GPX містить кілька треків, відкривається екран Імпорт треків, де можна переглянути всі треки у файлі, вибрати потрібні для імпорту, вибрати теку призначення або імпортувати всі треки як один за допомогою опції *Імпортувати як один трек*. Якщо файл містить шляхові точки, ви також можете переглянути та призначити їх перед завершенням імпорту.
