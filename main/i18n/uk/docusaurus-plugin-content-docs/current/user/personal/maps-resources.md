...
![Меню завантаження карт iOS](@site/static/img/personal/maps/map_type_1_ios.png) ![Карти регіонів Android](@site/static/img/personal/maps/map_type_2_ios_new.png)  
...
- [Глибини та контури морських карт](../plugins/nautical-charts.md). Морські карти, що показують берегові лінії, глибини, навігаційні засоби, гавані та небезпеки для безпечного плавання та вітрильного спорту.
...
Без активної підписки ці типи карт показують кнопку *Отримати* замість Завантажити. Торкніться *Отримати*, щоб відкрити екран підписки та обрати план.
...
- [Карти світу](#world-maps). Список карт усього світу: *World altitude correction (Android only)*, *World overview map*, *All world (Weather forecast)*
...
- **<Translate android="true" ids="index_item_world_altitude_correction"/>** (*тільки для Android*). Цей файл містить корекції висот для компенсації кривизни планети.
- **<Translate android="true" ids="index_item_world_basemap"/>**. Надає розширений огляд поверхні Землі, корисний для довгострокового планування маршрутів та звичайного дослідження.
- **Карта прогнозу погоди світу**. Дозволяє завантажити карту з інформацією про погоду.
...
Для країн, що складаються з **кількох регіональних карт**, ви можете використовувати пакетне завантаження кількох карт. Карти, доступні для цього типу завантаження, відображаються з іконками подвійної стрілки завантаження та міткою з кількістю регіонів всередині (наприклад, Лінії висот / Всі регіони: 10). Ви можете вибрати, скільки карт завантажити. Пакетне завантаження підтримує кілька типів карт, включаючи Стандартну, Лінії висот, Wikipedia та Рельєф.

Коли відкривається елемент країни з групуванням, відображається нижня панель з повним списком карт для цієї країни, що дозволяє вибирати окремі карти або завантажувати всі одразу.
...
Для стандартних карт деякі елементи можуть бути згруповані за регіоном. У таких випадках кожен запис карти відображає як підрегіон, так і батьківський регіон. Порядок цих назв залежить від вибраного методу сортування: при сортуванні за назвою спочатку показується підрегіон, за ним регіон; при сортуванні за країною спочатку показується регіон, за ним підрегіон.
...
![Меню Карти Оновлення карт Android](@site/static/img/personal/maps/maps_update_andr.png) ![Меню Карти Оновлення вкладка Android](@site/static/img/personal/maps/maps_update_tab_andr.png)
...
Якщо кілька карт однієї країни потребують оновлення, OsmAnd групує їх в один елемент у списку Оновлень. Групований елемент показує назву країни та кількість включених карт. При натисканні відкривається нижня панель *Оновити карти*, що показує всі карти, які потрібно оновити. 
...
Ви також можете керувати картами безпосередньо з вкладки Оновлення за допомогою жесту довгого натискання на будь-яку карту зі списку. Це відкриває контекстне меню з кількома опціями:
- **Інформація** — переглянути деталі про вибрану карту
- **Оновити** — завантажити останню доступну версію
- **Деактивувати** — тимчасово вимкнути карту без видалення її даних
- **Видалити** — повністю видалити дані карти з пристрою

Коли карта деактивована, вона може тимчасово зникнути з вкладки Локальні. Якщо оновлення стає доступним для деактивованої карти, вона все одно з'явиться у списку Оновлень і буде показана з сірою іконкою. Оновлення такої карти автоматично активує її знову.
...
![Меню Карти OsmAnd live  iOS](@site/static/img/personal/maps/maps_menu_live_updates_ios.png) ![Меню Карти OsmAnd live редагування iOS](@site/static/img/personal/maps/maps_menu_live_updates_edit_ios.png)
...
Live-оновлення застосовуються поверх завантаженої карти і не замінюють повний файл карти. Внаслідок цього дата, показана для карти на вкладці Локальні, не відображає Live-оновлення і може залишатися незмінною після їх застосування. Щоб завантажити новішу версію повної карти, скористайтеся Оновити карту або Оновити все в [Меню оновлень](#updates-menu).
