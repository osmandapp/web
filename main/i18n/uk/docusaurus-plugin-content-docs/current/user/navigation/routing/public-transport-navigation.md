...
![Максимальна кількість змін транспорту](@site/static/img/navigation/public/maximum_number_android.png) 

</TabItem>

<TabItem value="ios" label="iOS">  

![Максимальна кількість змін транспорту](@site/static/img/navigation/public/maximum_number_ios.png)

</TabItem>

</Tabs>

Перейдіть до: *<Translate android="true" ids="shared_string_settings"/>* *→ профіль Громадський транспорт →* *<Translate android="true" ids="routing_settings_2,help_article_navigation_routing_name"/>* *→ Максимальна кількість змін транспорту*

Продуктивність і споживання пам'яті маршрутизатора громадського транспорту залежать від глибини пошуку. Найважливіший параметр, який впливає на цю глибину, — це **Максимальна кількість змін транспорту**.

Цей параметр також безпосередньо впливає на доступність і якість маршрутів. Якщо значення встановлено занадто низьким (наприклад, 0), OsmAnd може не знайти відповідні маршрути або запропонувати неповноцінні варіанти, навіть якщо існують кращі маршрути з пересадками.

Ми використовуємо **2** як значення за замовчуванням, яке здається оптимальним для сучасних мереж громадського транспорту. Якщо ви стикаєтеся з низькою продуктивністю або помилками нестачі пам'яті, спробуйте зменшити це значення. Якщо ви в невеликому місті або подорожуєте короткими маршрутами, значення **1** може працювати краще.
<!--
