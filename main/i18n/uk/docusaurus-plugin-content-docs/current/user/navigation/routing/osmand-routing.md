...
sidebar_position: 0
title: Про маршрутизацію OsmAnd
---
...
- Офлайн-маршрутизацію OsmAnd можна **налаштувати, змінивши файл [.xml](https://github.com/osmandapp/OsmAnd-resources/blob/master/routing/routing.xml)**. Для отримання додаткової інформації див. статтю [*Налаштування офлайн-маршрутизації*](../routing/osmand-routing.md#customize-offline-routing).
...
- *[Велосипедна маршрутизація (MTB)](./bicycle-based-routing.md)* - Велосипедна маршрутизація надає навігаційні підказки спеціально для велосипедистів та ентузіастів MTB. Її також можуть використовувати водії мопедів (скутерів).
...
- *[Маршрутизація для верхової їзди](./horse-routing.md)* - надає вершникам інструменти, необхідні для планування та насолоди їхніми кінними прогулянками.
...
Ви також можете створити **власний файл routing.xml** або змінити стандартний **routing.xml** та використовувати його для навігації (лише для досвідчених користувачів OsmAnd).
...
- Після внесення змін новий файл *routing.xml* можна додати до OsmAnd, торкнувшись його та вибравши відкрити в додатку OsmAnd.
...
:::caution
Функція уникнення доріг є глобальною і впливає на всі **[Типи маршрутизації](#routing-types)**, крім *[Онлайн-маршрутизації](../routing/online-routing.md)* та *[BRouter](../routing/brouter.md)*.
:::

Існує два можливих способи вибрати, які дороги слід уникати при розрахунку маршруту:

- Виберіть тип дороги в розділі [Параметри маршруту](../guidance/navigation-settings.md#route-parameters) [Налаштувань навігації](../guidance/navigation-settings.md). Опис цього налаштування надано в статтях про налаштування [Типів маршрутизації](#routing-types). Як приклад, див. опис типу маршрутизації [Автомобіль / Вантажівка / Мотоцикл](../routing/car-based-routing.md#route-parameters---car).
...
- Дорога має бути позначена на максимальному масштабі, оскільки OsmAnd може неправильно інтерпретувати дотик і заблокувати, наприклад, неправильну сторону дороги з двома проїжджими частинами або тротуар.
- Функція Уникнення певних доріг некоректно працює з опцією [Навігація за треком](../setup/gpx-navigation.md) і не впливає на спрощену [Навігацію за маркерами](../setup/markers-navigation.md#use-markers-in-navigation).
...
Перейдіть до: *Кнопка Навігація → Налаштування → Уникнути доріг...*
...
Використовуючи опцію *Вибрати на мапі*, ви можете продовжити позначати дороги як непридатні для маршрутизації на мапі OsmAnd.
...
![Список уникнутих доріг на мапі Android](@site/static/img/navigation/routing/action_avoid_roads_list_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Список уникнутих доріг на мапі iOS](@site/static/img/navigation/routing/avoid_route_ios_2.png)
...
![Експорт уникнутих доріг на мапі Android 1](@site/static/img/navigation/routing/avoid_roads_export_andr_1.png) ![Експорт уникнутих доріг на мапі Android 2](@site/static/img/navigation/routing/avoid_roads_export_andr_2.png)
...
![Експорт уникнутих доріг на мапі iOS 1](@site/static/img/navigation/routing/avoid_roads_export_ios_1.png) ![Експорт уникнутих доріг на мапі iOS 2](@site/static/img/navigation/routing/avoid_roads_export_ios_2.png)

</TabItem>

</Tabs>

Залежно від налаштувань вашого пристрою та доступних додатків, весь ваш список доріг для уникнення або лише одна з них можуть бути [експортовані](../../personal/import-export.md#export) як файл `.osf` в різні місця. Виберіть дороги, які ви хочете експортувати, у запропонованому списку.
...
![Імпорт уникнутих доріг на мапі Android](@site/static/img/navigation/routing/avoid_roads_import_andr_1.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Імпорт уникнутих доріг на мапі iOS](@site/static/img/navigation/routing/avoid_roads_import_ios_1.png)
...
:::note
Найзручніший спосіб синхронізації доріг для уникнення між вашими пристроями — використовувати [OsmAnd Cloud](../../personal/osmand-cloud.md) (потрібна підписка Pro).
:::
...
**Розділ розробки** в налаштуваннях *Параметри маршруту* (*Меню → Налаштування → Профіль додатку → Налаштування навігації → Параметри маршруту → Розробка*) видно лише тоді, коли увімкнено [плагін розробки OsmAnd](../../plugins/development.md). Він включає такі налаштування, як *Тип маршрутизації (Android) / Алгоритм маршрутизації (iOS), Апроксимація GPX (лише Android), Автомасштабування та Дані OsmAnd Live (лише Android)*. Детальний опис читайте у статті [Налаштування навігації](../../navigation/guidance/navigation-settings.md#development-settings).
