...
Після розрахунку маршруту в [меню навігації](./route-navigation.md#start--stop-navigation) ви маєте доступ до "Деталей" цього маршруту. Ця інформація корисна для використання перед початком поїздки для покращення планування маршруту або для перевірки інформації про дорогу попереду під час руху.
Існує три способи доступу до *меню Деталі маршруту*.

1. Перейдіть до головного *Меню → Навігація*, встановіть маршрут та натисніть кнопку *Деталі*.
2. Натисніть піктограму *Навігація* на екрані мапи, встановіть маршрут та натисніть кнопку *Деталі*.
3. Перейдіть до меню *Мої місця*, торкніться будь-якого доступного треку у списку *Меню → Мої місця → Треки*, виберіть піктограму *Навігація* у контекстному меню треку в *Огляді* та торкніться *Деталі*.

Залежно від вибраних [налаштувань профілю](../../personal/profiles.md) та вибраного [типу маршрутизації](../../navigation/routing/osmand-routing.md#routing-types), деякі деталі маршруту можуть не відображатися. Крім того, деталі маршруту сильно залежать від *покриття даних OpenStreetMap*.
...
- ***Орієнтовний час у дорозі*** розраховується як сума часу на кожному сегменті та штрафного часу в певних точках. Кожен навігаційний профіль має різні правила для розрахунку швидкості на сегменті та які штрафи слід включити (налаштовується в [routing.xml](../../../technical/osmand-file-formats/osmand-routing-xml.md)). Наприклад, профілі автомобіля та велосипеда беруть середню швидкість на сегменті з даних OpenStreetMap, але профілі пішохода, човна та літака використовують *стандартну* швидкість з [Налаштувань навігації](../guidance/navigation-settings.md#default-speed--road-speeds).

- ***Орієнтовний час прибуття*** розраховується як час від поточного моменту за місцевим часом походження, використовуючи *Орієнтовний час у дорозі*.

- ***Дані про викиди CO2*** приблизно оцінюють, скільки CO2 буде вироблено під час руху за маршрутом. Вам потрібно налаштувати [тип паливного двигуна](../guidance/navigation-settings.md#fuel-used-by-motor) у параметрах транспортного засобу, щоб отримати точнішу оцінку.
...
- *Вгору/вниз*. Відображає співвідношення шляху до горизонталі, де нуль означає горизонтальність, а кількість градусів вказує на більший або крутіший ступінь нахилу. Градієнт показує максимальний нахил дороги у відсотках від вашого місця розташування до кінця видимого графіка або для вибраної ділянки маршруту без видимої точки розташування на екрані.

***Ліцензія на дані DEM, що використовуються OsmAnd для визначення рельєфу***
Дані про висоту на мапі (між 70 градусами північної широти та 70 градусами південної широти) були отримані з вимірювань, зроблених у рамках *Місії топографії радіолокаційного шаттла (SRTM)*. Вона використовувала *Розширений космічний тепловий емісійний та відбиваючий радіометр (ASTER)*, основний інструмент зображення в *Системі спостереження Землі НАСА*. Для отримання повної інформації дивіться [Ліцензію](https://github.com/osmandapp/OsmAnd/blob/master/LICENSE#L146).
...
- ***Збільшення/зменшення масштабу***. За замовчуванням графік показує маршрут від поточного місця розташування до точки призначення. Ви можете використовувати [рухи двома пальцями](../../map/interact-with-map.md#gestures) для збільшення та зменшення масштабу для більш детального перегляду. Ви також можете переміщати графік вправо, вперед по маршруту та назад до поточного місця розташування. *Такі маніпуляції також стосуються візуального блоку інформації в блоці [Атрибути дороги](#road-attributes).*

- ***Піктограма розташування***. Якщо ви хочете побачити інформацію в певній точці попереду на вашому маршруті, ви можете торкнутися будь-де на графіку, і з'явиться покажчик з висотою та відсотком нахилу.
...
- *Торкніться та перетягніть*. Торкніться графіка, щоб відобразити інформацію про точку треку та рух по ньому. Графік виділяє місце розташування точки на мапі та відображає інформацію про неї на лінійці.
- *Масштаб*. Масштабуйте графік [жестом двома пальцями](../../map/interact-with-map.md#gestures).
- *Слідувати за моїм розташуванням* (*лише Android*). Ви можете торкнутися [Моє розташування](../../map/interact-with-map.md#my-location-and-zoom), щоб синхронізувати вигляд мапи та графіка з вашим розташуванням. Під час руху графік буде зміщуватися зліва направо, відображаючи інформацію попереду вашого шляху.
...
Атрибути дороги доступні в [Деталях маршруту](#how-to-access) та відображають інформацію, пов'язану з сегментами, з OpenStreetMap та даних про висоту. Кольорове позначення пов'язане з тим, як конкретні атрибути відображаються на мапі (див. [Легенда мапи](../../map-legend/osmand.md#surface-smoothness)). Гістограми також інтерактивні, і можна побачити інформацію в певному місці, торкнувшись графіка.
...
- [**Поверхня**](https://wiki.openstreetmap.org/wiki/Key:surface). Дорожні покриття — це верхній шар дорожнього полотна, який контактує з колесами транспортних засобів. Вони можуть бути різних типів залежно від їхніх властивостей та функціонального призначення.

   Наприклад, [асфальтове](https://wiki.openstreetmap.org/wiki/Tag:surface%3Dasphalt) покриття забезпечує хороше зчеплення та шумоізоляцію, а [бетонне](https://wiki.openstreetmap.org/wiki/Tag:surface%3Dconcrete) покриття є більш довговічним та має більшу вантажопідйомність. Для створення дорожнього покриття також можуть використовуватися інші матеріали, такі як гравій, щебінь, дерев'яні дошки тощо.
...
- [**Складність кінних стежок**](https://wiki.openstreetmap.org/wiki/Riding). Цей атрибут дороги вказує на складність маршруту під час їзди верхи. Підходить для [маршрутизації для верхової їзди](../../navigation/routing/horse-routing.md).
...
Покрокова інформація надає детальні [інструкції](#instructions) щодо навігації за маршрутом. Окрім меню *Деталі*, вона також відображається у віджетах [*Маневри маршруту*](../../widgets/nav-widgets.md#route-maneuvers) та [*Смуги*](../../widgets/nav-widgets.md#lanes). Ця інформація допомагає вам краще орієнтуватися на маршруті та дозволяє планувати заздалегідь.

Торкніться певного розділу маршруту у списку, щоб переглянути детальну інформацію або внести необхідні зміни. Мапа збільшується, і відкривається [Контекстне меню](../../map/map-context-menu.md).
...
:::caution Тільки iOS
Цей тип інструкцій наразі *недоступний для версії OsmAnd для Android*.
:::

![Side indication](@site/static/img/navigation/route/side_indication.png)

У списку інструкцій **індикатор узбіччя** показує, в якому напрямку від вашого поточного місцезнаходження та напрямку руху до кінцевої точки знаходиться ваше призначення: ліворуч, праворуч або прямо.

Вимірюється кут між вашим призначенням (*[Азимут](https://en.wikipedia.org/w/index.php?title=Bearing_(navigation)&oldformat=true) від останньої точки*) та останнім сегментом (*Азимут до останніх 2 точок маршруту*), ділиться на 3 сектори по 120 градусів, і в результаті ви можете побачити, в якому напрямку знаходиться ваше призначення.
...
Функція **Друк** покрокового опису маршруту може бути корисною, якщо ви віддаєте перевагу мати фізичну копію опису маршруту замість використання електронних пристроїв, або якщо у вас немає доступу до навігаційних застосунків чи GPS-навігаторів, немає доступу до Інтернету, або якщо вам потрібна інформація для інших людей, що подорожують з вами.
...
Можливість **друкувати** або **зберігати** покрокову інформацію **у форматі PDF** особливо корисна під час подорожей на великі відстані або в незнайомих місцях, коли вам потрібно мати більш детальне уявлення про маршрут та час, необхідний для його подолання.
...
- **Експорт/Збереження**. Дозволяє зберегти інформацію про маршрут **як новий трек** для подальшого офлайн-використання. Експортований трек містить всю інформацію про атрибути дороги та інструкції щодо поворотів. Отже, [навігація за цим треком](./gpx-navigation.md) повинна надавати всі функції, як і навігація за маршрутом.
- **Поділитися**. Ділиться експортованим треком або посиланням на маршрут, який можна відкрити на іншому пристрої за допомогою OsmAnd.
