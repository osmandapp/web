---
source-hash: 8b4baa7add4df529ccfb9e631d439ed29e5af0f4c3bcf1359010edd095336ea6
sidebar_position: 4
...
OsmAnd дозволяє знайти найкращий маршрут та дослідити його деталі. Додаток надає детальну інформацію про маршрут, таку як відстань, [час у дорозі](#distance--time--co2), [тип покриття](#road-attributes), [перепади висот](#elevation-info), [дані про викиди CO2](#distance--time--co2) та [інформацію про повороти](#turn-by-turn-information). Ви також можете [аналізувати](#analyze-on-map) маршрут на карті, дізнаватися інформацію про сегменти треку, [друкувати](#print) план маршруту, [зберігати](#share--export-actions) маршрут та ділитися ним з іншими. Завдяки цій функціональності ви завжди знатимете деталі маршруту та зможете ефективніше планувати свою поїздку.
...
Існує три способи доступу до *меню Деталей маршруту*.

1. Перейдіть до головного *Меню → Навігація*, встановіть маршрут та натисніть кнопку *Деталі*.
2. Натисніть піктограму *Навігація* на екрані карти, встановіть маршрут та натисніть кнопку *Деталі*.
3. Перейдіть до меню *Мої місця*, натисніть будь-який доступний трек у списку *Меню → Мої місця → Треки*, виберіть піктограму *Навігація* у контекстному меню треку в *Огляді* та натисніть *Деталі*.
...
- ***Орієнтовний час у дорозі*** розраховується як сума часу на кожному сегменті та штрафів за час у певних точках. Кожен навігаційний профіль має різні правила для розрахунку швидкості на сегменті та які штрафи слід включати (налаштовується в [routing.xml](../../../technical/osmand-file-formats/osmand-routing-xml.md)). Наприклад, профілі автомобіля та велосипеда беруть середню швидкість на сегменті з даних OpenStreetMap, але профілі пішохода, човна та літака використовують *стандартну* швидкість з [Налаштувань навігації](../guidance/navigation-settings.md#default-speed).

- ***Орієнтовний час прибуття*** розраховується як час від поточного моменту за місцевим часом початкової точки, використовуючи *Орієнтовний час у дорозі*.

- ***Дані про викиди CO2*** приблизно оцінюють, скільки CO2 буде вироблено під час проходження маршруту. Вам потрібно налаштувати [Тип паливного двигуна](../guidance/navigation-settings.md#fuel-used-by-motor) у параметрах транспортного засобу, щоб отримати більш точне наближення.
...
- *Середня висота*. Відображає середньозважену висоту на маршруті.
- *Підйом/спуск*. Відображає співвідношення шляху до горизонталі, де нуль вказує на горизонтальність, а кількість градусів вказує на більший або крутіший ступінь нахилу. Ухил показує максимальний нахил дороги у відсотках від вашої точки розташування до кінця видимого графіка або для вибраної ділянки маршруту без видимої на екрані точки розташування.


***Ліцензія на дані DEM, що використовуються OsmAnd для виявлення рельєфу***
Дані про висоту на карті (між 70 градусами північної широти та 70 градусами південної широти) були отримані з вимірювань, зроблених у рамках *Місії радіолокаційного топографічного зйомки (SRTM)*. Вона використовувала *Удосконалений космічний тепловий емісійний та відбиваючий радіометр (ASTER)*, основний інструмент зображення в *Системі спостереження Землі НАСА*. Для повної інформації дивіться [Ліцензію](https://github.com/osmandapp/OsmAnd/blob/master/LICENSE#L146).
...
<summary>Дані DEM (DSM)</summary>
 ...
 - <a href="https://sonny.4lima.de">Цифрові моделі рельєфу Європи (DTM) за допомогою LiDAR від Sonny</a>.
...
- ***Збільшити/зменшити***. За замовчуванням графік показує маршрут від поточного місця до пункту призначення. Ви можете використовувати [рухи двома пальцями](../../map/interact-with-map.md#gestures) для збільшення та зменшення масштабу для більш детального перегляду. Ви також можете переміщати графік вправо, вперед по маршруту та назад до поточного місця. *Такі маніпуляції також стосуються візуального блоку інформації в блоці [Атрибути дороги](#road-attributes).*
...
- *Вісь Y*:
   ...
   - Графіки з двома типами даних: &nbsp;*Висота/Нахил*, &nbsp;*Висота/Швидкість*, &nbsp;*Нахил/Швидкість*, &nbsp;*Анімований зум/Швидкість*.
- *Вісь X*:
...
- *Торкніться та проведіть*. Торкніться графіка, щоб відобразити інформацію про точку треку та рух по ньому. Графік виділяє розташування точки на карті та відображає інформацію про неї на лінійці.
- *Масштаб*. Масштабуйте графік [жестом двома пальцями](../../map/interact-with-map.md#gestures).
- *Слідкувати за моїм розташуванням* (*Тільки Android*). Ви можете торкнутися [Моє розташування](../../map/interact-with-map.md#my-location-and-zoom), щоб синхронізувати вигляд карти та графік з вашим розташуванням. Під час руху графік буде зміщуватися зліва направо, відображаючи інформацію попереду вашого шляху.
...
Атрибути дороги доступні в [Деталях маршруту](#how-to-access) та відображають інформацію, пов'язану з сегментами, з OpenStreetMap та дані про висоту. Кольорова індикація пов'язана з тим, як конкретні атрибути відображаються на карті (див. [Легенда карти](../../map-legend/osmand.md#surface-smoothness)). Гістограми також інтерактивні, і можна побачити інформацію в певному місці, торкнувшись графіка.

- [**Тип дороги**](https://wiki.openstreetmap.org/wiki/Key:highway#Roads). Тип дороги визначається на основі її функціонального призначення та характеристик, таких як кількість смуг, обмеження швидкості, пропускна здатність, наявність пішохідних та велосипедних доріжок, узбіч та інших параметрів. Різні типи доріг можуть мати різні обмеження та правила дорожнього руху.
...
- [**Поверхня**](https://wiki.openstreetmap.org/wiki/Key:surface). Дорожні покриття — це верхній шар дорожнього полотна, який контактує з колесами транспортних засобів. Вони можуть бути різних типів залежно від їх властивостей та функціонального призначення.

   Наприклад, [асфальтове](https://wiki.openstreetmap.org/wiki/Tag:surface%3Dasphalt) покриття забезпечує хороше зчеплення та шумоізоляцію, а [бетонне](https://wiki.openstreetmap.org/wiki/Tag:surface%3Dconcrete) покриття є більш довговічним та має більшу вантажопідйомність. Інші матеріали, такі як гравій, щебінь, дерев'яні дошки та інші, також можуть використовуватися для створення дорожнього покриття.

- [**Крутизна**](https://wiki.openstreetmap.org/wiki/Key:incline#Common_&_extreme_inclines). Це вимірювання кута нахилу дороги, виражене у відсотках. Воно вказує, наскільки круто дорога нахилена вгору або вниз відносно горизонту, і розраховується шляхом вимірювання вертикальної зміни висоти дороги на горизонтальну відстань та множення отриманого значення на 100%.
...
Покрокова інформація надає детальні [інструкції](#instructions) щодо навігації маршрутом. Окрім меню *Деталі*, вона також відображається у віджетах [*Маневри маршруту*](../../widgets/nav-widgets.md#route-maneuvers) та [*Смуги*](../../widgets/nav-widgets.md#lanes). Ця інформація допомагає краще орієнтуватися на маршруті та дозволяє планувати заздалегідь.

Торкніться певного розділу маршруту у списку, щоб переглянути детальну інформацію або внести необхідні зміни. Карта збільшується, і відкривається [Контекстне меню](../../map/map-context-menu.md).
...
- [*Індикатор узбіччя*](#roadside-indicator) (*Тільки iOS*).
...
У списку інструкцій **індикатор узбіччя** показує, в якому напрямку від вашого поточного місцезнаходження та напрямку руху до вашої кінцевої точки знаходиться ваше призначення, це може бути ліворуч, праворуч та прямо.

Вимірюється кут між вашим призначенням (*[Напрямок](https://en.wikipedia.org/w/index.php?title=Bearing_(navigation)&oldformat=true) від останньої точки*) та останнім сегментом (*Напрямок до останніх 2 точок маршруту*), ділиться на 3 сектори по 120 градусів, і в результаті ви можете побачити, в якому напрямку знаходиться ваше призначення.
...
Друкований опис маршруту надає детальні *покрокові інструкції* щодо навігації маршрутом, включаючи відстань до наступного повороту, напрямок руху та назви вулиць або стежок, а також інформацію про відстань та час у дорозі.
