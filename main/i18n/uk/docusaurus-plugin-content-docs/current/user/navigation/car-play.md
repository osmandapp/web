...
:::tip Purchase
CarPlay є [платною функцією](../purchases/index.md).  
:::
...
Положення кнопок та віджета навігації в *CarPlay* відповідає стороні панелі стану CarPlay. Воно залежить від налаштування **Положення водія** (ліворучне чи праворучне) у головному блоці вашого автомобіля, а не від будь-якого налаштування всередині CarPlay або застосунку OsmAnd.
...
- [Масштабування](../widgets/map-buttons.md#my-position-and-zoom). Дозволяє збільшувати та зменшувати масштаб видимої частини карти.
...
1. Якщо ви хочете, щоб певне місце на карті було кінцевою точкою, виберіть відповідну категорію точок зі списку: [POI](#poi-categories), [Обране](#favorites) або [Маркери на карті](#map-markers).  
    ...
    - Список точок у папках відсортовано за відстанню до них, починаючи з найближчої. Кожна точка має назву або координати, фігурну іконку (колір та іконка встановлені за замовчуванням або обрані вами в застосунку OsmAnd) та відстань від поточного місцезнаходження до неї.
...
Коли відтворюються голосові підказки, OsmAnd регулює фонове аудіо залежно від його типу. Якщо ввімкнено опцію *<Translate ios="true" ids="pause_spoken_audio"/>*, аудіовміст, наприклад подкасти та аудіокниги, призупиняється під час навігаційних інструкцій, а відтворення музики продовжується зі зменшеною гучністю. Якщо опцію *<Translate ios="true" ids="pause_spoken_audio"/>* вимкнено, аудіовміст і музика продовжують відтворюватися зі зменшеною гучністю.  
...
Корисні сповіщення автоматично показуються перед початком навігації, коли потрібна додаткова дія для розрахунку маршруту. Ці сповіщення допомагають вирішити проблеми маршрутизації до початку навігації.

**Відсутні або застарілі карти**

Якщо деякі офлайн-карти, необхідні для розрахунку маршруту, відсутні або застарілі, відображається сповіщення. Ви можете вибрати один із таких варіантів:
- **Використовувати завантажені карти**. Розрахувати маршрут за допомогою карт, які зараз доступні на вашому пристрої.
- **Переглянути на телефоні**. Відкрити екран завантаження карт на iPhone, щоб завантажити або оновити потрібні карти. Після завантаження карт ви можете знову розпочати навігацію.
- **Скасувати**. Закрити сповіщення без початку навігації.

**Попередження про приватний доступ**

Якщо для досягнення пункту призначення маршрут потребує використання приватної дороги, відображається попередження: *Дозволити використання приватної дороги для досягнення пункту призначення?* Ви можете вибрати:
- **Дозволити**. Продовжити маршрут приватною дорогою.
- **Скасувати**. Закрити сповіщення та повернутися до попереднього екрана, де ви можете змінити пункт призначення або виконати іншу дію.
...
- У *CarPlay* неможливо налаштувати відображення сповіщень, вам потрібно налаштувати віджет у застосунку перед початком навігації та підключенням пристрою до автомобіля.
...
![Завершення навігації](@site/static/img/navigation/auto-car/finish_navigation_carplay_new.png)

Коли навігація завершена в CarPlay, OsmAnd відображає діалог **“Ви прибули”** на екрані автомобіля. Цей діалог використовує ту саму логіку виявлення прибуття, що й мобільний застосунок, і надає швидкий доступ до поширених дій після досягнення пункту призначення.

Доступні дії:
- **<Translate ios="true" ids="context_menu_item_add_parking_point"/>**. Зберігає вашу поточну позицію як місце паркування та розміщує маркер паркування на карті.
- **<Translate ios="true" ids="find_parking"/>**. Відкриває екран Пошуку з попередньо вибраною категорією Паркування, щоб допомогти вам знайти неподалік варіанти паркування.
- **<Translate ios="true" ids="recalculate_route"/>**. Будує новий маршрут до початкового пункту призначення, якщо ви хочете продовжити навігацію.
- **<Translate ios="true" ids="finish_navigation"/>**. Завершує поточний маршрут і повертає OsmAnd до стандартного стану карти.

Якщо CarPlay відключено під час активної навігації, OsmAnd застосовує додаткову логіку на iPhone:
- Якщо відстань до пункту призначення менше 100 м, навігація автоматично завершується, і відновлюється стандартний профіль. У цьому випадку сповіщення “Ви прибули” не відображається на екрані телефону.
- Якщо поточна швидкість нижче 1 м/с (наприклад, під час зупинки на заправці), навігація призупиняється. Навігація автоматично відновлюється при повторному підключенні CarPlay.
...
### Тема карта {#map-theme}

![Тема карти](@site/static/img/navigation/auto-car/map_theme_carplay.png)

CarPlay дозволяє налаштувати вигляд карта для кращої видимості в різних умовах водіння. Ви можете вибрати один із таких режимів вигляду:

- *Автоматично*. Вигляд карти змінюється автоматично залежно від системних налаштувань, часу доби або стану автомобіля.
- *Завжди темна*. CarPlay використовує темну тему карти.

Коли ввімкнено опцію *Завжди показувати темні карти*, карта завжди залишається в темному режимі незалежно від стану фар автомобіля чи часу доби.
