...
> **ПРИМІТКА.** *Майте на увазі, що профіль **Перегляд карти** не підтримує навігаційні віджети в жодному режимі. Перейдіть на відповідний **навігаційний профіль**, щоб увімкнути ці функції.*
...
| Ваше ***Поточне місцезнаходження*** на графіку включає значок місцезнаходження, висоту та значення відсотка нахилу. Права сторона віджета містить найвищу висоту та градієнт, середнє значення для них та найнижче значення на вибраній ділянці маршруту, видимій на графіку. |
...
|***Ухил*** — це відношення шляху до горизонталі, де нуль означає горизонтальність, а число градусів вказує на більший або крутіший ступінь нахилу.|
...
### Навігація за локсодромою {#navigation-along-the-rhumb-line}

- **Відносний азимут**. Якщо ви дотримуєтеся відносного азимута 0 градусів і рухаєтеся вперед, ви прибудете до пункту призначення, але не найкоротшим маршрутом ([Локсодрома](https://en.wikipedia.org/wiki/Rhumb_line)).
- **Магнітний азимут**. Якщо ви вирівняєте компас на своєму човні/транспортному засобі зі значенням віджета і рухатиметеся вперед, ви прибудете до пункту призначення, але не найкоротшим маршрутом ([Локсодрома](https://en.wikipedia.org/wiki/Rhumb_line)).
...
Віджет *Інформація про маршрут* надає дані в реальному часі про поточний навігаційний маршрут та швидкий спосіб [керування активним маршрутом](../navigation/setup/route-navigation.md#start--stop-navigation). Він допомагає відстежувати ключові деталі маршруту та дозволяє зупинити або відновити поточну навігаційну сесію з мінімальною взаємодією.

**Відображена інформація:**

- **Розрахунковий час прибуття (ETA)**. Показує очікуваний час прибуття до кінцевого пункту призначення.
- **Час, що залишився в дорозі**. Відображає розрахунковий час, що залишився до досягнення пункту призначення.
...
- Виберіть **Вигляд за замовчуванням**, щоб надати пріоритет одному з наступних значень: *Розрахунковий час прибуття*, *Час до прибуття*, *Відстань*.
- У **Пріоритет відображення** виберіть між *Пункт призначення перший (за замовчуванням)* або *Проміжний перший*, щоб надати пріоритет найбільш релевантній інформації про маршрут.
...
| Натисканням | Натисніть кнопку зі стрілкою, щоб відкрити панель [Старт/Стоп навігація](../navigation/setup/route-navigation.md#start--stop-navigation) |
| Довге натискання | Відкриває [Контекстне меню віджета](../widgets/configure-screen.md#widget-context-menu) |
...
Віджет відображає відстань, що залишилася до останнього пункту призначення під час руху за розрахованим маршрутом.
...
Віджет показує *Час прибуття* або *Час до прибуття* до першої проміжної точки. Під час навігації час постійно оновлюється, і як тільки проміжна точка пройдена, час буде оновлено до наступної проміжної точки. Наприклад, якщо ви зупините навігацію, *Час до прибуття* залишиться незмінним, а *Час прибуття* буде дорівнювати *Часу до прибуття* плюс *Поточний час*.
...
| Натисканням | Змінюється між *Часом прибуття* та *Часом до прибуття* і навпаки. |
| Довге натискання | Відкриває [Контекстне меню віджета](../widgets/configure-screen.md#widget-context-menu) |
...
Віджет відображає *Час прибуття* або *Час до прибуття* до останнього пункту призначення. Час постійно оновлюється під час навігації та дорівнює часу маршруту від вашого поточного місцезнаходження.
...
> ПРИМІТКА: Переконайтеся, що ваші [карти оновлені](../personal/maps-resources.md#updates) і що ви завантажили всі необхідні регіональні дані, щоб отримати доступ до найновішої інформації про обмеження швидкості.
...
| Натисканням | Якщо увімкнено [голосові підказки](../navigation/guidance/voice-navigation.md), вимовляється наступний маневр та відстань до нього. |
| Довге натискання | Відкриває [Контекстне меню віджета](../widgets/configure-screen.md#widget-context-menu) |
...
| ⚪ Сірий   | > 100 секунд  | Далеко    | Підготуйтеся до повороту |
...
Віджет **Смуги руху** виділяє поточну смугу руху, коли навігація активна, і відображає розташування смуг для фактичної дороги, коли навігація пасивна. Дані беруться з [проєкту OpenStreetMap](https://wiki.openstreetmap.org/wiki/Key:turn).
...
Відображає віджет за налаштуваннями часу наближення. Колір смуги пов'язаний з [голосовими підказками](../navigation/guidance/voice-navigation.md) та часом, що залишився до виконання маневру.
...
## Навігаційні підказки {#route-guidance}
...
Віджети *Навігаційні підказки* містять інформацію про:
...
| Натисканням  | **Наступний поворот**: Якщо увімкнено [голосові підказки](../navigation/guidance/voice-navigation.md), вимовляється наступний маневр та відстань до нього. <br/> **Смуга руху**: Без змін. |
...
Типи попереджень мають різний візуальний дизайн, який залежить від **<Translate android="true" ids="driving_region"/>** і може бути налаштований через *<Translate android="true" ids="shared_string_settings"/> → <Translate android="true" ids="general_settings_2"/>*. OsmAnd не прагне відображати 100% ідентичні дорожні знаки, але вказує на деякі схожості.
...
| *<Translate android="true" ids="show_pedestrian_warnings"/>* | [Пішохідний перехід](https://en.wikipedia.org/wiki/Pedestrian_crossing) (переважно британська англійська) або crosswalk — це місце, призначене для переходу пішоходами дороги, вулиці або проспекту. OsmAnd показує попередження про нерегульований пішохідний перехід.  | ![Alert traffic calming widgets](@site/static/img/widgets/warnings_pedestrian.png)  ![Alert traffic calming us widgets](@site/static/img/widgets/warnings_pedestrian_us.png) |
...
| *<Translate android="true" ids="show_tunnels"/>* | Тунель — це підземний прохід, що проходить під горою, річкою або перевантаженою міською територією. В OsmAnd попередження *Тунелі* містить інформацію про довжину тунелю. Якщо ви перебуваєте в тунелі, воно показує відстань до кінця тунелю.   ![Alert ex_tunnel widgets](@site/static/img/widgets/ex_warning_tunnel.png)  | ![Alert warnings_tunnel widgets](@site/static/img/widgets/warnings_tunnel.png)  ![Alert warnings_tunnel_us widgets](@site/static/img/widgets/warnings_tunnel_us.png) |
