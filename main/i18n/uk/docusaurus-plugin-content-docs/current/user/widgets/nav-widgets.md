...
Віджети навігації вмикаються під час навігації для відображення такої інформації, як відстань, час прибуття або час, що залишився, наступні повороти, азимут, назва поточної вулиці, інформація про смуги руху, максимальна швидкість, сповіщення про наближення, POI та точки маршруту.
...
Віджет **Смуги** виділяє поточну смугу, коли навігація активна, і відображає схему смуг для фактичної дороги, коли навігація пасивна. Також під схемою смуг показується індикатор відстані, що представляє, скільки залишається до маневру, де застосовується керівництво по смугах. Цей віджет доступний лише для верхньої та нижньої панелей. Дані взяті з [проєкту OpenStreetMap](https://wiki.openstreetmap.org/wiki/Key:turn).
...
- Більшість сповіщень відображаються на екрані та оголошуються голосовою підказкою протягом 15 секунд, а також час спрацьовування для проходження точки маршруту, улюбленої точки або POI. Детальніше читайте [тут](../../technical/algorithms/voice-prompt-triggering.md#trigger-table).
...
| *<Translate android="true" ids="show_pedestrian_warnings"/>* | [Пішохідний перехід](https://en.wikipedia.org/wiki/Pedestrian_crossing) (переважно британська англійська) або зебра - це місце, призначене для переходу пішоходами дороги, вулиці або проспекту. OsmAnd показує сповіщення про нерегульований пішохідний перехід.  | ![Віджети сповіщення про пішохідний перехід](@site/static/img/widgets/warnings_pedestrian.png)  ![Віджети сповіщення про пішохідний перехід у США](@site/static/img/widgets/warnings_pedestrian_us.png) |
...
| *<Translate android="true" ids="show_cameras"/>* | [Камери контролю дорожнього руху](https://en.wikipedia.org/wiki/Traffic_enforcement_camera) та [камери контролю швидкості](https://wiki.openstreetmap.org/wiki/Tag:highway%3Dspeed_camera) фіксують порушення правил дорожнього руху, включаючи перевищення швидкості, проїзд на червоне світло та інші види незаконної поведінки. <br />**ПРИМІТКА:** Сповіщення про камери контролю швидкості заборонені або є незаконними в деяких країнах. Щоб повністю [видалити камери контролю швидкості](../personal/global-settings.md#legal) з OsmAnd, перейдіть до головного *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_settings,shared_string_legal,uninstall_speed_cameras"/>*  | ![Віджети сповіщення про камери швидкості](@site/static/img/widgets/warnings_speed_camera.png)   |
...
Віджет **Наближення до POI/Обраного/Точок маршруту** поєднується з [Назвою вулиці](#street-name) і відображає наближення та проходження Точок інтересу, Точок маршруту відстежуваного треку та Обраного вздовж маршруту.  
...
**iOS** - На додаток до відстані до найближчої точки маршруту, також відображається сторона (ліва або права) маршруту, на якій знаходиться точка маршруту.
