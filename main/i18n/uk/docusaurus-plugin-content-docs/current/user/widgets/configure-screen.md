...
- [Маркери карти](../widgets/markers.md).  
    Набір показує відстань або орієнтований час прибуття (ETA) для перших двох маркерів у [списку](https://osmand.net/docs/user/personal/markers#itinerary-list).  
...
Ці налаштування застосовуються лише до вибраного віджета.
...
3. **Видимість текстових міток віджета**

    - Видимість текстових міток залежить від розміру віджета та розташування панелі. У середніх і великих розмірах мітки завжди відображаються.
    - У малому розмірі мітки не показуються на лівій і правій панелях. На верхній і нижній панелях мітка може бути автоматично прихована (коли розташована з правого боку), щоб числові значення залишалися повністю видимими.
    - Немає окремої опції для вмикання/вимикання текстових міток.
...
- **Показати трек на мапі** 
    - Доступно для віджетів [Запису треку](../plugins/trip-recording.md).
    - Відображає ваш поточний записаний трек на мапі.
...
## Макет екрана мапи {#map-screen-layout}
...
Налаштування **Макет екрана мапи** дозволяє контролювати, як панелі віджетів розташовані на екрані мапи. Воно допомагає запобігти накладанню віджетів і кнопок та покращити використання простору на екрані, особливо під час перемикання між портретною та альбомною орієнтаціями.

Ви можете відкрити налаштування макету екрана мапи з меню Налаштувати екран, безпосередньо з дій на екрані або через меню з трьома крапками.

### Одиночні та окремі макети {#single-and-separate-layouts}
...
Перейдіть до: *<Translate ios="true" ids="shared_string_menu,layer_map_appearance"/> → ⋮ → <Translate android="true" ids="map_screen_layout"/>*

OsmAnd підтримує два режими макету для екрана мапи:
- **<Translate android="true" ids="single_layout"/>** – використовує той самий макет віджетів для всіх орієнтацій екрана. Ця опція вибрана за замовчуванням.
- **<Translate android="true" ids="separate_layouts"/>** – дозволяє налаштувати різні макети віджетів для портретної та альбомної орієнтацій.

Коли ввімкнено окремі макети, зміни, внесені в одній орієнтації, не впливають на іншу. Це корисно, якщо ви хочете оптимізувати екран мапи по-різному для портретного та альбомного використання.

### Вкладки портретної та альбомної орієнтацій {#portrait-and-landscape-tabs}

![Map Screen Layout](@site/static/img/widgets/screen_layout_tabs.png)

Коли ввімкнено окремі макети, у Налаштувати екран стають доступними дві вкладки: **Портретна** та **Альбомна**. Кожна вкладка представляє незалежний макет для відповідної орієнтації екрана. Ви можете перемикатися між вкладками, натискаючи на них.

Ви можете копіювати макет між орієнтаціями. Щоб отримати доступ до цих дій, натисніть меню з трьома крапками для панелі віджетів:
- **<Translate android="true" ids="copy_from_portrait_layout"/>** – копіює список віджетів з портретного макету до альбомного макету.
- **<Translate android="true" ids="copy_from_landscape_layout"/>** – копіює список віджетів з альбомного макету до портретного макету.

Копіюється лише список віджетів, і операція застосовується в межах того самого профілю.
...
Налаштування **Макет панелей** контролює, як відображаються верхня та нижня панелі віджетів. 

**Примітка:** Ця опція доступна лише коли [Окремі макети](#single-and-separate-layouts) увімкнено.

Ви можете вибрати між двома режимами:
- **<Translate android="true" ids="panels_layout_wide"/>** – верхня та нижня панелі займають повну ширину екрана.
- **<Translate android="true" ids="panels_layout_compact"/>** – верхня та нижня панелі вміщуються між бічними панелями та кнопками мапи.

Макет панелей можна налаштувати незалежно для портретної та альбомної орієнтацій, дозволяючи кращу адаптацію до різних розмірів і орієнтацій екрана.
...
**Кнопки за замовчуванням** дозволяють налаштувати, які попередньо встановлені кнопки мапи будуть видимі на екрані мапи. Кожна кнопка в списку має перемикач Увімкнено/Вимкнено поруч з нею. Ви можете приховати або показати наступні кнопки:
    ...
    - [Моє місцезнаходження](../widgets/map-buttons.md#my-position-and-zoom). Центрує мапу на вашому поточному положенні.

    - [Навігація](../widgets/map-buttons.md#navigation). Важливо для планування маршруту та початку навігації.

    - [Пошук](../widgets/map-buttons.md#search). Відкриває інструмент пошуку для знаходження місць.

    - [Збільшити / зменшити](../widgets/map-buttons.md#my-position-and-zoom). Регулює рівень масштабування мапи, щоб показати більше або менше деталей.

Меню з трьома крапками на екрані кнопок за замовчуванням містить такі дії, як:

    - *Скинути до стандартних*. Дозволяє повернути налаштування кнопок до їх початкового вигляду.
    - *Копіювати з іншого профілю*. Виберіть профіль зі списку, щоб скопіювати налаштування видимості та зовнішнього вигляду кнопок.
    - *Зовнішній вигляд (Лише Android)*. Пропонує різноманітні варіанти налаштування для цього типу кнопок. Щоб змінити зовнішній вигляд кнопки, натисніть потрібну кнопку зі списку та виберіть опцію [Зовнішній вигляд](#button-appearance).
...
**Перейдіть до** (для користувацьких кнопок): *<Translate android="true" ids="shared_string_menu,layer_map_appearance,custom_buttons,quick_action_item"/>* → меню з трьома крапками → *<Translate android="true" ids="shared_string_appearance"/>*  
**Перейдіть до** (для всіх кнопок за замовчуванням): *<Translate android="true" ids="shared_string_menu,layer_map_appearance,shared_string_buttons,default_buttons"/>* → меню з трьома крапками → *<Translate android="true" ids="shared_string_appearance"/>*  
**Перейдіть до** (для конкретної кнопки за замовчуванням): *<Translate android="true" ids="shared_string_menu,layer_map_appearance,shared_string_buttons,default_buttons"/>* → виберіть кнопку → *<Translate android="true" ids="shared_string_appearance"/>*  
...
**Перейдіть до** (для користувацьких кнопок): *<Translate ios="true" ids="shared_string_menu,layer_map_appearance,custom_buttons,shared_string_quick_actions"/>* → меню з трьома крапками → *<Translate ios="true" ids="shared_string_appearance"/>*
**Перейдіть до** (для конкретної кнопки за замовчуванням): *<Translate ios="true" ids="shared_string_menu,layer_map_appearance,shared_string_buttons,default_buttons"/>* → виберіть кнопку → *<Translate ios="true" ids="shared_string_appearance"/>*
...
- **Розмір**. Виберіть розмір кнопки, який найкраще підходить для вашого екрана та виділяється за потреби. 
- **Прозорість фону** (Android) / **Фон** (iOS). Контролюйте видимість фону кнопки. Ви можете встановити його повністю видимим або частково прозорим, або відображати лише тінь рамки та іконку.
...
- [Відстань за дотиком](#distance-by-tap). [⬇](#distance-by-tap)
...
Дозволяє встановити розміщення курсора *[Моє місцезнаходження](../map/interact-with-map.md#my-position-and-zoom)* на мапі OsmAnd. Доступні три варіанти:  
...
Інструмент **Відстань за дотиком** дозволяє вимірювати відстані між точками на мапі без використання [Радіусної лінійки](../widgets/radius-ruler.md#radius-ruler-widget) або створення маршруту.

Існує два сценарії використання:

- **Відстань від вашого місцезнаходження до точки**  
  Торкніться будь-якої точки на мапі, і з’явиться пряма лінія від вашого поточного місцезнаходження до вибраної точки разом із міткою відстані. Лінія залишається на екрані, навіть якщо ви торкнетеся іншого місця.

- **Відстань між будь-якими двома точками**  
  Одночасно торкніться двох точок на мапі (мультитач). Лінія з’єднує дві точки, і відображається виміряна відстань.

Ви також можете налаштувати розмір тексту мітки відстані. Перейдіть до: *<Translate android="true" ids="shared_string_menu,layer_map_appearance,shared_string_other,map_widget_distance_by_tap,text_size"/>*. Виберіть між *<Translate android="true" ids="shared_string_normal"/>* (за замовчуванням) і *<Translate android="true" ids="shared_string_large"/>* (у 1,5 раза більше). Коли вибрано варіант «Великий», автоматично додається додатковий інтервал між лінією та міткою для кращої читабельності. Зміни застосовуються миттєво без перезапуску застосунку.
