...
- Створювати кілька **сторінок** на бічних панелях, вибирати розмір віджетів.
- Вибирати висоту рядка та перемикати піктограми віджетів.
...
Кожна панель підтримує різні поведінки макета:
...
- [Лінійка радіусу](../widgets/info-widgets.md#radius-ruler).  
    Показує відстань між вашим місцезнаходженням та точкою на мапі, обведену [інструментом лінійки радіусу](../widgets/radius-ruler.md).
...
- [Статус OsmAnd Tracker](../widgets/info-widgets.md#tracker-widget).  
    Швидкий доступ до [додатка OsmAnd Tracker](https://play.google.com/store/apps/details?id=net.osmand.telegram).
...
- [Запис поїздки](../widgets/info-widgets.md#trip-recording-widgets).  
    ...
    Включено 4 віджети: *Відстань*, *Тривалість*, *Вгору*, *Вниз*.
...
Функція **Режим рядків** дозволяє додавати стільки рядків, скільки потрібно, до верхньої та нижньої панелей. Ви можете відображати кілька віджетів в одному рядку, збільшуючи обсяг видимої інформації.
...
Режим дозволяє додавати **більше одного віджета в одному рядку** на верхній та нижній панелях, щоб відображати якомога більше інформації.  
...
- **Прості віджети**. Кілька простих віджетів можна додати до одного рядка. Ці віджети можуть відображати *Назву*, *Одиниці вимірювання*, *Значення* та *Піктограму*.
...
- **Організувати віджети за сторінками**. Ця функція корисна, коли для різних завдань потрібні різні набори віджетів, наприклад, для навігації порівняно з переглядом мапи.
...
2. **Приховування піктограм віджетів** (*для верхньої та нижньої панелей*):
...
3. **Індивідуальний розмір для різних рядків** (*для всіх панелей*):
    ...
    - Розмір віджетів можна налаштовувати індивідуально, регулюючи розмір рядка та стовпця.
    - **ПРИМІТКА:** Зміна розміру одного віджета в рядку/стовпці змінить розмір усіх віджетів у цьому рядку/стовпці.

4. **Щоб змінити розмір віджета та видимість піктограми:**

    - Виберіть віджет, який ви вже додали до панелі.  
    ...
    - Перемкніть у положення Вимкнено в полі *Показати піктограму*.
...
**Контекстне меню віджета** надає швидкий спосіб керування та налаштування віджетів безпосередньо з екрана мапи. Ви можете отримати до нього доступ, ***довго натиснувши*** будь-який віджет, щоб налаштувати макет віджетів, не виходячи з мапи.
...
- *Додати віджет вище/нижче:*
    ...
    - Виберіть, чи розмістити новий віджет вище або нижче поточного віджета.
...
OsmAnd дозволяє додавати кілька екземплярів одного віджета до різних панелей або сторінок. Ця функція особливо корисна, коли вам потрібна послідовна інформація в різних видах або профілях.  
...
- Довго натисніть **піктограму праворуч** від віджета.
- Перетягніть його до потрібної позиції у списку віджетів.

***Як видалити віджет:***  

- Виконайте ті ж кроки, що й вище.
- Натисніть **піктограму мінус** ліворуч від назви віджета.
- Підтвердіть видалення, коли з'явиться запит.
...
1. **Кнопки за замовчуванням** дозволяють налаштувати, які попередньо встановлені кнопки мапи відображаються на екрані мапи. Ви можете вибрати приховати або показати наступні кнопки:
    ...
    - [Моє місцезнаходження](../widgets/map-buttons.md#my-location-and-zoom). Центрує мапу на вашій поточній позиції.
...
3. **Вигляд кнопки за замовчуванням** (*Тільки Android*) пропонує різноманітні параметри налаштування для цих типів кнопок. Щоб змінити вигляд кнопки, натисніть потрібну кнопку зі списку та виберіть опцію [Вигляд](#button-appearance).
...
- **Піктограма**. Виберіть зі списку піктограм, які представляють дії, які ви вже додали, що полегшує ідентифікацію функцій з першого погляду.
- **Радіус кута**. Налаштуйте цей параметр, щоб змінити форму кнопки, від гострих, квадратних кутів до плавних, заокруглених країв.
...
- **Непрозорість фону**. Контролюйте видимість фону кнопки. Ви можете встановити його повністю видимим або частково прозорим, або відображати лише тінь рамки та піктограму.
...
- [Відстань за дотиком](../widgets/radius-ruler.md#distance-by-tap). Дає можливість виміряти відстань від вашої позиції до вибраної точки.
...
### Відображення позиції (Позиція місцезнаходження на екрані) {#display-position-location-position-on-screen}
...
- **<Translate android="true" ids="position_on_map_bottom"/>**. *Моє місцезнаходження* розташоване трохи нижче центру екрана. Цей режим дозволяє бачити більше інформації про мапу попереду вашого руху, що корисно під час навігації.
...
Доступ до додаткових дій у меню Налаштувати екран:

- *Android*. Розташовано в меню Налаштувати екран унизу екрана в розділі *Дії*: *Меню → Налаштувати екран → Дії*.
...
- Для пристроїв **Android** ви можете змінювати профілі у верхній частині головного екрана Налаштувати екран за допомогою доступних піктограм профілів.  
- Якщо ви використовуєте пристрій **iOS**, натисніть піктограму профілю у верхньому куті головного екрана Налаштувати екран та виберіть потрібний профіль зі списку, щоб активувати його.  
