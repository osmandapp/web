...
- [Розпочати навігацію](../navigation/index.md). Розпочніть покрокову навігацію.
...
Кнопка **Налаштувати мапу** надає доступ до [меню Налаштування мапи](../map/configure-map-menu.md). Іконка також відображає [Поточний профіль застосунку](../personal/profiles.md), що дозволяє ідентифікувати та перемикатися між різними профілями.
...
1. **Відображення кнопки**. (*Для Android*) Налаштування для кнопки *3D-режим* з'являється у списку Налаштування екрана лише якщо вибрано [Версію 2 (OpenGL)](../personal/global-settings.md#map-rendering-engine) механізму рендерингу мапи.

2. **Зберігає кут**. Після переходу в 3D-режим та ручної зміни кута жестом, кут зберігається та використовується наступного разу при активації 3D-режиму. Ця опція зберігається лише для профілю, вибраного на той момент.
...
[Віджет швидкої дії](./quick-action.md) — це кнопка, що налаштовується, з різними типами дій, які можна призначити. Також можна мати кілька [Користувацьких кнопок](./quick-action.md#custom-buttons).
