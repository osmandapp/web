...
### Градуси та хвилини {#degrees-and-minutes}

![Пошук за координатами Android](@site/static/img/search/coordinates_search_DM_andr.png)  

**<Translate android="true" ids="navigate_point_format_DM"/>**. У цьому форматі координати широти та довготи записуються за допомогою градусів та хвилин. Наприклад, 37:46.29 широти та -122:25.10 довготи для Сан-Франциско. Цей формат може бути менш точним, а його введення може зайняти більше часу.
...
### Градуси, хвилини та секунди {#degrees-minutes-and-seconds}

![Пошук за координатами Android](@site/static/img/search/coordinates_search_DMS_andr.png)  

**<Translate android="true" ids="navigate_point_format_DMS"/>**. Формат координат DMS записує географічні координати, де значення подаються в градусах, хвилинах і секундах. Градуси (DDD) можуть приймати значення від 0 до 180 для довготи та від 0 до 90 для широти. Додатні значення вказують на східну довготу та північну широту, а від'ємні — на західну довготу та південну широту. Хвилини (MM) можуть приймати значення від 0 до 59. Секунди (SS.S) представляють десяткову частину секунди і також можуть коливатися від 0 до 59.9.
...
### Швейцарська сітка (тільки Android) {#swiss-grid}
...
### Система локаторів Maidenhead (тільки Android) {#maidenhead-locator-system}

![Пошук за координатами Android](@site/static/img/search/coordinates_search_maidenhead.webp)

**Maidenhead**. Це система геокодів, яку в основному використовують радіоаматори. Вона кодує географічні місця у коротку послідовність літер і цифр, що зручно для визначення позицій та обміну інформацією про місцезнаходження. Приклад координат у форматі Maidenhead: KN29AS 19CX. Перша пара літер визначає велике поле, за нею йдуть цифри та додаткові пари літер, які підвищують точність визначення місцезнаходження. Детальніше про систему локаторів Maidenhead можна прочитати [тут](https://en.wikipedia.org/wiki/Maidenhead_Locator_System).  
- *Правила введення*. Координати складаються з чергування пар літер і цифр. Довші локатори забезпечують вищу точність позиціонування.  
- *Найбільш поширене використання*. Система локаторів Maidenhead широко використовується радіоаматорами для визначення розташування станцій, радіозв'язку та конкурсів.
...
## Налаштування формату координат (тільки Android) {#customize-coordinate-format}

![Пошук за координатами Android](@site/static/img/search/customising_format.webp) ![Пошук за координатами Android](@site/static/img/search/searching_format.webp)

Ви можете налаштувати список форматів координат, доступних для поточного профілю.  
Перейдіть до: *<Translate android="true" ids="shared_string_menu,configure_profile,general_settings_2,coordinates_format"/>*

На цьому екрані ви можете:  
- Натисніть кнопку *Редагувати*, щоб встановити основний формат координат, змінити порядок або видалити формати координат.
- Натисніть +, щоб додати інший формат координат.
- Відкрийте меню ⋮, щоб відновити список форматів координат за замовчуванням або скопіювати формати координат з іншого профілю.

Під час пошуку за координатами ви можете вибрати формат координат зі списку «Нещодавні» або натиснути *Вибрати інший формат*, щоб переглянути всі доступні системи координат. Ви також можете шукати за назвою системи координат або кодом EPSG.
...
- [Введення координат](../plan-route/coordinate-input.md)
- [Контекстне меню мапи](../map/map-context-menu.md)
- [Інформаційні віджети](../widgets/info-widgets.md)
