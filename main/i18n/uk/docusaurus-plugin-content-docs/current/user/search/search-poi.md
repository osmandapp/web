...
Функціонал [Точок інтересу (POI) OsmAnd](https://wiki.openstreetmap.org/wiki/Points_of_interest) забезпечує точність, наочність та зручність у пошуку цікавих місць, маршрутів та послуг поблизу вашого поточного місцеперебування або вибраної області на карті.

Рядок пошуку дозволяє вводити ключові слова, пов'язані з місцями, що вас цікавлять. Пошук POI також надає зручний список, відсортований за [категоріями](#poi-search-by-categories), щоб допомогти вам швидко знайти те, що ви шукаєте, а [фільтр](#save-new-custom-filters) дозволяє вибрати конкретні характеристики POI для відображення лише відповідних результатів.

[Користувацький пошук](#custom-poi-search) розширює функціональність пошуку, дозволяючи вибирати кілька категорій та підкатегорій. Ви можете зберігати створені фільтри для майбутнього використання, що полегшує швидкий пошук POI, які відповідають певним критеріям або уподобанням. Для отримання додаткової інформації зверніться до розділу про збереження користувацьких фільтрів [тут](#save-new-custom-filters).

OsmAnd пропонує кілька способів переходу до інструменту Пошуку, де знаходиться розділ **Пошук за категоріями**.
...
- **Пошук точок інтересу (POI):**
    ...
    - Результати пошуку показують місцеперебування POI, його контактну інформацію, рейтинги та відгуки.
...
- **Пошук за брендом:**
    - При введенні назви комерційного об'єкта або бренду (наприклад, Audi, Starbucks або Aldi) список результатів пошуку впорядковується з пріоритетом за назвою POI та сортується за відстанню від відповідного місцеперебування.

    - Назва населеного пункту відображається поруч з кожним результатом пошуку, щоб допомогти вам ідентифікувати правильне місцеперебування серед інших на подібній відстані від вас.
    ...
    - Приклад автосалону: *Тип POI* - Автодилер, *назва* - Octo Automobile, [***назва бренду***](https://wiki.openstreetmap.org/wiki/Key:brand) - Audi, BMW, Fiat, Jaguar, Land Rover.

    ![Search POI Android](@site/static/img/search/brand_search_andr.png)


:::note
Для виконання деяких з цих завдань (пошук адрес, POI) вам знадобиться офлайн-файл векторної карти. Спочатку пошук базується на даних, розташованих на карті у видимій області екрана пристрою. Якщо ви нічого не знайдете, OsmAnd пропонує збільшити радіус пошуку.
:::
...
- *Вибір категорії* — Ви вибираєте категорію, що вас цікавить, наприклад, ресторани, готелі, магазини, маршрути OSM, популярні місця Вікіпедії та інші, із запропонованого списку категорій.
- *Фільтри характеристик* — Після вибору категорії програма показує додаткові характеристики, які можна використовувати для уточнення пошуку. Наприклад, для категорії "Ресторани" можна вибрати фільтри, такі як тип кухні (італійська, китайська тощо), ціновий діапазон, рейтинг та наявність парковки.
...
За допомогою функції **Фільтр** ви можете вибрати необхідні характеристики та в результаті отримати список, що містить лише відповідні об'єкти. Щоб використовувати фільтр:

- Виберіть категорію, що вас цікавить, зі списку категорій.
- На екрані, що відкриється, натисніть піктограму, яка відображає фільтр. Вона розташована під полем введення, поруч із *Показати на карті*.
- У фільтрі виберіть тип характеристик.

![Search POI Android](@site/static/img/search/search_poi_filter_icon_andr.png)

Фільтр може складатися з досить великої кількості елементів, наявність яких залежить від обраної категорії. Кожна категорія має низку специфічних для неї фільтрів, організованих у папки за типом. Загалом існує *22 категорії за замовчуванням*. Деякі з них перераховані тут:
...
5. **<Translate android="true" ids="poi_filter_emergency"/>**. [Екстрені служби](https://wiki.openstreetmap.org/wiki/Key:emergency) включають об'єкти та місця, пов'язані з екстреними службами, такі як лікарні, поліцейські дільниці, пожежні станції та медичні центри першої допомоги.
    Не містить жодних фільтрів.

6. **Заправна станція**. [Заправна станція](https://wiki.openstreetmap.org/wiki/Tag:amenity%3Dfuel), також відома як станція заправки, бензоколонка, газова станція та бензиновий гараж. Це роздрібний об'єкт, де можна заправити автомобілі.
...
12. **<Translate android="true" ids="poi_filter_closest_poi"/>**. Включає POI, які знаходяться найближче до вас.
...
18. **Маршрути**. Включає [маршрути OSM](../map/routes.md) для різних видів діяльності.
    Включає *14* типів фільтрів: **Контрольна точка**, **Вузол велосипедної мережі**, **Вузол пішохідної мережі**, **Мережа маршрутів (велосипед)**, **Мережа маршрутів (пішохідна)**, **Категорія точки маршруту**, **Повітряні види спорту**, **Велоспорт**, **Водіння**, **Пішки**, **Мотоциклізм**, **Інші маршрути**, **Водні види спорту**, **Зимові види спорту**.
...
Функція "Перевпорядкувати категорії" дозволяє перерахувати лише потрібні вам категорії та розташувати їх у порядку, який є для вас найзручнішим.
...
- *Щоб приховати* непотрібні категорії, натисніть піктограму **мінус**.
- **Доступно**. Раніше *Приховані* категорії можна повернути до відображеного списку, натиснувши піктограму **плюс**.
- Використовуйте кнопку **Скинути** до стандартних налаштувань, щоб повернути список до попередньо встановленого стану.
- Категорії, створені за допомогою [Користувацького пошуку](#custom-poi-search) або [Фільтрів](#save-new-custom-filters), можна видалити за допомогою піктограми **Видалити**.
...
**Онлайн-пошук** дозволяє знаходити місцеперебування, адреси та точки інтересу в реальному часі. OsmAnd використовує [Nominatim](https://nominatim.openstreetmap.org/ui/search.html), онлайн-геокодер, розроблений OpenStreetMap, який перетворює текстові запити користувачів у географічні координати та навпаки ([*приклад*](#example-of-online-search-query)).

Онлайн-пошук включає такі функції:

1. **Пошук за місцеперебуванням** — Ви можете ввести назву міста, вулиці, району або іншого місцеперебування, і OsmAnd надасть результати, що відповідають запиту.
2. **Геокодування** — Онлайн-пошук перетворює ваші текстові запити в географічні координати, дозволяючи точне визначення місцеперебування.
3. **Зворотне геокодування** — Ви можете ввести географічні координати, і Пошук поверне відповідне місцеперебування як назву міста, назву вулиці або місце на карті.
...
- Онлайн-пошук відбувається в реальному часі, забезпечуючи швидкий доступ до результатів пошуку.
...
Щоб покращити результати, перед використанням інструменту Пошуку збільште карту, щоб наблизитися до місця, де вам потрібно знайти певне місцеперебування. І під час пошуку ви можете **збільшити радіус пошуку**, щоб знайти місця, які знаходяться далі від вашого місцеперебування. Під час пошуку в автономному режимі, якщо потрібні результати недоступні, OsmAnd пропонує використовувати онлайн-пошук.

Ви можете знайти інформацію про онлайн-навігацію в статті [Онлайн-маршрутизація](../navigation/routing/online-routing.md).
...
Користувацький пошук POI — це розширена функція пошуку, яка дозволяє знаходити POI з різних категорій та поєднувати їх для більш точного та персоналізованого пошуку. На відміну від стандартного пошуку POI, який обмежений деякими заздалегідь визначеними категоріями, користувацький пошук POI забезпечує гнучкість у виборі та поєднанні різних типів POI.

Наприклад, ви можете шукати ресторани та аптеки одночасно, щоб знайти найближче місце, де можна пообідати, а потім прийняти ліки. Або шукати найближчу заправку та автосервіс, коли у вас проблема з автомобілем.

Ця функціональність особливо корисна в різних випадках:

- *Подорожі*. Під час подорожей, використовуючи користувацький пошук, ви можете швидко знаходити різні цікаві місця та поєднувати їх для створення оптимального маршруту.
- *Надзвичайні ситуації*. Якщо вам потрібно швидко знайти кілька типів послуг, ви можете використовувати користувацький пошук POI, щоб отримати всю необхідну інформацію.
- *Пошук послуг поблизу*. Користувацький пошук дозволяє поєднувати різні категорії POI для зручного пошуку, якщо вам потрібно знайти кілька типів послуг у певному районі або на певній ділянці маршруту.
...
*Користувацький фільтр POI* включає набори категорій зручностей, що складаються з відповідних значень. OsmAnd дозволяє збирати та поєднувати необхідні типи POI з однієї або кількох запропонованих категорій.

Щоб створити користувацькі фільтри для пошуку місць на карті, виберіть відповідні категорії та підкатегорії, що цікавлять. Ви можете [зберегти](#save-new-custom-filters) ці фільтри для подальшого використання або [редагувати](#edit-an-existing-filter) їх за потреби. Крім того, ви можете переглядати відфільтровані дані без збереження, натиснувши кнопку *Показати*. Однак зверніть увагу, що якщо ви не збережете фільтри, усі вибори будуть скасовані, як тільки ви зміните пошук або вийдете з інструменту пошуку.
...
1. **<Translate android="true" ids="amenity_type_administrative"/>**. Включає 25 типів: *Ділянки, Атол, Район, Місто, Міський квартал, Країна, Суд, Митниця, Дипломатичне представництво, Ферма, Уряд, Село, Острів, Острівець, Окреме житло, Місцевість, Мікрорайон, Поліція, В'язниця, Квартал, Житловий район, Передмістя, Містечко, Ратуша, Селище*.

2. **Різдво**. Включає 5 типів фільтрів:
    *Різдвяна подія, Різдвяний ринок, Різдвяна піраміда, Різдвяний магазин, Різдвяна ялинка*.

3. **<Translate android="true" ids="amenity_type_education"/>**. Включає 16 типів: *Ділянки, Атол, Район, Місто, Міський квартал, Країна, Суд, Митниця, Дипломатичне представництво, Ферма, Уряд, Село, Острів, Острівець, Окреме житло, Місцевість, Мікрорайон, Поліція, В'язниця, Квартал, Житловий район, Передмістя, Містечко, Ратуша, Селище*.

4. **<Translate android="true" ids="amenity_type_emergency"/>**. Включає 14 типів фільтрів.

5. **Аварійна інфраструктура**. Включає 12 типів фільтрів.
...
- Натисніть кнопку *Показати* та виберіть кнопку *Зберегти* для iOS або кнопку з *піктограмою, що вказує на дію збереження* для Android.
...
- Виберіть *[категорію](#poi-search-by-categories) → [піктограму фільтра](#types-of-filters) →* виберіть *типи* характеристик *→ меню з трьома крапками → Зберегти фільтр*.
...
- Натисніть піктограму *Фільтр* поруч із *Показати на карті*, щоб відкрити екран *Фільтри*.
...
- У полі поруч із *Показати на карті* натисніть піктограму, що представляє *фільтр*.
