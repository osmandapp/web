...
Інструмент **Планування маршруту** (*Меню → Планування маршруту*) — це потужна функція програми OsmAnd, яка дозволяє [створювати нові маршрути](#create-new-route) як треки GPX, [редагувати та додавати нові сегменти](#segments) до вже збережених треків, [вимірювати відстані](#distance-measurement) на карті та [прикріплювати сегменти треку](#attach-track-to-roads) до найближчої доступної дороги за допомогою різних навігаційних профілів. Функція розроблена для роботи в *автономному режимі*.
...
Для створення нового треку у форматі GPX використовуйте основну функцію інструменту *Планування маршруту*. Попереднє створення маршруту має багато переваг, на відміну від запису поточного треку за допомогою *[Плагіна запису поїздок](../plugins/trip-recording.md)*. Ви можете [додати](#adding-points) скільки завгодно точок до маршруту, [видалити та перемістити їх](#point-context-menu), змінити типи маршрутів за [сегментами](#route-between-points) та отримати [детальну інформацію про маршрут](#graph).  

За замовчуванням тип маршрутизації відповідатиме раніше вибраному профілю, торкніться значка маршрутизації, щоб вибрати, як програма має розрахувати сегмент для з'єднання точок. Доступний профіль має бути налаштований [окремо](../navigation/routing/osmand-routing.md#routing-types).  

Ви можете **збільшувати та зменшувати масштаб** для зручного розміщення точок під час створення або редагування треку. При **зменшенні масштабу** точки на треку стають невидимими для кращого огляду всієї довжини треку та карти в цілому. Залишаються видимими лише остання додана точка та вказівник для додавання наступної точки.


### Змінення існуючого треку GPX {#modify-existing-gpx-track}
...
:::note
*Пряма лінія* потрібна і буде використовуватися для ділянок, не охоплених даними маршрутизації, таких як бездоріжжя та позатрекові ділянки.
:::
...
- [Використання найближчих доріг](#attach-track-to-roads). Цей режим використовує офлайн-карти для пошуку найближчих доріг для побудови треку, тому дані про висоту будуть отримані з приєднаних доріг. Геометрію треку можна налаштувати.  
- [Використання карт рельєфу](../map/tracks/track-context-menu.md#calculating-missing-elevation). ([OsmAnd Pro](../purchases/android.md#pro-features)) Режим розраховує висоту на основі даних карт рельєфу (3D). Різниця між висотами, записаними вашим пристроєм, може бути використана для корекції висоти. Геометрія треку залишається незмінною.


### Прикріплення треку до доріг {#attach-track-to-roads}
...
Налаштування **Прикріпити до доріг** дозволяє прикріпити записаний або імпортований трек до доріг з офлайн-карт OsmAnd для отримання додаткової інформації:
...
Ви можете вибрати [порогове значення](../navigation/setup/gpx-navigation.md#attach-to-the-roads) для відстані, на якій спрощені точки треку можуть знаходитися від вихідних точок треку.

:::note Невизначений значок профілю
Якщо ви вибираєте трек і поруч зі значком *Налаштування* відображається значок ***невизначеного*** профілю ("?"), торкніться його, щоб вибрати, чи прикріпити трек до доріг, чи просто з'єднати точки прямою лінією та розглядати його як записаний трек.
:::
...
Щоб виміряти відстань або спланувати поїздку, додайте точки в місці *Вказівника* одну за одною та торкніться кнопки *Додати точку*. Перейшовши до **списку точок** нижче, ви можете змінити порядок точок, видалити або отримати доступ до певного [контекстного меню точки](#point-context-menu).
...
Додані точки в редакторі можуть бути з'єднані як пряма лінія або як маршрут між точками вибраного профілю. *Маршрут між точками* можна отримати кількома способами:

1. З меню *Опції* *→* *Маршрут між точками*.
2. Торкніться *значка профілю* в нижньому лівому куті екрана карти. Не торкайтеся верхнього значка, він відкриє меню Налаштування карти.
...
- *Перерахувати маршрути*. Ви можете використовувати перерахунок маршруту без зміни типу профілю. Значок профілю, показаний на запланованому маршруті в інструменті, не зміниться, але тип маршруту відповідатиме вибраному. Це може знадобитися для пошуку альтернативних маршрутів.  

### Сегменти {#segments}

Сегмент треку — це сукупність точок, з'єднаних без **розривів**. В інструменті планування маршруту можна:
...
Кожна точка на вашому маршруті має своє контекстне меню. Воно показує *порядковий номер* точки, *відстань від початку маршруту*, функцію для її переміщення на карті та видалення, а також набір дій з ділянками маршруту до та після цієї точки. Для редагування торкніться потрібної точки у [списку точок](#adding-points) або безпосередньо на карті.

- ***Інформація про точку***. Кількість точок у списку. Відстань до першої точки базується на встановленому типі маршрутизації, відстань може змінюватися при перерахунку на новий тип, якщо параметр *Маршрут між точками* змінено.  

- ***<Translate ios="true" ids="move_point"/>***. Дозволяє змінити положення точки на карті. Переміщуйте карту за допомогою жестів, щоб змінити місце розташування точки. Індикатор точки буде в центрі видимої частини карти.  
...
- ***Зберегти як новий трек*** в [меню Опції](#options) відкриває діалогове вікно, де ви вказуєте назву треку та папку, куди буде збережено маршрут.
- ***Додати зміни до треку***. Прикріплює створений трек як [окремий сегмент](#segments) до іншого існуючого треку. Зміни до вибраного треку *не можуть бути скасовані*.

:::note Опція спрощеного треку
При збереженні нового треку ви можете вибрати опцію ***Спрощений*** трек, щоб зробити трек сумісним з іншими сторонніми програмами. Технічно трек буде збережений без інструкцій маршруту як чисто геометричний трек.
:::
...
- [<Translate android="true" ids="route_between_points"/>](#route-between-points). Показує вибраний профіль програми (за замовчуванням — пряма лінія). Натискання цієї дії те саме, що натискання кнопки *Профіль*, яка відкриває [меню профілів](../personal/profiles.md) програми.
...
- [<Translate android="true" ids="add_to_a_track"/>](#save-route). Додайте свій маршрут до треку з папки треків та збережіть новий трек.
...
- Для **Android** графік відображається на вкладці *Графік* при створенні або відкритті треку, а також при [Навігації](../navigation/setup/gpx-navigation.md) за допомогою *Планування маршруту*.
- Для **iOS** графік доступний лише через меню *[Опції](#options) → Навігація*.

При розрахунку маршруту для навігації в *Плануванні маршруту* ви можете дізнатися додаткову інформацію про трек, таку як [Інформація про висоту](../navigation/setup/route-details.md#elevation-info) та [Атрибути доріг](../navigation/setup/route-details.md#road-attributes), а також скористатися інструментом [Аналіз на карті](../navigation/setup/route-details.md#analyse-on-map). Торкніться кнопки *Деталі* під графіком.  
...
- [Запис поїздок](../plugins/trip-recording.md)
