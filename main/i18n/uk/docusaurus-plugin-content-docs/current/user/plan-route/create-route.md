...
- Оновлений профіль висоти на основі геометрії доріг.
- Можливий перерахунок геометрії треку (спрощення або коригування точок до мережі доріг).

Коли трек прив'язується до доріг, наступні параметри можуть змінитися, оскільки геометрія та висота треку перераховуються: *Відстань, Підйом / Спад, Середня швидкість (та графік швидкості), Максимальна швидкість, Час у русі* та *Тривалість*. Ці значення можуть відрізнятися від оригінального треку GPX, якщо перерахована висота або геометрія на основі доріг значно відхиляється від записаних даних GPS.
...
![Планування маршруту Android](@site/static/img/plan-route/plan_route_points_list_andr.png) ![Планування маршруту Android](@site/static/img/plan-route/plan_route_favorites_and.png)
