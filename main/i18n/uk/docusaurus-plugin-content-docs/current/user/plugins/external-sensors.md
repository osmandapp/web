...
4. OsmAnd скануватиме наявні датчики **Bluetooth LE** або **ANT+** (залежно від вашої платформи).
5. Натисніть кнопку **+** поруч із датчиком, який потрібно підключити.
...
- На **Android** можуть підтримуватися як датчики BLE, так і ANT+ залежно від вашого пристрою. Докладніше див. у [посібнику сумісності ANT+](https://www.thisisant.com/consumer/ant-101/ant-in-phones).
...
Кожен підключений зовнішній датчик, перелічений в OsmAnd, має власний екран конфігурації з такими параметрами:
...
- На **iOS** ви можете відображати **рівень заряду батареї** підключених датчиків BLE безпосередньо у [віджеті](#widgets) карти. Щоб увімкнути це, перейдіть до: *Меню → Налаштувати екран → Вибрати панель → Додати віджет → Зовнішній датчик → Налаштування → Показати → Рівень заряду батареї*
- З'являється спливаюче сповіщення, коли рівень заряду батареї **підключеного датчика падає нижче 15%**.
...
Щоб записати дані датчика за допомогою [плагіна «Запис поїздки»](../plugins/trip-recording.md#recording-settings) під час вашої поїздки:
...
4. Почніть запис поїздки. OsmAnd збереже вибрані дані датчика в трек GPX.
...
Після запису поїздки за допомогою [плагіна «Запис поїздки»](../plugins/trip-recording.md#recording-settings) ви можете переглянути дані датчика в інтерфейсі аналізу треку.
...
3. Виберіть *Параметри → Аналізувати на карті* з контекстного меню треку.
...
- Відкрийте *Налаштувати екран* і додайте необхідні віджети до відповідної панелі.
...
За замовчуванням віджет показує перший підключений датчик цього типу. Якщо ви хочете використовувати певний датчик, натисніть **Джерело даних** і виберіть зі списку підключених датчиків.
