...
#### Поведінка імпорту GPX

Під час імпорту файлу GPX OsmAnd створює єдиний об'єкт треку на основі файлу:
- Назва імпортованого треку походить від назви файлу GPX. Елемент `<name>` всередині `<trk>` не використовується як основна назва треку під час імпорту.
- Якщо файл GPX містить кілька елементів `<trk>`, OsmAnd імпортує їх як сегменти одного треку. У цьому випадку значення `<name>` можуть використовуватися як мітки для відповідних сегментів у межах треку.
...
| `<split_interval>` | - Встановлює інтервал для сегментації треку на основі обраного типу. <br/> - *Подвійне:* Ціле число (метри для `"distance"`, секунди для `"time"`) |
...
| `<elevation_meters>` | - Вказує фіксовану висоту в метрах для `"fixed_height"` у `<line_3d_visualization_by_type>`. <br/> - *Число з плаваючою комою:* За замовчуванням: `1000` |
