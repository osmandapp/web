...
Você pode usar o widget para descobrir a elevação do centro do mapa atual acima do nível do mar. Um [mapa de terreno (relevo sombreado e inclinação)](../plugins/topography.md#download-maps) da área necessária é necessário para exibir informações no **widget Elevação: centro do mapa**.
...
2. ***Intervalo de tempo.***
   ...
   `Velocidade Média = MÉDIA(GPS_LOCALIZAÇÃO.velocidade)`.

3. ***Levar paradas em consideração.***

   **Ativado** — Se a velocidade registrada estiver abaixo de 1 unidade por hora, ela é ignorada, evitando quedas desnecessárias na velocidade média ao parar em semáforos ou fazer pequenas pausas.
...
|:--------|:---------|
|![Razão de planagem](@site/static/img/widgets/glide_ratio_3.png)|![Razão de planagem](@site/blog/2023-12-22-android-4-6/img/glide_ratio_2.png)|
...
|Widgets| Widgets na tela|
|:--------|:---------|
|![Razão de planagem](@site/static/img/widgets/glide_ratio_1_widget_ios.png)|![Razão de planagem](@site/static/img/widgets/glide_ratio_widget_ios.png)|

</TabItem>

</Tabs>

O [simples](../widgets/configure-screen.md#widgets-for-all-panels) widget *Razão de Planagem* fornece informações valiosas sobre a razão de planagem para ajudar a tomar decisões informadas ao planejar rotas aéreas. O widget *Razão de Planagem* no OsmAnd ajuda você a avaliar a eficiência de sua descida ou subida ao navegar para um local específico ([marcador](../personal/markers.md) na posição do mapa). Ele calcula a razão de planagem como a razão entre a distância e a diferença de altitude entre sua posição atual e seu destino pretendido. Você pode ler mais sobre isso [aqui](https://en.wikipedia.org/wiki/Lift-to-drag_ratio#Glide_ratio).

**Uso de widgets:**

- *Planejamento de voos de planagem.* Para pilotos de planadores, o widget *Razão de planagem para o alvo* torna-se uma ferramenta indispensável ao planejar e executar voos de planagem. Os pilotos podem estimar o quão longe podem voar até o alvo dadas as condições atuais e a razão de planagem necessária.
- *Otimizar subidas e descidas.* A razão de planagem média permite que os pilotos otimizem subidas e descidas com base nas condições atuais. Os pilotos podem escolher o momento ideal para subidas para maximizar o tempo de voo, ou para descidas para atingir um ponto alvo.
- *Reagindo a condições variáveis.* O widget fornece atualizações contínuas da razão de planagem, permitindo que os pilotos reajam instantaneamente a mudanças nas condições atmosféricas ou rotas. Eles podem ajustar seu plano de voo para garantir segurança e eficiência.
- *Uso máximo da distância horizontal.* Conhecer a razão de planagem permite que os pilotos maximizem o uso da distância horizontal para se moverem até o alvo, o que é especialmente importante ao planejar voos longos.
- *Redução de riscos*. O widget *Razão de planagem para o alvo* ajuda os pilotos a evitar situações em que atingir o alvo se torna impossível devido à razão de planagem insuficiente. Isso ajuda a reduzir riscos e melhorar a segurança do voo.

**Valores Positivos e Negativos:**

- **Valores positivos** indicam que seu destino está em uma altitude mais baixa do que sua posição atual, exigindo descida.
- **Valores negativos** indicam que seu destino está em uma altitude mais alta, exigindo subida. Por exemplo, "-90:1" significa que, após percorrer uma distância de 90 quilômetros, você precisa subir 1 quilômetro para chegar ao seu destino.
...
### Razão de Planagem para o Alvo {#glide-ratio-to-target}

- **Razão de planagem para o alvo** mostra a razão de planagem exata necessária para atingir o ponto alvo.
- **Elevação do alvo** mostra a elevação do ponto alvo. O [marcador do mapa](../personal/markers.md) deve ser usado como ponto alvo.

| | |
|:------------|:------------|
| Ativar | **Android:** *<Translate android="true" ids="shared_string_menu,map_widget_config"/> → Escolha um painel → Adicionar widget → Razão de planagem* |
| | **iOS:** *<Translate ios="true" ids="shared_string_menu,layer_map_appearance"/> → Escolha um painel → Adicionar widget → Razão de planagem* |
| Ao tocar | Altera *Razão de planagem para o alvo* ou *Elevação do alvo* |
| Toque longo | Abre o [Menu de contexto do widget](../widgets/configure-screen.md#widget-context-menu) |

### Razão de Planagem Média {#average-glide-ratio}

- **Razão de planagem média** mostra a razão de planagem média para um intervalo de tempo especificado.
...
| Ativar | **Android:** *<Translate android="true" ids="shared_string_menu,map_widget_config"/> → Escolha um painel → Adicionar widget → Razão de planagem* |
| | **iOS:** *<Translate ios="true" ids="shared_string_menu,layer_map_appearance"/> → Escolha um painel → Adicionar widget → Razão de planagem* |
| Ao tocar | Altera *Razão de planagem média* ou *Velocidade vertical média* |
| Toque longo | Abre o [Menu de contexto do widget](../widgets/configure-screen.md#widget-context-menu) |
...
O widget **Nível da Bateria** exibe a porcentagem da bateria do seu dispositivo diretamente na tela do mapa e é atualizado automaticamente pelo menos uma vez por minuto.
...
| Ao tocar | Alternar entre o tempo restante e o horário do próximo nascer/pôr do sol. |
| Toque longo | Abre o [Menu de contexto do widget](../widgets/configure-screen.md#widget-context-menu) |
...
| Ao tocar | Altera o estado do widget entre os modos de escala preta, escala de cinza e invisível. |
| Toque longo | Abre o [Menu de contexto do widget](../widgets/configure-screen.md#widget-context-menu) |
...
Esses widgets são ativados e funcionam em combinação com seus plugins correspondentes.
...
Os widgets se referem ao [plugin Sensores externos](../plugins/external-sensors.md#widgets). Você pode usá-los para visualizar informações breves de seus sensores externos, como: **<Translate android="true" ids="map_widget_ant_heart_rate"/>, <Translate android="true" ids="map_widget_ant_bicycle_power"/>** (*somente Android*), **<Translate android="true" ids="map_widget_ant_bicycle_cadence"/>, <Translate android="true" ids="map_widget_ant_bicycle_speed"/>, <Translate android="true" ids="map_widget_ant_bicycle_dist"/>** e **<Translate android="true" ids="external_device_characteristic_temperature"/>**. *No iOS*, você também pode selecionar o **nível da bateria** dos sensores BLE conectados para ser exibido diretamente no widget.
...
Os widgets de clima exibem informações meteorológicas atuais para um ponto central no mapa. Eles são automaticamente ativados na tela quando visualizados com o plugin (*Menu → Clima*), e você pode escolher quais colocar na tela inicial para uso diário (*Menu → Personalizar Tela*). Você pode ler mais sobre isso no artigo [Plugin Clima](../plugins/weather.md).
...
O widget do plugin Estacionamento mostra a distância do centro da tela até o local de estacionamento. Para definir um local de estacionamento no mapa, amplie para o nível necessário e, em seguida, toque longamente no local no mapa para abrir o menu de contexto. Siga [estas configurações](../plugins/parking.md#set-a-spot).
...
![Widget de notas de áudio e vídeo](@site/static/img/plugins/audio-video-notes/audio_video_notes_widget.png)
...
- *Sob demanda*. Estado padrão. A caixa de diálogo de seleção de ação é exibida a cada vez.
...
:::note
Para adicionar o widget Rastreador à tela do mapa, ative o plugin [Rastreador OsmAnd](../plugins/osmand-tracker.md).
:::
...
:::note
Para adicionar widgets de Desenvolvedor à tela do mapa, ative o plugin [Desenvolvimento OsmAnd](../plugins/development.md).
:::
...
| Ao tocar | Alternar entre os modos (*Nível de Zoom*) / Nenhuma alteração (*Outros*) |
| Toque longo | Abre o [Menu de contexto do widget](../widgets/configure-screen.md#widget-context-menu) |
...
- **RAM Total do Aplicativo** — Indica a memória total alocada para o OsmAnd, incluindo memória usada e disponível.
