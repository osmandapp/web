...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_overview_ios_1.png)

</TabItem>

</Tabs>

## Painéis de Widget {#widget-panels}
...
O OsmAnd permite organizar a interface do mapa usando painéis de widget personalizáveis.
...
- **Painéis superior e inferior** exibem widgets em linhas. Você pode colocar um ou dois widgets por linha e adicionar várias linhas, se necessário.
- **Painéis esquerdo e direito** suportam várias páginas. Você pode agrupar widgets em páginas separadas e alternar entre elas.
...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_panels_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_panels_ios.png)
...
O conjunto mostra a distância restante para um destino, ponto intermediário, tempo de chegada a um ponto intermediário ou tempo de viagem para uma rota calculada.
...
Distância do centro da tela até a localização de estacionamento salva.

- [Gravação de viagem](../widgets/info-widgets.md#trip-recording-widgets).
Inicia e para a gravação de trilhas e exibe dados de gravação de trilhas.
...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_rows_1_andr.png) ![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_rows_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_rows_1_ios.png) ![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_rows_ios.png)
...
- **Mover widgets**: Arraste widgets entre diferentes linhas.
...
#### Tipos e Restrições de Widget {#widget-types-and-restrictions}
...
- **Widgets complexos**. Apenas um widget complexo pode ser adicionado por linha. Isso inclui:
...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_settings_1_andr.png) ![Menu de configuração de tela](@site/static/img/widgets/configure_screen_widgets_settings_2_andr.png)
...
Você pode personalizar o tamanho dos **Widgets Simples** nos **painéis Superior, Inferior, Esquerdo e Direito**:
    ...
    - **Grande**. Fornece informações do widget maiores e mais visíveis.
...
- *Configurações*.
    - Acesse as configurações do widget (se disponíveis) para personalizar sua aparência ou comportamento.
...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_remove_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu de configuração de tela](@site/static/img/widgets/configure_screen_remove_ios.png)
...
1. Os **botões padrão** permitem personalizar quais botões de mapa predefinidos são visíveis na tela do mapa. Você pode optar por ocultar ou mostrar os seguintes botões:
    ...
    - [Modo 3D](../widgets/map-buttons.md#3d-mode). Para iOS, o botão está sempre disponível. Para Android, ele aparece na lista e está disponível para configuração se o motor de renderização de mapa [Versão 2 (OpenGL)](../personal/global-settings.md#map-rendering-engine) for selecionado.
    ...
    - [Minha localização](../widgets/map-buttons.md#my-location-and-zoom). Centraliza o mapa na sua posição atual.
...
3. A **aparência do botão padrão** (*Somente Android*) oferece uma variedade de opções de personalização para esses tipos de botões. Para modificar a aparência de um botão, toque no botão desejado na lista e selecione a opção [Aparência](#button-appearance).
...
- [Distância por toque](../widgets/radius-ruler.md#distance-by-tap). Permite medir a distância da sua posição até o ponto selecionado.
- [Velocímetro](../widgets/info-widgets.md#speedometer). Mostra a velocidade atual com base nos dados do GPS e a velocidade limitada com base nos [dados do OpenStreetMap](https://wiki.openstreetmap.org/wiki/Key:maxspeed). Os dados exibidos também dependem da configuração [Tolerância de Limite de Velocidade](../navigation/guidance/voice-navigation.md#speed-limit) do OsmAnd.
...
- **<Translate android="true" ids="position_on_map_bottom"/>**. *Minha localização* é posicionada ligeiramente abaixo do centro da tela. Este modo permite que você veja mais informações do mapa à frente do seu movimento, o que é útil durante a navegação.
- **<Translate android="true" ids="shared_string_automatic"/>**. Posiciona *Minha localização* dependendo da *[Orientação do mapa](../map/interact-with-map.md#map-orientation-modes)* (*Inferior* - para direção do movimento, *Centro* - para todos os outros).
...
O acesso a ações adicionais na tela Configurar está em:
...
- **Redefinir para o padrão**. Este recurso permite redefinir todas as configurações para os valores padrão se você quiser começar do zero, ou usar apenas as predefinidas ao iniciar o OsmAnd.
...
![Menu de configuração de tela](@site/static/img/widgets/configure_screen_switch_2_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Menu de configuração de tela](@site/static/img/widgets/configure_screen_switch_ios.png)
