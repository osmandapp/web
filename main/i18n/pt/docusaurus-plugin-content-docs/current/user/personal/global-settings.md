...
Esta seção contém as configurações básicas do OsmAnd, como a seleção de um [perfil](#default-profile) ao carregar o aplicativo, a seleção do [motor de renderização](#map-rendering-engine) e da [pasta](#data-storage) onde os dados serão salvos.  
...
![Perfil padrão das configurações gerais Android](@site/static/img/personal/profiles/default_profile_andr.png)  

</TabItem>

<TabItem value="ios" label="iOS">

![Perfil padrão das configurações gerais Android](@site/static/img/personal/profiles/default_profile_ios.png)

</TabItem>

</Tabs>

O OsmAnd usa o perfil padrão, *Navegar no mapa*, como o primeiro perfil para novos usuários, e se este perfil tiver sido selecionado por você, ele será usado cada vez que o aplicativo for reiniciado. Você pode selecionar qualquer perfil da lista padrão, incluindo *Último usado* e *Navegar no mapa*.
...
![Configurações de Armazenamento de Perfis Android](@site/static/img/personal/profiles/main_storage.webp) ![Configurações de Armazenamento de Perfis Android](@site/static/img/personal/profiles/media_storage.webp)

Vá para: *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_settings,data_storage,main_storage"/>*

1. A **Memória do dispositivo** mostra quanto espaço livre resta no seu dispositivo. Toque em um campo para acessar o menu [*Mapas e Recursos*](../personal/maps-resources.md#local-menu), onde você pode visualizar e gerenciar informações detalhadas sobre seus dados do OsmAnd.

2. Em **<Translate android="true" ids="change_data_storage_folder"/>**, você pode selecionar uma pasta para salvar os dados do OsmAnd no seu dispositivo.  

    - *Memória interna do aplicativo*. Somente o aplicativo OsmAnd tem acesso aos seus dados e nenhum dos aplicativos externos.
    - *Memória compartilhada*. Não está mais disponível.
    - *Armazenamento externo 1*. Somente 1 aplicativo OsmAnd tem acesso e aplicativos de administração e USB.
    - *Armazenamento multiusuário*. Somente 1 aplicativo OsmAnd tem acesso, mas é compartilhado entre vários usuários Android.
...
Vá para: *<Translate android="true" ids="shared_string_menu,shared_string_settings,osmand_settings,data_storage,media_storage"/>*

A configuração **Armazenamento de mídia** determina onde as fotos, vídeos e arquivos de áudio anexados a favoritos, trilhas e outros itens do OsmAnd são armazenados. Você pode escolher um local de armazenamento dependendo se deseja que os arquivos permaneçam privados ou sejam acessíveis a partir de outros aplicativos.
- **Armazenamento principal** — <Translate android="true" ids="media_storage_main_storage_descr"/>
- **Armazenamento compartilhado** — <Translate android="true" ids="media_storage_shared_storage_descr"/>
- **Pasta da câmera** — <Translate android="true" ids="media_storage_camera_folder_descr"/>
- **Especificado manualmente** — <Translate android="true" ids="media_storage_manually_specified_descr"/>

Há também um interruptor *Sempre manter uma cópia local*. Quando ativado, o OsmAnd copia a mídia importada da Galeria ou Arquivos para seu próprio armazenamento, para que a mídia anexada permaneça disponível mesmo se o arquivo original for excluído da Galeria ou do sistema de arquivos.
...
### Motor de Renderização de Mapas {#map-rendering-engine}
...
![Motor de renderização das configurações gerais Android](@site/static/img/personal/global-settings/rendering_engine_andr.png)  

- Na versão **Android** do aplicativo, você pode alternar entre a renderização de mapa da Versão 1, originalmente implementada no aplicativo OsmAnd, e a Versão 2, que usa a interface de programação OpenGL (Open Graphics Library) para renderizar objetos gráficos bidimensionais e tridimensionais.

- A versão **iOS** usa apenas OpenGL para renderização de mapas.
...
![Análise das configurações gerais Android](@site/static/img/personal/profiles/general_settings_analitics_andr.png)  
...
:::note
Dados sobre sua localização ou os locais que você visualiza no mapa não são coletados.
:::  
...
![Histórico das configurações gerais Android](@site/static/img/personal/profiles/general_settings_history_android.png) ![Histórico das configurações gerais Android](@site/static/img/personal/profiles/general_settings_history_android_new_2.png)
...
![Histórico das configurações gerais Android](@site/static/img/personal/profiles/general_settings_history_android_2.png)

</TabItem>

<TabItem value="ios" label="iOS">  

![Histórico das configurações gerais iOS](@site/static/img/personal/profiles/history_settings_ios_2.png) ![Histórico das configurações gerais iOS](@site/static/img/personal/profiles/history_settings_ios_3.png)
...
![Proxy das configurações gerais Android](@site/static/img/personal/profiles/general_settings_proxy_android.png)
...
Nesta seção das configurações do OsmAnd, você pode configurar as notificações que recebe, o serviço de localização e como as configurações do seu aplicativo são salvas.
...
![Notificações das configurações gerais Android](@site/static/img/personal/profiles/general_settings_notifications_android.png)  

</TabItem>

<TabItem value="ios" label="iOS">

![Diálogos das configurações gerais iOS](@site/static/img/personal/profiles/general_settings_dialogs_2_ios.png)  
...
- [*Diálogo de download de mapa*](../start-with/download-maps.md#downloading-maps) informa que mapas específicos podem ser baixados.
...
- O sistema iOS usa a *API iOS* para determinar a localização.  
...
![Notificações das configurações gerais Android](@site/static/img/personal/profiles/auto_backup_2_andr.png)

- O Android ou o OsmAnd Cloud podem salvar seus dados e configurações do aplicativo.

- Por padrão, suas *configurações do OsmAnd* e marcadores de mapa [*Favoritos*](../personal/favorites.md#automatic-favorites-backup) são automaticamente copiados pelo sistema Android. Isso permite que você os restaure em novos dispositivos ou após reinstalar o aplicativo.
...
![Câmeras das configurações gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_1_andr.png)   ![Câmeras das configurações gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_2_andr.png)
...
![Câmeras das configurações gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_1_ios.png)   ![Câmeras das configurações gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_2_ios.png)
...
Em alguns países ou regiões, o uso de aplicativos de aviso de radar de velocidade é ilegal. Você deve fazer uma escolha dependendo das leis do seu país. Selecione **Manter ativo** e você receberá alertas e notificações de radar de velocidade. Selecione **Desinstalar** e todos os dados relacionados a radares de velocidade, como avisos, notificações e POIs, serão excluídos até que você reinstale completamente o OsmAnd.  

Leia sobre alertas de radares de velocidade em sua rota no artigo Widgets de navegação na seção *[Widget de alerta](../widgets/nav-widgets.md#alert-widget)*.
