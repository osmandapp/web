...
Esta seção contém as configurações básicas do OsmAnd, como a seleção de um [perfil](#default-profile) ao carregar o aplicativo, a seleção do [motor de renderização](#map-rendering-engine) e da [pasta](#data-storage-folder) onde os dados serão salvos.
...
![Perfil Padrão de Configurações Gerais Android](@site/static/img/personal/profiles/default_profile_andr.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Perfil Padrão de Configurações Gerais Android](@site/static/img/personal/profiles/default_profile_ios.png)

</TabItem>

</Tabs>

O OsmAnd usa o perfil padrão, *Navegar mapa*, como o primeiro perfil para novos usuários, e se este perfil foi selecionado por você, ele será usado cada vez que o aplicativo for reiniciado. Você pode selecionar qualquer perfil da lista padrão, incluindo *Último usado* e *Navegar mapa*.
...
![Configurações de Armazenamento de Perfis Android](@site/static/img/personal/profiles/settings_data_storage_andr.png)
...
2. Em **<Translate android="true" ids="change_data_storage_folder"/>**, você pode selecionar uma pasta para salvar os dados do OsmAnd em seu dispositivo.
    ...
    - *Armazenamento externo 1*. Apenas 1 aplicativo OsmAnd tem acesso e aplicativos Admin e USB.
...
![Armazenamento de Configurações Gerais iOS](@site/static/img/personal/profiles/general_settings_storage_ios.png)
...
![Armazenamento de Configurações Gerais iOS](@site/static/img/personal/profiles/files-1.png) ![Armazenamento de Configurações Gerais iOS](@site/static/img/personal/profiles/files-2.png)
...
![Motor de renderização de configurações gerais Android](@site/static/img/personal/global-settings/rendering_engine_andr.png)
...
| Sobreposição / subposição de símbolos transparentes | Suporta todas as configurações. | Não suporta uma camada raster sobre o texto vetorial. |
...
![Análise de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_analitics_andr.png)
...
Um **UUID** (*Unique User Identifier*) é gerado para cada instalação do aplicativo OsmAnd e enviado aos servidores durante o carregamento do mapa.
...
![Histórico de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_history_android.png) ![Histórico de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_history_android_new_2.png)
...
![Histórico de Configurações Gerais iOS](@site/static/img/personal/profiles/history_settings_ios.png) ![Histórico de Configurações Gerais iOS](@site/static/img/personal/profiles/history_settings_ios_4.png)
...
- ***Fazer backup como arquivo*** (*Android*) ou ***Exportar*** (*iOS*) é usado para fazer [um arquivo de backup](../personal/import-export.md#export) do seu histórico.
...
![Histórico de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_history_android_2.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Histórico de Configurações Gerais iOS](@site/static/img/personal/profiles/history_settings_ios_2.png) ![Histórico de Configurações Gerais iOS](@site/static/img/personal/profiles/history_settings_ios_3.png)
...
- Para excluir ou exportar vários registros simultaneamente, selecione as caixas de seleção ao lado dos registros desejados e toque nos botões **Exportar** ou **Excluir**.
...
![Proxy de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_proxy_android.png)
...
![Notificações de Configurações Gerais Android](@site/static/img/personal/profiles/general_settings_notifications_android.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Diálogos de Configurações Gerais iOS](@site/static/img/personal/profiles/general_settings_dialogs_2_ios.png)
...
- Você pode escolher quais serviços *Google Play* ou *Android API* o OsmAnd usará para determinar sua localização. Isso pode ser útil para dispositivos sem *Google Play Services* ou em caso de localização imprecisa ou dados de altitude ao gravar rotas.
...
![Notificações de Configurações Gerais Android](@site/static/img/personal/profiles/auto_backup_2_andr.png)

- O Android ou o OsmAnd Cloud podem salvar seus dados e configurações do aplicativo.

- Por padrão, suas *configurações do OsmAnd* e marcadores de mapa de [*Favoritos*](../personal/favorites.md#automatic-favorites-backup) são automaticamente copiados pelo sistema Android. Isso permite que você os restaure em novos dispositivos ou após reinstalar o aplicativo.
...
- Para encontrar os arquivos de backup, use um aplicativo gerenciador de arquivos de terceiros.
    *NOTA: Você pode encontrar os arquivos de backup usando o gerenciador de arquivos do Android: Android → data → net.osmand → files → backup, mas esteja ciente de que nas versões recentes do Android, o acesso a diretórios do sistema é limitado.*
...
O OsmAnd pode [avisá-lo sobre radares](../navigation/guidance/navigation-settings.md#screen-alerts), mas apenas se as leis do seu país permitirem.
...
![Câmeras de Configurações Gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_1_andr.png) ![Câmeras de Configurações Gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_2_andr.png)
...
![Câmeras de Configurações Gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_1_ios.png) ![Câmeras de Configurações Gerais Android](@site/static/img/personal/profiles/uninstall_speed_camera_2_ios.png)

</TabItem>

</Tabs>

Esta configuração permite ativar ou desativar POI com radares, você precisa reiniciar o aplicativo OsmAnd para que as alterações sejam aplicadas.

Em alguns países ou regiões, o uso de aplicativos de aviso de radar é ilegal. Você deve fazer uma escolha dependendo das leis do seu país. Selecione **Manter ativo**, e você receberá alertas e notificações de radar. Selecione **Desinstalar** e todos os dados relacionados a radares, como avisos, notificações e POI, serão excluídos até que você reinstale completamente o OsmAnd.

Leia sobre alertas de radares em sua rota no artigo Widgets de navegação na seção *[Widget de alerta](../widgets/nav-widgets.md#alert-widget)*.
