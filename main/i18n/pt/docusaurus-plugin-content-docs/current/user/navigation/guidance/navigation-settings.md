---
source-hash: 4edc8227e12e54c71b7dd2deeee541b10757820486015a14fa18e3b0e954bbf8
sidebar_position: 7
...
- Toque no ícone *Navegação* no ecrã do mapa e, em seguida, selecione o *ícone Definições → Definições de navegação*.
...
:::info
O perfil predefinido *Navegar no mapa* não tem configurações de navegação.
:::
...
- **Navegação online**
Utiliza principalmente um sistema de roteamento especial ou website que permite à aplicação OsmAnd traçar uma rota com base em informações atuais e outros fatores. Pode ler sobre como escolher a melhor rota online no artigo [Roteamento online](../routing/online-routing.md). Este tipo de navegação só está disponível para a *versão Android* da aplicação.
...
O tipo de navegação determina quais regras são usadas para calcular rotas. Se precisar de um tipo específico de navegação, pode **importar o seu próprio ficheiro *XML* de roteamento modificado**. Leia mais sobre as regras de roteamento na nossa [página do GitHub](https://github.com/osmandapp/OsmAnd-resources/blob/master/routing). E há um artigo separado sobre o formato XML e como usá-lo, [Estilo de renderização do mapa](../../../technical/osmand-file-formats/osmand-rendering-style.md), que pode ajudá-lo a criar um ficheiro de rota.
...
**<Translate android="true" ids="route_parameters"/>** são configurações que determinam como a aplicação calculará uma rota para chegar a um destino, considerando fatores como tipo de transporte, prioridades de roteamento, condições da rota, complexidade da rota e preferências do utilizador.

Para cada [perfil](../../personal/global-settings.md#default-profile), exceto para *Navegar no Mapa*, o OsmAnd seleciona por padrão o [Tipo de navegação](#navigation-type) ideal e os [parâmetros de rota](../routing/osmand-routing.md#routing-types) relevantes. No entanto, pode escolher qualquer tipo que desejar e alterar os parâmetros dependendo das suas preferências e das condições externas da próxima viagem.
...
|:------------|:---------------|:---------------|
| *<Translate android="true" ids="route_recalculation_dist_title"/> em caso de desvio* | <Translate android="true" ids="recalculate_route_distance_promo"/> | Se os [Comandos de voz](../guidance/voice-navigation.md#voice-settings) estiverem ativados, o OsmAnd anuncia que a rota está a ser recalculada. |
...
Ir para: *Menu → Definições → perfil da aplicação → Definições de navegação → Parâmetros da rota → Desenvolvimento*
...
| *<Translate android="true" ids="use_live_routing"/>* (*apenas Android*) | Utilizar [atualizações OsmAnd Live](../../personal/maps-resources.md#osmand-live) durante o roteamento. Note que recomendamos usar esta opção apenas para fins de teste. |
...
A primeira parte do artigo *[Comandos de Voz / Notificações](./voice-navigation.md)* descreve como configurar os comandos de voz em detalhe.
...
Se selecionar um percurso como ponto de destino, precisa de usar a opção [Anexar às estradas](../setup/gpx-navigation.md#attach-to-the-roads) para que os comandos de voz funcionem corretamente.

Os *comandos de voz* permitem-lhe concentrar-se na estrada e não se distrair com o mapa ou o dispositivo de navegação. Também melhoram a segurança na condução, reduzem o tempo necessário para navegar em terrenos desconhecidos e podem ser úteis para diferentes [tipos de rotas](../routing/osmand-routing.md#routing-types).
...
Ir para: *<Translate android="true" ids="shared_string_menu,configure_profile,routing_settings_2"/>*
...
Ir para: *<Translate ios="true" ids="shared_string_menu,shared_string_settings,application_profiles,routing_settings_2"/>*
...
- **Limite de velocidade** ([informação OSM Wiki](https://wiki.openstreetmap.org/wiki/Speed_limits)). Na configuração de Alertas no Ecrã, o *Limite de velocidade* é exibido apenas no ecrã com *Avisos de tráfego* ativados. Se quiser ver os limites de velocidade durante a navegação sem outros avisos, use o [widget](../../widgets/nav-widgets.md#speed-limit).
- **Avisos de tráfego** ([informação OSM Wiki](https://wiki.openstreetmap.org/wiki/Key:hazard#Traffic_hazards)). Informações adicionais podem ser encontradas no artigo [Ecrã do mapa durante a navegação](https://osmand.net/docs/user/navigation/guidance/map-during-navigation/#traffic-warnings).
...
Ir para: *<Translate android="true" ids="shared_string_menu,configure_profile,routing_settings_2,map_during_navigation"/>*

</TabItem>

<TabItem value="ios" label="iOS">

Ir para: *<Translate ios="true" ids="shared_string_menu,shared_string_settings,application_profiles,routing_settings_2,map_during_navigation"/>*
...
## Personalizar Linha da Rota {#customize-route-line}

<Tabs groupId="operating-systems">

<TabItem value="android" label="Android">

Ir para: *<Translate android="true" ids="profile_type_user_string,shared_string_settings,configure_profile,routing_settings_2,customize_route_line"/>*
...
Ir para: *<Translate ios="true" ids="shared_string_menu,shared_string_settings,application_profiles,routing_settings_2,customize_route_line"/>*
...
Com a configuração **Personalizar Linha da Rota**, pode escolher a aparência da linha da rota que será visível no mapa durante a navegação. Pode selecionar a *cor*, a *largura da linha* e a exibição das *setas de viragem*. Para cada perfil, pode escolher uma visualização de linha diferente. Todos estes parâmetros são descritos em detalhe no artigo *Ecrã do mapa durante a navegação* na secção [Aparência da linha da rota](../../navigation/guidance/map-during-navigation.md#route-line-appearance).
...
1. Defina a [*Velocidade Padrão* ou *Velocidades de Estrada*](#default-speed--road-speeds), como a [velocidade mínima e máxima](#road-speeds) do veículo. Isso ajudará a aplicação a determinar o tempo necessário para concluir a rota e permitirá que ela escolha a melhor rota, considerando os limites de velocidade em diferentes segmentos da estrada.
...
3. Insira a [*capacidade do seu depósito de combustível*](#fuel-tank-capacity) para rastrear com precisão o seu nível de combustível e consumo.
...
A configuração correta dos parâmetros na aplicação OsmAnd ajudá-lo-á a evitar problemas ao navegar numa rota, a escolher a mais adequada de acordo com o tipo de veículo e as restrições da estrada, e a calcular o tempo da sua viagem.
...
- Se selecionar manualmente o parâmetro de medição do veículo, a aplicação irá oferecer-lhe o valor mais próximo da lista pronta. Isso é necessário para evitar erros e construir a rota de forma mais correta.
...
A configuração **Combustível usado pelo motor** só está disponível em navegação baseada em veículos, como *[Carro, Motocicleta e Camião](../../navigation/routing/car-based-routing.md)*.
...
A configuração **Capacidade do depósito de combustível** só está disponível em navegação baseada em veículos. Para tipos de navegação como *Bicicleta*, *Equitação*, *A pé* e *Esqui*, este parâmetro não é exibido nos Parâmetros do Veículo.
...
Para garantir cálculos e sugestões de navegação precisos, verifique se a unidade selecionada corresponde ao padrão de medição do seu depósito.
...
Para todos os tipos de navegação, a velocidade mínima e máxima permitida, tanto por padrão como por estrada, pode ser diferente, de acordo com certos valores ([Limites de velocidade padrão](https://wiki.openstreetmap.org/wiki/Default_speed_limits)) e restrições para veículos. A velocidade também é definida em certos incrementos para tornar as configurações da aplicação o mais utilizáveis possível. Para os perfis *Caminhada*, *Equitação* e *Ciclismo*, em pequenos incrementos equivalentes a 0,1 km/h ([Unidades e formatos](https://osmand.net/docs/user/personal/profiles#units--formats)), e para os outros perfis, em incrementos equivalentes a 1 km/h.
As configurações de velocidade determinam quando os [avisos de voz](../guidance/voice-navigation.md) são ativados.
...
- se forem selecionados percursos gerados pelo utilizador ou importados.

#### Velocidade Padrão {#default-speed}

A **Velocidade padrão** é a velocidade de movimento predefinida. É utilizada para calcular o tempo de chegada e determinar a rota ideal com base na velocidade de movimento que a aplicação considera típica para este modo de transporte. Por exemplo, carro, transporte público, pedestre, ou a velocidade que definiu manualmente.
...
:::note
Estas configurações no iOS estão disponíveis em *Menu → Definições → perfil da aplicação → Definições de navegação → Parâmetros da rota → Velocidades da estrada*.
:::
-->
...
## Orientação Detalhada do Percurso {#detailed-track-guidance}

<Tabs groupId="operating-systems">

<TabItem value="android" label="Android">

Ir para: *<Translate android="true" ids="shared_string_menu,configure_profile,routing_settings_2,detailed_track_guidance"/>*
...
Ir para: *<Translate ios="true" ids="shared_string_menu,shared_string_settings,application_profiles,routing_settings_2"/>* → *Orientação detalhada do percurso*
...
A funcionalidade **Orientação detalhada do percurso** melhora a precisão da navegação de rota ao utilizar **rotas baseadas em percursos**. Ao selecionar um percurso como destino, pode ativar a configuração [Anexar às estradas](../setup/gpx-navigation.md#attach-to-the-roads) a partir do menu [Seguir percurso](../setup/gpx-navigation.md#follow-track-options). Isto garante que o percurso está alinhado com as estradas existentes para uma navegação mais suave e precisa.

Existem duas opções para utilizar a *Orientação detalhada do percurso*:

- **<Translate android="true" ids="ask_every_time"/>** (*configurações padrão*) – A opção *Anexar às estradas* aparecerá na secção [Detalhes da Rota](../setup/route-details.md) cada vez que uma rota baseada em percurso for calculada. Isto permite-lhe decidir se aplica o anexo à estrada para cada sessão de navegação.

- **<Translate android="true" ids="shared_string_always"/>** – O processo de anexo à estrada será aplicado automaticamente para cada rota de *navegação por percurso* sem exigir confirmação manual.

Para mais informações sobre como usar a navegação baseada em percursos, visite [Navegação por Percurso](../setup/gpx-navigation.md).

## Artigos Relacionados {#related-articles}

- [Parâmetros da rota](../routing/osmand-routing.md#routing-types)
- [Preparação da rota](../setup/route-navigation.md)
- [Navegação por percurso](../setup/gpx-navigation.md)
