...
Durante a navegação, o mapa é uma ferramenta visual para localizar sua posição, identificar seu destino, planejar sua rota e visualizar a orientação de navegação. Você pode usar o zoom para fazer isso e arrastar e girar o mapa conforme necessário. O mapa também pode exibir informações da estrada, nomes de ruas, edifícios e outros objetos para ajudá-lo a determinar facilmente sua rota.
...
|:------------|:---------------|:---------------|
| *<Translate android="true" ids="choose_auto_follow_route"/>* | O tempo pelo qual a visualização do mapa é sincronizada com a posição atual após o movimento. | *Valor:* <br /> Nunca, 5 seg, 10 seg, 15 seg, 20 seg, 25 seg, 30 seg, 45 seg, 60 seg, 50 seg. |
...
| *<Translate android="true" ids="approximate_bearing"/>* | Determina a direção com base na rota de navegação que você está seguindo. | A configuração só é visível quando o [plugin de desenvolvimento do OsmAnd](../../plugins/development.md) está ativado. <br /> Essas configurações devem ser ativadas se a orientação do mapa direcional estiver invertida ou tremeluzir ao usar o [Android Auto](../auto-car.md#common-issues-and-solutions). |


## Mostrar Pontos ao Longo da Rota {#show-points-along-the-route}

A configuração *Mostrar ao longo da rota* permite configurar parâmetros adicionais da rota e é necessária para o funcionamento de widgets como [Nome da Rua](../../widgets/nav-widgets#street-name) e [Widget de Alerta](../../widgets/nav-widgets.md#alert-widget). As opções incluem exibir [**POIs**](#points-of-interest-pois) e [**Meus Favoritos**](#my-favorites) ao longo da rota ou usá-los como um acréscimo aos já configurados para o perfil, bem como exibir uma lista completa de [**Alertas de Tráfego**](#traffic-warnings) ao longo da rota.
...
- Para a configuração *Mostrar ao longo da rota*, é recomendado usar [Avisos de Voz](../guidance/voice-navigation.md) para navegação.
- *POIs, Favoritos e Alertas de Tráfego* não são exibidos na lista para uma rota que você já percorreu.
...
Quando você especifica a exibição de certos POIs no *Configurar mapa*, todos eles são exibidos nos mapas que você baixou, quer você tenha selecionado categorias ou, se não se importa com qual categoria, selecionado POIs próximos.

- O número e a identificação dos POIs são *dependentes da escala*.

- A configuração *Mostrar ao longo da rota* exibe a mesma categoria que na *sobreposição de POIs*, mas você vê a *lista completa* de POIs selecionados de uma vez, começando do ponto da localização atual na distância definida dentro de sua rota.
...
- A lista contém os [tipos de POI](../../map/point-layers-on-map.md#poi-types) selecionados e informações breves sobre cada um deles, como o *ícone do tipo, nome, distância do ponto de localização atual até o POI ao longo da rota*, e *indicações de qual lado da rota em linha reta e a que distância o POI está*.


### Meus Favoritos {#my-favorites}

A lista contém todos os pontos [Favoritos](../../personal/favorites.md#favorite-point) adicionados anteriormente perto da rota que você criou. Assim como nos POIs, você pode selecionar a distância em que esses pontos estão localizados.
...
- Cada ponto contém um nome ou coordenadas, um grupo, a distância do ponto de localização atual até o *Favorito* diretamente na linha da rota, informações sobre a que distância à direita ou à esquerda o ponto está da linha e a direção da rota.


### Alertas de Tráfego {#traffic-warnings}

*Alertas de tráfego* não são exibidos diretamente no mapa como *POIs* ou *Meus Favoritos*.
...
- A escolha de exibir *Alertas de tráfego* ao longo de sua rota permite que você veja a lista completa pouco antes de iniciar sua rota, o que é útil ao planejar sua viagem.
...
| Parâmetro | Descrição | Observação |
|:------------|:---------------|:---------------|
| **<Translate android="true" ids="screen_alerts"/>** | Notificações, como alertas de tráfego ou limites de velocidade, aparecerão na tela como um widget. Elas aparecem no canto inferior esquerdo enquanto você está navegando. | [Tipos de widgets de alerta](../../widgets/nav-widgets.md#alert-widget) |
...
Você pode selecionar a aparência da linha da rota por estilo ou selecionar manualmente a cor, largura e transparência da linha. Além disso, você pode escolher se deseja exibir setas de curva e setas de direção na linha.
...
A configuração **Cor** altera os tons de cor das linhas da rota. A cor geral delas muda dependendo do tipo selecionado da coleção OsmAnd e de acordo com a **Legenda do Mapa**. Alternativamente, a cor delas se torna a cor e a transparência que você definir manualmente.
...
- ***<ProFeature/> &nbsp; <Translate android="true" id="shared_string_slope"/>***. A linha da rota é colorida em diferentes cores dependendo do perfil de elevação da rota. Uma descrição detalhada está no artigo do *plugin Topografia*, seção [Inclinação](../../plugins/topography.md#hillshade-slope-and-altitude-layers).
...
- [Avisos de voz / Notificações](./voice-navigation.md)
