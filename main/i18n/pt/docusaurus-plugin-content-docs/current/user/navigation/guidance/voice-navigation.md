...
sidebar_position: 6
title: Avisos de Voz / Notificações
---
...
- As notificações de texto refletem totalmente o tempo de acionamento e as mensagens dos avisos de voz.

:::

## Configurando Avisos de Voz {#setting-up-voice-prompts}

O OsmAnd oferece várias opções para controlar os avisos de voz para que você possa seguir sua rota com conveniência. Para configurar essas definições, comece nas seções apropriadas do aplicativo.

- Ative os avisos de voz na seção [Navegação](../guidance/navigation-settings.md) tocando em **Configurações**. Você também pode ativar e configurar os avisos de voz através de *Configurar Perfil* e *Configurações de Navegação*.
- Ative ou desative os avisos de voz usando *Menu → Navegação →* toque no *botão de Som*,
    ou *Menu → Navegação →* botão de Configurações *→ Som →* alternar ligar/desligar.

Para obter mais informações sobre como e quando os avisos de voz são acionados, consulte a documentação [Acionamento de Avisos de Voz de Navegação](../../../technical/algorithms/voice-prompt-triggering.md).
...
**Anúncio**. Permite configurar os seguintes tipos de avisos:

- *Nomes de ruas (TTS), números de saída, avisos de tráfego, faixas de pedestres* e *Túneis.*
- *[Câmeras de velocidade](#speed-cameras)*.
- Também configure o **[widget de Alerta](../../widgets/nav-widgets.md#alert-widget)** para usar com anúncios.

**Pontos do usuário**:

- Ative os avisos de voz para os [Waypoints](../../map/tracks/track-context-menu.md#add-waypoint-to-a-track), [Favoritos](../../personal/favorites.md) ou [POIs](../../map/point-layers-on-map.md#points-of-interest-pois) predefinidos e adicionados. Enquanto dirige, os pontos selecionados serão anunciados quando você se aproximar ou passar por eles.

| Tipo de Aviso | Tempo de Antecedência [s]:<br/>Correspondente<br/>Distância de Antecedência @ Velocidade Padrão [m] | Limite |
| :- | :- | :- |
...
- *Repetir instruções de navegação*. Permite repetir as instruções de navegação em intervalos regulares de 1 min a 30 min. Ou manualmente - se você perder um aviso de voz, pode ouvi-lo novamente tocando simplesmente na [seta de próxima curva](../../widgets/nav-widgets.md#next-turn) na tela do aplicativo.
- *[Tempo de anúncio](#announcement-time)*.

**Saída** (*Somente Android*):

- *[Saída de orientação por voz](#voice-guidance-output)*.
- *Pausar música*. Os avisos de voz pausam a reprodução de música por um tempo.
...
A opção **Limite de velocidade** permite que o OsmAnd o informe quando sua velocidade atual excede um limite especificado. Os dados sobre os limites máximos de velocidade legal são obtidos do OpenStreetMap.

**Anunciar quando excedido**
*Tolerância de limite de velocidade* permite selecionar um desvio aceitável (por exemplo, -10 km/h a +20 km/h) do [limite máximo de velocidade legal](https://wiki.openstreetmap.org/wiki/Key:maxspeed) na estrada atual. Quando sua velocidade excede essa tolerância, o OsmAnd emite um aviso de voz.

*Regras para ativar avisos de voz* quando o limite de velocidade é excedido:
...
- *Atraso de repetição do anúncio*. Se você continuar excedendo a velocidade, o próximo aviso é atrasado por 120 segundos para evitar notificações excessivamente frequentes.
- *Reinicialização do temporizador*. O temporizador é reiniciado se você permanecer dentro do limite legal por 30 segundos. Qualquer excesso de velocidade futuro esperará novamente 5 segundos antes que o próximo aviso soe.
...
O tempo de anúncio dos diferentes avisos de voz depende do perfil selecionado, do tipo de aviso, da velocidade de navegação atual e da velocidade de navegação padrão. Com esta configuração, você pode alterar a distância antes que os avisos de voz sejam ativados aplicando um multiplicador de distância: *<Translate android="true" ids="arrival_distance_factor_normally" />* - 1.0, *<Translate android="true" ids="arrival_distance_factor_early" />* - 1.5, *<Translate android="true" ids="arrival_distance_factor_late" />* - 0.5, *<Translate android="true" ids="arrival_distance_factor_at_last" />* - 0.25.

Na lista suspensa *Intervalos de Tempo e Distância*, você pode visualizar informações detalhadas sobre a ativação de avisos para os diferentes multiplicadores de distância. Para mais informações, consulte o [Acionamento de Avisos de Voz de Navegação](../../../technical/algorithms/voice-prompt-triggering.md).
...
Para evitar a reprodução de áudio no mesmo fluxo de saída ao mesmo tempo, o foco de áudio é implementado no Android. O OsmAnd usará o alto-falante selecionado na lista nesta configuração para saída de áudio. Outros aplicativos pausarão a reprodução ou diminuirão o volume para facilitar a audição dos avisos de voz do OsmAnd.

- Áudio de mídia/navegação.
- Áudio de notificação.
- Áudio de chamada telefônica (para interromper [sistemas de som de carro Bluetooth](../auto-car.md)).

### Teste de Avisos de Voz {#testing-of-voice-prompts}

Você pode testar os avisos de voz com:
...
- [Testar avisos de voz](../../plugins/development.md#application-testing) (*Somente Android*). Use o *plugin de Desenvolvimento* para testar avisos de voz:
    - Vá para *Menu → Plugins → Ativar Desenvolvimento do OsmAnd*.
    - Vá para *Configurações → Testar avisos de voz*.
    - Selecione um idioma e verifique se o sistema reproduz os avisos corretamente.

### Solução de Problemas de Áudio {#solving-audio-issues}

Se você não estiver ouvindo **avisos de voz** ou [avisos de limite de velocidade](#speed-limit):

- Certifique-se de que o volume do seu dispositivo esteja ligado e aumentado.
- Confirme se o som está ativado durante a navegação: *Menu → Navegação → Botão de som Ligar/Desligar* ou *Menu → Navegação → Botão de Configurações → Som* para configuração detalhada de áudio.
...
***Testando avisos de voz:***

Para verificar se o TTS está funcionando corretamente no OsmAnd:

- Ative o **Plugin de Desenvolvimento**: *Menu → Configurações → Plugins → Desenvolvimento do OsmAnd*.
- Abra [Testar avisos de voz](../../plugins/development.md#application-testing): *Menu → Configurações → Plugins → Desenvolvimento do OsmAnd → Testar Avisos de Voz*.
...
### Idioma do Aviso de Voz {#voice-prompt-language}
...
O OsmAnd oferece dois tipos de avisos de voz:

- **<Translate android="true" ids="tts_title"/>**
    - Usa o mecanismo TTS do dispositivo para gerar avisos de voz dinamicamente.
...
- **<Translate android="true" ids="shared_string_recorded"/>** (*Somente Android*)
    - Usa avisos de voz pré-gravados para navegação.
    - Parece mais natural que o TTS, mas tem limitações.
...
#### Configurações de Aviso de Voz em Nível de Sistema {#system-level-voice-prompt-settings}

O comportamento do aviso de voz, incluindo *velocidade de reprodução, tom e pausas*, pode ser ajustado apenas nas configurações do sistema do dispositivo.

**Android:**

1. Abra *Configurações do dispositivo → Acessibilidade → Saída de texto para fala*.
2. Escolha seu mecanismo TTS e idioma preferidos.
3. Ajuste a taxa de fala, tom e velocidade de reprodução.

**iOS:**

1. Abra *Configurações do dispositivo → Acessibilidade → Conteúdo Falado*.
...
Atualmente, há um total de 45 idiomas. Nem todos os idiomas listados abaixo podem ser suportados por todos os mecanismos TTS. Veja [aqui](https://accessibleandroid.com/list-of-languages-with-available-tts-engines-on-android/).
...
## Avisos de Voz Gravados {#recorded-voice-prompts}

### Baixar Pacotes de Voz {#download-voice-packages}

<InfoAndroidOnly />

O uso de vozes gravadas no OsmAnd deve ser apenas um recurso de fallback. Elas são bastante limitadas e não podem pronunciar nomes de ruas, nomes de lugares, etc. Você pode baixar avisos de voz de dois conjuntos diferentes da lista.
...
- O segundo tipo são avisos de voz gravados, com um conjunto incompleto de recursos:
...
- **Complexo**. Adequado para ciclismo de estrada, o padrão complexo usa bipes de diferentes comprimentos e tons para informá-lo sobre as próximas curvas.
...
Uma vez iniciada uma rota, você pode visualizar as informações no menu suspenso do sistema na lista de notificações. As notificações silenciosas do OsmAnd contêm informações como instruções curva a curva, setas de direção de curva, tempo de chegada e tempo restante, velocidade atual e distância até o destino.
...
2. **<Translate android="true" ids="wake_time"/>**. Se *Manter Tela Ligada* estiver ativado, a tela do dispositivo não aplica um tempo limite após o despertar. Se estiver desativado, você pode definir o tempo após o qual a tela do dispositivo será desligada se você não interagir com ela, de 5 a 60 segundos.

    ![Tempo limite após o despertar Android](@site/static/img/navigation/route/timeout_after_wakeup_android.png) ![Tempo limite após o despertar Android](@site/static/img/navigation/route/timeout_after_wakeup_1_android.png)

### Ligar Tela {#turn-screen-on}

![ligar tela Android](@site/static/img/navigation/voice/voice_navigation_Turnscreenon.png)

Selecione as opções de despertar da tela e certifique-se de que o OsmAnd permaneça em primeiro plano quando o dispositivo estiver bloqueado.
...
2. [Teste de Avisos de Voz.](#testing-of-voice-prompts)
