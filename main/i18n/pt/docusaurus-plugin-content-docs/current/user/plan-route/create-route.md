...
Você pode **ampliar e reduzir** para facilitar a colocação de pontos ao criar ou editar uma trilha. Ao **reduzir o zoom**, os pontos na trilha ficam invisíveis para uma melhor visualização de todo o comprimento da trilha e do mapa como um todo. Apenas o último ponto adicionado e o ponteiro para adicionar o próximo ponto permanecem visíveis.
...
A ferramenta *Planejar uma rota* permite modificar trilhas GPX existentes, bem como uma [trilha importada](../personal/tracks/manage-tracks.md#import). No entanto, caso você precise excluir um grande número de pontos com base em critérios genéricos, [<Translate android="true" ids="shared_string_gps_filter"/>](../map/tracks/track-context-menu.md#gps-filter) pode ser uma ferramenta mais adequada.
...
Se os [dados de elevação](../map/tracks/track-context-menu.md#calculating-missing-elevation) estiverem ausentes em uma trilha existente, é possível adicioná-los usando as seguintes ferramentas:
...
Você pode selecionar um [valor de limite](../navigation/setup/gpx-navigation.md#attach-to-the-roads) para a distância que os pontos da trilha simplificados podem ter dos pontos da trilha original.
...
Para medir uma distância ou planejar uma viagem, adicione pontos no local do *Ponteiro* um por um e toque no botão *Adicionar Ponto*. Ao acessar a **lista de pontos** abaixo, você pode reordenar pontos, excluir ou acessar um [menu de contexto de ponto](#point-context-menu) específico.
...
- Unir segmentos: A opção [Unir segmentos](#point-context-menu) remove a lacuna para segmentos previamente separados.
...
- ***Informações sobre o ponto***. O número de pontos na lista. A distância até o primeiro ponto é baseada no tipo de roteamento definido, a distância pode mudar ao ser recalculada para o novo tipo se o parâmetro *Rota entre pontos* for alterado.
...
- ***<Translate ios="true" ids="trim_before"/>***. Você pode cortar toda a seção da rota, pontos e segmentos, do início até o ponto selecionado. A distância a ser cortada é indicada sob o nome da opção.

- ***<Translate ios="true" ids="trim_after"/>***. Você pode cortar uma rota, pontos e segmentos, do último ponto adicionado até o ponto selecionado. A distância a ser cortada é indicada sob o nome da opção.
...
- ***<Translate ios="true" ids="join_segments"/>***. O último ponto em um dos segmentos divididos e o próximo na lista de pontos no outro segmento têm esta configuração de conexão.
...
- ***<Translate ios="true" ids="delete_point"/>***. Remover o ponto selecionado da rota. Semelhante a excluir um ponto em uma lista.
...
- [<Translate android="true" ids="route_between_points"/>](#route-between-points). Mostra o perfil do aplicativo selecionado (o padrão é uma linha reta). Tocar nesta ação é o mesmo que tocar no botão *Perfil*, que abre o [menu de perfis](../personal/profiles.md) do aplicativo.
...
- **<Translate android="true" ids="reverse_route"/>**. Você troca o ponto *Inicial* da rota e o último ponto adicionado. As configurações do segmento de rota não mudam quando a Inversão é aplicada.
...
Durante o planejamento da rota, o [gráfico](../navigation/setup/route-details.md#elevation-graph) exibe informações de altitude da rota e porcentagem de inclinação, além de você poder tocar em qualquer lugar do gráfico para exibir um ponteiro com dados específicos.
