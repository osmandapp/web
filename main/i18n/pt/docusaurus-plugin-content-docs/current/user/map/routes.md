...
O OsmAnd possui muitos recursos poderosos para exibir diferentes [rotas](#type-of-routes) no mapa. Por padrão, essas rotas fazem parte dos [mapas vetoriais](./vector-maps.md#routes) (dados do OpenStreetMap), no entanto, funcionalidade semelhante é fornecida por [trilhas](./tracks/index.md) que podem ser criadas com [Planejar uma Rota](../plan-route/create-route.md), importadas como [trilhas GPX](#save-as-a-track), gravadas com o [plugin de Gravação de Viagem](../plugins/trip-recording.md).


## Tipos de Rotas {#type-of-routes}
...
- ***<Translate android="true" ids="mtb_scale"/>***. Mostra trilhas de acordo com a escala de MTB. Mais informações estão no artigo [Legenda do Mapa](../map-legend/index.md).
...
### Rotas de caminhada {#hiking--walking-routes}
...
O recurso **Trilhas de Moto de Trilha** permite exibir rotas específicas para motos de trilha no mapa com base na tag `dirtbike:scale`. Essa escala destaca trilhas adequadas para motos de trilha, com uma sobreposição colorida mostrando os níveis de dificuldade.

Você pode ativar esse recurso em *Menu → Configurar mapa → Rotas → Rotas de moto de trilha*. O sistema funciona de forma semelhante à escala de MTB, oferecendo uma representação visual de rotas adaptadas para motos de trilha — mais sobre a [`dirtbike` tag pode ser encontrada aqui](https://wiki.openstreetmap.org/wiki/Key:dirtbike:scale).
...
O OsmAnd suporta *áreas de escalada* (`climbing=area`) e *rochedos de escalada* (`climbing=crag`), permitindo que você encontre locais de [escalada em rocha](https://wiki.openstreetmap.org/wiki/Climbing) diretamente no mapa.

- **Áreas de escalada** representam **zonas maiores** onde existem várias rotas de escalada.

- **Rochedos de escalada** referem-se a **formações rochosas específicas** ou paredes adequadas para escalada.
...
Alguns segmentos de estrada podem fazer parte de muitas rotas, mas podem ser coloridos individualmente para representar a dificuldade do segmento em áreas montanhosas. Atualmente, o OsmAnd suporta classificações italianas e suíças, e cada uma tem uma lista específica de requisitos de equipamento para acessar este segmento.
...
As [rotas de cavalo](https://wiki.openstreetmap.org/wiki/Tag:route%3Dhorse) são exibidas como rotas coloridas com símbolos para equitação.
...
As [rotas de águas brancas](https://wiki.openstreetmap.org/wiki/Tag:route%3Dcanoe) são mostradas juntamente com ícones de acesso, zonas de perigo para [turismo de águas brancas](https://wiki.openstreetmap.org/wiki/Whitewater_sports#Whitewater_Map).
...
Você pode personalizar a exibição de certos ***Livros de Viagem*** e tipos de ***trilhas***, exibir trilhas como pontos ou ***Grupos de Pontos*** (por exemplo, para livros de viagem do Wikivoyage). Saiba mais sobre [Guias de Viagem aqui](../plan-route/travel-guides.md).
...
- O OsmAnd pode destacar [rotas presentes no OpenStreetMap](https://wiki.openstreetmap.org/wiki/Relation:route). Elas podem ser selecionadas tocando [no símbolo na rota](#save-as-a-track), e se o conjunto visível de rotas estiver configurado corretamente, você pode seguir a cor e os ícones.
...
No mapa, você pode selecionar rotas para [caminhada, ciclismo, viagem e muito mais](#type-of-routes), que são marcadas com [símbolos OSMC](https://wiki.openstreetmap.org/wiki/Key:osmc:symbol) quando aplicável.
...
Para salvar a rota como um *arquivo GPX*, toque no botão **Salvar (Download)** sob o [painel de Informações](../map/tracks/track-context-menu.md#info-panel). Isso abrirá o painel de botões de [Ações rápidas](../map/tracks/track-context-menu.md#track-actions) onde você terá acesso a ações de trilha, como alterar [Aparência](./tracks/appearance.md), [Navegação](../navigation/setup/route-navigation.md) ou editar [Planejar uma rota](../plan-route/create-route.md).
