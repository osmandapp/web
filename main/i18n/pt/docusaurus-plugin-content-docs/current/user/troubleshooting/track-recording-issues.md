...
### Configurações de Energia Testadas para Android 9, 10 e 11 (Hardy, 2020-08-25) {#tested-power-settings-for-android-9-10-and-11-hardy-2020-08-25}
...
Para conseguir a ativação do GPS, até agora usamos o Gerenciador de Alarmes do Android para ativar o dispositivo periodicamente (também do modo Doze, que foi introduzido no Android 6). Novas versões do Android introduziram os seguintes problemas:

- **(B1) O setRepeating() do Gerenciador de Alarmes tornou-se impreciso a partir do Android 4.4:**  
Mitigação: Agora usamos *setRepeating()* apenas até o Android 4.2, o novo método *setExact()* a partir do Android 4.4, e *setExactAndAllowWhileIdle()* para Android 8+. ([Issue \#5632](https://github.com/osmandapp/Osmand/issues/5632))
...
Nenhuma boa solução encontrada por enquanto. A mitigação atual é que não usamos a ativação do Gerenciador de Alarmes, em vez disso, mantemos o GPS sempre ligado para registro de trajeto em segundo plano em dispositivos com Android 5+ para todos os intervalos de gravação mais curtos que 5 minutos. Isso produz trajetos confiáveis e precisos ao custo de maior uso da bateria. ([Issue \#5632](https://github.com/osmandapp/Osmand/issues/5632))