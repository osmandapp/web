...
- Para **Parar** ou **Salvar** uma trilha de gravação, vá para *<Translate android="true" ids="shared_string_menu,shared_string_my_places"/>* *→* [*guia <Translate android="true" ids="shared_string_gpx_files"/>*](../personal/myplaces.md#tracks) e toque no botão apropriado no campo *Trilha atualmente gravando*.
...
| **Velocidade mínima** | Define um limite para ignorar pontos registrados abaixo de uma certa velocidade.<ul><li>*Efeitos colaterais*: Seções onde a velocidade cai abaixo de um limite especificado não serão registradas.</li><li>*Recomendação*: Use o filtro *Deslocamento mínimo* em vez disso, pois ele pode dar melhores resultados sem perder dados importantes.</li></ul><details><summary>*Observação*</summary>Tente usar a detecção de movimento através do filtro de deslocamento mínimo de registro (B) primeiro, ele pode produzir melhores resultados, e você perderá menos dados. Se suas trilhas permanecerem ruidosas em baixas velocidades, tente valores diferentes de zero aqui. Observe que algumas medições podem não relatar nenhum valor de velocidade (alguns métodos baseados em rede), caso em que você não registraria nada.<br/><br/>Verificação de velocidade > 0: A maioria dos chipsets GPS relata um valor de velocidade apenas se o algoritmo determinar que você está em movimento, e nenhum se você não estiver. Portanto, usar a configuração > 0 neste filtro, em certo sentido, usa a detecção de movimento do chipset GPS. Mas mesmo que não seja filtrado aqui no momento da gravação, ainda usamos esse recurso em nossa análise GPX para determinar a Distância corrigida, ou seja, o valor exibido nesse campo é a distância registrada enquanto em movimento.</details> |
...
Para personalizar sua interface, você pode adicionar ou remover o widget de Gravação de Viagem e [outros widgets](../plugins/trip-recording#additional-widgets) através do menu Configurar Tela.
...
![Modos de Exibição](@site/static/img/plugins/trip-recording/trip_rec_widgets_mode.png) ![Modos de Exibição](@site/static/img/plugins/trip-recording/average_slope_mode.png)

</TabItem>

<TabItem value="ios" label="iOS">

![Modos de Exibição](@site/static/img/widgets/tr_rec_wid_conf_scr_2_new.png) ![Modos de Exibição](@site/static/img/plugins/trip-recording/average_slope_mode_ios.png)
...
**Notas:** Os modos se aplicam à viagem gravada atualmente e são atualizados à medida que a gravação continua.
...
### Widgets Adicionais {#additional-widgets}
