...
3. Defina o [Estilo](#set-nautical-map-style) de mapa náutico para o [perfil](../personal/profiles.md) necessário.
...
Para alterar o estilo de mapa atual para Náutico, você precisa fazer as seguintes configurações:

1. Selecione o [perfil](../personal/profiles.md) necessário.
2. Abra [Configurar mapa](../map/configure-map-menu.md).
3. Role para baixo até Estilo do mapa, abra-o e marque a opção **Náutico**.
...
Os pacotes de contornos de profundidade permitem definir áreas de igual profundidade. Eles são destinados a visualizar mudanças no relevo abaixo da superfície da água. Os contornos de profundidade podem ser baixados para certas áreas e, em seguida, desativados se você não precisar deles.
...
- Altere o estilo do mapa para qualquer outra coisa que não seja *Náutico*.

:::info NOTA
Desativar o plugin Náutico não remove os dados náuticos carregados, então mesmo que você altere o método de renderização de náutico para qualquer outro método quando os dados náuticos são baixados, eles permanecem visíveis no mapa.
:::
...
Os dados do fundo do mar contêm informações sobre a vegetação e o material geral da superfície, como rochas rochosas, conchas, cascalho, coral, lodo, etc. Devido à classificação internacional dos dados do fundo do mar, existem opções para exibir esses detalhes em um mapa: *simples*, *categoria*, *todos* ou *omitir*. Use a [legenda do mapa da área do fundo do mar](../map-legend/nautical-map.md#seabed-area) para mais informações.

- **Simples** (*para a versão Android*) - mostra os símbolos de sinalização marítima de acordo com INT-1 Ref, indicando a natureza da superfície do fundo do mar.
- **Categoria** - além do símbolo de sinalização marítima, também mostra a tag de sinalização marítima relevante, indicando o material natural, ou a categoria de algas e ervas marinhas.
- **Todos** - além do símbolo, tag ou categoria de sinalização marítima, também mostra qualificações como fina, pegajosa, grossa, etc. Para ervas marinhas e algas, mostra dados marcados com as tags *taxon* e *genus*.
- **Omitir** - não mostra detalhes do fundo do mar.

:::info NOTA
Para mais detalhes sobre a classificação dos detalhes da superfície e as opções de renderização, consulte a [wiki de sinalização marítima do OSM](https://wiki.openstreetmap.org/wiki/Seamarks/INT-1_Section_J).
:::
