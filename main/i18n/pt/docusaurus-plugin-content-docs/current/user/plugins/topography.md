...
Topografia é um recurso importante da cartografia que fornece informações para avaliar visualmente o relevo do terreno.
Informações topográficas como [Curvas de Nível](#contour-lines), [Terreno](#terrain) (*Hillshade* e *Inclinação*), e [Relevo 3D](#3d-relief) ajudam a fazer uma avaliação visual da configuração do terreno, observando elevação, relevo, extremos, inclinação ou pontos de igual altura.
...
- [Hillshade](#hillshade-slope-and-altitude-layers). Tipos de mapas com sombreamento de colinas e encostas tornam o relevo mais visível e ajudam a interpretar visualmente o terreno.
...
- [Relevo 3D](#3d-relief). É um [mapa vetorial](../map/vector-maps.md) que fornece uma representação tridimensional do terreno, disponível apenas com [assinatura OsmAnd Pro](../purchases/index.md).
...
| Curvas de Nível | Hillshade | Inclinação |
|:---|:---|:---|
...
Para exibir dados de **Curvas de Nível** e **Terreno (Hillshade, Inclinação)** no mapa:
...
3. [Baixe](#download-maps): Curvas de Nível, Hillshade, Inclinação ou mapas de terreno (3D).
4. **Ative e ajuste**: Curvas de Nível, Hillshade ou Inclinação para a visualização do mapa.
...
Para um trabalho estável e para economizar recursos, você pode baixar um mapa não de todo o país, mas de suas regiões específicas, se tais regiões forem oferecidas no aplicativo. As informações sobre o tamanho de cada tipo de mapa são listadas sob seu nome.
...
Você precisa baixar mapas de **Terreno (3D)** para exibir Hillshade, Inclinação e Relevo 3D. Após baixar os mapas, você pode exibir **Curvas de Nível** e **Terreno** usando a seção [Configurar mapa](../map/configure-map-menu.md) do *Menu Principal*.
...
**As opções de unidade não são intercambiáveis**, portanto, se você precisar alternar de metros para pés ou o inverso, será necessário desinstalar a versão anterior do mapa de Curvas de Nível para baixar a nova versão.
...
- **Versão 1**. As curvas de nível não são suportadas ao usar mapas de blocos obtidos da Internet.
...
A opção **Terreno** ativa e permite personalizar três recursos, como *Hillshade*, *Inclinação* e *Altitude*.
Recursos específicos:

- Apenas uma opção pode ser ativada ao mesmo tempo, seja Hillshade, Inclinação ou Altitude.
...
## Camadas de Hillshade, Inclinação e Altitude {#hillshade-slope-and-altitude-layers}

| Hillshade | Inclinação | Altitude |
| ------ | ------- | ------- |
...
**Hillshade** é baseado na simulação da iluminação da superfície usando dados de terreno. Este método envolve a criação de sombras e realces com base no ângulo da superfície em relação à fonte de luz. Como resultado, você vê colinas naturais, vales e outros detalhes do terreno no mapa.
...
Os mapas raster de **Hillshade**, **Inclinação** e **Altitude** são criados com base em dados de terreno raster, como Modelos Digitais de Elevação (DEM).

**Uso:**

- *Navegação.* Ajuda a identificar encostas íngremes, tanto em declive quanto em aclive, o que pode ser crucial para uma navegação segura.
...
- *Hillshade* usa tons escuros para mostrar encostas, picos e planícies. O Sol Virtual tem um azimute (direção) fixo de 315 graus.

- *Inclinação* usa cores para visualizar a inclinação do terreno. Você pode ler mais sobre isso [aqui](https://en.wikipedia.org/wiki/Grade_(slope)). Cada cor corresponde a um ângulo de desvio da horizontal. Um esquema de cores *Inclinação* adicional, ***Avalanche***, está disponível no menu **Modificar**.
...
> *Leia o artigo [Esquema de Cores](../personal/color-palette-schemes.md) para mais informações.*
...
A função *Visibilidade* é usada para ajustar a transparência das sombras para Hillshade e as cores usadas para representar o ângulo no parâmetro Inclinação.

### Níveis de Zoom {#zoom-levels}

![Zoom](../../../blog/2023-08-28-terrain/img/zoom.png)

A função *Níveis de Escala* permite definir os valores mínimo e máximo dos níveis de zoom do mapa, variando de 4 a 19, nos quais as camadas do mapa Hillshade ou Inclinação serão exibidas.

### Tamanho do Cache {#cache-size}

O **Tamanho do cache** é uma seção informativa que exibe a quantidade de memória em seu dispositivo atualmente usada para dados de *Terreno*. Cada vez que você visualiza informações de *Hillshade* ou *Inclinação* em um mapa, todos esses dados são temporariamente armazenados no cache para acesso rápido e uso posterior, evitando carga adicional no processador do seu dispositivo.
...
**Hillshade** e **Inclinação** são exibidos e ajustados em qualquer [Mecanismo de renderização de mapa](../personal/global-settings.md#map-rendering-engine) selecionado.

1. Se você usar o **Mecanismo de renderização de mapa Versão 1**, você precisa usar o [download](../start-with/download-maps.md) normal de mapas raster de Hillshade e Inclinação.

2. Se você usar o **Mecanismo de renderização de mapa Versão 2 (OpenGL)**:
    - Você pode continuar a usar o tipo de download normal de mapas raster de Hillshade e Inclinação. No entanto, para fazer isso, você precisará ativar o [plugin de desenvolvimento OsmAnd](../plugins/development.md) e habilitar a configuração [Usar formato SQLite raster para hillshade e inclinação](../plugins/development/index.md#terrain).

    - Alternativamente, você pode usar o download do [Mapa de Terreno (3D)](../personal/maps-resources.md#paid-features-for-the-selected-location). Isso economiza espaço de memória em seu dispositivo, e os efeitos de Hillshade, Inclinação e Relevo 3D serão gerados a partir deles usando seu dispositivo.
...
Você pode usar os botões de *Ação Rápida* na tela do mapa para alternar a visibilidade das [Curvas de Nível](#contour-lines), camadas de [Terreno](#terrain) e [esquema de cores do Terreno](#default-color-scheme). Dependendo da camada selecionada no menu Configurar mapa, atribuir uma ação *Terreno* ao botão exibirá *Hillshade*, *Inclinação* ou *Altitude*.
...
O recurso Relevo 3D produz um relevo elevado e fornece uma representação tridimensional da paisagem. O Relevo 3D funciona offline e pode ser usado com [mapas vetoriais OsmAnd](../map/vector-maps.md) ou quaisquer [mapas raster](../map/raster-maps.md#select-raster-maps) como [Fonte de Mapa](../map/raster-maps.md#main) ou como [Subcamada/Sobreposição](../map/raster-maps.md#overlay-layer).
...
**Hillshade** é um tipo de mapa que exibe o terreno usando sombras, criando uma representação visual da inclinação e forma da superfície terrestre.
...
- A combinação de **Curvas de Nível** e **Hillshade** é ideal para estimar visualmente e numericamente a inclinação da encosta da montanha.
- A combinação das camadas de **Curvas de Nível** e **Inclinação** é melhor para estimar a inclinação da encosta e encontrar pontos com a mesma altura.
