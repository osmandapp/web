...
- **Usar formato raster [SQLIte](../../technical/osmand-file-formats/osmand-sqlite) para sombreamento e inclinação** (*Android*). Carrega mapas raster de formato antigo.


### Solução de Problemas {#troubleshotting}

- **Simular sua posição** (*Android* / *iOS*). [Simula](#gpx-track-simulation) a localização e o movimento usando uma trilha GPX.
...
- **Desativar camadas do mapa** (*Android*). Desativa todas as camadas do mapa sobre o mapa vetorial (reinicialização necessária).
...
- **<Translate android="true" ids="write_bearing"/>** (*Android*). <Translate android="true" ids="write_bearing_description"/>. Rolamento é a direção da bússola da sua posição atual para o seu destino pretendido. Afeta a *[gravação de trilhas](../plugins/trip-recording.md)*.
- **<Translate android="true" ids="write_heading"/>** (*Android*). <Translate android="true" ids="write_heading_description"/>. Rumo é a direção da bússola da sua posição atual em direção ao norte verdadeiro. Esta opção está ativada por padrão. Afeta a *[gravação de trilhas](../plugins/trip-recording.md)*.


### Teste de Aplicativos {#application-testing}

:::caution Somente Android
:::

- **Simular primeira inicialização do aplicativo**. Define o sinalizador indicando a primeira inicialização do aplicativo, mantendo todas as outras configurações inalteradas.
...
- A alocação de memória mais alta pode afetar o desempenho de outros aplicativos.
...
- **Estatísticas de renderização (média de 1 / 5 / 15 minutos)**. Mostra estatísticas de renderização, incluindo FPS nos últimos 1k quadros, CPU (blocos), CPU (tempo ocioso) e CPU (em espera) médias em intervalos de 1, 5 e 15 minutos.
...
## Widget FPS de Renderização de Mapa {#map-rendering-fps-widget}
...
O [Widget FPS](../widgets/info-widgets.md#map-rendering-fps) mostra a rapidez com que o mapa e os elementos do mapa são exibidos e atualizados. O valor numérico é indicado em quadros por segundo (FPS).
...
Você pode usar os [widgets de posição da câmera](../widgets/info-widgets.md#camera-widgets) para dar ao mapa do OsmAnd uma aparência realista e combinar fotos da Terra tiradas tanto de cima quanto do espaço.
...
Ao mesmo tempo, devido ao movimento imaginário do visualizador, a distância da câmera à superfície muda.
:::
...
O widget possui duas visualizações que são alternadas ao tocá-lo:
...
- A distorção máxima é observada em zooms grandes (zoom 6-7 para inclinação da câmera de 90 graus).
...
|Inclinação (90) |Latitude |Zoom Máximo |Altura, km |Distorção|
|-----|-----|-----|-----|-----|
|90|26|6|5500|5%|
|90|50|6|2500|5%|
|90|66|7|1300|5%|
|**Inclinação (60)** |**Latitude** |**Zoom Máximo** |**Altura, km** |**Distorção**|
|65|26|8|1100|6.5%|
|65|50|8|800|6.5%|
|65|66|9|630|6.5%|
|**Inclinação (45)** |**Latitude** |**Zoom Máximo** |**Altura, km** |**Distorção**|
|45|26|9|350|7.5%|
|45|50|9|320|7.5%|
|45|66|8|210|7.5%|
|**Inclinação (20)** |**Latitude** |**Zoom Máximo** |**Altura, km** |**Distorção**|
|20| 26|12|30|10%|
|20| 50|11|30|10%|
|20| 66|11|30|10%|
...
O OsmAnd oferece a oportunidade de simular a localização e o movimento do seu dispositivo usando uma trilha GPX. Para esses fins, use a opção *<Translate android="true" ids="simulate_location_by_gpx"/>* no plugin de desenvolvimento do OsmAnd [configurações](#plugin-settings):
...
Após executar a simulação, você verá na tela principal do aplicativo que a marca de navegação está se movendo de acordo com a trilha GPX. Toque no [botão minha localização](../map/interact-with-map#my-location-and-zoom) para sincronizar *Minha Localização* (geolocalização simulada do dispositivo) com o centro do mapa.
Para parar a simulação do movimento do seu dispositivo, volte para as [configurações](#plugin-settings) de desenvolvimento do OsmAnd e toque em **parar** na opção **simular sua posição**.

- *<Translate android="true" ids="simulate_location_by_gpx"/>* também pode ser acessado através do **[Menu de ação rápida](../widgets/quick-action.md#navigation)**.
