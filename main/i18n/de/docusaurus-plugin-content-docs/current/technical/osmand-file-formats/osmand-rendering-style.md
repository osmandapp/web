...
Falls Sie den Standard-Rendering-Stil ändern möchten, müssen Sie die [Definition auf GitHub](https://github.com/osmandapp/OsmAnd-resources/blob/master/rendering_styles/default.render.xml) überprüfen. Unter diesem [Link](https://github.com/osmandapp/OsmAnd-resources/tree/master/rendering_styles) finden Sie alle Arten von Stilen. Wenn Sie also Ihren eigenen Stil erstellen möchten, finden Sie dort das einfachste Beispiel.

Falls Sie sich entscheiden, Ihren eigenen Rendering-Stil zu erstellen, müssen Sie möglicherweise [Ihre eigenen Karten](../map-creation/create-offline-maps-yourself.md#custom-vector-map-tags) erstellen, um die Anzeige benutzerdefinierter Funktionen hinzuzufügen.
...
|---------|-------------|
| **Header** | Besteht aus `name` (Stilname), `defaultColor` (Standardkartenfarbe, die durch das Attribut defaultColor überschrieben werden kann) und `depends` (erbt alle Eigenschaften vom übergeordneten Stil). |
...
| **Order** Abschnitt | Definiert als Block [`<order>`](https://github.com/osmandapp/OsmAnd-resources/blob/master/rendering_styles/default.render.xml#L2876). Jedes Kartenobjekt wird im Order-Abschnitt gesucht, um zu ermitteln, ob es als Punkt, Linie oder Polygon (`objectType`) und in welcher `order` es angezeigt werden soll. **Suchparameter** (Eingabe): `tag`, `value`, `zoom`, `point` (true oder false), `area` (osm type), `cycle` (Start- und Endpunkt sind gleich). **Suchergebnis** (Ausgabe): `objectType` (Punkt = 1, Linie = 2, Polygon = 3), `order` (0-255). Die Reihenfolge des Renderings ist definiert als: Polygone, Linienschatten, Linien, Punkte. Wenn die `order` für Polygone gleich ist, werden die Polygone vom größten zum kleinsten Polygon angezeigt. Hinweis: Polygone sollten sich nicht teilweise überlappen, da sonst das Verhalten nicht definiert ist. |
| **Text** Abschnitt | Definiert als Block [`<text>`](https://github.com/osmandapp/OsmAnd-resources/blob/master/rendering_styles/default.render.xml#L3811). Jedes angezeigte Kartenobjekt wird gesucht, um zu ermitteln, ob es Text anzeigen soll und wie dieser angezeigt werden soll. **Suchparameter** (Eingabe): `tag`, `value`, `zoom`, `nameTag` (jeder Text-Tag wird geprüft). **Suchergebnis** (Ausgabe): `textOnPath`, `textMinDistance` (Abstand zwischen gleichen Texten, z. B. Straßennamen, Referenzen), `textSize`, `textColor`, `textHaloRadius`, `textOrder`, `textDy`, `textBold`, `nameTag2` (zusätzlicher Name in Klammern, falls vorhanden), `textShield` (Hintergrund um Text), `textItalic`, `textWrapWidth`, `intersectionMargin` (zusätzlicher Abstand, um Textüberlappungen zu vermeiden). |
...
- Alle Interna von `<case>` werden sequentiell tief angewendet und Ausgabeparameter werden gesammelt
 - `<case><apply output="1"/><apply_if zoom="15" output="2"> <case>` - Die Ausgabe ist 2, wenn `zoom=15`, und 1 sonst.
...
Auswertungsregeln:
- In jedem 'case', `apply_if`, `switch`, `apply` werden die Eingabeattribute auf Übereinstimmung geprüft (evaluieren zu true), andernfalls wird der Abschnitt übersprungen und die Ausgabeattribute werden nicht gesammelt.
- Alle Regeln werden sequentiell ausgewertet und Ausgabeparameter können durch folgende Regeln überschrieben werden.
...
Rendering-Konstanten können nur mit einem gegebenen Wert verwendet werden: `<renderingConstant name="motorwayShadowRadius" value="1.6"/>` und später in Funktionsselektoren als `<apply_if shadowRadius="$motorwayShadowRadius"/>`.
...
Spezielle Attribute sind `<renderingAttribute>`, die nicht von Selektoren verwendet werden, sondern direkt vom Code abgefragt werden, um bestimmte Funktionen abzurufen, die in der Anwendung gezeichnet werden, wie z. B. eine Navigationsroute, ein Lineal, eine GPX-Spur usw.


| Spezielles Attribut | Beschreibung |
|-------------------|-------------|
| `measureDistanceLine` | Linien-Rendering für die Funktion "Route planen" |
| `markerGuideline`, `markerPlanRouteline` | Rendering von Linien zu den Markierungen |
| `route` | Rendering einer Navigationsroute |
| `gpx` | Rendering einer GPX-Spur |
| `publicTransportLine` | Rendering einer öffentlichen Verkehrs-Navigationsroute |
| `walkingRouteLine` | Rendering einer öffentlichen Verkehrs-Navigation: Fußgängerroute zwischen Haltestellen |
| `rulerLineFont`, `rulerLine`, `rulerCircleAlt`, `rulerCircle` | Wie das Radius-Lineal-Widget gerendert und die Entfernung durch Tippen gemessen wird |
|||
| `defaultColor` | Standardfarbe zum Füllen der Karte (schaltet Nacht-/Tagmodus um) |
| `shadowRendering` | Intern wie Schatten mit Skia gerendert werden |
|||
...
|||
| `debugTextDisplayBBox` | Attribute zum Debuggen der Textdarstellung und -positionierung |
| `debugTextDisplayShieldBBox` | Attribute zum Debuggen der Textdarstellung und -positionierung |
| `debugTextDoNotFindIntersections` | Attribute zum Debuggen der Textdarstellung und -positionierung |
| `debugTextDoNotFindIntersectionsSameName` | Attribute zum Debuggen der Textdarstellung und -positionierung |
| `debugTextDisplayShortRoadNames` | Attribute zum Debuggen der Textdarstellung und -positionierung |

## Kartenstilparameter {#map-style-parameters}

Kartenstilparameter ermöglichen die Kombination mehrerer Rendering-Stile innerhalb einer Dateidefinition, d. h. es ist nicht erforderlich, separate 'my_custom_style_night_mode.render.xml' zu haben. Es ist möglich, einen Parameter wie `night_mode` (standardmäßig aktiviert) zu definieren und bestimmte Regeln (wie Farben) mithilfe dieses Parameters anzupassen. Später in der Benutzeroberfläche ist es möglich, diesen Parameter einfach umzuschalten und einen anderen Kartenstil in OsmAnd zu haben.

Hier ist eine Definition des Parameters `baseAppMode`. `possibleValues` sind Werte, die in der OsmAnd-Benutzeroberfläche angezeigt werden, `category` hilft dabei, zu welcher UI-Kategorie diese Eigenschaft gehört.
```
