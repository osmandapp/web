...
- alle von Text-To-Speech unterstützten Pakete werden mit einer Anwendung gebündelt und aktualisiert
- ein aufgenommenes Paket muss manuell installiert und aktualisiert werden (obwohl die Konfigurationsdatei mit dem App-Update aktualisiert werden könnte)
...
Ein Sprachpaket wird normalerweise als .voice.zip verteilt, wobei die Zip-Datei einen Ordner mit [lang_tts.js](https://github.com/osmandapp/OsmAnd-resources/blob/master/voice/de/de_tts.js) (Haupt-**Konfigurationsdatei**) und ggf. Sounddateien enthält.

Die Konfigurationsdateien sind in JavaScript geschrieben. Alle Konfigurationsdateien sind standardmäßig TTS-Pakete, auch wenn noch kein TTS zur Unterstützung der Sprache vorhanden ist. Später wird die Konfiguration durch die Zuordnung von 'Wörterbuchphrasen' zu 'Namen aufgenommener Dateien' erweitert.
...
### Generierung eines benutzerdefinierten Pakets {#generation-custom-package}
