...
In der Android-Version von OsmAnd kann die Auswahl von *Multiuser Storage 1* als Speicherort zum Löschen aller [lokalen Karten](../personal/maps-resources.md#local-maps) führen, wenn die App automatisch aktualisiert wird, z. B. von Version 4.1.9 auf 4.1.10, 4.1.11 oder höher (**Android 11, 12**). Dieses Problem ist auf [Github](https://github.com/osmandapp/OsmAnd/issues/13404) dokumentiert.

Um den Verlust Ihrer Kartendaten während Updates zu vermeiden, sollten Sie diese Lösungen in Betracht ziehen:

1. *Sichern Sie Ihre Dateien*. Erstellen Sie vor dem Update von OsmAnd ein [Backup](../personal/import-export.md) der Karten, die in *..Android/obb/net.osmand* gespeichert sind. Stellen Sie die Dateien nach dem Update wieder her, bevor Sie die aktualisierte OsmAnd-App starten.

2. *Ändern Sie den Speicherpfad*. Verschieben Sie den OsmAnd-Ordner an einen Speicherort, der von allen Apps beschreibbar ist (z. B. Wi-Fi FTP-Server, X-plore oder über ein USB-Kabel an einen PC). Ändern Sie den Ordnerpfad von:
...
Bei den OsmAnd 4.2 Android-Versionen kann die [Kartenanzeige auf Google Pixel-Geräten fehlschlagen](https://github.com/osmandapp/OsmAnd/issues/15045), sodass Benutzer anstelle einer Karte einen leeren weißen Bildschirm sehen.
...
### Behebung der langsamen Kartenanzeige in OsmAnd {#resolving-slow-map-rendering-in-osmand}
...
Um das Problem der langsamen Kartenanzeige zu lösen, insbesondere bei der Verwendung von Offline-Vektorkarten, sollten die folgenden Schritte unternommen werden:

1. **Deaktivieren Sie unnötige Kartenfunktionen.** Schalten Sie 3D-Relief, Höhenlinien, Schattierungen und Neigungen aus, die den für die Kartenanzeige benötigten Rechenaufwand erheblich erhöhen können. Sie können dies in *Menü → Karte konfigurieren* tun.

2. **Vereinfachen Sie Kartendetails.** Blenden Sie weniger kritische Elemente wie Grenzen, POIs, OSM-Notizen, Wetterebenen, zusätzliche Details (*Menü → Karte konfigurieren → Kartenanzeige*) aus, um die Kartenanzeige zu optimieren und die Geschwindigkeit zu verbessern.

3. **Begrenzen Sie die Anzahl der angezeigten POIs.** Zeigen Sie nur die wichtigsten POIs an oder begrenzen Sie die während der Navigation angezeigten Kategorien, was zur Beschleunigung der Kartenanzeige beitragen kann. Greifen Sie auf diese Einstellungen in *Menü → Navigation → Einstellungen* zu.

4. **Passen Sie den Kartenvergrößerer an.** Die Einstellung des Kartenvergrößerers könnte ein Schlüsselfaktor für die Verlangsamung der Karte sein. Stellen Sie den Vergrößerer auf 100 %, indem Sie lange auf die Schaltfläche **"+"** oder **"-"** auf dem Kartenbildschirm drücken, um die Anzeige zu optimieren.
...
- Überprüfen Sie, ob die Daten der richtigen Region heruntergeladen wurden. Gehen Sie zu ***Menü → Karten & Ressourcen → Downloads → Ihre Region***. [So laden Sie Karten herunter](../start-with/first-steps.md#how-to-download-maps).
...
- Überprüfen Sie, ob die Elemente im Abschnitt Topographie in ***Menü → Karte konfigurieren → Topographie → Höhenlinien / Gelände / 3D-Relief*** aktiviert sind.
...
- **Falscher Straßenname**. Der mit dem Haus verbundene Straßenname ist möglicherweise in OpenStreetMap falsch. Überprüfen Sie das Tag `addr:street`, um sicherzustellen, dass der Straßenname genau mit den Tagging-Informationen der Straße übereinstimmt.

- **Problem mit Nominatim**. Das Haus ist möglicherweise in OpenStreetMap vorhanden, wird aber nicht über Nominatim (die von OpenStreetMap verwendete Suchmaschine) gefunden. Sie können mehr darüber erfahren, wie Sie adressbezogene Probleme beheben können [hier](https://wiki.openstreetmap.org/wiki/Addresses).

- **Mögliches Problem in OsmAnd**. Wenn das Haus in Nominatim vorhanden ist, aber immer noch nicht in OsmAnd gefunden wird, liegt das Problem möglicherweise spezifisch bei OsmAnd. Sie können zur Lösung beitragen, indem Sie weitere Untersuchungen anstellen. Weitere Details finden Sie in diesem [technischen Artikel](../../technical/algorithms/trace-address-search-issues.md).
...
- *[Favoriten](../personal/favorites.md)*. Dies sind permanente Punkte auf der Karte, denen Sie Beschreibungen hinzufügen können. Um einen Favoriten zu erstellen, machen Sie einen *langen Tipp → Tippen Sie auf Hinzufügen*. Favoriten eignen sich hervorragend zum Markieren von Orten, die Sie häufig besuchen oder für zukünftige Referenz speichern möchten.

- *[Marker](../personal/markers.md)*. Marker sind temporäre Punkte, die oft für Navigation oder Planung verwendet werden. Sie können die Entfernung von Ihrem aktuellen Standort oder einem anderen Punkt anzeigen und können schnell entfernt werden, wenn sie nicht mehr benötigt werden. Um einen Marker hinzuzufügen, machen Sie einen *langen Tipp → Tippen Sie auf Marker*.
