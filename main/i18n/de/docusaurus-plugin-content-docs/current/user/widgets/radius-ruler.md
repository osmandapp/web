...
Die **Lineal-Werkzeuge** in OsmAnd wurden entwickelt, um Ihnen zu helfen, Entfernungen zu messen und den [Kartenmaßstab](https://de.wikipedia.org/wiki/Ma%C3%9Fstab_(Kartografie)) präzise zu verstehen.

- Das **Lineal**-Werkzeug zeigt einen dynamischen Kartenmaßstab in Einheiten wie Metern oder Fuß an, um Ihnen bei der visuellen Einschätzung von Entfernungen zwischen Objekten auf dem Bildschirm zu helfen.
- Mit dem **Radius-Lineal** können Sie einen Radius von einem beliebigen Punkt auf der Karte festlegen, wobei konzentrische Kreise angezeigt werden, die Entfernungsintervalle darstellen.
- Mit dem Werkzeug **Entfernung nach Tippen** können Sie Entfernungen messen, indem Sie auf die Karte tippen — entweder von Ihrem aktuellen Standort aus oder zwischen zwei beliebigen Standorten.

Diese Werkzeuge sind nützlich für die Routenplanung, die Schätzung der Entfernung zu Sehenswürdigkeiten und die Analyse von Gebieten — sei es für Outdoor-Aktivitäten oder präzise Navigation.
...
Das **Lineal** zeigt den aktuellen Kartenmaßstab als Liniensegment an, das mit seiner realen Länge (z. B. 100 m oder 500 ft) beschriftet ist, am unteren Bildschirmrand. Eigenschaften:

- Der Maßstab passt sich dynamisch an, wenn Sie hinein- oder herauszoomen.
- Er bietet eine visuelle Referenz zur Schätzung realer Entfernungen zwischen Objekten auf der Karte.
- Die Segmentlänge wird auf praktische Einheiten gerundet (z. B. 100 m), die ungefähr der Breite eines Daumens entsprechen, um die Verwendung zu erleichtern.

Dieses Werkzeug ist hilfreich, wenn Sie schnell die Größe von Objekten oder Entfernungen verstehen müssen, ohne Marker oder Routen zu setzen.
...
Gehe zu: *<Translate android="true" ids="shared_string_menu,map_widget_config,shared_string_widgets"/> → Wählen Sie ein Feld → Widget hinzufügen → <Translate android="true" ids="radius_ruler_item"/>*
...
- Der erste Kreis spiegelt den aktuellen [Kartenmaßstab](#ruler) wider, um eine schnelle visuelle Referenz zu ermöglichen.
- Passt sich beim Zoomen automatisch an, um genaue Radiuswerte zu erhalten.
...
Wenn das **Radius-Lineal** aktiv ist, zeigt die Karte eine Kompassüberlagerung an, die Ihnen hilft, sich im Raum zu orientieren. Die Überlagerung zeigt, wie die Karte relativ zum geografischen Norden ausgerichtet ist, und kann ein blaues Dreieck enthalten, das die aktuelle Richtung des Geräts anzeigt.

- Der Kompassring zeigt die Kartenausrichtung — ob Norden oben oder gedreht ist.
- Ein **blaues Dreieck** zeigt die Richtung an, in die Ihr Gerät zeigt.
- Um diese Funktion nutzen zu können, muss das Gerät einen **Kompasssensor** (GPS-Kompass, Magnetometer) unterstützen und aktivieren.
...
| | |
|------------|------------|
| Aktivieren | **Android**: *<Translate android="true" ids="shared_string_menu,map_widget_config,shared_string_widgets"/> → Wählen Sie ein Feld → Widget hinzufügen → <Translate android="true" ids="radius_ruler_item"/>* |
|   |  **iOS**: *<Translate android="true" ids="shared_string_menu,map_widget_config,shared_string_widgets"/> → Wählen Sie ein Feld → Widget hinzufügen → <Translate android="true" ids="radius_ruler_item"/>* |
...
Gehe zu: *<Translate ios="true" ids="shared_string_menu,layer_map_appearance"/>* *→ Andere → Standortposition auf dem Bildschirm*
...
Sie können zur **3D-Ansicht** wechseln, um Entfernungen und das umliegende Gelände bei Verwendung des Radius-Lineals besser visualisieren zu können. Diese Ansicht neigt die Karte und bietet einen Perspektive-Effekt, der den sichtbaren Radius erweitert.
...
## Entfernung nach Tippen {#distance-by-tap}

![Distance between two random points on the map](@site/static/img/widgets/distance_between_two_random_points.png)

Das Werkzeug **Entfernung nach Tippen** ermöglicht es Ihnen, Entfernungen zwischen Punkten auf der Karte zu messen, ohne das Radius-Lineal zu verwenden oder eine Route zu erstellen.

Es gibt zwei Anwendungsfälle:

- **Entfernung von Ihrem Standort zu einem Punkt**  
  Tippen Sie irgendwo auf die Karte, und eine gerade Linie erscheint von Ihrem aktuellen Standort zu dem angetippten Punkt, zusammen mit einer Entfernungsbeschriftung. Die Linie bleibt auf dem Bildschirm, auch wenn Sie an anderer Stelle tippen.
