...
- **Raster [SQLIte-Format](../../technical/osmand-file-formats/osmand-sqlite) für Schummerung und Hangneigung verwenden** (*Android*). Lädt alte Rasterkartenformate.
...
- **Batteriesparmodus** (*Android* / *iOS*). Diese Einstellung reduziert die [Bildschirmaktualisierungsrate](#map-rendering-fps-widget) um 20 Bilder pro Sekunde, um Batteriestrom bei der Verwendung von Anwendungen zu sparen.
...
### Info und Statistiken {#info-and-statistics}
...
Gleichzeitig ändert sich durch die imaginäre Bewegung des Betrachters die Entfernung von der Kamera zur Oberfläche.
:::
...
- **<Translate android="true" ids="map_widget_zoom_level"/>**. Zeigt die aktuelle Kartenzoomstufe an.
...
OsmAnd bietet die Möglichkeit, den Standort und die Bewegung Ihres Geräts mithilfe eines GPX-Tracks zu simulieren. Verwenden Sie hierfür die Option *<Translate android="true" ids="simulate_location_by_gpx"/>* in den [Einstellungen](#plugin-settings) des OsmAnd-Entwicklungs-Plugins:
...
Nach dem Start der Simulation sehen Sie auf dem Hauptbildschirm der App, dass sich die Navigationsmarkierung entsprechend dem GPX-Track bewegt. Tippen Sie auf die Schaltfläche [Mein Standort](../map/interact-with-map#my-location-and-zoom), um *Mein Standort* (simulierte Geolokalisierung des Geräts) mit dem Mittelpunkt der Karte zu synchronisieren.
Um die Simulation der Bewegung Ihres Geräts zu beenden, gehen Sie zurück zu den OsmAnd-Entwicklungs-[Einstellungen](#plugin-settings) und tippen Sie bei der Option **Ihre Position simulieren** auf **Stopp**.
