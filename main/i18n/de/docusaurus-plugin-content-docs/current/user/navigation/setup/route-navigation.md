...
- Konfigurieren Sie die [Darstellung der Routenlinie](../guidance/map-during-navigation#route-line-appearance) (*optional*, verfügbar für [Pro-Benutzer](../../purchases/index.md#purchases)).
...
3. [Starten Sie](#start--stop-navigation) die Navigation oder [simulieren Sie](#simulated-navigation) zuerst die Navigation (mit dieser Funktion können Sie Ihre Route testen, bevor Sie tatsächlich entlangfahren).
...
- **Hinzufügen** &nbsp;&#43; - [Zwischenpunkte](../../widgets/nav-widgets.md#intermediate-destination) entlang der Route einfügen.
- **Tauschen** &nbsp;&#8595;&#8593; - Start- und Zielpunkte umkehren.
- **Tontaste** - [Sprachansagen](../guidance/voice-navigation.md) stummschalten.
...
- **Startpunkt und Ziel tauschen** &nbsp;&#8595;&#8593; - Ermöglicht das Tauschen von *Start*- und *Ziel*-Punkten.
...
- **<Translate android="true" ids="add_intermediate_point"/> / <Translate ios="true" ids="add_waypoint_short"/>**. Sie können von diesem Menü aus Zwischenziele hinzufügen.
...
Wenn die Aufzeichnung des [Navigationsverlaufs](../../personal/global-settings.md#history) aktiviert ist, können Sie die vorherigen Routen oder Ziele aus der Verlaufsliste für die Navigation verwenden. Diese Option kann für den schnellen Zugriff auf häufig verwendete Routen nützlich sein. Die Elemente in der Verlaufsliste sind nach der letzten Verwendung sortiert, beginnend mit dem neuesten.
...
Sobald Start- und Endpunkte ausgewählt sind, erstellt die App eine Route zwischen ihnen unter Verwendung eines aktiven Profils. Das [App-Profil](../../personal/profiles.md) und die [Widgets](../../widgets/index.md) werden erst gewechselt, wenn Sie auf die Schaltfläche *Start* der Navigation tippen.
...
- Verwenden Sie das *Schnellaktionsmenü* oder den Dialog, der erscheint, wenn Sie auf die *[Navigationstaste](../../widgets/map-buttons.md#directions)* tippen (siehe Screenshot).
...
:::note
Wenn Sie **auf die Navigationstaste tippen**, während Sie auf Ihrer Route sind, **stoppt die Navigation nicht und die Sprachansagen werden weiterhin ausgegeben**, obwohl die vorgeschlagenen Optionen Fortsetzen oder Start verwirrend sein können.
:::


### Tipps zum Energiesparen {#power-saving-tips}

Die Navigation verwendet GPS und Hintergrundprozesse, die den Akku belasten können. Um den Stromverbrauch zu optimieren:
...
- &nbsp;**<Translate android="true" ids="nav_type_hint"/>** bestimmt, wie Routen berechnet werden. Zum Beispiel hat Ihr Fahrradprofil einen Navigationsmodus für Radfahren, der die Routingregeln festlegt. Sie können diese Regeln (als routing.xml-Datei) in OsmAnd importieren. Mehr über Routing können Sie auf unserer [GitHub](https://github.com/osmandapp/OsmAnd-resources/blob/master/routing)-Seite lesen.&nbsp;

- &nbsp;**<Translate android="true" ids="route_parameters"/>** steuert, welche Parameter während des Routings verwendet werden (dies ist der Prozess der Auswahl einer Route für die Navigation).
...
Das Werkzeug **Simulierte Navigation** ermöglicht es Ihnen, eine Route vor der Reise zu überprüfen. Dies hilft bei der Planung, Orientierung und dem Testen des Navigationssystems von OsmAnd.
...
4. [Wie löst man die Situation, wenn die Navigation aufgrund fehlender notwendiger Karten nicht möglich ist.](#maps-missing-or-not-synchronized)
