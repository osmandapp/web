...
- Wenn Sie ein **OsmAnd Pro**-Abonnement haben, können Sie nach Ablauf Ihres Abonnements **ein weiteres Jahr** lang auf Ihr OsmAnd Cloud-Konto zugreifen.  

:::tip Fehlerbehebung bei Login-Problemen
Wenn Sie Probleme bei der Registrierung haben:

- Stellen Sie sicher, dass Sie sich auf dem **gleichen Gerät** registrieren, auf dem der Kauf getätigt wurde.  
...
- [Kostenloses Einstellungen-Backup](../personal/profiles.md#free-cloud-backup) – Speichern Sie Ihre OsmAnd-App-Einstellungen in OsmAnd Cloud und stellen Sie sie auf anderen Geräten wieder her, um ein nahtloses Erlebnis zu gewährleisten.  
...
**Sie benötigen ein OsmAnd Cloud-Konto, um OsmAnd Start zu verwenden**.
...
Die Funktion **Papierkorb** ermöglicht Ihnen, Dateien anzuzeigen und zu verwalten, die aus OsmAnd Cloud gelöscht wurden. Sie dient dazu, versehentliches Löschen von Dateien und unwiderruflichen Datenverlust zu verhindern.

- **Dateien anzeigen**. Im *Papierkorb* werden alle zuvor aus dem Ordner *Cloud-Änderungen* gelöschten Dateien aufgelistet. Alle Dateien enthalten Informationen über Datum und Uhrzeit der Löschung. Sie sind nach Monaten gruppiert und in der Reihenfolge vom zuletzt gelöschten zum ältesten im Papierkorb verbliebenen sortiert.
- **Papierkorb leeren**.  

    ![Trash OsmAnd Cloud](@site/static/img/personal/osmand-cloud/cloud_trash_3_andr.png)  

    Um Speicherplatz von allen aus der Cloud gelöschten Dateien freizugeben, können Sie die Funktion *Papierkorb leeren* verwenden. Sie ist über das *Drei-Punkte-Menü* in der oberen rechten Ecke des Papierkorb-Bildschirms verfügbar. Die Dateien werden dauerhaft gelöscht, ohne die Möglichkeit der Wiederherstellung.
...
- **Verwaltung einzelner Dateien**.  
    ...
    - *<Translate ios="true" ids="download_to_device"/>*. Diese Funktion ermöglicht Ihnen das Herunterladen einer Datei auf das Gerät. Wenn die ausgewählte Datei mit demselben Namen im Ordner vorhanden ist, wird das Dialogfeld *Datei existiert bereits* angezeigt, in dem Sie eine Aktionsoption auswählen können: *Ersetzen* oder *Beide behalten*.
    - *<Translate ios="true" ids="shared_string_delete_immediately"/>*. Diese Funktion ermöglicht Ihnen das dauerhafte Löschen der ausgewählten Datei, ohne sie wiederherstellen zu können. Seien Sie vorsichtig bei der Verwendung dieser Funktion, da der Vorgang irreversibel ist.
...
Gehen Sie zu: *<Translate ios="true" ids="shared_string_menu,shared_string_settings,osmand_cloud,shared_string_settings,backup_data"/>*
...
1. **Einstellungen**. Dieser Tab enthält alle Ihre Einstellungen, wie z. B. allgemeine Präferenzen, spezifische Profile, Schnellaktionen, POIs und Einstellungen zur Straßenvermeidung.

2. **Meine Orte**. Dieser Tab listet Daten aus dem Menü [Meine Orte](../personal/myplaces) auf, einschließlich Favoriten, Tracks, OSM-Notizen, Markern und anderen Elementen.
...
Alle [Karten](../start-with/download-maps.md) werden auf dem Gerät gespeichert, auf dem sie zuvor heruntergeladen wurden. OsmAnd Cloud speichert nur Informationen darüber, welche Karten Sie haben und wann sie zuletzt geändert wurden.
...
Der Menüabschnitt **<Translate android="true" ids="backup_version_history"/>** (Android) / **<Translate ios="true" ids="manage_storage"/>** (iOS) enthält Informationen darüber, wie viel der 3,15 Gigabyte Speicher in *Einstellungen*, *Meine Orte* und *Ressourcen* verwendet werden. Sie können Änderungen an einzelnen Datentypen aus OsmAnd Cloud löschen, in diesem Fall kann die Aktion jedoch nicht rückgängig gemacht werden.  
...
Dieser Abschnitt enthält Informationen darüber, welches Ihrer Konten mit OsmAnd Cloud verbunden ist, und enthält eine Abmeldetaste. Um Daten zu sichern oder wiederherzustellen, müssen Sie sich erneut anmelden.
...
Die Aktionen, die Sie im Abschnitt **Gefahrenzone** des Einstellungsmenüs durchführen können, sind irreversibel und erfordern daher besondere Aufmerksamkeit.

1. **Alle meine Daten löschen**. Diese Option ermöglicht Ihnen, alle zuvor in OsmAnd Cloud hochgeladenen Daten zu entfernen, einschließlich des gesamten Versionsverlaufs. Bitte beachten Sie, dass diese Daten nach dem Löschen nicht wiederhergestellt werden können.

2. **Alte Versionen entfernen**. Diese Option ermöglicht es Ihnen, den Änderungsverlauf für zuvor hochgeladene Daten zu löschen. Die aktuelle Datenversion bleibt auf dem Server erhalten, aber Sie können nicht auf frühere Versionen zurückgreifen.
...
Tippen Sie im Dropdown-Menü im Abschnitt [Letzte Synchronisierung](#last-sync) auf ein beliebiges Element, um den Abschnitt "Änderungen" zu öffnen. Dieser Abschnitt enthält drei Tabs, die alle verfügbaren Änderungen zur Bearbeitung anzeigen. Wenn die Liste leer ist, sind seit der letzten Synchronisierung keine Änderungen aufgetreten.
...
- In jedem Tab sind die Daten **nach Typ und alphabetisch sortiert**, um Ihnen zu helfen, bestimmte Elemente schnell zu finden, insbesondere wenn Sie mehrere Dateien in OsmAnd Cloud verwalten.
...
- **Lokal** - Sie können alle lokalen Änderungen, die noch nicht in der Cloud sind, synchronisieren, indem Sie auf **Alle hochladen** tippen. Alternativ können Sie bestimmte Änderungen hochladen, indem Sie sie aus der Liste auswählen und **Lokale Version hochladen** wählen.  
    ...
    Änderungen, die erfolgreich in die Cloud hochgeladen wurden, werden nach einem festgelegten Zeitraum automatisch aus dem Tab *Änderungen → Lokal* entfernt.  

- **Cloud** - Der Tab **Cloud** zeigt eine Liste von Änderungen an, die aus dem Cloud-Speicher zum Herunterladen verfügbar sind. Sie können alle Änderungen auf einmal mit der Schaltfläche am unteren Bildschirmrand herunterladen oder sie einzeln herunterladen.  
...
- Sie können entweder die **Cloud-Version durch lokale Änderungen ersetzen** oder die **Cloud-Version** auf Ihr Gerät **herunterladen**.
...
- Durch Tippen auf eine Datei im Ordner **Konflikte** werden Details zu den **lokalen** und **Cloud-Versionen** angezeigt, einschließlich der letzten Änderungs- und Upload-Zeiten.
