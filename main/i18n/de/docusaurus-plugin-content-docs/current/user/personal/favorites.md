...
**Favoriten** ist eine Funktion von OsmAnd, mit der Sie Lesezeichen (Notizen) auf der Karte erstellen können. Diese Favoritenpunkte werden standardmäßig als gelbe Sterne angezeigt, aber Sie können sie vollständig mit verschiedenen Farben, Formen und Symbolen anpassen. Favoriten werden in einer eigenen Kartenebene gruppiert, die ab der *6. Zoomstufe* sichtbar wird.
...
- **Gruppe** — Wählen Sie aus einer vorhandenen [Favoritengruppe](#manage-favorites) oder erstellen Sie eine neue.
...
- **Löschen** — Entfernen Sie den Favoriten über die Option Löschen, die über das Menü Bearbeiten oder über die Schaltfläche Löschen unter Android zugänglich ist.
...
Im Bereich **Meine Favoriten** können Sie:
...
1. Um mehrere Favoriten zu verwalten, tippen Sie auf das Symbol **Löschen** (*Papierkorbsymbol unter Android*) oder die Schaltfläche **Bearbeiten** (*Bleistiftsymbol unter iOS*).
...
Gehe zu: *Langes Tippen auf Ordner → Aussehen ändern*
...
- **<Translate android="true" ids="shared_string_add_to_map_markers"/>** oder **Aus Kartenmarkierungen entfernen** (*nur Android*) — Fügen Sie alle Favoritenpunkte aus dem Ordner zur *Liste der Kartenmarkierungen* hinzu oder entfernen Sie sie nach Bedarf zur einfachen Referenz.
...
- **Anpassung** — Überschreiben Sie die Standardeinstellungen für vorhandene Punkte im Ordner, nur für neue Favoriten oder wenden Sie sie auf alle an.
...
Die [Kostenlose Favoriten-Sicherung](../personal/osmand-cloud.md#osmand-start) ist ein spezieller Kaufplan, mit dem Sie Ihre Favoritenpunkte (Standorte) für die **Android-, iOS-** oder **Web-**Versionen in der OsmAnd-Anwendung sichern und aus der OsmAnd Cloud wiederherstellen können. Dieser Plan ist für Benutzer von [OsmAnd Free oder Maps+](../purchases/index.md) verfügbar.
...
Sie benötigen ein OsmAnd Cloud-Konto, um die Funktion *Kostenlose Einstellungen-Sicherung* nutzen zu können. Wenn Sie ein Konto in *OsmAnd Pro* oder ein aktives Konto in *OsmAnd Cloud* haben, wird Ihnen das Werbebanner nicht angezeigt.

- *Kostenloses Favoriten-Sicherungsbanner*. Tippen Sie auf dieses Banner, um den Sicherungsvorgang zu starten.
- *Schließen Sie den Registrierungsschritt ab*, wenn Sie kein OsmAnd Cloud-Konto haben, indem Sie den Anweisungen im [Registrierungsleitfaden](../personal/osmand-cloud.md#login) folgen.
...
Gehe zu: *Dateien → Auf meinem iPhone → OsmAnd Maps → favorites_backup*
...
OsmAnd erstellt jedes Mal, wenn Favoriten bearbeitet werden, eine **Sicherungsdatei**.
...
- **iOS**: Dateien befinden sich in *Dateien → Auf meinem iPhone → OsmAnd Maps → favorites_backup*.

**Manueller Sicherungsexport**

- Gehe zu *<Translate android="true" ids="shared_string_menu,shared_string_settings,export_to_file,shared_string_resources,favorites_backup"/>*.
