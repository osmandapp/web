...
Falls die Favoriten in einem Ordner bereits unterschiedliche Symbole, Farben oder Formen haben, wird der *Originalzustand* angezeigt. Das bedeutet, dass jeder Favorit seinen aktuellen Stil beibehält, es sei denn, Sie wählen einen neuen aus. Beim Speichern werden Sie auch aufgefordert, auszuwählen, wie die Änderungen angewendet werden sollen:
...
Diese Flexibilität ermöglicht es Ihnen, das Erscheinungsbild eines Ordners zu standardisieren oder einzigartige Anpassungen beizubehalten, während Sie Standards für neue Punkte festlegen.
