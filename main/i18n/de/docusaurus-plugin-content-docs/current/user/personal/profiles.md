...
OsmAnd bietet eine Vielzahl von Standardprofilen, die auf verschiedene Aktivitäten zugeschnitten sind, darunter Autofahren, Radfahren, Wandern und speziellere Aktivitäten wie Skifahren und Bootfahren. Diese Profile wurden entwickelt, um Ihr Navigationserlebnis entsprechend der Aktivität, an der Sie teilnehmen, zu verbessern. Jedes Profil enthält konfigurierbare Optionen, mit denen Sie Einstellungen anpassen und zusätzliche Plugins integrieren können, um Ihren Präferenzen zu entsprechen.
...
- Es ist wichtig zu beachten, dass einige Profile, wie **LKW**, **Motorrad**, **Moped**, **Zug**, **Flugzeug**, **Boot** und **Reiten**, standardmäßig nicht aktiviert sind.
     ...
     - Diese Methode hilft, eine organisierte und relevante Liste von Profilen zu pflegen, indem nur die für bestimmte Aktivitäten benötigten Profile aktiviert werden.
...
:::caution Einstellungen nur für ein bestimmtes Profil übernehmen
...
OsmAnd ermöglicht es Ihnen, Ihre Anwendungsprofileinstellungen in der OsmAnd Cloud [zu sichern](../personal/osmand-cloud.md#osmand-start), um sicherzustellen, dass Ihre Konfigurationen sicher gespeichert und einfach über Android-, iOS- und Web-Versionen wiederhergestellt werden können. Diese Sicherungsfunktion schützt Ihre Einstellungen im Falle eines Geräteausfalls oder einer Neuinstallation der App. Sie können von diesem speziellen Plan profitieren, indem Sie [OsmAnd Start](../personal/osmand-cloud.md#osmand-start) verwenden.

Sie müssen ein [OsmAnd Cloud](../personal/osmand-cloud.md#login)-Konto haben, um das *Kostenlose Einstellungen-Backup* nutzen zu können. Wenn Sie ein *OsmAnd Pro*-Abonnement oder ein aktives *OsmAnd Cloud*-Konto haben, wird das Werbebanner nicht angezeigt.
...
- **<Translate android="true" ids="rotate_map_to"/>**. Ermöglicht Ihnen, [den Kartenorientierungsmodus](../map/interact-with-map.md#map-orientation-modes) für das ausgewählte Profil zu wählen.
...
- **[<Translate android="true" ids="screen_control"/>](../navigation/guidance/voice-navigation.md#screen-control)** (*Nur Android*). Öffnet das Bildschirmsteuerungsmenü, in dem Sie Optionen für den Gerätebildschirm während der Navigation auswählen können, um den Batterieverbrauch des Geräts zu sparen.
...
|          | <Translate android="true" ids="shared_string_milliradians"/> | Alle Winkelwerte haben [Milliradiant-Werte](https://en.wikipedia.org/wiki/Milliradian).  |
|  |  |  |
...
|          | <Translate android="true" ids="round_up"/> | *Für bessere Lesbarkeit* werden die in den Navigations-Widgets angezeigten Zahlen (Distanz zu einem Punkt, nächste Abbiegung oder Spur) abgerundet, um weniger Ziffern zu haben. <br/> Beispiele: 3672 m *→* 3.6 km,&nbsp; 462 m *→* 400 m,&nbsp; 184 m *→* 150 m,&nbsp; 47 m *→* 40 m,&nbsp; 18 m *→* 15 m. |
...
- [<Translate android="true" ids="use_kalman_filter_compass"/>](../map/interact-with-map.md#extra-compass-settings) (*Nur Android*) – Glättet die Kartenrotation mit [allmählicher Bewegung](https://en.wikipedia.org/wiki/Kalman_filter), wodurch plötzliche Positionsverschiebungen reduziert werden. Dies führt zu einer kleinen Verzögerung (*weniger als 1 Sekunde*).
...
- **<Translate android="true" ids="tap_on_map_to_hide_interface"/>** (*Nur Android*) – Tippen Sie auf einen leeren Bereich der Karte, um Steuerelemente und Widgets auszublenden und die Sichtbarkeit der Karte zu maximieren.
...
- **<Translate android="true" ids="position_animation"/>** (*Nur Android*) – Das [Positions-](../map/interact-with-map.md#my-location-and-zoom)Symbol animiert mit jedem empfangenen GPS-Punkt (einmal pro Sekunde). Eine leichte Verzögerung in seiner Bewegung kann bemerkbar sein, besonders bei schneller Bewegung oder GPS-Signalfluktuationen. Sie können diesen Effekt mit der Einstellung *Vorhersagezeit* anpassen: Höhere Werte erhöhen die Glätte, fügen aber Verzögerung hinzu, niedrigere Werte reduzieren die Verzögerung, können aber die Bewegung weniger glatt machen.
...
Für eine detaillierte Übersicht der Navigationsoptionen, siehe den Artikel [Navigationseinstellungen](../navigation/guidance/navigation-settings.md), der erklärt, wie OsmAnd für ein optimales Navigationserlebnis konfiguriert wird.
...
- [Routenparameter](../navigation/guidance/navigation-settings.md#route-parameters) - Definieren Sie, wie OsmAnd Routen berechnet, einschließlich Präferenzen für das Vermeiden bestimmter Straßen oder die Auswahl von Routentypen.
...
- [Karte während der Navigation](../navigation/guidance/navigation-settings.md#map-during-navigation) – Zeigen Sie die Echtzeitpositionierung auf der Karte an, verfolgen Sie den Fortschritt entlang der Route und verbessern Sie die Orientierung mithilfe von Landmarken.

- [Detaillierte Routenführung](../navigation/guidance/navigation-settings.md) – Verbessern Sie die Turn-by-Turn-Navigation, indem Sie Ihre Route an kartierte Straßen anpassen. Verfügbare Einstellungen: *Jedes Mal fragen* oder *Immer*.
...
Das [<Translate android="true" ids="map_widget_config"/>](../widgets/configure-screen.md) ist ein Menü, das es Ihnen ermöglicht, die Widgets für das ausgewählte Profil zu konfigurieren, die auf der Karte angezeigt werden sollen. Auf diesem Bildschirm können Sie [Informations-](../widgets/info-widgets.md) und [Navigations-Widgets](../widgets/nav-widgets.md) sowie andere Elemente aktivieren und konfigurieren.
...
Die Einstellungen für das **Erscheinungsbild meines Standorts** ermöglichen es Ihnen, anzupassen, wie Ihr Standort auf der Karte angezeigt wird, sowohl im Stillstand als auch in Bewegung. Bewegung wird vom Betriebssystem erkannt, wenn die Geräteschwindigkeit **größer als Null** ist.

#### 1. Blickwinkel und Standortradius {#1-view-angle-and-location-radius}

Sie können die visuellen Elemente im Zusammenhang mit Ihrem Standort anpassen und diese separat oder zusammen für **Ruhepositionen** und **Navigationspositionen** anwenden. Sowohl im 2D- als auch im 3D-Modus werden *Blickwinkel* und *Standortradius* in der gleichen Farbe angezeigt, die für das [Navigationsprofil](#profile-appearance) ausgewählt wurde, direkt unter dem Symbol [Mein Standort](../map/interact-with-map.md#my-location-and-zoom).
...
- Erfahren Sie mehr über Peilung und Richtungsverfolgung im Abschnitt [Peilung-Widget](../widgets/nav-widgets#bearing-widget).
...
- **<Translate android="true" ids="copy_from_other_profile"/>** - Ermöglicht es Ihnen, das *Schubladenlayout* von einem anderen OsmAnd-Profil zu kopieren, um eine konsistente Einrichtung über Profile hinweg zu gewährleisten.
...
- **Karte konfigurieren Menü** - Im Abschnitt *UI anpassen* können Sie Elemente aus dem Menü [Karte konfigurieren](../map/configure-map-menu.md) *neu anordnen oder ausblenden*, um einen schnellen Zugriff auf häufig verwendete Einstellungen zu ermöglichen.
...
- **<Translate android="true" ids="copy_from_other_profile"/>** - Kopiert das *Karte konfigurieren Menülayout* von einem anderen OsmAnd-Profil.
...
- **<Translate android="true" ids="reset_to_default"/>** – Stellt die *Standard-Elementliste* im Kartenkontextmenü wieder her und macht alle Änderungen sofort rückgängig.

- **<Translate android="true" ids="copy_from_other_profile"/>** – Kopiert die *Kontextmenü-Anordnung* von einem anderen OsmAnd-Profil, um die Konsistenz über verschiedene Profile hinweg zu gewährleisten.


## Plugin-Einstellungen {#plugin-settings}

:::caution Zugriff auf Plugin-Einstellungen
Um auf die Einstellungen des Plugins zuzugreifen, müssen Sie das Plugin zunächst im *Plugin-Bereich des Hauptmenüs* [**aktivieren**](../plugins/index.md#enable--disable).
:::
