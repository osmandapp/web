...
Go to: *<Translate android="true" ids="shared_string_menu,configure_map,layer_map,gpx_add_track"/>*  
...
Go to: *<Translate ios="true" ids="shared_string_menu,configure_map,map_settings_type,shared_string_online_maps,map_settings_install_more"/>*  
...
### Add New Online Source {#add-new-online-source}

<Tabs groupId="operating-systems" queryString="current-os">

<TabItem value="android" label="Android">  

Go to: *<Translate android="true" ids="shared_string_menu,configure_map,layer_map,shared_string_add_manually"/>*
...
| *<Translate ios="true" ids="edit_tilesource_url_to_load"/>* | Enter or paste the URL for the online tile source. Make sure it follows the tile URL format. The URL can contain certain placeholders, which OsmAnd will automatically replace based on the specific tile needed. The most commonly used placeholders are based on the [OpenStreetMap slippy map tilenames convention](https://wiki.openstreetmap.org/wiki/Slippy_map_tilenames): <ul><li>`{z}` or `{0}`: Zoom level</li><li>`{x}` or `{1}`: Tile X index</li><li>`{y}` or `{2}`: Tile Y index</li></ul> For examples of the less commonly used placeholders, see the [predefined online raster map sources](https://github.com/osmandapp/web/blob/main/main/static/tile_sources.xml). |
...
You find an added Online map in the list of the [Main / Underlay / Overlay layer](#layers) menu.
...
- **Android**: *<Translate android="true" ids="shared_string_menu,maps_and_resources,download_tab_local,quick_action_map_source_title"/> → choose online maps →  
...
Go to: *<Translate android="true" ids="shared_string_menu,maps_and_resources,download_tab_local,quick_action_map_source_title"/> → choose online maps →  
...
Go to: *<Translate ios="true" ids="shared_string_menu,res_mapsres,download_tab_local,online_raster_maps"/> → i → <Translate ios="true" ids="shared_string_clear_cache"/>*
...
For the maps to automatically update the tiles after a while, you can set an [Expiry time](#add-new-online-source), then OsmAnd will reload the tiles as soon as they are displayed.  
...
- To download or update raster tiles, you need to select the source of the online map as the [Main map source](#layers) (**Android / iOS**). You can also select online tiles separately for the [Overlay](#overlay) map or for the [Underlay](#underlay) (Only for **Android**).
...
Go to: *<Translate android="true" ids="shared_string_menu,maps_and_resources,download_tab_local,quick_action_map_source_title"/> → choose online maps →  &#8942; → <Translate android="true" ids="shared_string_edit"/>*
