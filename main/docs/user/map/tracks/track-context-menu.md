...
At the top of the information panel, you can see the track’s name and the symbol used to mark it (*Android only*). The track name can be changed using the rename function in the [Options menu](#options). Some tracks (mostly the *[Travel guides](../../plan-route/travel-guides.md#manage-as-gpx-track)*) may have a short *[description](#description-and-info)* and (or) a picture below the *Track name*. An arrow shows *Direction to the nearest point of the track* from [My position](../../map/interact-with-map.md#my-position-and-zoom).  
...
In case your track was created in OsmAnd or any other tracking app (so its points have [`time` and  `speed`](../../plugins/trip-recording#recorded-gpx-file) tags), the informational panel will also contain information about *<Translate android="true" ids="average_speed"/>*, *<Translate android="true" ids="max_speed"/>*, *<Translate android="true" ids="map_widget_trip_recording_duration"/>* (the total time between the start and end points of the track), and *<Translate android="true" ids="shared_string_time_moving"/>* (the sum of time while in motion).
...
- [Activity tags for GPX tracks](#description-and-info). [Recorded tracks](../../plugins/trip-recording.md#recording-settings) and tracks saved via [Plan a route](../../plan-route/create-route.md) automatically receive an activity type based on the profile used to create them. This helps categorize and filter them later. You can change the activity manually if needed.
...
- *<Translate android="true" ids="map_widget_trip_recording_duration"/>*. The time interval between the start and end points of the track.
...
When you tap on any segment in the list, the map view shows the full track with its intervals and opens the context menu for the track at the bottom of the screen. On the map, interval labels support two actions: tap to open the track overview panel, and long-press to open the track context menu.
