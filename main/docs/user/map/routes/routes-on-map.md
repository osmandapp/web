...
OsmAnd displays routes from OpenStreetMap data directly on the map, highlighting them with colors and icons based on route type. Tapping a route symbol opens a context menu with detailed information and available actions, such as [saving the route](#save-as-a-track) as a GPX track or [starting navigation](#start-navigation). You can configure which routes are visible in the [Configure Map](../../map/configure-map-menu.md) menu.
