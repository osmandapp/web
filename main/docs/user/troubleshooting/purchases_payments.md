...
A cross-platform purchase, such as _Maps+_ and _Pro_, is linked to the OsmAnd Cloud account that was last activated on the device holding the original purchase receipt (from the App Store or Google Play). If a user on this device logs out of their primary account (`OsmAnd Cloud account A`) and logs into a new one (`OsmAnd Cloud account B`), the OsmAnd system automatically transfers the license to the new `OsmAnd Cloud account B`. As a result, the old account loses its puchase status on other platforms, as the license can only be active on one OsmAnd Cloud account at a time.
...
> For more information, read [Cross platfrom purchases](../purchases/cross.md) article.
