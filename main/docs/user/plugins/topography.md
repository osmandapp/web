...
You can use the *Quick Action* buttons on the map screen to switch the visibility of [Contour Lines](#contour-lines), [Terrain](#terrain) layers, and [Terrain color scheme](../../user/personal/color-palette-schemes.md#quick-actions). Depending on the layer selected in the Configure map menu, assigning a *Terrain* action to the button will display *Hillshade*, *Slope*, or *Altitude*.  
...
**3D Buildings** feature displays buildings as volumetric 3D models instead of flat shapes. Buildings are generated from [OpenStreetMap data](https://wiki.openstreetmap.org/wiki/Simple_3D_Buildings), using height information from tags such as `height` and `building:levels` when available. For complex structures, OsmAnd uses `building:part` to render individual sections of a building with different heights and shapes. If [OpenStreetMap data](https://wiki.openstreetmap.org/wiki/Tag:tunnel%3Dbuilding_passage) includes passages through buildings mapped with the tag `tunnel=building_passage`, OsmAnd renders visible openings in the 3D building model so that roads or pedestrian ways passing through the building are displayed correctly. 
...
On Android, this option is available only when the Topography plugin is enabled.  
