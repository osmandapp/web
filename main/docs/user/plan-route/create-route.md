...
The *Plan a route* tool allows modifying existing GPX track and an [imported track](../personal/tracks/manage-tracks.md#import). However, in case you need to exclude many points based on generic criteria, [*<Translate android="true" ids="shared_string_gps_filter"/>*](../map/tracks/track-context-menu.md#gps-filter) might be a more suitable tool.
...
The *Plan a route* is a quick and easy way to measure the distance between points.
...
*Plan a route → Straight line method → <Translate android="true" ids="shared_string_graph,get_altitude_data"/>*

![Plan a route android](@site/static/img/plan-route/plan_route_graph_4_andr.png)  

If [*Elevation data*](../map/tracks/track-context-menu.md#calculating-missing-elevation) is missing (*Plan a route → Straight line method → <Translate android="true" ids="shared_string_graph,get_altitude_data"/>*) in an existing or newly created track, it's possible to add it using the following tools:
...
You can select a distance threshold for how far points of a simplified track could be from points of an original track.

:::note
If you open a track and see the ***undefined*** profile icon, you can select whether to attach the track to roads or simply connect points with a straight line and treat it as a recorded track.
:::
...
:::note
You can also **Undo**/**Redo** every action you made in the *Plan a route* tool.
:::
  
### Route Between Points

Added points in the editor could be connected as a straight line or a route between points of a selected profile. *Route between points* can be accessed in several ways:

1. From the *Options* menu *→* *Route between points*.
2. Tap the *profile icon* in the bottom left corner of the map screen. If you do not tap the top icon, the Configure Map menu opens.
...
A track segment is a collection of points connected without **gaps**. In the *Plan a route* tool, it's possible:

- To merge segments: [Join segments](#point-context-menu) option removes gap to previously separated segments.
...
With the *Plan a route* tool you can create multi-modal routes where the first part can be a *cycling* part, then a straight line, and then a *pedestrian*.
You can do it via option [Route between points](#route-between-points). Multi-modal navigation is not possible, so you will need to select one of the most suited profiles to follow turn instructions.
...
- ***Quick save***. The top right button ***Done*** / ***Save*** (for existing tracks) allows you to quickly save your changes and exit the *Plan a route* tool. The name is generated based on the current date.
- ***Save changes*** in [Options menu](#options) allows you to save changes to a file and continue planning the route.
- ***Save as new track*** in [Options menu](#options) opens a dialog where you can specify the track name and the track folder route will be saved to.
...
When saving a new track you can select option ***Simplified*** track, so the track might be compatible with other 3rd party applications. Technically, the track will be saved without route instructions just as a pure geometry track.
...
- [*<Translate android="true" ids="route_between_points"/>*](#route-between-points). Shows the chosen application profile (the straight line by default). Tap this action is equal the *Profile* button with opening application [profile menu](../personal/profiles.md).
...
![Plan a route snap-road-ios](@site/static/img/plan-route/plan_route_graph_2_ios.png)
...
When calculating a route for navigation in the *Plan a route*, you can find out additional track information such as [*Elevation info*](../navigation/setup/route-details.md#elevation-info) and [*Road attributes*](../navigation/setup/route-details.md#road-attributes), and use the [*Analyze on map*](../navigation/setup/route-details.md#analyse-on-map) tool. Tap the *Details* button below the graph.  
