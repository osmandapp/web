...
In Settings you can select to always display **the current street name** and use it with [**Next turn**](#next-turn) widget to always see the next street name as well. **Next turn information** is OFF by default. If you enable it, then switching between the current street name and the next street name happens when you approach the place where you want to perform the maneuver (~20 seconds), same to the voice prompt [Turn in X m](../../technical/algorithms/voice-prompt-triggering.md#trigger-table).
...
| Settings | Select to always display **the current street name** or display **the next street name** when approaching to the maneuver |
