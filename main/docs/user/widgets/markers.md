...
- Distance from your current location (or the centre map point when the Location is off) to the selected point indicated by the Marker on the map ([Units of length](../personal/profiles/#general-settings) can be chosen in the Profile settings).
- An arrow of the same color as the marker indicates the direction from the current location (or the centre map point when the Location is off) and orientation in space of your device (heading) towards the marker.
- Location Address.
