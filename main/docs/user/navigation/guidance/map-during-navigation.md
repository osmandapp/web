...
|:------------|:---------------|:---------------|
| *<Translate android="true" ids="choose_auto_follow_route"/>* |  The time for which the map view is synchronized with the current position after moving.  |  *Value:* <br /> Never, 5 sec, 10 sec, 15 sec, 20 sec, 25 sec, 30 sec, 45 sec, 60 sec, 90 sec.|
| *<Translate android="true" ids="auto_zoom_map"/>*  |  Automatically scale the map according to your speed, as long as the map is synchronized with your current position. | *Value:* <br /> *<Translate android="true" ids="auto_zoom_none"/>* - manually zoom. <br /> *<Translate android="true" ids="auto_zoom_farthest"/>* - zoom is 200 m.<br /> *<Translate android="true" ids="auto_zoom_far"/>* - zoom is 100 m. <br /> *<Translate android="true" ids="auto_zoom_close"/>* - zoom is 5 m. <br /> Auto-zoom changes can be animated (Smooth) or step-based (Discrete), depending on [Development settings](navigation-settings.md#development-settings). In Smooth mode, zoom changes use a controlled animation (around 0.1 zoom/second). Auto-zoom does not adjust zoom at very low speeds (below ~7 km/h). If the required zoom change would take less than ~1.5 seconds, the animation is not started. <br /> Auto-zoom aims to keep the upcoming maneuver within a stable on-screen focus area, so the visible distance ahead remains consistent while driving.|
| *Auto zoom 3D angle* | Sets the tilt of the map when switching to 3D view during navigation. A higher angle makes the horizon appear further away, giving more forward visibility. |It is applied only when Auto zoom is enabled. Values: 20°, 25°, 30°, 35°, 40°. Default: 25°. <br /> When approaching a maneuver/intersection, the app can gradually reduce the 3D tilt toward a 2D view to keep the next turn readable. |
| *Preview next turn* | Automatically rotates the map slightly ahead of time to show the next turn or maneuver during navigation. Helps anticipate upcoming actions.  | Enabled by default. If you export and re-import a profile, make sure to verify this setting, as in some earlier versions it could reset to “enabled.”  <br /> Rotation/preview is triggered once the next maneuver point falls into the focus area (so the map starts “looking” at the turn when it becomes relevant). |
