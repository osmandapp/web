...
Apostrophes and missing punctuation are no obstacle either. Type "guys" or "Guy's," "mcdonalds" or "McDonald's" — the engine recognizes both as the same place, so a quick, imperfectly typed search still finds what you're after.

|| |
|:--------|:---------|
|![Spatial Search Guys](./img/guys_1.webp)| ![Spatial Search Guys ](./img/guys_2.webp) |


## Finding Things That Belong Together

Some of the most useful searches are really about two places that need to sit near each other — like a gas station next to a specific fast-food chain. Spatial Search can look for a brand alongside a category and return only the places where both are true.

Type "Fuel McDonald's" and you'll get gas stations that sit next to a McDonald's — not a long, separate list of one or the other. The same works the other way around ("McDonald's Fuel"). It's the kind of query that used to mean two separate searches and a lot of squinting at the map. Now it's one line.

![Spatial Search Fuel](./img/fuel.webp)
