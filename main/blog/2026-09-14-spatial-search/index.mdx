...
Some searches are really about two things that need to be true at once — a gas station that sells diesel, a café that's vegan, a gas station with a McDonald's nearby. Spatial Search can look for the intersection of two categories, whether both apply to the same place or to two different places near each other.

Type "Fuel Diesel" or "Cafe Vegan" and you'll get places matching both. Type "Fuel McDonald's" and you'll get gas stations near a McDonald's — not a long, separate list of one or the other. It's the kind of query that used to mean two separate searches and a lot of squinting at the map. Now it's one line.
