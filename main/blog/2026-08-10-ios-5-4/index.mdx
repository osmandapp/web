...
* [**Tracks**](../docs/user/personal/myplaces#tracks) provides access to saved, recorded, and imported GPX files from the same redesigned My Places interface, with search, sorting, folder management, statistics, and quick actions for working with individual tracks or multiple selected items.

* [**OSM Edits**](../docs/user/personal/myplaces#openstreetmap-edits) keeps your OpenStreetMap edits, notes, and uploaded changes in a dedicated section. This tab is available when the **OSM Editing** plugin is enabled.

* [**Travel Guides**](../docs/user/personal/myplaces#travel-guides) contains bookmarked travel articles and guides, allowing saved travel content to be accessed directly from My Places. The tab appears when multiple guides have been bookmarked.
...
The [**Plan a Route**](../docs/user/plan-route/create-route) tool has been rebuilt with a new interface for creating, editing, and analyzing routes.
...
You can now choose from [the complete collection of OsmAnd icons](../docs/user/personal/profiles#profile-appearance) when creating or editing a profile. The redesigned icon selector uses grouped categories and includes the same icons available for Favorites, making profiles easier to identify and personalize.

![Icons list](./img/icons_list.webp)

## CarPlay updates

OsmAnd 5.4 includes several fixes and improvements for [**Apple CarPlay**](../docs/user/navigation/car-play).
...
You can now set a [**default appearance**](../docs/user/personal/tracks/manage-tracks) for each Track folder. New tracks added to the folder can automatically use the selected coloring, line width, direction arrows, start and finish icons, and split interval.
...
**Smart Folders** can now [automatically organize tracks](../docs/user/personal/tracks/smart-folder#managing-smart-folders) into groups using the new **Organize by** option. Instead of displaying every track in one list, you can group them by activity, creation date, location, distance, speed, altitude, elevation, or recorded sensor data.
...
When [importing a GPX file containing multiple tracks](../docs/user/personal/tracks/manage-tracks#import), you can now review and select the individual tracks you want to import. Each selected track is saved separately, making it possible to manage its visibility, appearance, and other settings independently.
...
Track waypoint lists now include a dedicated [**Show on map** button](../docs/user/map/tracks/track-context-menu#points--waypoints). Tap the pin icon next to a waypoint to center the map on its location without changing the current zoom level.
...
* Added a [**bicycle width** setting](../docs/user/navigation/guidance/vehicle-parameters#limits). OsmAnd can now consider the width of your bicycle when calculating routes and avoid cycleways that are too narrow. 
...
* Added an [**OBD-II adapter voltage widget**](../docs/user/plugins/vehicle-metrics#metrics-list) that displays the current voltage reported by a connected OBD-II adapter. 

* Added support for [**Roads-only maps**](../docs/user/personal/maps-resources#free-map-content) on iOS. These smaller maps contain the road network required for navigation and address search without the complete set of regular map details. 

* [Planned tracks](../docs/user/plan-route/create-route#save-route) now automatically receive an [**activity type**](../docs/user/map/tracks/track-context-menu#ttrack-activity-type) based on the active profile or selected navigation type. 

* Track lists now display the [**track folder name on the second line**](../docs/user/personal/tracks/manage-tracks#sort-by), making tracks easier to identify in Search, Smart Folders, and Configure Map. 

* Added the [**Pause spoken audio**](../docs/user/navigation/guidance/voice-navigation#voice-guidance-output) option. Podcasts, audiobooks, and other spoken content can now be paused during navigation instructions instead of playing underneath them. {/*[More details](https://github.com/osmandapp/OsmAnd-iOS/issues/5134).*/}
...
* Added the ability to [**share a route using a link**](../docs/user/navigation/setup/route-details#share--export-actions) making it easier to send a planned route to another user or open it on another device. {/*[More details](https://github.com/osmandapp/OsmAnd-Issues/issues/1860)*/}

* POIs containing [**multiple website links**](../docs/user/map/map-context-menu#object-info) now display all available URLs, allowing you to choose which one to open. {/*[More details](https://github.com/osmandapp/OsmAnd-iOS/issues/5417).*/}

* Updated the [**Wikipedia action**](../docs/user/map/map-context-menu#object-info) in the map context menu for easier access to related Wikipedia articles. {/*[More details](https://github.com/osmandapp/OsmAnd-Issues/issues/3215).*/}

* [Added the ability](../docs/user/search/search-all) to open matching [**Travel Guides directly from Search**](../docs/user/map/map-context-menu#guides). {/*[More details](https://github.com/osmandapp/OsmAnd-Issues/issues/3212).*/}

* Added an option to hide the **expand button** in [the Route Information widget](../docs/user/widgets/nav-widgets#route-information) for a cleaner navigation interface. {/*[More details](https://github.com/osmandapp/OsmAnd-iOS/issues/5162).*/}

* Increased the size of [**road shields in the Street Name widget**](../docs/user/widgets/nav-widgets#route-guidance), improving the readability of route numbers during navigation. {/*[More details](https://github.com/osmandapp/OsmAnd-iOS/issues/5331).*/}

* [Updated turn and lane arrows](../docs/user/widgets/nav-widgets#route-guidance) to use consistent **stroke thickness** both on the device and in CarPlay. {/*[More details](https://github.com/osmandapp/OsmAnd-iOS/issues/4787).*/}
