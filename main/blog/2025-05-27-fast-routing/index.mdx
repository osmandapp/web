...
Diagram illustrating the Highway Hierarchy approach in OsmAnd:
The map is divided into multiple colored area clusters, each representing a logical section of the road network. Border points are marked along the boundaries between clusters. Within a cluster, shortcut lines connect key border points to optimize local routing. Additionally, a prominent shortcut connects border points between two adjacent clusters, enabling efficient long-distance routing across the hierarchy.

![OsmAnd HH](./img/hh-image.png)
