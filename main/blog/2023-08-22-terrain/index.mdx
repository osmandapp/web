...
At first you need to buy OsmAnd package and download Terrain map data. It is available in [Download maps menu](https://osmand.net/docs/user/plugins/contour-lines#how-to-download): _OsmAnd menu → Download maps → Regions → Chosen country → Terrain map (3D)_. Here you can find Terrain map for each country.
...
When you download Terrain data, next step is activate it for your map. 
...
How to choose colour scheme and other parameters for layers read below.
...
**_Zoom levels_** is a parameter for setting the minimum and maximum zoom levels at which the layr will be displayed.

![Zoom](./img/zoom.png)

**_Cache size_** is a parameter that shows the size of the cache. Each time you view terrain info on the map (hillshade or slope), all this data is cached for quick use in the future.
...
Now, you can see 3D Relief on the map with any map layers:
...
To display [_Elevation: map center widget_](https://osmand.net/docs/user/widgets/info-widgets#-elevation-map-center):

_OsmAnd Menu → Configure screen → Left / Right panel → Altitude_

## Elevation calculation

This feature allows to calculate elevation for any GPX tracks without altitude data: [Altitude menu](https://osmand.net/docs/user/map/track-context-menu#altitude)
