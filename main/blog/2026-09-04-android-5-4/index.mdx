...
![OsmAnd 5.4 for Android](./img/5-4-preview.webp)
...
We introduced a refreshed [**Search interface**](https://osmand.net/docs/user/search/search-all#search-explore) designed to make finding places faster and easier.
...
We've made **[Favorites](https://osmand.net/docs/user/personal/favorites) and [Waypoints](https://osmand.net/docs/user/navigation/setup/gpx-navigation#waypoints) more informative** by allowing you to attach media files directly to them.

Similar to the [**Audio/Video Notes**](https://osmand.net/docs/user/plugins/audio-video-notes) feature, you can now add photos, videos, and audio notes to a saved Favorite or GPX Waypoint. You can take a new photo, record a video or audio note, or choose existing media from your device.
...
Favorites can now be organized using [**folders and subfolders**](https://osmand.net/docs/user/personal/favorites#bulk-edit--delete), similar to the folder structure already available for Tracks. This allows you to create a clear hierarchy instead of keeping all Favorite groups at the same level.
...
We introduce new [**appearance settings for map widgets**](https://osmand.net/docs/user/widgets/configure-screen#widget-panels), giving you more control over how information is displayed on the map.
...
OsmAnd 5.4 brings another major update to [**3D Buildings**](https://osmand.net/docs/user/plugins/topography#3d-buildings), making cityscapes more detailed and closer to the real-world geometry stored in OpenStreetMap.
...
In [**Coordinate Search**](https://osmand.net/docs/user/search/search-address#coordinates-search), you can select recently used formats or browse the full list of available coordinate systems. You can also quickly find the required system by searching for its **name or EPSG code**.
...
Previously, [route calculation](https://osmand.net/docs/user/navigation/setup/route-navigation) could be interrupted by several dialogs asking you to update maps or manually continue using the downloaded maps. In some cases, this could even happen during route recalculation while navigation was already active.

Now, OsmAnd can handle these situations more smoothly. If the selected routing method cannot calculate the route correctly, the app can switch to an alternative [calculation method](https://osmand.net/docs/user/navigation/routing/car-based-routing#route-parameters---car) and continue navigation without unnecessary interruptions.
...
OsmAnd 5.4 brings several improvements to [**Android Auto**](https://osmand.net/docs/user/navigation/auto-car), making the search, and navigation experience more convenient on the car display.
...
The Star Map now also includes a dedicated [**Lunar Eclipse Explorer**](https://osmand.net/docs/user/plugins/astronomy#lunar-eclipse), supporting:
...
OsmAnd 5.4 brings several improvements to the [**Vessel Tracker (AIS)**](https://osmand.net/docs/user/plugins/ais-tracker) plugin, making it easier to configure AIS connections and monitor nearby vessels.
...
> **Experimental feature:** In OsmAnd 5.4 for Android, the new Spatial Search is disabled by default. To enable it, go to: *[OsmAnd Development plugin](https://osmand.net/docs/user/plugins/development) → Use spatial text search*. This allows us to collect feedback and continue improving the search experience before enabling it by default in a future release. 
...
- **[More icons for profiles.](https://osmand.net/docs/user/personal/profiles#profile-appearance)** The profile icon selection has been expanded, giving you more options to visually distinguish profiles for activities such as running and other use cases.  
  {/*[#22508](https://github.com/osmandapp/OsmAnd/issues/22508)*/}

- [**Extended AIDL support for external apps.**](https://osmand.net/docs/technical/osmand-api-sdk/#android-osmand-aidl-api) External apps and plugins can now organize their OsmAnd widgets into groups and use custom widget icons, providing more flexibility for third-party integrations.  
  {/*[#24980](https://github.com/osmandapp/OsmAnd/issues/24980)*/}

- [**Correct altitude on Android 16.**](https://osmand.net/docs/user/personal/maps-resources#world-maps) Fixed an issue where OsmAnd could display ellipsoidal altitude instead of altitude above mean sea level (MSL) on Android 16 devices.  
