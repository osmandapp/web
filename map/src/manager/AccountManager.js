...
export async function userLogin({ ctxl, username, pwd, setError, handleClose, lang = DEFAULT_AUTH_API_LANG }) {
    ...
    if (await isRequestOk(response, setError)) {
        setError('');
        ctxl.setLoginUser(username);
        ctxl.setEmailCookie(username, { days: 30, SameSite: 'Strict' }); // for next login
        handleClose();
    }
}

export async function userLogout({ ctxl, username, handleClose, lang = DEFAULT_AUTH_API_LANG }) {
