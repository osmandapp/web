...
export function createLayeredPinIcon(options = {}) {
    ...
    const { color, shape, size, iconSize, iconHtml, className, iconAnchor, iconColor } = merged;
    ...
    const html = `
        <div class="map-layered-pin ${className ?? ''}" style="position:relative;width:${size}px;height:${size}px;">
            ...
            ${
                markerIconHtml
                    ? `<div class="map-layered-pin__icon" style="position:absolute;left:50%;top:50%;transform:translate(-50%, -50%) translateY(${translateY}px);width:${iconWrapperSize}px;height:${iconWrapperSize}px;display:flex;align-items:center;justify-content:center;${iconColor ? 'filter:brightness(0) invert(1);' : ''}">
                            ${markerIconHtml}
                       </div>`
                    : ''
            }
