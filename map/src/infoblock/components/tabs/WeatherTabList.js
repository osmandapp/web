...
export default class WeatherTabList {
    ...
    create(ctx) {
        ...
        tabs['Day forecast'] = ctx.weatherPoint?.day && (
            <WeatherForecast key="weatherday" data={ctx.weatherPoint.day} loc={loc} />
        );
        tabs['Week forecast'] = ctx.weatherPoint?.week && (
            <WeatherForecast key="weatherweek" data={ctx.weatherPoint.week} loc={loc} />
        );
